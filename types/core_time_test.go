...
func (s *testCoreTimeSuite) TestCompareTime(c *C) {
	tests := []struct {
		...
		{FromDate(2006, 1, 2, 3, 4, 5, 6), FromDate(2020, 1, 2, 3, 4, 5, 0), -1},
...
func (s *testCoreTimeSuite) TestMixDateAndTime(c *C) {
	tests := []struct {
		...
		{
			date:   FromDate(2020, 12, 31, 0, 0, 0, 0),
			dur:    NewDuration(24, 0, 0, 0, DefaultFsp),
			neg:    false,
			expect: FromDate(2020, 1, 1, 0, 0, 0, 0),
		},
		{
			date:   FromDate(2020, 12, 0, 0, 0, 0, 0),
			dur:    NewDuration(24, 0, 0, 0, DefaultFsp),
			neg:    false,
			expect: FromDate(2020, 12, 1, 0, 0, 0, 0),
		},
		{
			date:   FromDate(2020, 1, 12, 3, 23, 15, 0),
			dur:    NewDuration(2, 21, 10, 0, DefaultFsp),
			neg:    true,
			expect: FromDate(2020, 1, 12, 1, 2, 5, 0),
		},
	}
...
func (s *testCoreTimeSuite) TestIsLeapYear(c *C) {
	tests := []struct {
		...
		{FromDate(2020, 4, 24, 0, 0, 0, 0), false},
		...
		{FromDate(2020, 6, 29, 0, 0, 0, 0), true},
		{FromDate(2020, 6, 29, 0, 0, 0, 0), false},
		{FromDate(2020, 9, 31, 0, 0, 0, 0), false},
...
func (s *testCoreTimeSuite) TestgetFixDays(c *C) {
	tests := []struct {
		...
		{2020, 04, 05, time.Date(2020, 04, 01, 1, 2, 3, 4, time.UTC), 0},
	}
...
func (s *testCoreTimeSuite) TestAddDate(c *C) {
	tests := []struct {
		...
		{01, 04, 05, time.Date(2020, 04, 01, 1, 2, 3, 4, time.UTC)},
	}
...
func (s *testCoreTimeSuite) TestWeekday(c *C) {
	tests := []struct {
		...
		{FromDate(2020, 01, 01, 0, 0, 0, 0), "Tuesday"},
		{FromDate(2020, 02, 31, 0, 0, 0, 0), "Sunday"},
		{FromDate(2020, 04, 31, 0, 0, 0, 0), "Wednesday"},
	}
