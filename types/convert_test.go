...
func (s *testTypeConvertSuite) TestConvertType(c *C) {
	...
	vd, err := ParseTime(sc, "2010-10-10 10:11:11.12345", allegrosql.TypeDatetime, 2)
	...
	vt, err := ParseTime(sc, "2010-10-10 10:11:11.12345", allegrosql.TypeTimestamp, 2)
	...
	// For allegrosql.TypeTimestamp, allegrosql.TypeDatetime, allegrosql.TypeDate
	...
	// For TypeLonglong
	ft = NewFieldType(allegrosql.TypeLonglong)
	...
	v, err = Convert(NewTime(FromDate(2020, 1, 1, 12, 59, 59, 555000), allegrosql.TypeDatetime, MaxFsp), ft)
	c.Assert(err, IsNil)
	c.Assert(v, Equals, int64(20170101130000))

	// For TypeBit
	ft = NewFieldType(allegrosql.TypeBit)
	ft.Flen = 24 // 3 bytes.
	v, err = Convert("100", ft)
	c.Assert(err, IsNil)
	...
	// Test Causet.ToDecimal with bad number.
	d := NewCauset("hello")
	...
	v, err = Convert("2020", ft)
	c.Assert(err, IsNil)
	c.Assert(v, Equals, int64(2020))
	v, err = Convert(2020, ft)
	c.Assert(err, IsNil)
	c.Assert(v, Equals, int64(2020))
	_, err = Convert(1800, ft)
	c.Assert(err, NotNil)
	dt, err := ParseDate(nil, "2020-11-11")
	c.Assert(err, IsNil)
	v, err = Convert(dt, ft)
	c.Assert(err, IsNil)
	c.Assert(v, Equals, int64(2020))
...
func (s *testTypeConvertSuite) TestConvertToString(c *C) {
	...
	testToString(c, NewBinaryLiteralFromUint(0x4D7953514C, -1), "MyALLEGROSQL")
	...
	t, err := ParseTime(&stmtctx.StatementContext{TimeZone: time.UTC},
		"2011-11-10 11:11:11.999999", allegrosql.TypeTimestamp, 6)
	c.Assert(err, IsNil)
	...
	for _, tt := range tests {
		...
		inputCauset := NewStringCauset(tt.input)
		sc := new(stmtctx.StatementContext)
		outputCauset, err := inputCauset.ConvertTo(sc, ft)
		if tt.input != tt.output {
			c.Assert(ErrDataTooLong.Equal(err), IsTrue)
		} else {
			c.Assert(err, IsNil)
		}
		c.Assert(outputCauset.GetString(), Equals, tt.output)
	}
}
...
func (s *testTypeConvertSuite) TestStrToNum(c *C) {
	...
	testSelectUFIDelateDeleteEmptyStringError(c)
}

func testSelectUFIDelateDeleteEmptyStringError(c *C) {
	testCases := []struct {
		inSelect bool
		inDelete bool
	}{
...
func (s *testTypeConvertSuite) TestFieldTypeToStr(c *C) {
	defer testleak.AfterTest(c)()
	v := TypeToStr(allegrosql.TypeUnspecified, "not binary")
	c.Assert(v, Equals, TypeStr(allegrosql.TypeUnspecified))
	v = TypeToStr(allegrosql.TypeBlob, charset.CharsetBin)
	c.Assert(v, Equals, "blob")
	v = TypeToStr(allegrosql.TypeString, charset.CharsetBin)
...
func (s *testTypeConvertSuite) TestConvert(c *C) {
	defer testleak.AfterTest(c)()
	// integer ranges
	signedDeny(c, allegrosql.TypeTiny, -129, "-128")
	signedAccept(c, allegrosql.TypeTiny, -128, "-128")
	signedAccept(c, allegrosql.TypeTiny, 127, "127")
	signedDeny(c, allegrosql.TypeTiny, 128, "127")
	signedAccept(c, allegrosql.TypeTiny, NewBinaryLiteralFromUint(127, -1), "127")
	signedDeny(c, allegrosql.TypeTiny, NewBinaryLiteralFromUint(128, -1), "127")
	unsignedDeny(c, allegrosql.TypeTiny, -1, "255")
	unsignedAccept(c, allegrosql.TypeTiny, 0, "0")
	unsignedAccept(c, allegrosql.TypeTiny, 255, "255")
	unsignedDeny(c, allegrosql.TypeTiny, 256, "255")
	unsignedAccept(c, allegrosql.TypeTiny, NewBinaryLiteralFromUint(0, -1), "0")
	unsignedAccept(c, allegrosql.TypeTiny, NewBinaryLiteralFromUint(255, -1), "255")
	unsignedDeny(c, allegrosql.TypeTiny, NewBinaryLiteralFromUint(256, -1), "255")

	signedDeny(c, allegrosql.TypeShort, int64(math.MinInt16)-1, strvalue(int64(math.MinInt16)))
	signedAccept(c, allegrosql.TypeShort, int64(math.MinInt16), strvalue(int64(math.MinInt16)))
	signedAccept(c, allegrosql.TypeShort, int64(math.MaxInt16), strvalue(int64(math.MaxInt16)))
	signedDeny(c, allegrosql.TypeShort, int64(math.MaxInt16)+1, strvalue(int64(math.MaxInt16)))
	signedAccept(c, allegrosql.TypeShort, NewBinaryLiteralFromUint(math.MaxInt16, -1), strvalue(int64(math.MaxInt16)))
	signedDeny(c, allegrosql.TypeShort, NewBinaryLiteralFromUint(math.MaxInt16+1, -1), strvalue(int64(math.MaxInt16)))
	unsignedDeny(c, allegrosql.TypeShort, -1, "65535")
	unsignedAccept(c, allegrosql.TypeShort, 0, "0")
	unsignedAccept(c, allegrosql.TypeShort, uint64(math.MaxUint16), strvalue(uint64(math.MaxUint16)))
	unsignedDeny(c, allegrosql.TypeShort, uint64(math.MaxUint16)+1, strvalue(uint64(math.MaxUint16)))
	unsignedAccept(c, allegrosql.TypeShort, NewBinaryLiteralFromUint(0, -1), "0")
	unsignedAccept(c, allegrosql.TypeShort, NewBinaryLiteralFromUint(math.MaxUint16, -1), strvalue(uint64(math.MaxUint16)))
	unsignedDeny(c, allegrosql.TypeShort, NewBinaryLiteralFromUint(math.MaxUint16+1, -1), strvalue(uint64(math.MaxUint16)))

	signedDeny(c, allegrosql.TypeInt24, -1<<23-1, strvalue(-1<<23))
	signedAccept(c, allegrosql.TypeInt24, -1<<23, strvalue(-1<<23))
	signedAccept(c, allegrosql.TypeInt24, 1<<23-1, strvalue(1<<23-1))
	signedDeny(c, allegrosql.TypeInt24, 1<<23, strvalue(1<<23-1))
	signedAccept(c, allegrosql.TypeInt24, NewBinaryLiteralFromUint(1<<23-1, -1), strvalue(1<<23-1))
	signedDeny(c, allegrosql.TypeInt24, NewBinaryLiteralFromUint(1<<23, -1), strvalue(1<<23-1))
	unsignedDeny(c, allegrosql.TypeInt24, -1, "16777215")
	unsignedAccept(c, allegrosql.TypeInt24, 0, "0")
	unsignedAccept(c, allegrosql.TypeInt24, 1<<24-1, strvalue(1<<24-1))
	unsignedDeny(c, allegrosql.TypeInt24, 1<<24, strvalue(1<<24-1))
	unsignedAccept(c, allegrosql.TypeInt24, NewBinaryLiteralFromUint(0, -1), "0")
	unsignedAccept(c, allegrosql.TypeInt24, NewBinaryLiteralFromUint(1<<24-1, -1), strvalue(1<<24-1))
	unsignedDeny(c, allegrosql.TypeInt24, NewBinaryLiteralFromUint(1<<24, -1), strvalue(1<<24-1))

	signedDeny(c, allegrosql.TypeLong, int64(math.MinInt32)-1, strvalue(int64(math.MinInt32)))
	signedAccept(c, allegrosql.TypeLong, int64(math.MinInt32), strvalue(int64(math.MinInt32)))
	signedAccept(c, allegrosql.TypeLong, int64(math.MaxInt32), strvalue(int64(math.MaxInt32)))
	signedDeny(c, allegrosql.TypeLong, uint64(math.MaxUint64), strvalue(uint64(math.MaxInt32)))
	signedDeny(c, allegrosql.TypeLong, int64(math.MaxInt32)+1, strvalue(int64(math.MaxInt32)))
	signedDeny(c, allegrosql.TypeLong, "1343545435346432587475", strvalue(int64(math.MaxInt32)))
	signedAccept(c, allegrosql.TypeLong, NewBinaryLiteralFromUint(math.MaxInt32, -1), strvalue(int64(math.MaxInt32)))
	signedDeny(c, allegrosql.TypeLong, NewBinaryLiteralFromUint(math.MaxUint64, -1), strvalue(int64(math.MaxInt32)))
	signedDeny(c, allegrosql.TypeLong, NewBinaryLiteralFromUint(math.MaxInt32+1, -1), strvalue(int64(math.MaxInt32)))
	unsignedDeny(c, allegrosql.TypeLong, -1, "4294967295")
	unsignedAccept(c, allegrosql.TypeLong, 0, "0")
	unsignedAccept(c, allegrosql.TypeLong, uint64(math.MaxUint32), strvalue(uint64(math.MaxUint32)))
	unsignedDeny(c, allegrosql.TypeLong, uint64(math.MaxUint32)+1, strvalue(uint64(math.MaxUint32)))
	unsignedAccept(c, allegrosql.TypeLong, NewBinaryLiteralFromUint(0, -1), "0")
	unsignedAccept(c, allegrosql.TypeLong, NewBinaryLiteralFromUint(math.MaxUint32, -1), strvalue(uint64(math.MaxUint32)))
	unsignedDeny(c, allegrosql.TypeLong, NewBinaryLiteralFromUint(math.MaxUint32+1, -1), strvalue(uint64(math.MaxUint32)))

	signedDeny(c, allegrosql.TypeLonglong, math.MinInt64*1.1, strvalue(int64(math.MinInt64)))
	signedAccept(c, allegrosql.TypeLonglong, int64(math.MinInt64), strvalue(int64(math.MinInt64)))
	signedAccept(c, allegrosql.TypeLonglong, int64(math.MaxInt64), strvalue(int64(math.MaxInt64)))
	signedDeny(c, allegrosql.TypeLonglong, math.MaxInt64*1.1, strvalue(int64(math.MaxInt64)))
	signedAccept(c, allegrosql.TypeLonglong, NewBinaryLiteralFromUint(math.MaxInt64, -1), strvalue(int64(math.MaxInt64)))
	signedDeny(c, allegrosql.TypeLonglong, NewBinaryLiteralFromUint(math.MaxInt64+1, -1), strvalue(int64(math.MaxInt64)))
	unsignedAccept(c, allegrosql.TypeLonglong, -1, "18446744073709551615")
	unsignedAccept(c, allegrosql.TypeLonglong, 0, "0")
	unsignedAccept(c, allegrosql.TypeLonglong, uint64(math.MaxUint64), strvalue(uint64(math.MaxUint64)))
	unsignedDeny(c, allegrosql.TypeLonglong, math.MaxUint64*1.1, strvalue(uint64(math.MaxUint64)))
	unsignedAccept(c, allegrosql.TypeLonglong, NewBinaryLiteralFromUint(0, -1), "0")
	unsignedAccept(c, allegrosql.TypeLonglong, NewBinaryLiteralFromUint(math.MaxUint64, -1), strvalue(uint64(math.MaxUint64)))

	// integer from string
	signedAccept(c, allegrosql.TypeLong, "	  234  ", "234")
	signedAccept(c, allegrosql.TypeLong, " 2.35e3  ", "2350")
	signedAccept(c, allegrosql.TypeLong, " 2.e3  ", "2000")
	signedAccept(c, allegrosql.TypeLong, " -2.e3  ", "-2000")
	signedAccept(c, allegrosql.TypeLong, " 2e2  ", "200")
	signedAccept(c, allegrosql.TypeLong, " 0.002e3  ", "2")
	signedAccept(c, allegrosql.TypeLong, " .002e3  ", "2")
	signedAccept(c, allegrosql.TypeLong, " 20e-2  ", "0")
	signedAccept(c, allegrosql.TypeLong, " -20e-2  ", "0")
	signedAccept(c, allegrosql.TypeLong, " +2.51 ", "3")
	signedAccept(c, allegrosql.TypeLong, " -9999.5 ", "-10000")
	signedAccept(c, allegrosql.TypeLong, " 999.4", "999")
	signedAccept(c, allegrosql.TypeLong, " -3.58", "-4")
	signedDeny(c, allegrosql.TypeLong, " 1a ", "1")
	signedDeny(c, allegrosql.TypeLong, " +1+ ", "1")

	// integer from float
	signedAccept(c, allegrosql.TypeLong, 234.5456, "235")
	signedAccept(c, allegrosql.TypeLong, -23.45, "-23")
	unsignedAccept(c, allegrosql.TypeLonglong, 234.5456, "235")
	unsignedDeny(c, allegrosql.TypeLonglong, -23.45, "18446744073709551593")

	// float from string
	signedAccept(c, allegrosql.TypeFloat, "23.523", "23.523")
	signedAccept(c, allegrosql.TypeFloat, int64(123), "123")
	signedAccept(c, allegrosql.TypeFloat, uint64(123), "123")
	signedAccept(c, allegrosql.TypeFloat, 123, "123")
	signedAccept(c, allegrosql.TypeFloat, float32(123), "123")
	signedAccept(c, allegrosql.TypeFloat, float64(123), "123")
	signedAccept(c, allegrosql.TypeDouble, " -23.54", "-23.54")
	signedDeny(c, allegrosql.TypeDouble, "-23.54a", "-23.54")
	signedDeny(c, allegrosql.TypeDouble, "-23.54e2e", "-2354")
	signedDeny(c, allegrosql.TypeDouble, "+.e", "0")
	signedAccept(c, allegrosql.TypeDouble, "1e+1", "10")

	// year
	signedDeny(c, allegrosql.TypeYear, 123, "0")
	signedDeny(c, allegrosql.TypeYear, 3000, "0")
	signedAccept(c, allegrosql.TypeYear, "2000", "2000")
	signedAccept(c, allegrosql.TypeYear, "abc", "0")
	signedAccept(c, allegrosql.TypeYear, "00abc", "2000")
	signedAccept(c, allegrosql.TypeYear, "0019", "2020")
	signedAccept(c, allegrosql.TypeYear, 2155, "2155")
	signedAccept(c, allegrosql.TypeYear, 2155.123, "2155")
	signedDeny(c, allegrosql.TypeYear, 2156, "0")
	signedDeny(c, allegrosql.TypeYear, 123.123, "0")
	signedDeny(c, allegrosql.TypeYear, 1900, "0")
	signedAccept(c, allegrosql.TypeYear, 1901, "1901")
	signedAccept(c, allegrosql.TypeYear, 1900.567, "1901")
	signedDeny(c, allegrosql.TypeYear, 1900.456, "0")
	signedAccept(c, allegrosql.TypeYear, 1, "2001")
	signedAccept(c, allegrosql.TypeYear, 69, "2069")
	signedAccept(c, allegrosql.TypeYear, 70, "1970")
	signedAccept(c, allegrosql.TypeYear, 99, "1999")
	signedDeny(c, allegrosql.TypeYear, 100, "0")
	signedDeny(c, allegrosql.TypeYear, "99999999999999999999999999999999999", "0")

	// time from string
	signedAccept(c, allegrosql.TypeDate, "2012-08-23", "2012-08-23")
	signedAccept(c, allegrosql.TypeDatetime, "2012-08-23 12:34:03.123456", "2012-08-23 12:34:03")
	signedAccept(c, allegrosql.TypeDatetime, ZeroDatetime, "0000-00-00 00:00:00")
	signedAccept(c, allegrosql.TypeDatetime, int64(0), "0000-00-00 00:00:00")
	signedAccept(c, allegrosql.TypeDatetime, NewDecFromFloatForTest(20010101100000.123456), "2001-01-01 10:00:00")
	signedAccept(c, allegrosql.TypeTimestamp, "2012-08-23 12:34:03.123456", "2012-08-23 12:34:03")
	signedAccept(c, allegrosql.TypeTimestamp, NewDecFromFloatForTest(20010101100000.123456), "2001-01-01 10:00:00")
	signedAccept(c, allegrosql.TypeDuration, "10:11:12", "10:11:12")
	signedAccept(c, allegrosql.TypeDuration, ZeroDatetime, "00:00:00")
	signedAccept(c, allegrosql.TypeDuration, ZeroDuration, "00:00:00")
	signedAccept(c, allegrosql.TypeDuration, 0, "00:00:00")

	signedDeny(c, allegrosql.TypeDate, "2012-08-x", "0000-00-00")
	signedDeny(c, allegrosql.TypeDatetime, "2012-08-x", "0000-00-00 00:00:00")
	signedDeny(c, allegrosql.TypeTimestamp, "2012-08-x", "0000-00-00 00:00:00")
	signedDeny(c, allegrosql.TypeDuration, "2012-08-x", "00:00:00")

	// string from string
	signedAccept(c, allegrosql.TypeString, "abc", "abc")

	// string from integer
	signedAccept(c, allegrosql.TypeString, 5678, "5678")
	signedAccept(c, allegrosql.TypeString, ZeroDuration, "00:00:00")
	signedAccept(c, allegrosql.TypeString, ZeroDatetime, "0000-00-00 00:00:00")
	signedAccept(c, allegrosql.TypeString, []byte("123"), "123")

	//TODO add more tests
	signedAccept(c, allegrosql.TypeNewDecimal, 123, "123")
	signedAccept(c, allegrosql.TypeNewDecimal, int64(123), "123")
	signedAccept(c, allegrosql.TypeNewDecimal, uint64(123), "123")
	signedAccept(c, allegrosql.TypeNewDecimal, float32(123), "123")
	signedAccept(c, allegrosql.TypeNewDecimal, 123.456, "123.456")
	signedAccept(c, allegrosql.TypeNewDecimal, "-123.456", "-123.456")
	signedAccept(c, allegrosql.TypeNewDecimal, NewDecFromInt(12300000), "12300000")
	dec := NewDecFromInt(-123)
	dec.Shift(-5)
	dec.Round(dec, 5, ModeHalfEven)
	signedAccept(c, allegrosql.TypeNewDecimal, dec, "-0.00123")
}
...
func testConvertTimeTimeZone(c *C, sc *stmtctx.StatementContext) {
	...
	tests := []struct {
		...
		{
			input:  NewTime(raw, allegrosql.TypeDatetime, DefaultFsp),
			target: NewFieldType(allegrosql.TypeTimestamp),
			expect: NewTime(raw, allegrosql.TypeTimestamp, DefaultFsp),
		},
		{
			input:  NewTime(raw, allegrosql.TypeDatetime, DefaultFsp),
			target: NewFieldType(allegrosql.TypeTimestamp),
		...
		{
			input:  NewTime(raw, allegrosql.TypeTimestamp, DefaultFsp),
			target: NewFieldType(allegrosql.TypeDatetime),
			expect: NewTime(raw, allegrosql.TypeDatetime, DefaultFsp),
		},
	}
...
func (s *testTypeConvertSuite) TestNumberToDuration(c *C) {
	var testCases = []struct {
		...
		{20171222020005, 0, false, 2020, 12, 22, 02, 00, 05},
