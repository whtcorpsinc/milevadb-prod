...
func (s *testTimeSuite) TestTimeFormatMethod(c *C) {
	...
	tblDate := []struct {
		...
		{
			// For invalid date month or year = 0, MyALLEGROSQL behavior is confusing, %U (which format Week()) is 52, but Week() is 0.
			// It's because in MyALLEGROSQL, Week() checks invalid date before processing, but DateFormat() don't.
			// So there are some difference to MyALLEGROSQL here (%U %u %V %v), MilevaDB user should not rely on those corner case behavior.
	...
	for i, t := range tblDate {
		tm, err := types.ParseTime(sc, t.Input, allegrosql.TypeDatetime, 6)
...
func (s *testTimeSuite) TestStrToDate(c *C) {
	...
	tests := []struct {
		...
		{`01,05,2020`, `%d,%m,%Y`, types.FromDate(2020, 5, 1, 0, 0, 0, 0)},
		{`May 01, 2020`, `%M %d,%Y`, types.FromDate(2020, 5, 1, 0, 0, 0, 0)},
		...
		{`2020-January:7 432101`, `%Y-%M:%l %f`, types.FromDate(2020, 1, 0, 7, 0, 0, 432101)},
		...
		{`18/10/22`, `%y/%m/%d`, types.FromDate(2020, 10, 22, 0, 0, 0, 0)},
		...
		{`18/10/22`, `%Y/%m/%d`, types.FromDate(2020, 10, 22, 0, 0, 0, 0)},
		{`2020/10/22`, `%Y/%m/%d`, types.FromDate(2020, 10, 22, 0, 0, 0, 0)},
		{`8/10/22`, `%Y/%m/%d`, types.FromDate(2008, 10, 22, 0, 0, 0, 0)},
		{`69/10/22`, `%Y/%m/%d`, types.FromDate(2069, 10, 22, 0, 0, 0, 0)},
		{`70/10/22`, `%Y/%m/%d`, types.FromDate(1970, 10, 22, 0, 0, 0, 0)},
		{`18/10/22`, `%Y/%m/%d`, types.FromDate(2020, 10, 22, 0, 0, 0, 0)},
