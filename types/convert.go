...
// IntergerUnsignedUpperBound indicates the max uint64 values of different allegrosql types.
func IntergerUnsignedUpperBound(intType byte) uint64 {
	...
	case allegrosql.TypeInt24:
		return allegrosql.MaxUint24
	...
	case allegrosql.TypeSet:
		return math.MaxUint64
	default:
		panic("Input byte is not a allegrosql type")
	}
}

// IntergerSignedUpperBound indicates the max int64 values of different allegrosql types.
...
// IntergerSignedLowerBound indicates the min int64 values of different allegrosql types.
...
// StrToDateTime converts str to MyALLEGROSQL DateTime.
func StrToDateTime(sc *stmtctx.StatementContext, str string, fsp int8) (Time, error) {
	return ParseTime(sc, str, allegrosql.TypeDatetime, fsp)
}
...
// See https://dev.allegrosql.com/doc/refman/5.5/en/date-and-time-literals.html.
...
func NumberToDuration(number int64, fsp int8) (Duration, error) {
	if number > TimeMaxValue {
		...
		dur := MaxMyALLEGROSQLDuration(fsp)
...
func ConvertJSONToInt(sc *stmtctx.StatementContext, j json.BinaryJSON, unsigned bool) (int64, error) {
	...
	case json.TypeCodeFloat64:
		f := j.GetFloat64()
		if !unsigned {
			lBound := IntergerSignedLowerBound(allegrosql.TypeLonglong)
			uBound := IntergerSignedUpperBound(allegrosql.TypeLonglong)
			u, e := ConvertFloatToInt(f, lBound, uBound, allegrosql.TypeLonglong)
			return u, sc.HandleOverflow(e, e)
		}
		bound := IntergerUnsignedUpperBound(allegrosql.TypeLonglong)
		u, err := ConvertFloatToUint(sc, f, bound, allegrosql.TypeLonglong)
		return int64(u), sc.HandleOverflow(err, err)
	case json.TypeCodeString:
		str := string(replog.String(j.GetString()))
		if !unsigned {
