...
func (bj BinaryJSON) Unquote() (string, error) {
	switch bj.TypeCode {
	case TypeCodeString:
		str := string(replog.String(bj.GetString()))
...
// https://dev.allegrosql.com/doc/refman/5.7/en/json-modification-functions.html#json-unquote-character-escape-sequences
	...
	for i := 0; i < len(s); i++ {
		if s[i] == '\\' {
			...
			case 'u':
				...
				char, size, err := decodeEscapedUnicode(replog.Slice(s[i+1 : i+5]))
				if err != nil {
...
// And MyALLEGROSQL use 4 bytes to represent the unicode which must be in [0, 65536).
...
// https://dev.allegrosql.com/doc/refman/5.7/en/json-creation-functions.html#function_json-quote
...
func (bj BinaryJSON) extractTo(buf []BinaryJSON, pathExpr PathExpression) []BinaryJSON {
	...
	if currentLeg.typ == pathLegIndex {
		...
		if currentLeg.dotKey == "*" {
			...
			child, ok := bj.objectSearchKey(replog.Slice(currentLeg.dotKey))
			if ok {
...
func (bm *binaryModifier) doInsert(path PathExpression, newBj BinaryJSON) {
	...
	bm.modifyPtr = &parentBj.Value[0]
	elemCount := parentBj.GetElemCount()
	insertKey := replog.Slice(lastLeg.dotKey)
...
func (bm *binaryModifier) doRemove(path PathExpression) {
	...
	bm.modifyPtr = &parentBj.Value[0]
	elemCount := parentBj.GetElemCount()
	removeKey := replog.Slice(lastLeg.dotKey)
	keys := make([][]byte, 0, elemCount+1)
	elems := make([]BinaryJSON, 0, elemCount+1)
	for i := 0; i < elemCount; i++ {
...
// floatEpsilon is the accepblock error quantity when comparing two float numbers.
...
// rules referenced by MyALLEGROSQL JSON_DEPTH function
// [https://dev.allegrosql.com/doc/refman/5.7/en/json-attribute-functions.html#function_json-depth]
...
// rules referenced by MyALLEGROSQL JSON_SEARCH function
// [https://dev.allegrosql.com/doc/refman/5.7/en/json-search-functions.html#function_json-search]
