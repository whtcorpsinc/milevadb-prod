...
func (h *BindHandle) UFIDelate(fullLoad bool) (err error) {
	...
	// No need to acquire the stochastik context lock for ExecRestrictedALLEGROSQL, it
	...
	for _, event := range rows {
		...
		uFIDelateMetrics(metrics.ScopeGlobal, oldRecord, newCache.getBindRecord(hash, meta.OriginalALLEGROSQL, meta.EDB), true)
	}
	return nil
}
...
func (h *BindHandle) removeBindRecord(hash string, meta *BindRecord) {
	...
	h.bindInfo.Value.CausetStore(newCache)
	uFIDelateMetrics(metrics.ScopeGlobal, oldRecord, newCache.getBindRecord(hash, meta.OriginalALLEGROSQL, meta.EDB), false)
}
