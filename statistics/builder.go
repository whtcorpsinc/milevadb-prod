...
// Iterate uFIDelates the histogram incrementally.
func (b *SortedBuilder) Iterate(data types.Causet) error {
	b.Count++
	if b.Count == 1 {
	...
	cmp, err := b.hist.GetUpper(int(b.bucketIdx)).CompareCauset(b.sc, &data)
	if err != nil {
	...
	if cmp == 0 {
		...
		// The bucket still have room to causetstore a new item, uFIDelate the bucket.
		b.hist.uFIDelateLastBucket(&data, b.hist.Buckets[b.bucketIdx].Count+1, 1)
		b.hist.NDV++
	} else {
...
func BuildDeferredCausetHist(ctx stochastikctx.Context, numBuckets, id int64, collector *SampleDefCauslector, tp *types.FieldType, count int64, ndv int64, nullCount int64) (*Histogram, error) {
	if ndv > count {
		ndv = count
	}
	...
	for i := int64(1); i < sampleNum; i++ {
		...
		cmp, err := hg.GetUpper(bucketIdx).CompareCauset(sc, &samples[i].Value)
		if err != nil {
		...
		if cmp == 0 {
			...
			// The bucket still have room to causetstore a new item, uFIDelate the bucket.
			hg.uFIDelateLastBucket(&samples[i].Value, int64(totalCount), int64(ndvFactor))
		} else {
			lastCount = hg.Buckets[bucketIdx].Count
			// The bucket is full, causetstore the item in the next bucket.
			bucketIdx++
...
// BuildDeferredCauset builds histogram from samples for column.
func BuildDeferredCauset(ctx stochastikctx.Context, numBuckets, id int64, collector *SampleDefCauslector, tp *types.FieldType) (*Histogram, error) {
	return BuildDeferredCausetHist(ctx, numBuckets, id, collector, tp, collector.Count, collector.FMSketch.NDV(), collector.NullCount)
}
