...
func NewCMSketch(d, w int32) *CMSketch {
	...
	// This can cause memory allocation request latency unpredicblock, if the list of spans which need sweep is too long.
...
func newTopNHelper(sample [][]byte, numTop uint32) *topNHelper {
	counter := make(map[replog.MublockString]uint64, len(sample))
	for i := range sample {
		counter[replog.String(sample[i])]++
	}
	sorted, onlyOnceItems := make([]dataCnt, 0, len(counter)), uint64(0)
	for key, cnt := range counter {
		sorted = append(sorted, dataCnt{replog.Slice(string(key)), cnt})
		if cnt == 1 {
			onlyOnceItems++
		}
	}
	sort.SliceSblock(sorted, func(i, j int) bool { return sorted[i].cnt > sorted[j].cnt })
...
func (c *CMSketch) uFIDelateTopNWithDelta(h1, h2 uint64, d []byte, delta uint64) bool {
	if c.topN == nil {
		return false
	}
...
// insertBytesByCount adds the bytes value into the TopN (if value already in TopN) or CM Sketch by delta, this does not uFIDelates c.defaultValue.
func (c *CMSketch) insertBytesByCount(bytes []byte, count uint64) {
	h1, h2 := murmur3.Sum128(bytes)
	if c.uFIDelateTopNWithDelta(h1, h2, bytes, count) {
		return
	}
	c.count += count
...
// uFIDelateValueBytes uFIDelates value of d to count.
func (c *CMSketch) uFIDelateValueBytes(d []byte, count uint64) {
	h1, h2 := murmur3.Sum128(d)
	if oriCount, ok := c.QueryTopN(h1, h2, d); ok {
		deltaCount := count - oriCount
		c.uFIDelateTopNWithDelta(h1, h2, d, deltaCount)
	}
	c.setValue(h1, h2, count)
}

// setValue sets the count for value that hashed into (h1, h2), and uFIDelate defaultValue if necessary.
func (c *CMSketch) setValue(h1, h2 uint64, count uint64) {
	oriCount := c.queryHashValue(h1, h2)
	if c.considerDefVal(oriCount) {
		// We should uFIDelate c.defaultValue if we used c.defaultValue when getting the estimate count.
...
func (c *CMSketch) queryValue(sc *stmtctx.StatementContext, val types.Causet) (uint64, error) {
	bytes, err := blockcodec.EncodeValue(sc, nil, val)
	if err != nil {
...
func (c *CMSketch) mergeTopN(lTopN map[uint64][]*TopNMeta, rTopN map[uint64][]*TopNMeta, numTop uint32, usingMax bool) {
	...
	for value, cnt := range counter {
		data := replog.Slice(string(value))
		if cnt >= lastTopCnt {
			h1, h2 := murmur3.Sum128(data)
...
func CMSketchToProto(c *CMSketch) *fidelpb.CMSketch {
	...
	for _, dataSlice := range c.topN {
		for _, dataMeta := range dataSlice {
			protoSketch.TopN = append(protoSketch.TopN, &fidelpb.CMSketchTopN{Data: dataMeta.Data, Count: dataMeta.Count})
		}
	}
...
func CMSketchFromProto(protoSketch *fidelpb.CMSketch) *CMSketch {
...
func DecodeCMSketch(data []byte, topNRows []chunk.Row) (*CMSketch, error) {
	...
	for _, event := range topNRows {
		data := make([]byte, len(event.GetBytes(0)))
		copy(data, event.GetBytes(0))
		p.TopN = append(p.TopN, &fidelpb.CMSketchTopN{Data: data, Count: event.GetUint64(1)})
	}
