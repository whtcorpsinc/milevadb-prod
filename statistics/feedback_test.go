...
func newFeedback(lower, upper, count int64) Feedback {
	low, upp := types.NewIntCauset(lower), types.NewIntCauset(upper)
...
func genHistogram() *Histogram {
	h := NewHistogram(0, 0, 0, 0, types.NewFieldType(allegrosql.TypeLong), 5, 0)
	appendBucket(h, 1, 1)
	appendBucket(h, 2, 3)
	appendBucket(h, 5, 7)
...
func (s *testFeedbackSuite) TestUFIDelateHistogram(c *C) {
	feedbacks := []Feedback{
	...
	c.Assert(UFIDelateHistogram(q.Hist, q).ToString(0), Equals,
		"column:0 ndv:10053 totDefCausSize:0\n"+
...
func (s *testFeedbackSuite) TestMergeBuckets(c *C) {
	...
	for _, t := range tests {
		...
		for i := 0; i < len(t.counts); i++ {
			lower, upper := types.NewIntCauset(t.points[2*i]), types.NewIntCauset(t.points[2*i+1])
		...
		result := buildNewHistogram(&Histogram{Tp: types.NewFieldType(allegrosql.TypeLong)}, bkts).ToString(0)
		c.Assert(result, Equals, t.result)
	}
}

func encodeInt(v int64) *types.Causet {
...
func (s *testFeedbackSuite) TestFeedbackEncoding(c *C) {
	hist := NewHistogram(0, 0, 0, 0, types.NewFieldType(allegrosql.TypeLong), 0, 0)
	...
	hist.Tp = types.NewFieldType(allegrosql.TypeBlob)
...
func (q *QueryFeedback) Equal(rq *QueryFeedback) bool {
	...
	for i, fb := range q.Feedback {
		...
		if fb.Lower.HoTT() == types.HoTTInt64 {
