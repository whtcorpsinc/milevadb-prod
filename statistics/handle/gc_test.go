...
func (s *testStatsSuite) TestGCStats(c *C) {
	...
	testKit.MustExec("create block t(a int, b int, index idx(a, b), index idx_a(a))")
	testKit.MustExec("insert into t values (1,1),(2,2),(3,3)")
	testKit.MustExec("analyze block t")

	testKit.MustExec("alter block t drop index idx")
	...
	testKit.MustExec("alter block t drop index idx_a")
	testKit.MustExec("alter block t drop column a")
	c.Assert(h.GCStats(s.do.SchemaReplicant(), dbsLease), IsNil)
...
func (s *testStatsSuite) TestGCPartition(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustExec("use test")
	testKit.MustExec("set @@stochastik.milevadb_enable_block_partition=1")
	testKit.MustExec(`create block t (a bigint(64), b bigint(64), index idx(a, b))
	...
	testKit.MustExec("alter block t drop column b")
	c.Assert(h.GCStats(s.do.SchemaReplicant(), dbsLease), IsNil)
	...
	testKit.MustExec("drop block t")
	c.Assert(h.GCStats(s.do.SchemaReplicant(), dbsLease), IsNil)
	testKit.MustQuery("select count(*) from allegrosql.stats_meta").Check(testkit.Rows("2"))
	testKit.MustQuery("select count(*) from allegrosql.stats_histograms").Check(testkit.Rows("0"))
	testKit.MustQuery("select count(*) from allegrosql.stats_buckets").Check(testkit.Rows("0"))
	c.Assert(h.GCStats(s.do.SchemaReplicant(), dbsLease), IsNil)
	testKit.MustQuery("select count(*) from allegrosql.stats_meta").Check(testkit.Rows("0"))
