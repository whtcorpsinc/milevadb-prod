...
func (c *CMSketch) insert(val *types.Causet) error {
...
func prepareCMSWithTopN(d, w int32, vals []*types.Causet, n uint32, total uint64) (*CMSketch, error) {
...
func buildCMSketchAndMapWithOffset(d, w int32, seed int64, total, imax uint64, s float64, offset int64) (*CMSketch, map[int64]uint32, error) {
	...
	for i := uint64(0); i < total; i++ {
		val := types.NewIntCauset(int64(zipf.Uint64()) + offset)
		err := cms.insert(&val)
		if err != nil {
...
func buildCMSketchTopNAndMap(d, w, n, sample int32, seed int64, total, imax uint64, s float64) (*CMSketch, map[int64]uint32, error) {
	mp := make(map[int64]uint32)
	zipf := rand.NewZipf(rand.New(rand.NewSource(seed)), s, 1, imax)
	vals := make([]*types.Causet, 0)
	for i := uint64(0); i < total; i++ {
		val := types.NewIntCauset(int64(zipf.Uint64()))
		mp[val.GetInt64()]++
		if i < uint64(sample) {
...
func averageAbsoluteError(cms *CMSketch, mp map[int64]uint32) (uint64, error) {
	...
	for num, count := range mp {
		estimate, err := cms.queryValue(sc, types.NewIntCauset(num))
		if err != nil {
...
func (s *testStatisticsSuite) TestCMSketchTopNUniqueData(c *C) {
	...
	vals := make([]*types.Causet, 0)
	for i := uint64(0); i < total; i++ {
		val := types.NewIntCauset(int64(i))
		mp[val.GetInt64()]++
		if i < uint64(1000) {
			vals = append(vals, &val)
		}
...
func (s *testStatisticsSuite) TestCMSketchCodingTopN(c *C) {
	...
	unsignedLong := types.NewFieldType(allegrosql.TypeLonglong)
	unsignedLong.Flag |= allegrosql.UnsignedFlag
	chk := chunk.New([]*types.FieldType{types.NewFieldType(allegrosql.TypeBlob), unsignedLong}, 20, 20)
	var rows []chunk.Row
	for i := 0; i < 20; i++ {
