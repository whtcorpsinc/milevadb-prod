...
func (d Driver) Open(path string) (ekv.CausetStorage, error) {
	...
	s, err := newEinsteinDBStore(uuid, &codecFIDelClient{FIDelCli}, spkv, newRPCClient(security), !disableGC, coprCacheConfig)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func newEinsteinDBStore(uuid string, FIDelClient fidel.Client, spkv SafePointKV, client Client, enableGC bool, coprCacheConfig *config.CoprocessorCache) (*einsteindbStore, error) {
