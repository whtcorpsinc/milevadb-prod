...
const (
	...
	testStochastikStatus = "test_stochastik_status"
	testStatusVal     = "test_status_val"
)

var specificStatusScopes = map[string]ScopeFlag{
	testStochastikStatus: ScopeStochastik,
}
...
func (ms *mockStatistics) Stats(vars *StochastikVars) (map[string]interface{}, error) {
...
func (s *testStatusVarSuite) TestStatusVar(c *C) {
	...
	scope = s.ms.GetScope(testStochastikStatus)
	c.Assert(scope, Equals, ScopeStochastik)
