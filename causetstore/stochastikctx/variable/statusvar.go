...
var DefaultStatusVarScopeFlag = ScopeGlobal | ScopeStochastik
...
type Statistics interface {
	...
	Stats(*StochastikVars) (map[string]interface{}, error)
}
...
func GetStatusVars(vars *StochastikVars) (map[string]*StatusVal, error) {
...
var defaultStatus = map[string]*StatusVal{
	"Ssl_cipher":      {ScopeGlobal | ScopeStochastik, ""},
	"Ssl_cipher_list": {ScopeGlobal | ScopeStochastik, ""},
	"Ssl_verify_mode": {ScopeGlobal | ScopeStochastik, 0},
	"Ssl_version":     {ScopeGlobal | ScopeStochastik, ""},
}
...
func (s defaultStatusStat) Stats(vars *StochastikVars) (map[string]interface{}, error) {
	...
	if vars != nil && vars.TLSConnectionState != nil {
		statusVars["Ssl_cipher"] = soliton.TLSCipher2String(vars.TLSConnectionState.CipherSuite)
...
func init() {
	...
	for _, v := range tlsCiphers {
		ciphersBuffer.WriteString(soliton.TLSCipher2String(v))
