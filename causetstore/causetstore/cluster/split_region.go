...
func (s *einsteindbStore) splitBatchRegionsReq(bo *Backoffer, keys [][]byte, scatter bool, blockID *int64) (*einsteindbrpc.Response, error) {
	...
	srResp := &ekvrpcpb.SplitRegionResponse{Regions: make([]*spacetimepb.Region, 0, len(keys)*2)}
	for i := 0; i < len(batches); i++ {
		...
		if batchResp.resp != nil {
			spResp := batchResp.resp.Resp.(*ekvrpcpb.SplitRegionResponse)
...
func (s *einsteindbStore) batchSendSingleRegion(bo *Backoffer, batch batch, scatter bool, blockID *int64) singleBatchResp {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdSplitRegion, &ekvrpcpb.SplitRegionRequest{
		SplitKeys: batch.keys,
	}, ekvrpcpb.Context{
		Priority: ekvrpcpb.CommandPri_Normal,
	})
	...
	spResp := resp.Resp.(*ekvrpcpb.SplitRegionResponse)
	regions := spResp.GetRegions()
	if len(regions) > 0 {
