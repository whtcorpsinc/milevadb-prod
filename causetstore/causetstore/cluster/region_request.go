...
func (ss *RegionBatchRequestSender) sendStreamReqToAddr(bo *Backoffer, ctxs []copTaskAndRPCContext, req *einsteindbrpc.Request, timout time.Duration) (resp *einsteindbrpc.Response, retry bool, cancel func(), err error) {
		...
		case "GCNotLeader":
			if req.Type == einsteindbrpc.CmdGC {
				failpoint.Return(&einsteindbrpc.Response{
					Resp: &ekvrpcpb.GCResponse{RegionError: &errorpb.Error{NotLeader: &errorpb.NotLeader{}}},
				}, nil, nil)
			}
		case "GCServerIsBusy":
			if req.Type == einsteindbrpc.CmdGC {
				failpoint.Return(&einsteindbrpc.Response{
					Resp: &ekvrpcpb.GCResponse{RegionError: &errorpb.Error{ServerIsBusy: &errorpb.ServerIsBusy{}}},
				}, nil, nil)
			}
...
func (s *RegionRequestSender) sendReqToRegion(bo *Backoffer, rpcCtx *RPCContext, req *einsteindbrpc.Request, timeout time.Duration) (resp *einsteindbrpc.Response, retry bool, err error) {
	...
	if s.Stats != nil {
		...
		failpoint.Inject("einsteindbStoreRespResult", func(val failpoint.Value) {
			if val.(bool) {
				if req.Type == einsteindbrpc.CmdCop && bo.totalSleep == 0 {
					failpoint.Return(&einsteindbrpc.Response{
...
func pbIsolationLevel(level ekv.IsoLevel) ekvrpcpb.IsolationLevel {
	switch level {
	case ekv.RC:
		return ekvrpcpb.IsolationLevel_RC
	case ekv.SI:
		return ekvrpcpb.IsolationLevel_SI
	default:
		return ekvrpcpb.IsolationLevel_SI
	}
}
