...
func (l *mvccLock) check(ts uint64, key []byte, resolvedLocks []uint64) (uint64, error) {
	...
	// Pessimistic dagger doesn't causet read.
	if l.startTS > ts || l.op == ekvrpcpb.Op_Lock || l.op == ekvrpcpb.Op_PessimisticLock {
		return ts, nil
	}
...
func (e *mvccEntry) Get(ts uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error) {
	if isoLevel == ekvrpcpb.IsolationLevel_SI && e.dagger != nil {
		var err error
...
type MVCCStore interface {
	Get(key []byte, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error)
	Scan(startKey, endKey []byte, limit int, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) []Pair
	ReverseScan(startKey, endKey []byte, limit int, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) []Pair
	BatchGet(ks [][]byte, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) []Pair
	PessimisticLock(req *ekvrpcpb.PessimisticLockRequest) *ekvrpcpb.PessimisticLockResponse
	PessimisticRollback(keys [][]byte, startTS, forUFIDelateTS uint64) []error
	Prewrite(req *ekvrpcpb.PrewriteRequest) []error
	...
	ScanLock(startKey, endKey []byte, maxTS uint64) ([]*ekvrpcpb.LockInfo, error)
	...
	CheckTxnStatus(primaryKey []byte, lockTS uint64, startTS, currentTS uint64, rollbackIfNotFound bool) (uint64, uint64, ekvrpcpb.CausetAction, error)
	Close() error
}
...
type MVCCDebugger interface {
	MvccGetByStartTS(starTS uint64) (*ekvrpcpb.MvccInfo, []byte)
	MvccGetByKey(key []byte) *ekvrpcpb.MvccInfo
}
