...
func putMutations(ekvpairs ...string) []*ekvrpcpb.Mutation {
	var mutations []*ekvrpcpb.Mutation
	for i := 0; i < len(ekvpairs); i += 2 {
		mutations = append(mutations, &ekvrpcpb.Mutation{
			Op:    ekvrpcpb.Op_Put,
			Key:   []byte(ekvpairs[i]),
			Value: []byte(ekvpairs[i+1]),
		})
	}
	return mutations
}

func dagger(key, primary string, ts uint64) *ekvrpcpb.LockInfo {
	return &ekvrpcpb.LockInfo{
		Key:         []byte(key),
...
func (s *testMockEinsteinDBSuite) mustGetNone(c *C, key string, ts uint64) {
	val, err := s.causetstore.Get([]byte(key), ts, ekvrpcpb.IsolationLevel_SI, nil)
	c.Assert(err, IsNil)
	c.Assert(val, IsNil)
}
...
func (s *testMockEinsteinDBSuite) mustGetRC(c *C, key string, ts uint64, expect string) {
	val, err := s.causetstore.Get([]byte(key), ts, ekvrpcpb.IsolationLevel_RC, nil)
	c.Assert(err, IsNil)
	c.Assert(string(val), Equals, expect)
}
...
func (s *testMockEinsteinDBSuite) mustRangeScanOK(c *C, start, end string, limit int, ts uint64, expect ...string) {
	pairs := s.causetstore.Scan([]byte(start), []byte(end), limit, ts, ekvrpcpb.IsolationLevel_SI, nil)
...
func (s *testMockEinsteinDBSuite) mustRangeReverseScanOK(c *C, start, end string, limit int, ts uint64, expect ...string) {
	pairs := s.causetstore.ReverseScan([]byte(start), []byte(end), limit, ts, ekvrpcpb.IsolationLevel_SI, nil)
	c.Assert(len(pairs)*2, Equals, len(expect))
	for i := 0; i < len(pairs); i++ {
		c.Assert(pairs[i].Err, IsNil)
		c.Assert(pairs[i].Key, BytesEquals, []byte(expect[i*2]))
...
func MustPrewriteOK(c *C, causetstore MVCCStore, mutations []*ekvrpcpb.Mutation, primary string, startTS uint64, ttl uint64) {
	s := testMockEinsteinDBSuite{causetstore}
	s.mustPrewriteWithTTLOK(c, mutations, primary, startTS, ttl)
}

func (s *testMockEinsteinDBSuite) mustPrewriteOK(c *C, mutations []*ekvrpcpb.Mutation, primary string, startTS uint64) {
	s.mustPrewriteWithTTLOK(c, mutations, primary, startTS, 0)
}

func (s *testMockEinsteinDBSuite) mustPrewriteWithTTLOK(c *C, mutations []*ekvrpcpb.Mutation, primary string, startTS uint64, ttl uint64) {
	req := &ekvrpcpb.PrewriteRequest{
		Mutations:    mutations,
		PrimaryLock:  []byte(primary),
		StartVersion: startTS,
...
func (s *testMockEinsteinDBSuite) mustScanLock(c *C, maxTs uint64, expect []*ekvrpcpb.LockInfo) {
...
func (s *testMockEinsteinDBSuite) TestBatchGet(c *C) {
	...
	pairs := s.causetstore.BatchGet(batchKeys, 5, ekvrpcpb.IsolationLevel_SI, nil)
...
func (s *testMockEinsteinDBSuite) TestScanLock(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(locks, DeepEquals, []*ekvrpcpb.LockInfo{
		dagger("p1", "p1", 5),
		dagger("p2", "p2", 10),
	})

	s.mustScanLock(c, 10, []*ekvrpcpb.LockInfo{
		dagger("p1", "p1", 5),
		dagger("p2", "p2", 10),
		dagger("s1", "p1", 5),
		dagger("s2", "p2", 10),
...
func (s *testMockEinsteinDBSuite) TestScanWithResolvedLock(c *C) {
	...
	pairs := s.causetstore.Scan([]byte("p1"), nil, 3, 10, ekvrpcpb.IsolationLevel_SI, nil)
	...
	pairs = s.causetstore.Scan([]byte("p1"), nil, 3, 10, ekvrpcpb.IsolationLevel_SI, []uint64{dagger.StartTS})
	for _, pair := range pairs {
		c.Assert(pair.Err, IsNil)
	}
}
...
func (s *testMockEinsteinDBSuite) TestBatchResolveLock(c *C) {
	...
	s.mustScanLock(c, 30, []*ekvrpcpb.LockInfo{
		dagger("p5", "p5", 15),
		dagger("s5", "p5", 15),
	})
	txnInfos = map[uint64]uint64{
...
func (s *testMVCCLevelDB) TestCheckTxnStatus(c *C) {
	...
	c.Assert(action, Equals, ekvrpcpb.CausetAction_MinCommitTSPushed)
	...
	c.Assert(ttl, Equals, uint64(0))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, ekvrpcpb.CausetAction_NoCausetAction)
	...
	c.Assert(ttl, Equals, uint64(0))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, ekvrpcpb.CausetAction_TTLExpireRollback)
	...
	c.Assert(ttl, Equals, uint64(0))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, ekvrpcpb.CausetAction_LockNotExistRollback)
...
func (s *testMVCCLevelDB) TestMvccGetByKey(c *C) {
	...
	except := &ekvrpcpb.MvccInfo{
		Lock: &ekvrpcpb.MvccLock{
			Type:       ekvrpcpb.Op_Put,
			StartTs:    5,
			Primary:    []byte("p1"),
			ShortValue: []byte("v5"),
		},
