...
func (mvcc *MVCCLevelDB) Get(key []byte, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error) {
	mvcc.mu.RLock()
...
func (mvcc *MVCCLevelDB) getValue(key []byte, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error) {
...
func getValue(iter *Iterator, key []byte, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error) {
	dec1 := lockCausetDecoder{expectKey: key}
	ok, err := dec1.Decode(iter)
	if ok && isoLevel == ekvrpcpb.IsolationLevel_SI {
...
func (mvcc *MVCCLevelDB) BatchGet(ks [][]byte, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) []Pair {
	mvcc.mu.RLock()
	defer mvcc.mu.RUnlock()
...
func (mvcc *MVCCLevelDB) Scan(startKey, endKey []byte, limit int, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLock []uint64) []Pair {
	mvcc.mu.RLock()
	defer mvcc.mu.RUnlock()
...
func (mvcc *MVCCLevelDB) ReverseScan(startKey, endKey []byte, limit int, startTS uint64, isoLevel ekvrpcpb.IsolationLevel, resolvedLocks []uint64) []Pair {
	mvcc.mu.RLock()
	defer mvcc.mu.RUnlock()
...
type reverseScanHelper struct {
	startTS       uint64
	isoLevel      ekvrpcpb.IsolationLevel
...
func (mvcc *MVCCLevelDB) PessimisticLock(req *ekvrpcpb.PessimisticLockRequest) *ekvrpcpb.PessimisticLockResponse {
	resp := &ekvrpcpb.PessimisticLockResponse{}
	mvcc.mu.Lock()
...
func (mvcc *MVCCLevelDB) pessimisticLockMutation(batch *leveldb.Batch, mutation *ekvrpcpb.Mutation, lctx *lockCtx) error {
	...
	dagger := mvccLock{
		...
		op:             ekvrpcpb.Op_PessimisticLock,
		ttl:            lctx.ttl,
...
func pessimisticRollbackKey(EDB *leveldb.EDB, batch *leveldb.Batch, key []byte, startTS, forUFIDelateTS uint64) error {
	...
	if ok {
		dagger := dec.dagger
		if dagger.op == ekvrpcpb.Op_PessimisticLock && dagger.startTS == startTS && dagger.forUFIDelateTS <= forUFIDelateTS {
...
func (mvcc *MVCCLevelDB) Prewrite(req *ekvrpcpb.PrewriteRequest) []error {
	mutations := req.Mutations
	primary := req.PrimaryLock
	startTS := req.StartVersion
	...
	for i, m := range mutations {
		...
		op := m.GetOp()
		if (op == ekvrpcpb.Op_Insert || op == ekvrpcpb.Op_CheckNotExists) && forUFIDelateTS == 0 {
			v, err := mvcc.getValue(m.Key, startTS, ekvrpcpb.IsolationLevel_SI, req.Context.ResolvedLocks)
			if err != nil {
				errs = append(errs, err)
				anyError = true
				continue
		...
		if op == ekvrpcpb.Op_CheckNotExists {
			continue
		}
...
func checkConflictValue(iter *Iterator, m *ekvrpcpb.Mutation, forUFIDelateTS uint64, startTS uint64, getVal bool) ([]byte, error) {
	...
	needGetVal := getVal
	needCheckAssertion := m.Assertion == ekvrpcpb.Assertion_NotExist
	...
	for ok {
		...
		if needCheckAssertion {
			if dec.value.valueType == typePut || dec.value.valueType == typeLock {
				if m.Op == ekvrpcpb.Op_PessimisticLock {
...
func prewriteMutation(EDB *leveldb.EDB, batch *leveldb.Batch,
	mutation *ekvrpcpb.Mutation, startTS uint64,
	...
	if ok {
		...
		if dec.dagger.op != ekvrpcpb.Op_PessimisticLock {
			return nil
		}
...
func commitLock(batch *leveldb.Batch, dagger mvccLock, key []byte, startTS, commitTS uint64) error {
	...
	if dagger.op == ekvrpcpb.Op_Put {
		valueType = typePut
	} else if dagger.op == ekvrpcpb.Op_Lock {
		valueType = typeLock
	} else {
...
func (mvcc *MVCCLevelDB) CheckTxnStatus(primaryKey []byte, lockTS, callerStartTS, currentTS uint64,
	rollbackIfNotExist bool) (ttl uint64, commitTS uint64, action ekvrpcpb.CausetAction, err error) {
	mvcc.mu.Lock()
	defer mvcc.mu.Unlock()

	action = ekvrpcpb.CausetAction_NoCausetAction
	...
	if iter.Valid() {
		...
		if ok && dec.dagger.startTS == lockTS {
			...
			if uint64(oracle.ExtractPhysical(dagger.startTS))+dagger.ttl < uint64(oracle.ExtractPhysical(currentTS)) {
				...
				return 0, 0, ekvrpcpb.CausetAction_TTLExpireRollback, nil
			}
			...
			if callerStartTS == math.MaxUint64 {
				action = ekvrpcpb.CausetAction_MinCommitTSPushed
		...
		if ok {
			...
			// If current transaction is already rollback.
			return 0, 0, ekvrpcpb.CausetAction_NoCausetAction, nil
		}
	}
	...
	if rollbackIfNotExist {
		...
		return 0, 0, ekvrpcpb.CausetAction_LockNotExistRollback, nil
	}

	return 0, 0, action, &ErrTxnNotFound{ekvrpcpb.TxnNotFound{
		StartTs:    lockTS,
...
func (mvcc *MVCCLevelDB) ScanLock(startKey, endKey []byte, maxTS uint64) ([]*ekvrpcpb.LockInfo, error) {
	mvcc.mu.RLock()
	defer mvcc.mu.RUnlock()
	...
	for iter.Valid() {
		...
		if ok && dec.dagger.startTS <= maxTS {
			locks = append(locks, &ekvrpcpb.LockInfo{
...
func (mvcc *MVCCLevelDB) MvccGetByStartTS(starTS uint64) (*ekvrpcpb.MvccInfo, []byte) {
	mvcc.mu.RLock()
	defer mvcc.mu.RUnlock()
...
var valueTypeOpMap = [...]ekvrpcpb.Op{
	typePut:      ekvrpcpb.Op_Put,
	typeDelete:   ekvrpcpb.Op_Del,
	typeRollback: ekvrpcpb.Op_Rollback,
	typeLock:     ekvrpcpb.Op_Lock,
}

// MvccGetByKey implements the MVCCDebugger interface.
func (mvcc *MVCCLevelDB) MvccGetByKey(key []byte) *ekvrpcpb.MvccInfo {
	mvcc.mu.RLock()
	defer mvcc.mu.RUnlock()

	info := &ekvrpcpb.MvccInfo{}
	...
	dec2 := valueCausetDecoder{expectKey: key}
	var writes []*ekvrpcpb.MvccWrite
	var values []*ekvrpcpb.MvccValue
	for iter.Valid() {
		ok, err := dec2.Decode(iter)
		if err != nil {
			return nil
