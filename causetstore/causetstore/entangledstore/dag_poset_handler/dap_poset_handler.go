...
func buildResp(chunks []fidelpb.Chunk, counts []int64, posetPosetDagReq *fidelpb.PosetDagRequest, err error, warnings []stmtctx.ALLEGROSQLWarn, dur time.Duration) *interlock.Response {
	...
	resp.InterDircDetails = &ekvrpcpb.InterDircDetails{
		HandleTime: &ekvrpcpb.HandleTime{ProcessMs: int64(dur / time.Millisecond)},
	}
...
func extractKVRanges(startKey, endKey []byte, keyRanges []*interlock.KeyRange, descScan bool) (ekvRanges []ekv.KeyRange, err error) {
	ekvRanges = make([]ekv.KeyRange, 0, len(keyRanges))
