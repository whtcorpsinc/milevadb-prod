...
func (w *GCWorker) calSafePointByMinStartTS(ctx context.Context, safePoint time.Time) time.Time {
	ekvs, err := w.causetstore.GetSafePointKV().GetWithPrefix(infosync.ServerMinStartTSPath)
	if err != nil {
...
func (w *GCWorker) doUnsafeDestroyRangeRequest(ctx context.Context, startKey []byte, endKey []byte, concurrency int) error {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdUnsafeDestroyRange, &ekvrpcpb.UnsafeDestroyRangeRequest{
		StartKey: startKey,
		EndKey:   endKey,
	})
		...
		go func() {
			...
			if err1 == nil {
				if resp == nil || resp.Resp == nil {
					...
					errStr := (resp.Resp.(*ekvrpcpb.UnsafeDestroyRangeResponse)).Error
					if len(errStr) > 0 {
...
func (w *GCWorker) resolveLocksForRange(ctx context.Context, safePoint uint64, startKey []byte, endKey []byte) (einsteindb.RangeTaskStat, error) {
	...
	// cleaned.
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdScanLock, &ekvrpcpb.ScanLockRequest{
...
retryScanAndResolve:
	for {
		...
		locksResp := resp.Resp.(*ekvrpcpb.ScanLockResponse)
...
func (w *GCWorker) registerLockObservers(ctx context.Context, safePoint uint64, stores map[uint64]*spacetimepb.CausetStore) error {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRegisterLockObserver, &ekvrpcpb.RegisterLockObserverRequest{
		MaxTs: safePoint,
	})
	...
	for _, causetstore := range stores {
		...
		errStr := resp.Resp.(*ekvrpcpb.RegisterLockObserverResponse).Error
		if len(errStr) > 0 {
...
func (w *GCWorker) checkLockObservers(ctx context.Context, safePoint uint64, stores map[uint64]*spacetimepb.CausetStore) (map[uint64]interface{}, error) {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdCheckLockObserver, &ekvrpcpb.CheckLockObserverRequest{
		MaxTs: safePoint,
	})
	...
	for _, causetstore := range stores {
		...
		respInner := resp.Resp.(*ekvrpcpb.CheckLockObserverResponse)
...
func (w *GCWorker) removeLockObservers(ctx context.Context, safePoint uint64, stores map[uint64]*spacetimepb.CausetStore) {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRemoveLockObserver, &ekvrpcpb.RemoveLockObserverRequest{
		MaxTs: safePoint,
	})
	...
	for _, causetstore := range stores {
		...
		errStr := resp.Resp.(*ekvrpcpb.RemoveLockObserverResponse).Error
		if len(errStr) > 0 {
...
func (w *GCWorker) doGCForRegion(bo *einsteindb.Backoffer, safePoint uint64, region einsteindb.RegionVerID) (*errorpb.Error, error) {
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdGC, &ekvrpcpb.GCRequest{
		SafePoint: safePoint,
	})
	...
	gcResp := resp.Resp.(*ekvrpcpb.GCResponse)
...
func (s *mergeLockScanner) physicalScanLocksForStore(ctx context.Context, safePoint uint64, causetstore *spacetimepb.CausetStore, lockCh chan<- scanLockResult) error {
	address := causetstore.Address
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdPhysicalScanLock, &ekvrpcpb.PhysicalScanLockRequest{
		MaxTs: safePoint,
	...
	for {
		...
		resp := response.Resp.(*ekvrpcpb.PhysicalScanLockResponse)
		if len(resp.Error) > 0 {
