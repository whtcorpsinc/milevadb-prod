...
// Package einsteindb provides tcp connection to ekvserver.
package einsteindb
...
func (c *rpcClient) SendRequest(ctx context.Context, addr string, req *einsteindbrpc.Request, timeout time.Duration) (*einsteindbrpc.Response, error) {
	...
	if req.Type == einsteindbrpc.CmdBatchCop {
		return c.getBatchCopStreamResponse(ctx, client, req, timeout, connArray)
	}

	if req.Type == einsteindbrpc.CmdCopStream {
...
func (c *rpcClient) getCopStreamResponse(ctx context.Context, client einsteindbpb.EinsteinDBClient, req *einsteindbrpc.Request, timeout time.Duration, connArray *connArray) (*einsteindbrpc.Response, error) {
	...
	copStream := resp.Resp.(*einsteindbrpc.CopStreamResponse)
...
func (c *rpcClient) getBatchCopStreamResponse(ctx context.Context, client einsteindbpb.EinsteinDBClient, req *einsteindbrpc.Request, timeout time.Duration, connArray *connArray) (*einsteindbrpc.Response, error) {
	// Coprocessor streaming request.
	// Use context to support timeout for grpc streaming client.
	ctx1, cancel := context.WithCancel(ctx)
	// Should NOT call defer cancel() here because it will cancel further stream.Recv()
	...
	// Put the lease object to the timeout channel, so it would be checked periodically.
	copStream := resp.Resp.(*einsteindbrpc.BatchCopStreamResponse)
	copStream.Timeout = timeout
	copStream.Lease.Cancel = cancel
	connArray.streamTimeout <- &copStream.Lease
