...
func (s *Scanner) resolveCurrentLock(bo *Backoffer, current *pb.EkvPair) error {
...
func (s *Scanner) getData(bo *Backoffer) error {
	...
	for {
		...
		ekvPairs := cmdScanResp.Pairs
		// Check if ekvPair contains error, it should be a Lock.
		for _, pair := range ekvPairs {
		...
		s.cache, s.idx = ekvPairs, 0
		if len(ekvPairs) < s.batchSize {
