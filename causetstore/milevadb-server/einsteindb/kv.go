...
func (d Driver) Open(path string) (ekv.CausetStorage, error) {
	...
	coprCacheConfig := &config.GetGlobalConfig().EinsteinDBClient.CoprCache
	s, err := newEinsteinDBStore(uuid, &codecFIDelClient{FIDelCli}, spekv, newRPCClient(security), !disableGC, coprCacheConfig)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func newEinsteinDBStore(uuid string, FIDelClient fidel.Client, spekv SafePointKV, client Client, enableGC bool, coprCacheConfig *config.CoprocessorCache) (*einsteindbStore, error) {
	...
	causetstore.coprCache = coprCache
