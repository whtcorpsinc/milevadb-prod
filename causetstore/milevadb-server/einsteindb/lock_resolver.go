...
func NewLockResolver(etcdAddrs []string, security config.Security, opts ...fidel.ClientOption) (*LockResolver, error) {
	...
	spekv, err := NewEtcdSafePointKV(etcdAddrs, tlsConfig)
	if err != nil {
		return nil, errors.Trace(err)
	}

	s, err := newEinsteinDBStore(uuid, &codecFIDelClient{FIDelCli}, spekv, newRPCClient(security), false, nil)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
type TxnStatus struct {
	ttl         uint64
	commitTS    uint64
	action      ekvrpcpb.CausetAction
	primaryLock *ekvrpcpb.LockInfo
}
...
func (s TxnStatus) CausetAction() ekvrpcpb.CausetAction { return s.action }
...
func NewLock(l *ekvrpcpb.LockInfo) *Lock {
	return &Lock{
		Key:                l.GetKey(),
...
func (lr *LockResolver) BatchResolveLocks(bo *Backoffer, locks []*Lock, loc RegionVerID) (bool, error) {
	...
	listTxnInfos := make([]*ekvrpcpb.TxnInfo, 0, len(txnInfos))
	for txnID, status := range txnInfos {
		listTxnInfos = append(listTxnInfos, &ekvrpcpb.TxnInfo{
			Txn:    txnID,
			Status: status,
		})
	}

	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdResolveLock, &ekvrpcpb.ResolveLockRequest{TxnInfos: listTxnInfos})
	startTime = time.Now()
	...
	cmdResp := resp.Resp.(*ekvrpcpb.ResolveLockResponse)
...
func (lr *LockResolver) resolveLocks(bo *Backoffer, callerStartTS uint64, locks []*Lock, forWrite bool, lite bool) (int64, []uint64 /*pushed*/, error) {
	...
	for _, l := range locks {
		...
		if status.ttl == 0 {
			...
			} else if l.LockType == ekvrpcpb.Op_PessimisticLock {
			...
			if forWrite {
				...
				if l.LockType != ekvrpcpb.Op_PessimisticLock && l.TxnID > callerStartTS {
				...
				if status.action != ekvrpcpb.CausetAction_MinCommitTSPushed {
					pushFail = true
					continue
				}
...
func (lr *LockResolver) getTxnStatusFromLock(bo *Backoffer, l *Lock, callerStartTS uint64) (TxnStatus, error) {
	...
	for {
		...
		failpoint.Inject("txnNotFoundRetTTL", func() {
			failpoint.Return(TxnStatus{ttl: l.TTL, action: ekvrpcpb.CausetAction_NoCausetAction}, nil)
		})
		...
		if lr.causetstore.GetOracle().UntilExpired(l.TxnID, l.TTL) <= 0 {
			...
			if l.LockType == ekvrpcpb.Op_PessimisticLock {
				failpoint.Inject("txnExpireRetTTL", func() {
					failpoint.Return(TxnStatus{ttl: l.TTL, action: ekvrpcpb.CausetAction_NoCausetAction},
...
func (lr *LockResolver) getTxnStatus(bo *Backoffer, txnID uint64, primary []byte, callerStartTS, currentTS uint64, rollbackIfNotExist bool) (TxnStatus, error) {
	...
	var status TxnStatus
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdCheckTxnStatus, &ekvrpcpb.CheckTxnStatusRequest{
		PrimaryKey:         primary,
		LockTs:             txnID,
	...
	for {
		...
		cmdResp := resp.Resp.(*ekvrpcpb.CheckTxnStatusResponse)
		if keyErr := cmdResp.GetError(); keyErr != nil {
...
func (data *asyncResolveData) addKeys(locks []*ekvrpcpb.LockInfo, expected int, startTS uint64, commitTS uint64) error {
	data.mutex.Lock()
...
func (lr *LockResolver) checkSecondaries(bo *Backoffer, txnID uint64, curKeys [][]byte, curRegionID RegionVerID, shared *asyncResolveData) error {
	checkReq := &ekvrpcpb.CheckSecondaryLocksRequest{
		Keys:         curKeys,
		StartVersion: txnID,
	}
	...
	checkResp := resp.Resp.(*ekvrpcpb.CheckSecondaryLocksResponse)
...
func (lr *LockResolver) resolveRegionLocks(bo *Backoffer, l *Lock, region RegionVerID, keys [][]byte, status TxnStatus) error {
	lreq := &ekvrpcpb.ResolveLockRequest{
...
func (lr *LockResolver) resolvePessimisticLock(bo *Backoffer, l *Lock, cleanRegions map[RegionVerID]struct{}) error {
	...
	for {
		...
		pessimisticRollbackReq := &ekvrpcpb.PessimisticRollbackRequest{
			StartVersion:   l.TxnID,
		...
		cmdResp := resp.Resp.(*ekvrpcpb.PessimisticRollbackResponse)
