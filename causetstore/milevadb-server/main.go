...
import (
	...
	ekvstore "github.com/whtcorpsinc/milevadb/causetstore"
...
func registerStores() {
	err := ekvstore.Register("einsteindb", einsteindb.Driver{})
	terror.MustNil(err)
	einsteindb.NewGCHandlerFunc = gcworker.NewGCWorker
	err = ekvstore.Register("mockeinsteindb", mockstore.MockEinsteinDBDriver{})
	terror.MustNil(err)
	err = ekvstore.Register("entangledstore", mockstore.EmbedEntangledStoreDriver{})
	terror.MustNil(err)
}
...
func createStoreAndPetri() {
	...
	var err error
	storage, err = ekvstore.New(fullPath)
	terror.MustNil(err)
...
func reloadConfig(nc, c *config.Config) {
	...
	if nc.Performance.ServerMemoryQuota != c.Performance.ServerMemoryQuota {
		causetembedded.PreparedCausetCacheMaxMemory.CausetStore(nc.Performance.ServerMemoryQuota)
	}
	if nc.Performance.CrossJoin != c.Performance.CrossJoin {
		causetembedded.AllowCartesianProduct.CausetStore(nc.Performance.CrossJoin)
	}
	...
	if nc.PreparedCausetCache.Enabled != c.PreparedCausetCache.Enabled {
		causetembedded.SetPreparedCausetCache(nc.PreparedCausetCache.Enabled)
	}
...
func setGlobalVars() {
	...
	causetembedded.AllowCartesianProduct.CausetStore(cfg.Performance.CrossJoin)
	...
	causetembedded.SetPreparedCausetCache(config.CheckTableBeforeDrop || cfg.PreparedCausetCache.Enabled)
	if causetembedded.PreparedCausetCacheEnabled() {
		causetembedded.PreparedCausetCacheCapacity = cfg.PreparedCausetCache.Capacity
		causetembedded.PreparedCausetCacheMemoryGuardRatio = cfg.PreparedCausetCache.MemoryGuardRatio
		if causetembedded.PreparedCausetCacheMemoryGuardRatio < 0.0 || causetembedded.PreparedCausetCacheMemoryGuardRatio > 1.0 {
			causetembedded.PreparedCausetCacheMemoryGuardRatio = 0.1
		}
		causetembedded.PreparedCausetCacheMaxMemory.CausetStore(cfg.Performance.ServerMemoryQuota)
		total, err := memory.MemTotal()
		terror.MustNil(err)
		if causetembedded.PreparedCausetCacheMaxMemory.Load() > total || causetembedded.PreparedCausetCacheMaxMemory.Load() <= 0 {
			causetembedded.PreparedCausetCacheMaxMemory.CausetStore(total)
		}
	}
	...
	ekvcache.GlobalLRUMemUsageTracker.AttachToGlobalTracker(interlock.GlobalMemoryUsageTracker)
