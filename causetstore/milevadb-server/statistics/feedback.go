...
// DefCauslectFeedback decides whether to collect the feedback. It returns false when:
...
func DefCauslectFeedback(sc *stmtctx.StatementContext, q *QueryFeedback, numOfRanges int) bool {
	if !sc.InSelectStmt {
		return false
	}
...
func (q *QueryFeedback) DecodeToRanges(isIndex bool) ([]*ranger.Range, error) {
	...
	for _, val := range q.Feedback {
		...
		var lowVal, highVal []types.Causet
		if isIndex {
			var err error
			...
			lowVal = []types.Causet{types.NewIntCauset(lowInt)}
			highVal = []types.Causet{types.NewIntCauset(highInt)}
		}
...
func (q *QueryFeedback) DecodeIntValues() *QueryFeedback {
	...
	for _, fb := range q.Feedback {
		...
		low, high := types.NewIntCauset(lowInt), types.NewIntCauset(highInt)
		nq.Feedback = append(nq.Feedback, Feedback{Lower: &low, Upper: &high, Count: fb.Count})
	}
	return nq
}

// StoreRanges stores the ranges for uFIDelate.
...
// Actual gets the actual event count.
...
// UFIDelate uFIDelates the query feedback. `startKey` is the start scan key of the partial result, used to find
// the range for uFIDelate. `counts` is the scan counts of each range, used to uFIDelate the feedback count info.
func (q *QueryFeedback) UFIDelate(startKey ekv.Key, counts []int64) {
	...
	metrics.DistALLEGROSQLScanKeysPartialHistogram.Observe(float64(sum))
	q.actual += sum
	...
	if q.Tp == IndexType {
		startKey = blockcodec.CutIndexPrefix(startKey)
	} else {
		startKey = blockcodec.CutRowKeyPrefix(startKey)
	}
	...
	// UFIDelate the feedback count info.
...
func NonOverlappedFeedbacks(sc *stmtctx.StatementContext, fbs []Feedback) ([]Feedback, bool) {
	...
	sort.Slice(fbs, func(i, j int) bool {
		res, err := fbs[i].Upper.CompareCauset(sc, fbs[j].Upper)
		if err != nil {
			existsErr = true
		}
		if existsErr || res != 0 {
		...
		res, err = fbs[i].Lower.CompareCauset(sc, fbs[j].Lower)
		if err != nil {
			existsErr = true
		}
		return res < 0
	...
	previousEnd := &types.Causet{}
	for _, fb := range fbs {
		res, err := previousEnd.CompareCauset(sc, fb.Lower)
		if err != nil {
			return fbs, false
		}
		if res <= 0 {
...
type BucketFeedback struct {
	...
	lower    *types.Causet // The lower bound of the new bucket.
	upper    *types.Causet // The upper bound of the new bucket.
}

// outOfRange checks if the `val` is between `min` and `max`.
func outOfRange(sc *stmtctx.StatementContext, min, max, val *types.Causet) (int, error) {
	result, err := val.CompareCauset(sc, min)
	if err != nil {
		return 0, err
	}
	if result < 0 {
	...
	result, err = val.CompareCauset(sc, max)
	if err != nil {
		return 0, err
	}
	if result > 0 {
...
func (f *Feedback) adjustFeedbackBoundaries(sc *stmtctx.StatementContext, min, max *types.Causet) (bool, error) {
...
func buildBucketFeedback(h *Histogram, feedback *QueryFeedback) (map[int]*BucketFeedback, int) {
	...
	for _, fb := range feedback.Feedback {
		...
		// UFIDelate the bound if necessary.
		res, err := bkt.lower.CompareCauset(nil, fb.Lower)
		if err != nil {
			logutil.BgLogger().Debug("compare causet failed", zap.Any("value1", bkt.lower), zap.Any("value2", fb.Lower), zap.Error(err))
			continue
		}
		if res > 0 {
			bkt.lower = fb.Lower
		}
		res, err = bkt.upper.CompareCauset(nil, fb.Upper)
		if err != nil {
			logutil.BgLogger().Debug("compare causet failed", zap.Any("value1", bkt.upper), zap.Any("value2", fb.Upper), zap.Error(err))
			continue
		}
		if res < 0 {
			bkt.upper = fb.Upper
...
func (b *BucketFeedback) getBoundaries(num int) []types.Causet {
	...
	for i := 1; i < len(vals); i++ {
		cmp, err := vals[total-1].CompareCauset(nil, &vals[i])
		if err != nil {
			logutil.BgLogger().Debug("compare causet failed", zap.Any("value1", vals[total-1]), zap.Any("value2", vals[i]), zap.Error(err))
			continue
		}
		if cmp == 0 {
			continue
...
// There are only two types of causet in bucket: one is `Blob`, which is for index; the other one
...
func getOverlapFraction(fb Feedback, bkt bucket) (float64, float64) {
	datums := make([]types.Causet, 0, 4)
	datums = append(datums, *fb.Lower, *fb.Upper)
	datums = append(datums, *bkt.Lower, *bkt.Upper)
	err := types.SortCausets(nil, datums)
	if err != nil {
		return 0, 0
	}
	minValue, maxValue := &datums[0], &datums[3]
	fbLower := calcFraction4Causets(minValue, maxValue, fb.Lower)
	fbUpper := calcFraction4Causets(minValue, maxValue, fb.Upper)
	bktLower := calcFraction4Causets(minValue, maxValue, bkt.Lower)
	bktUpper := calcFraction4Causets(minValue, maxValue, bkt.Upper)
...
type bucketSembedded struct {
	id        int
	sembedded float64
}

type bucketSembeddeds []bucketSembedded

func (bs bucketSembeddeds) Len() int           { return len(bs) }
func (bs bucketSembeddeds) Swap(i, j int)      { bs[i], bs[j] = bs[j], bs[i] }
func (bs bucketSembeddeds) Less(i, j int) bool { return bs[i].sembedded < bs[j].sembedded }
...
// getBucketSembedded gets the sembedded for merge this bucket with previous one.
// TODO: We also need to consider the bucket hit count.
func getBucketSembedded(bkts []bucket, totalCount float64, id int) bucketSembedded {
	...
	err := calcFraction4Causets(low, high, mid)*(preCount+count) - preCount
	return bucketSembedded{id, math.Abs(err / (preCount + count))}
}
...
// UFIDelateHistogram uFIDelates the histogram according buckets.
func UFIDelateHistogram(h *Histogram, feedback *QueryFeedback) *Histogram {
	...
	// UFIDelate the NDV of primary key column.
...
// UFIDelateCMSketch uFIDelates the CMSketch by feedback.
func UFIDelateCMSketch(c *CMSketch, eqFeedbacks []Feedback) *CMSketch {
	if c == nil || len(eqFeedbacks) == 0 {
		return c
	}
	newCMSketch := c.Copy()
	for _, fb := range eqFeedbacks {
		newCMSketch.uFIDelateValueBytes(fb.Lower.GetBytes(), uint64(fb.Count))
	}
	return newCMSketch
}

func buildNewHistogram(h *Histogram, buckets []bucket) *Histogram {
	hist := NewHistogram(h.ID, h.NDV, h.NullCount, h.LastUFIDelateVersion, h.Tp, len(buckets), h.TotDefCausSize)
	preCount := int64(0)
	for _, bkt := range buckets {
...
type queryFeedback struct {
	...
	IndexPoints [][]byte
	// Counts is the number of scan keys in each range. It first stores the count for `IntRanges`, `IndexRanges` or `DeferredCausetRanges`.
	// After that, it stores the Ranges for `HashValues`.
	Counts               []int64
	DeferredCausetRanges [][]byte
}

func encodePKFeedback(q *QueryFeedback) (*queryFeedback, error) {
	pb := &queryFeedback{}
	for _, fb := range q.Feedback {
		// There is no need to uFIDelate the point queries.
		if bytes.Compare(ekv.Key(fb.Lower.GetBytes()).PrefixNext(), fb.Upper.GetBytes()) >= 0 {
			continue
		}
...
func encodeDeferredCausetFeedback(q *QueryFeedback) (*queryFeedback, error) {
	pb := &queryFeedback{}
	sc := stmtctx.StatementContext{TimeZone: time.UTC}
	for _, fb := range q.Feedback {
		...
		pb.DeferredCausetRanges = append(pb.DeferredCausetRanges, lowerBytes, upperBytes)
		pb.Counts = append(pb.Counts, fb.Count)
	}
	return pb, nil
}
...
func EncodeFeedback(q *QueryFeedback) ([]byte, error) {
	...
	case DefCausType:
		pb, err = encodeDeferredCausetFeedback(q)
	}
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func decodeFeedbackForIndex(q *QueryFeedback, pb *queryFeedback, c *CMSketch) {
	...
	for i := 0; i < len(pb.IndexRanges); i += 2 {
		lower, upper := types.NewBytesCauset(pb.IndexRanges[i]), types.NewBytesCauset(pb.IndexRanges[i+1])
	...
	if c != nil {
		...
		if len(pb.HashValues) > 0 {
			// It needs raw values to uFIDelate the top n, so just skip it here.
			if len(c.topN) > 0 {
				return
			}
		...
		for i := 0; i < len(pb.IndexPoints); i++ {
			c.uFIDelateValueBytes(pb.IndexPoints[i], uint64(pb.Counts[start+i]))
		}
	}
}
...
// ConvertCausetsType converts the datums type to `ft`.
func ConvertCausetsType(vals []types.Causet, ft *types.FieldType, loc *time.Location) error {
	for i, val := range vals {
		if val.HoTT() == types.HoTTMinNotNull || val.HoTT() == types.HoTTMaxValue {
			continue
		}
		newVal, err := blockcodec.UnflattenCausets([]types.Causet{val}, []*types.FieldType{ft}, loc)
		if err != nil {
			return err
		}
		vals[i] = newVal[0]
...
func decodeDeferredCausetBounds(data []byte, ft *types.FieldType) ([]types.Causet, error) {
	vals, _, err := codec.DecodeRange(data, 1, nil, nil)
	if err != nil {
		return nil, err
	}
	err = ConvertCausetsType(vals, ft, time.UTC)
	return vals, err
}

func decodeFeedbackForDeferredCauset(q *QueryFeedback, pb *queryFeedback, ft *types.FieldType) error {
	q.Tp = DefCausType
	for i := 0; i < len(pb.DeferredCausetRanges); i += 2 {
		low, err := decodeDeferredCausetBounds(pb.DeferredCausetRanges[i], ft)
		if err != nil {
			return err
		}
		high, err := decodeDeferredCausetBounds(pb.DeferredCausetRanges[i+1], ft)
		if err != nil {
			return err
		}
...
func DecodeFeedback(val []byte, q *QueryFeedback, c *CMSketch, ft *types.FieldType) error {
	...
	if len(pb.IndexRanges) > 0 || len(pb.HashValues) > 0 || len(pb.IndexPoints) > 0 {
		...
		decodeFeedbackForPK(q, pb, allegrosql.HasUnsignedFlag(ft.Flag))
	} else {
		err = decodeFeedbackForDeferredCauset(q, pb, ft)
	}
	return err
}
...
// SupportDeferredCausetType checks if the type of the column can be uFIDelated by feedback.
func SupportDeferredCausetType(ft *types.FieldType) bool {
	switch ft.Tp {
	case allegrosql.TypeTiny, allegrosql.TypeShort, allegrosql.TypeInt24, allegrosql.TypeLong, allegrosql.TypeLonglong, allegrosql.TypeFloat,
		allegrosql.TypeDouble, allegrosql.TypeString, allegrosql.TypeVarString, allegrosql.TypeVarchar, allegrosql.TypeBlob, allegrosql.TypeTinyBlob, allegrosql.TypeMediumBlob, allegrosql.TypeLongBlob,
		allegrosql.TypeNewDecimal, allegrosql.TypeDuration, allegrosql.TypeDate, allegrosql.TypeDatetime, allegrosql.TypeTimestamp:
		return true
	}
	return false
}
