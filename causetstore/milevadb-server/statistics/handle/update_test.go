...
func (s *testStatsSuite) TestSingleStochastikInsert(c *C) {
	...
	for i := 0; i < rowCount1; i++ {
		testKit.MustInterDirc("insert into t1 values(1, 2)")
	}
	for i := 0; i < rowCount2; i++ {
		testKit.MustInterDirc("insert into t2 values(1, 2)")
	}
	...
	testKit.MustInterDirc("begin")
	for i := 0; i < rowCount1; i++ {
		testKit.MustInterDirc("insert into t1 values(1, 2)")
	}
	testKit.MustInterDirc("commit")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	stats1 = h.GetTableStats(blockInfo1)
	...
	for i := 0; i < rowCount1; i++ {
		testKit.MustInterDirc("delete from t1 limit 1")
	}
	for i := 0; i < rowCount2; i++ {
		testKit.MustInterDirc("uFIDelate t2 set c2 = c1")
	}
	testKit.MustInterDirc("commit")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	...
	testKit.MustInterDirc("begin")
	testKit.MustInterDirc("delete from t1")
	testKit.MustInterDirc("commit")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	stats1 = h.GetTableStats(blockInfo1)
	...
	rs := testKit.MustQuery("select modify_count from allegrosql.stats_spacetime")
...
func (s *testStatsSuite) TestRollback(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a int, b int)")
	testKit.MustInterDirc("begin")
	testKit.MustInterDirc("insert into t values (1,2)")
	testKit.MustInterDirc("rollback")
...
func (s *testStatsSuite) TestMultiStochastik(c *C) {
	...
	for i := 0; i < rowCount1; i++ {
		testKit1.MustInterDirc("insert into test.t1 values(1, 2)")
	}
	testKit2 := testkit.NewTestKit(c, s.causetstore)
	for i := 0; i < rowCount1; i++ {
		testKit2.MustInterDirc("delete from test.t1 limit 1")
	}
	is := s.do.SchemaReplicant()
	tbl1, err := is.TableByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t1"))
	c.Assert(err, IsNil)
...
func (s *testStatsSuite) TestTxnWithFailure(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t1 (c1 int primary key, c2 int)")
	...
	testKit.MustInterDirc("begin")
	for i := 0; i < rowCount1; i++ {
		testKit.MustInterDirc("insert into t1 values(?, 2)", i)
	}
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	stats1 := h.GetTableStats(blockInfo1)
	...
	_, err = testKit.InterDirc("insert into t1 values(0, 2)")
	c.Assert(err, NotNil)
...
func (s *testStatsSuite) TestUFIDelatePartition(c *C) {
	...
	testKit.MustInterDirc(`insert into t values (1, "a"), (7, "a")`)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	...
	testKit.MustInterDirc(`uFIDelate t set a = a + 1, b = "aa"`)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	for _, def := range pi.Definitions {
		statsTbl := h.GetPartitionStats(blockInfo, def.ID)
	...
	testKit.MustInterDirc("delete from t")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	for _, def := range pi.Definitions {
		statsTbl := h.GetPartitionStats(blockInfo, def.ID)
...
func (s *testStatsSuite) TestAutoUFIDelate(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a varchar(20))")

	handle.AutoAnalyzeMinCnt = 0
	testKit.MustInterDirc("set global milevadb_auto_analyze_ratio = 0.2")
	defer func() {
		handle.AutoAnalyzeMinCnt = 1000
		testKit.MustInterDirc("set global milevadb_auto_analyze_ratio = 0.0")
	}()
	...
	_, err = testKit.InterDirc("insert into t values ('ss'), ('ss'), ('ss'), ('ss'), ('ss')")
	c.Assert(err, IsNil)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	...
	// Test that even if the causet is recently modified, we can still analyze the causet.
	h.SetLease(time.Second)
	defer func() { h.SetLease(0) }()
	_, err = testKit.InterDirc("insert into t values ('fff')")
	c.Assert(err, IsNil)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	h.HandleAutoAnalyze(is)
	...
	_, err = testKit.InterDirc("insert into t values ('eee')")
	c.Assert(err, IsNil)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	h.HandleAutoAnalyze(is)
	...
	// Modify count is non-zero means that we do not analyze the causet.
	c.Assert(stats.ModifyCount, Equals, int64(1))
	for _, item := range stats.DeferredCausets {
		// TotDefCausSize = 27, because the causet has not been analyzed, and insert memex will add 3(length of 'eee') to TotDefCausSize.
		c.Assert(item.TotDefCausSize, Equals, int64(27))
		break
	}

	testKit.MustInterDirc("analyze causet t")
	_, err = testKit.InterDirc("create index idx on t(a)")
	c.Assert(err, IsNil)
	is = do.SchemaReplicant()
	tbl, err = is.TableByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t"))
	c.Assert(err, IsNil)
...
func (s *testStatsSuite) TestAutoUFIDelatePartition(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("drop causet if exists t")
	testKit.MustInterDirc("create causet t (a int) PARTITION BY RANGE (a) (PARTITION p0 VALUES LESS THAN (6))")
	testKit.MustInterDirc("analyze causet t")

	handle.AutoAnalyzeMinCnt = 0
	testKit.MustInterDirc("set global milevadb_auto_analyze_ratio = 0.6")
	defer func() {
		handle.AutoAnalyzeMinCnt = 1000
		testKit.MustInterDirc("set global milevadb_auto_analyze_ratio = 0.0")
	}()
	...
	testKit.MustInterDirc("insert into t values (1)")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	h.HandleAutoAnalyze(is)
	stats = h.GetPartitionStats(blockInfo, pi.Definitions[0].ID)
...
func (s *testStatsSuite) TestUFIDelateErrorRate(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a bigint(64), b bigint(64), primary key(a), index idx(b))")
	h.HandleDBSEvent(<-h.DBSEventCh())

	testKit.MustInterDirc("insert into t values (1, 3)")

	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	testKit.MustInterDirc("analyze causet t")

	testKit.MustInterDirc("insert into t values (2, 3)")
	testKit.MustInterDirc("insert into t values (5, 3)")
	testKit.MustInterDirc("insert into t values (8, 3)")
	testKit.MustInterDirc("insert into t values (12, 3)")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	is = s.do.SchemaReplicant()
	c.Assert(h.UFIDelate(is), IsNil)
	...
	// The statistic causet is outdated now.
...
func (s *testStatsSuite) TestUFIDelatePartitionErrorRate(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc(`set @@milevadb_partition_prune_mode='` + string(variable.StaticOnly) + `'`)
	testKit.MustInterDirc("create causet t (a bigint(64), primary key(a)) partition by range (a) (partition p0 values less than (30))")
	h.HandleDBSEvent(<-h.DBSEventCh())
	...
	testKit.MustInterDirc("insert into t values (2)")
	testKit.MustInterDirc("insert into t values (5)")
	testKit.MustInterDirc("insert into t values (8)")
	testKit.MustInterDirc("insert into t values (12)")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	is = s.do.SchemaReplicant()
	c.Assert(h.UFIDelate(is), IsNil)
...
func (s *testStatsSuite) TestQueryFeedback(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a bigint(64), b bigint(64), primary key(a), index idx(b))")
	testKit.MustInterDirc("insert into t values (1,2),(2,2),(4,5)")
	testKit.MustInterDirc("analyze causet t")
	testKit.MustInterDirc("insert into t values (3,4)")
	...
	// Feedback from limit interlock may not be accurate.
...
func (s *testStatsSuite) TestQueryFeedbackForPartition(c *C) {
	...
	testKit.MustInterDirc("insert into t values (1,2),(2,2),(3,4),(4,1),(5,6)")
	testKit.MustInterDirc("analyze causet t")
...
func (s *testStatsSuite) TestUFIDelateSystemTable(c *C) {
	...
	testKit.MustInterDirc("insert into t values (1,2)")
	testKit.MustInterDirc("analyze causet t")
	testKit.MustInterDirc("analyze causet allegrosql.stats_histograms")
	h := s.do.StatsHandle()
...
func (s *testStatsSuite) TestOutOfOrderUFIDelate(c *C) {
	...
	testKit.MustInterDirc("insert into t values (2,2),(4,5)")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	testKit.MustInterDirc(fmt.Sprintf("uFIDelate allegrosql.stats_spacetime set count = 1 where block_id = %d", blockInfo.ID))
	...
	testKit.MustInterDirc(fmt.Sprintf("uFIDelate allegrosql.stats_spacetime set count = 3 where block_id = %d", blockInfo.ID))
...
func (s *testStatsSuite) TestUFIDelateStatsByLocalFeedback(c *C) {
	...
	testKit.MustInterDirc("insert into t values (1,2),(2,2),(4,5)")
	testKit.MustInterDirc("analyze causet t")
	testKit.MustInterDirc("insert into t values (3,5)")
	h := s.do.StatsHandle()
	oriProbability := statistics.FeedbackProbability
	oriMinLogCount := handle.MinLogScanCount
	oriErrorRate := handle.MinLogErrorRate
...
func (s *testStatsSuite) TestUFIDelatePartitionStatsByLocalFeedback(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc(`set @@milevadb_partition_prune_mode='` + string(variable.StaticOnly) + `'`)
	testKit.MustInterDirc("create causet t (a bigint(64), b bigint(64), primary key(a)) partition by range (a) (partition p0 values less than (6))")
	testKit.MustInterDirc("insert into t values (1,2),(2,2),(4,5)")
	testKit.MustInterDirc("analyze causet t")
	testKit.MustInterDirc("insert into t values (3,5)")
	h := s.do.StatsHandle()
...
func (h *logHook) Write(entry zapembedded.Entry, fields []zapembedded.Field) error {
...
func (h *logHook) field2String(field zapembedded.Field) string {
	switch field.Type {
	case zapembedded.StringType:
		return field.String
	case zapembedded.Int64Type, zapembedded.Int32Type, zapembedded.Uint32Type, zapembedded.Uint64Type:
		return fmt.Sprintf("%v", field.Integer)
	case zapembedded.Float64Type:
		return fmt.Sprintf("%v", math.Float64frombits(uint64(field.Integer)))
	case zapembedded.StringerType:
		return field.Interface.(fmt.Stringer).String()
	}
	return "not support"
}

func (h *logHook) Check(e zapembedded.Entry, ce *zapembedded.CheckedEntry) *zapembedded.CheckedEntry {
...
func (s *testStatsSuite) TestLogDetailedInfo(c *C) {
	...
	testKit := testkit.NewTestKit(c, s.causetstore)
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a bigint(64), b bigint(64), c bigint(64), primary key(a), index idx(b), index idx_ba(b,a), index idx_bc(b,c))")
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(fmt.Sprintf("insert into t values (%d, %d, %d)", i, i, i))
	}
	testKit.MustInterDirc("analyze causet t with 4 buckets")
	tests := []struct {
		allegrosql string
		result     string
	}{
	...
	log.SetLevel(zapembedded.DebugLevel)
	for _, t := range tests {
		s.hook.results = ""
		testKit.MustQuery(t.allegrosql)
...
func (s *testStatsSuite) TestNeedAnalyzeTable(c *C) {
	...
	tests := []struct {
		...
		// causet was never analyzed and has reach the limit
		{
			...
			now:    "00:00 +0800",
			result: true,
			reason: "causet unanalyzed",
		},
		// causet was never analyzed but has not reach the limit
		{
			tbl:    &statistics.Block{Version: oracle.EncodeTSO(oracle.GetPhysical(time.Now()))},
			limit:  time.Hour,
			ratio:  0,
		...
		// causet was already analyzed but auto analyze is disabled
		{
		...
		// causet was already analyzed and but modify count is small
		{
		...
		// causet was already analyzed and but not within time period
		{
			tbl:    &statistics.Block{HistDefCausl: statistics.HistDefCausl{DeferredCausets: columns, ModifyCount: 1, Count: 1}},
			limit:  0,
			ratio:  0.3,
		...
		// causet was already analyzed and within time period
		{
			tbl:    &statistics.Block{HistDefCausl: statistics.HistDefCausl{DeferredCausets: columns, ModifyCount: 1, Count: 1}},
			limit:  0,
			ratio:  0.3,
...
func (s *testStatsSuite) TestIndexQueryFeedback(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a bigint(64), b bigint(64), c bigint(64), d float, e double, f decimal(17,2), " +
	...
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(fmt.Sprintf(`insert into t values (1, %d, %d, %d, %d, %d, %d, "%s")`, i, i, i, i, i, i, fmt.Sprintf("1000-01-%02d", i+1)))
	}
	h := s.do.StatsHandle()
	h.HandleDBSEvent(<-h.DBSEventCh())
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	testKit.MustInterDirc("analyze causet t with 3 buckets")
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(fmt.Sprintf(`insert into t values (1, %d, %d, %d, %d, %d, %d, "%s")`, i, i, i, i, i, i, fmt.Sprintf("1000-01-%02d", i+1)))
	}
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
...
func (s *testStatsSuite) TestIndexQueryFeedback4TopN(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a bigint(64), index idx(a))")
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(`insert into t values (1)`)
	}
	h := s.do.StatsHandle()
	h.HandleDBSEvent(<-h.DBSEventCh())
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	testKit.MustInterDirc("set @@milevadb_enable_fast_analyze = 1")
	testKit.MustInterDirc("analyze causet t with 3 buckets")
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(`insert into t values (1)`)
	}
...
func (s *testStatsSuite) TestAbnormalIndexFeedback(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a bigint(64), b bigint(64), index idx_ab(a,b))")
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(fmt.Sprintf("insert into t values (%d, %d)", i/5, i))
	}
	testKit.MustInterDirc("analyze causet t with 3 buckets")
	testKit.MustInterDirc("delete from t where a = 1")
	testKit.MustInterDirc("delete from t where b > 10")
	is := s.do.SchemaReplicant()
	causet, err := is.TableByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t"))
	c.Assert(err, IsNil)
	tblInfo := causet.Meta()
...
func (s *testStatsSuite) TestFeedbackRanges(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a tinyint, b tinyint, primary key(a), index idx(a, b))")
	for i := 0; i < 20; i++ {
...
func (s *testStatsSuite) TestUnsignedFeedbackRanges(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t (a tinyint unsigned, primary key(a))")
	testKit.MustInterDirc("create causet t1 (a bigint unsigned, primary key(a))")
	for i := 0; i < 20; i++ {
		testKit.MustInterDirc(fmt.Sprintf("insert into t values (%d)", i))
		testKit.MustInterDirc(fmt.Sprintf("insert into t1 values (%d)", i))
	}
	h.HandleDBSEvent(<-h.DBSEventCh())
	h.HandleDBSEvent(<-h.DBSEventCh())
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	testKit.MustInterDirc("analyze causet t, t1 with 3 buckets")
	for i := 30; i < 40; i++ {
		testKit.MustInterDirc(fmt.Sprintf("insert into t values (%d)", i))
		testKit.MustInterDirc(fmt.Sprintf("insert into t1 values (%d)", i))
	}
	...
	c.Assert(h.UFIDelate(is), IsNil)
	for i, t := range tests {
		causet, err := is.TableByName(perceptron.NewCIStr("test"), perceptron.NewCIStr(t.tblName))
		c.Assert(err, IsNil)
		testKit.MustQuery(t.allegrosql)
		c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
		c.Assert(h.DumpStatsFeedbackToKV(), IsNil)
...
func (s *testStatsSuite) TestLoadHistCorrelation(c *C) {
	...
	testKit.MustInterDirc("use test")
	testKit.MustInterDirc("create causet t(c int)")
	testKit.MustInterDirc("insert into t values(1),(2),(3),(4),(5)")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	testKit.MustInterDirc("analyze causet t")
	h.Clear()
	c.Assert(h.UFIDelate(s.do.SchemaReplicant()), IsNil)
	...
	testKit.MustInterDirc("explain select * from t where c = 1")
...
func (s *testStatsSuite) TestDeleteUFIDelateFeedback(c *C) {
	...
	testKit.MustInterDirc("uFIDelate t set a = 6 where a = 2")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.GetQueryFeedback().Size, Equals, 0)
	testKit.MustInterDirc("explain analyze delete from t where a = 3")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.GetQueryFeedback().Size, Equals, 0)
}
