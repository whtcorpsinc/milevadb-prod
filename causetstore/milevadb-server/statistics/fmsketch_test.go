...
// extractSampleItemsCausets is for test purpose only to extract Causet slice
// from SampleItem slice.
func extractSampleItemsCausets(items []*SampleItem) []types.Causet {
	datums := make([]types.Causet, len(items))
...
func (s *testStatisticsSuite) TestSketch(c *C) {
	...
	maxSize := 1000
	sampleSketch, ndv, err := buildFMSketch(sc, extractSampleItemsCausets(s.samples), maxSize)
	c.Check(err, IsNil)
