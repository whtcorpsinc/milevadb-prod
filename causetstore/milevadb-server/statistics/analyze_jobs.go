...
func GetAllAnalyzeJobs() []*AnalyzeJob {
	...
	analyzeStatus.Unlock()
	sort.Slice(jobs, func(i int, j int) bool { return jobs[i].getUFIDelateTime().Before(jobs[j].getUFIDelateTime()) })
	return jobs
}

// Start marks status of the analyze job as running and uFIDelate the start time.
func (job *AnalyzeJob) Start() {
	job.Mutex.Lock()
	job.State = running
	now := time.Now()
	job.StartTime = now
	job.uFIDelateTime = now
	job.Mutex.Unlock()
}

// UFIDelate uFIDelates the event count of analyze job.
func (job *AnalyzeJob) UFIDelate(rowCount int64) {
	job.Mutex.Lock()
...
// Finish uFIDelate the status of analyze job to finished or failed according to `meetError`.
...
func (job *AnalyzeJob) getUFIDelateTime() time.Time {
	job.Mutex.Lock()
	defer job.Mutex.Unlock()
	return job.uFIDelateTime
}
