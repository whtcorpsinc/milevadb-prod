...
func (s *testMemoSuite) TestNewExprIterFromGroupElem(c *C) {
	g0 := NewGroupWithSchema(NewGroupExpr(causetembedded.LogicalSelection{}.Init(s.sctx, 0)), s.schemaReplicant)
	g0.Insert(NewGroupExpr(causetembedded.LogicalLimit{}.Init(s.sctx, 0)))
	g0.Insert(NewGroupExpr(causetembedded.LogicalProjection{}.Init(s.sctx, 0)))
	g0.Insert(NewGroupExpr(causetembedded.LogicalLimit{}.Init(s.sctx, 0)))

	g1 := NewGroupWithSchema(NewGroupExpr(causetembedded.LogicalSelection{}.Init(s.sctx, 0)), s.schemaReplicant)
	g1.Insert(NewGroupExpr(causetembedded.LogicalLimit{}.Init(s.sctx, 0)))
	g1.Insert(NewGroupExpr(causetembedded.LogicalProjection{}.Init(s.sctx, 0)))
	g1.Insert(NewGroupExpr(causetembedded.LogicalLimit{}.Init(s.sctx, 0)))

	expr := NewGroupExpr(causetembedded.LogicalJoin{}.Init(s.sctx, 0))
...
func (s *testMemoSuite) TestExprIterNext(c *C) {
	g0 := NewGroupWithSchema(NewGroupExpr(
		causetembedded.LogicalProjection{Exprs: []memex.Expression{memex.NewZero()}}.Init(s.sctx, 0)), s.schemaReplicant)
	g0.Insert(NewGroupExpr(
		causetembedded.LogicalLimit{Count: 1}.Init(s.sctx, 0)))
	g0.Insert(NewGroupExpr(
		causetembedded.LogicalProjection{Exprs: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0)))
	g0.Insert(NewGroupExpr(
		causetembedded.LogicalLimit{Count: 2}.Init(s.sctx, 0)))
	g0.Insert(NewGroupExpr(
		causetembedded.LogicalProjection{Exprs: []memex.Expression{memex.NewNull()}}.Init(s.sctx, 0)))

	g1 := NewGroupWithSchema(NewGroupExpr(
		causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewNull()}}.Init(s.sctx, 0)), s.schemaReplicant)
	g1.Insert(NewGroupExpr(
		causetembedded.LogicalLimit{Count: 3}.Init(s.sctx, 0)))
	g1.Insert(NewGroupExpr(
		causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0)))
	g1.Insert(NewGroupExpr(
		causetembedded.LogicalLimit{Count: 4}.Init(s.sctx, 0)))
	g1.Insert(NewGroupExpr(
		causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewZero()}}.Init(s.sctx, 0)))
...
func (s *testMemoSuite) TestExprIterReset(c *C) {
	...
	sel1 := NewGroupExpr(causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewNull()}}.Init(s.sctx, 0))
	sel2 := NewGroupExpr(causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0))
	sel3 := NewGroupExpr(causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewZero()}}.Init(s.sctx, 0))
	g1 := NewGroupWithSchema(sel1, s.schemaReplicant)
	g1.Insert(NewGroupExpr(causetembedded.LogicalLimit{Count: 3}.Init(s.sctx, 0)))
	g1.Insert(sel2)
	g1.Insert(NewGroupExpr(causetembedded.LogicalLimit{Count: 4}.Init(s.sctx, 0)))
	g1.Insert(sel3)
	...
	// create memex iterator for the pattern on join
...
func (s *testMemoSuite) TestExprIterWithEngineType(c *C) {
	g1 := NewGroupWithSchema(NewGroupExpr(
		causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0)), s.schemaReplicant).SetEngineType(EngineTiFlash)
	g1.Insert(NewGroupExpr(
		causetembedded.LogicalLimit{Count: 1}.Init(s.sctx, 0)))
	g1.Insert(NewGroupExpr(
		causetembedded.LogicalProjection{Exprs: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0)))
	...
	g2 := NewGroupWithSchema(NewGroupExpr(
		causetembedded.LogicalSelection{Conditions: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0)), s.schemaReplicant).SetEngineType(EngineEinsteinDB)
	g2.Insert(NewGroupExpr(
		causetembedded.LogicalLimit{Count: 2}.Init(s.sctx, 0)))
	g2.Insert(NewGroupExpr(
		causetembedded.LogicalProjection{Exprs: []memex.Expression{memex.NewOne()}}.Init(s.sctx, 0)))
	...
	flashGather := NewGroupExpr(
		causetembedded.EinsteinDBSingleGather{}.Init(s.sctx, 0))
	...
	join := NewGroupExpr(
		causetembedded.LogicalJoin{}.Init(s.sctx, 0))
