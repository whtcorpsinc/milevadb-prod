...
// ExprIter enumerates all the equivalent memexs in the Group according to
// the memex pattern.
type ExprIter struct {
	// Group and Element solely identify a Group memex.
	*Group
	*list.Element

	// matched indicates whether the current Group memex bound by the
	// iterator matches the pattern after the creation or iteration.
	matched bool

	// Pattern describes the node of pattern tree.
	// The Operand type of the Group memex and the EngineType of the Group
	// must be matched with it.
	*Pattern

	// Children is used to iterate the child memexs.
	Children []*ExprIter
}

// Next returns the next Group memex matches the pattern.
func (iter *ExprIter) Next() (found bool) {
	...
	// Otherwise, iterate itself to find more matched equivalent memexs.
...
// Matched returns whether the iterator founds a Group memex matches the
// pattern.
func (iter *ExprIter) Matched() bool {
	return iter.matched
}

// Reset resets the iterator to the first matched Group memex.
...
// newExprIterFromGroupExpr creates the iterator on the Group memex.
