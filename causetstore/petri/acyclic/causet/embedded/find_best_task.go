...
// CausetCounterTp is used in hint nth_plan() to indicate which plan to use.
type CausetCounterTp int64

// CausetCounterDisabled is the default value of CausetCounterTp, indicating that optimizer needn't force a plan.
var CausetCounterDisabled CausetCounterTp = -1

// Dec minus CausetCounterTp value by x.
func (c *CausetCounterTp) Dec(x int64) {
	if *c <= 0 {
		return
	}
	*c = CausetCounterTp(int64(*c) - x)
	if *c < 0 {
		*c = 0
	}
}

// Empty indicates whether the CausetCounterTp is clear now.
func (c *CausetCounterTp) Empty() bool {
	return *c == 0
}

// IsForce indicates whether to force a plan.
func (c *CausetCounterTp) IsForce() bool {
	return *c != -1
}

// wholeTaskTypes records all possible HoTTs of task that a plan can return. For Agg, TopN and Limit, we will try to get
// these tasks one by one.
var wholeTaskTypes = [...]property.TaskType{property.CopSingleReadTaskType, property.CoFIDeloubleReadTaskType, property.RootTaskType}

var invalidTask = &rootTask{cst: math.MaxFloat64}

// GetPropByOrderByItems will check if this sort property can be pushed or not. In order to simplify the problem, we only
// consider the case that all memex are columns.
func GetPropByOrderByItems(items []*soliton.ByItems) (*property.PhysicalProperty, bool) {
	...
	for _, item := range items {
		col, ok := item.Expr.(*memex.DeferredCauset)
		if !ok {
			return nil, false
		}
...
// problem, we only consider the case that all memex are columns or some special scalar functions.
...
func (p *LogicalBlockDual) findBestTask(prop *property.PhysicalProperty, planCounter *CausetCounterTp) (task, int64, error) {
...
func (p *LogicalShow) findBestTask(prop *property.PhysicalProperty, planCounter *CausetCounterTp) (task, int64, error) {
...
func (p *LogicalShowDBSJobs) findBestTask(prop *property.PhysicalProperty, planCounter *CausetCounterTp) (task, int64, error) {
	if !prop.IsEmpty() || planCounter.Empty() {
		return invalidTask, 0, nil
	}
...
func (p *baseLogicalCauset) rebuildChildTasks(childTasks *[]task, pp PhysicalCauset, childCnts []int64, planCounter int64, TS uint64) error {
	...
	*childTasks = (*childTasks)[:0]
	for j, child := range p.children {
		multAll /= childCnts[j]
		curClock = CausetCounterTp((planCounter-1)/multAll + 1)
		...
		if curClock != 0 {
			return errors.Errorf("CausetCounterTp planCounter is not handled")
		}
...
func (p *baseLogicalCauset) enumeratePhysicalCausets4Task(physicalCausets []PhysicalCauset, prop *property.PhysicalProperty, planCounter *CausetCounterTp) (task, int64, error) {
	var bestTask task = invalidTask
	var curCntCauset, cntCauset int64
	childTasks := make([]task, 0, len(p.children))
	childCnts := make([]int64, len(p.children))
	cntCauset = 0
	for _, pp := range physicalCausets {
		// Find best child tasks firstly.
		childTasks = childTasks[:0]
		// The curCntCauset records the number of possible plans for pp
		curCntCauset = 1
		TimeStampNow := p.GetlogicalTS4TaskMap()
		savedCausetID := p.ctx.GetStochastikVars().CausetID
		for j, child := range p.children {
			childTask, cnt, err := child.findBestTask(pp.GetChildReqProps(j), &CausetCounterDisabled)
			childCnts[j] = cnt
			if err != nil {
				return nil, 0, err
			}
			curCntCauset = curCntCauset * cnt
			if childTask != nil && childTask.invalid() {
				break
			}
			childTasks = append(childTasks, childTask)
		...
		// If the target plan can be found in this physicalCauset(pp), rebuild childTasks to build the corresponding combination.
		if planCounter.IsForce() && int64(*planCounter) <= curCntCauset {
			p.ctx.GetStochastikVars().CausetID = savedCausetID
			curCntCauset = int64(*planCounter)
		...
		if prop.Enforced {
			curTask = enforceProperty(prop, curTask, p.baseCauset.ctx)
		}

		// Optimize by shuffle interlock to running in parallel manner.
		if prop.IsEmpty() {
			// Currently, we do not regard shuffled plan as a new plan.
			curTask = optimizeByShuffle(pp, curTask, p.baseCauset.ctx)
		}

		cntCauset += curCntCauset
		planCounter.Dec(curCntCauset)
	...
	return bestTask, cntCauset, nil
}

// findBestTask implements LogicalCauset interface.
func (p *baseLogicalCauset) findBestTask(prop *property.PhysicalProperty, planCounter *CausetCounterTp) (bestTask task, cntCauset int64, err error) {
	...
	var plansFitsProp, plansNeedEnforce []PhysicalCauset
	var hintWorksWithProp bool
	// Maybe the plan can satisfy the required property,
	// so we try to get the task without the enforced sort first.
	plansFitsProp, hintWorksWithProp = p.self.exhaustPhysicalCausets(newProp)
	if !hintWorksWithProp && !newProp.IsEmpty() {
		// If there is a hint in the plan and the hint cannot satisfy the property,
		// we enforce this property and try to generate the PhysicalCauset again to
		// make sure the hint can work.
		newProp.Enforced = true
	}

	if newProp.Enforced {
		// Then, we use the empty property to get physicalCausets and
		...
		plansNeedEnforce, hintCanWork = p.self.exhaustPhysicalCausets(newProp)
	...
	var cnt int64
	var curTask task
	if bestTask, cnt, err = p.enumeratePhysicalCausets4Task(plansFitsProp, newProp, planCounter); err != nil {
		return nil, 0, err
	}
	cntCauset += cnt
	if planCounter.Empty() {
	...
	newProp.Enforced = true
	curTask, cnt, err = p.enumeratePhysicalCausets4Task(plansNeedEnforce, newProp, planCounter)
	if err != nil {
		return nil, 0, err
	}
	cntCauset += cnt
...
func (p *LogicalMemBlock) findBestTask(prop *property.PhysicalProperty, planCounter *CausetCounterTp) (t task, cntCauset int64, err error) {
	if !prop.IsEmpty() || planCounter.Empty() {
		return invalidTask, 0, nil
	}
...
// tryToGetDualTask will check if the push down predicate has false constant. If so, it will return causet dual.
func (ds *DataSource) tryToGetDualTask() (task, error) {
	for _, cond := range ds.pushedDownConds {
		if con, ok := cond.(*memex.Constant); ok && con.DeferredExpr == nil && con.ParamMarker == nil {
			result, _, err := memex.EvalBool(ds.ctx, []memex.Expression{cond}, chunk.Row{})
			if err != nil {
				return nil, err
			}
			if !result {
...
// compareCandidates is the embedded of skyline pruning. It compares the two candidate paths on three dimensions:
...
func (ds *DataSource) getBlockCandidate(path *soliton.AccessPath, prop *property.PhysicalProperty) *candidatePath {
	...
	candidate.columnSet = memex.ExtractDeferredCausetSet(path.AccessConds)
...
func (ds *DataSource) skylinePruning(prop *property.PhysicalProperty) []*candidatePath {
	...
	for _, path := range ds.possibleAccessPaths {
		...
		if path.StoreType != ekv.TiFlash && (prop.TaskTp == property.CopTiFlashLocalReadTaskType || prop.TaskTp == property.CopTiFlashGlobalReadTaskType) {
			continue
		}
...
// findBestTask implements the PhysicalCauset interface.
// It will enumerate all the available indices and choose a plan with least cost.
func (ds *DataSource) findBestTask(prop *property.PhysicalProperty, planCounter *CausetCounterTp) (t task, cntCauset int64, err error) {
	...
	defer func() {
		...
		if prop.Enforced {
			...
			t = enforceProperty(prop, t, ds.baseCauset.ctx)
		}
		ds.storeTask(prop, t)
	}()
	...
	for _, candidate := range candidates {
		...
		if ds.causet.Meta().GetPartitionInfo() != nil && ds.ctx.GetStochastikVars().UseDynamicPartitionPrune() {
			canConvertPointGet = false
		}
...
func (ds *DataSource) convertToIndexMergeScan(prop *property.PhysicalProperty, candidate *candidatePath) (task task, err error) {
	...
	scans := make([]PhysicalCauset, 0, len(path.PartialIndexPaths))
	cop := &copTask{
...
func (ds *DataSource) convertToPartialIndexScan(prop *property.PhysicalProperty, path *soliton.AccessPath) (
	...
	if indexConds != nil {
		...
		indexCauset := PhysicalSelection{Conditions: indexConds}.Init(is.ctx, stats, ds.blockOffset)
		indexCauset.SetChildren(is)
		partialCost += rowCount * rowSize * sessVars.NetworkFactor
		return indexCauset, partialCost, rowCount
	}
	partialCost += rowCount * rowSize * sessVars.NetworkFactor
	indexCauset = is
	return indexCauset, partialCost, rowCount
...
func (ds *DataSource) convertToPartialBlockScan(prop *property.PhysicalProperty, path *soliton.AccessPath) (
	...
	if len(ts.filterCondition) > 0 {
		...
		blockCauset = PhysicalSelection{Conditions: ts.filterCondition}.Init(ts.ctx, ts.stats.ScaleByExpectCnt(selectivity*rowCount), ds.blockOffset)
		blockCauset.SetChildren(ts)
		partialCost += rowCount * sessVars.CopCPUFactor
		partialCost += selectivity * rowCount * rowSize * sessVars.NetworkFactor
		return blockCauset, partialCost, rowCount
	}
	partialCost += rowCount * rowSize * sessVars.NetworkFactor
	blockCauset = ts
	return blockCauset, partialCost, rowCount
...
func (ds *DataSource) buildIndexMergeBlockScan(prop *property.PhysicalProperty, blockFilters []memex.Expression, totalRowCount float64) (PhysicalCauset, float64, error) {
...
func indexCoveringDefCaus(col *memex.DeferredCauset, indexDefCauss []*memex.DeferredCauset, idxDefCausLens []int) bool {
...
func (ds *DataSource) isCoveringIndex(columns, indexDeferredCausets []*memex.DeferredCauset, idxDefCausLens []int, tblInfo *perceptron.BlockInfo) bool {
...
// If there is a causet reader which needs to keep order, we should append a pk to causet scan.
func (ts *PhysicalBlockScan) appendExtraHandleDefCaus(ds *DataSource) (*memex.DeferredCauset, bool) {
	handleDefCauss := ds.handleDefCauss
...
func (ds *DataSource) convertToIndexScan(prop *property.PhysicalProperty, candidate *candidatePath) (task task, err error) {
	...
	if candidate.isMatchProp {
		if cop.blockCauset != nil && !ds.blockInfo.IsCommonHandle {
			col, isNew := cop.blockCauset.(*PhysicalBlockScan).appendExtraHandleDefCaus(ds)
			cop.extraHandleDefCaus = col
			cop.doubleReadNeedProj = isNew
		}
		cop.keepOrder = true
		// IndexScan on partition causet can't keep order.
...
func (is *PhysicalIndexScan) indexScanRowSize(idx *perceptron.IndexInfo, ds *DataSource, isForScan bool) float64 {
	scanDefCauss := make([]*memex.DeferredCauset, 0, len(idx.DeferredCausets)+1)
...
//   PhysicalIndexScan.IdxDefCauss       []*memex.DeferredCauset
//   PhysicalIndexScan.DeferredCausets       []*perceptron.DeferredCausetInfo
func (is *PhysicalIndexScan) initSchema(idxExprDefCauss []*memex.DeferredCauset, isDoubleRead bool) {
	indexDefCauss := make([]*memex.DeferredCauset, len(is.IdxDefCauss), len(is.Index.DeferredCausets)+1)
	copy(indexDefCauss, is.IdxDefCauss)
	...
	for i := len(is.IdxDefCauss); i < len(is.Index.DeferredCausets); i++ {
		if idxExprDefCauss[i] != nil {
			...
			indexDefCauss = append(indexDefCauss, &memex.DeferredCauset{
				...
				UniqueID: is.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
			})
		}
	}
	...
	if is.NeedCommonHandle {
		...
		is.SetSchema(memex.NewSchema(indexDefCauss...))
		return
	}
...
func (is *PhysicalIndexScan) addPushedDownSelection(copTask *copTask, p *DataSource, path *soliton.AccessPath, finalStats *property.StatsInfo) {
	// Add filter condition to causet plan now.
	indexConds, blockConds := path.IndexFilters, path.BlockFilters

	blockConds, copTask.rootTaskConds = SplitSelCondsWithVirtualDeferredCauset(blockConds)

	var newRootConds []memex.Expression
	indexConds, newRootConds = memex.PushDownExprs(is.ctx.GetStochastikVars().StmtCtx, indexConds, is.ctx.GetClient(), ekv.EinsteinDB)
	copTask.rootTaskConds = append(copTask.rootTaskConds, newRootConds...)

	blockConds, newRootConds = memex.PushDownExprs(is.ctx.GetStochastikVars().StmtCtx, blockConds, is.ctx.GetClient(), ekv.EinsteinDB)
	copTask.rootTaskConds = append(copTask.rootTaskConds, newRootConds...)
	...
	if indexConds != nil {
		...
		copTask.indexCauset = indexSel
	}
	if len(blockConds) > 0 {
		copTask.finishIndexCauset()
		copTask.cst += copTask.count() * sessVars.CopCPUFactor
		blockSel := PhysicalSelection{Conditions: blockConds}.Init(is.ctx, finalStats, is.blockOffset)
		blockSel.SetChildren(copTask.blockCauset)
		copTask.blockCauset = blockSel
	}
}

// SplitSelCondsWithVirtualDeferredCauset filter the select conditions which contain virtual column
func SplitSelCondsWithVirtualDeferredCauset(conds []memex.Expression) ([]memex.Expression, []memex.Expression) {
	var filterConds []memex.Expression
	for i := len(conds) - 1; i >= 0; i-- {
		if memex.ContainVirtualDeferredCauset(conds[i : i+1]) {
...
func matchIndicesProp(idxDefCauss []*memex.DeferredCauset, colLens []int, propItems []property.Item) bool {
...
func (ds *DataSource) splitIndexFilterConditions(conditions []memex.Expression, indexDeferredCausets []*memex.DeferredCauset, idxDefCausLens []int,
	causet *perceptron.BlockInfo) (indexConds, blockConds []memex.Expression) {
	var indexConditions, blockConditions []memex.Expression
	for _, cond := range conditions {
		if ds.isCoveringIndex(memex.ExtractDeferredCausets(cond), indexDeferredCausets, idxDefCausLens, causet) {
...
func getMostCorrDefCausFromExprs(exprs []memex.Expression, histDefCausl *statistics.Block, threshold float64) (*memex.DeferredCauset, float64) {
	var defcaus []*memex.DeferredCauset
	defcaus = memex.ExtractDeferredCausetsFromExpressions(defcaus, exprs, nil)
	if len(defcaus) == 0 {
		return nil, 0
	}
	colSet := set.NewInt64Set()
	var corr float64
	var corrDefCaus *memex.DeferredCauset
...
// convertRangeFromExpectedCnt builds new ranges used to estimate event count we need to scan in causet scan before finding specified
...
// crossEstimateRowCount estimates event count of causet scan using histogram of another column which is in BlockFilters
...
// if order of column `a` is strictly correlated with column `pk`, the event count of causet scan should be:
...
func (ds *DataSource) crossEstimateRowCount(path *soliton.AccessPath, expectedCnt float64, desc bool) (float64, bool, float64) {
	...
	// If causet scan is not full range scan, we cannot use histogram of other columns for estimation, because
	// the histogram reflects value distribution in the whole causet level.
...
func (s *LogicalIndexScan) GetPhysicalIndexScan(schemaReplicant *memex.Schema, stats *property.StatsInfo) *PhysicalIndexScan {
	ds := s.Source
...
// convertToBlockScan converts the DataSource to causet scan.
func (ds *DataSource) convertToBlockScan(prop *property.PhysicalProperty, candidate *candidatePath) (task task, err error) {
	...
	if candidate.isMatchProp {
		...
		// BlockScan on partition causet can't keep order.
		if ds.blockInfo.GetPartitionInfo() != nil {
			return invalidTask, nil
		}
	}
...
func (ds *DataSource) convertToPointGet(prop *property.PhysicalProperty, candidate *candidatePath) task {
	...
	rTsk := &rootTask{p: pointGetCauset}
	var cost float64
	if candidate.path.IsIntHandlePath {
		pointGetCauset.Handle = ekv.IntHandle(candidate.path.Ranges[0].LowVal[0].GetInt64())
		pointGetCauset.UnsignedHandle = allegrosql.HasUnsignedFlag(ds.handleDefCauss.GetDefCaus(0).RetType.Flag)
		pointGetCauset.PartitionInfo = partitionInfo
		cost = pointGetCauset.GetCost(ds.TblDefCauss)
		// Add filter condition to causet plan now.
		if len(candidate.path.BlockFilters) > 0 {
			sessVars := ds.ctx.GetStochastikVars()
			cost += pointGetCauset.stats.RowCount * sessVars.CPUFactor
			sel := PhysicalSelection{
				Conditions: candidate.path.BlockFilters,
			}.Init(ds.ctx, ds.stats.ScaleByExpectCnt(prop.ExpectedCnt), ds.blockOffset)
			sel.SetChildren(pointGetCauset)
			rTsk.p = sel
		}
	} else {
		pointGetCauset.IndexInfo = candidate.path.Index
		pointGetCauset.IdxDefCauss = candidate.path.IdxDefCauss
		pointGetCauset.IdxDefCausLens = candidate.path.IdxDefCausLens
		pointGetCauset.IndexValues = candidate.path.Ranges[0].LowVal
		pointGetCauset.PartitionInfo = partitionInfo
		if candidate.isSingleScan {
			cost = pointGetCauset.GetCost(candidate.path.IdxDefCauss)
		} else {
			cost = pointGetCauset.GetCost(ds.TblDefCauss)
		}
		// Add index condition to causet plan now.
...
func (ds *DataSource) convertToBatchPointGet(prop *property.PhysicalProperty, candidate *candidatePath) task {
	...
	batchPointGetCauset := BatchPointGetCauset{
		ctx:              ds.ctx,
		AccessConditions: candidate.path.AccessConds,
		TblInfo:          ds.BlockInfo(),
	...
	if batchPointGetCauset.KeepOrder {
		batchPointGetCauset.Desc = prop.Items[0].Desc
	}
	rTsk := &rootTask{p: batchPointGetCauset}
	var cost float64
	if candidate.path.IsIntHandlePath {
		for _, ran := range candidate.path.Ranges {
			batchPointGetCauset.Handles = append(batchPointGetCauset.Handles, ekv.IntHandle(ran.LowVal[0].GetInt64()))
		}
		cost = batchPointGetCauset.GetCost(ds.TblDefCauss)
		// Add filter condition to causet plan now.
		if len(candidate.path.BlockFilters) > 0 {
			sessVars := ds.ctx.GetStochastikVars()
			cost += batchPointGetCauset.stats.RowCount * sessVars.CPUFactor
			sel := PhysicalSelection{
				Conditions: candidate.path.BlockFilters,
			}.Init(ds.ctx, ds.stats.ScaleByExpectCnt(prop.ExpectedCnt), ds.blockOffset)
			sel.SetChildren(batchPointGetCauset)
			rTsk.p = sel
		}
	} else {
		batchPointGetCauset.IndexInfo = candidate.path.Index
		batchPointGetCauset.IdxDefCauss = candidate.path.IdxDefCauss
		batchPointGetCauset.IdxDefCausLens = candidate.path.IdxDefCausLens
		for _, ran := range candidate.path.Ranges {
			batchPointGetCauset.IndexValues = append(batchPointGetCauset.IndexValues, ran.LowVal)
		}
		if !prop.IsEmpty() {
			batchPointGetCauset.KeepOrder = true
			batchPointGetCauset.Desc = prop.Items[0].Desc
		}
		if candidate.isSingleScan {
			cost = batchPointGetCauset.GetCost(candidate.path.IdxDefCauss)
		} else {
			cost = batchPointGetCauset.GetCost(ds.TblDefCauss)
		}
		// Add index condition to causet plan now.
...
func (ts *PhysicalBlockScan) addPushedDownSelection(copTask *copTask, stats *property.StatsInfo) {
	ts.filterCondition, copTask.rootTaskConds = SplitSelCondsWithVirtualDeferredCauset(ts.filterCondition)
	var newRootConds []memex.Expression
	ts.filterCondition, newRootConds = memex.PushDownExprs(ts.ctx.GetStochastikVars().StmtCtx, ts.filterCondition, ts.ctx.GetClient(), ts.StoreType)
	copTask.rootTaskConds = append(copTask.rootTaskConds, newRootConds...)
...
func (ds *DataSource) getOriginalPhysicalBlockScan(prop *property.PhysicalProperty, path *soliton.AccessPath, isMatchProp bool) (*PhysicalBlockScan, float64, float64) {
	...
	if prop.ExpectedCnt < ds.stats.RowCount {
		...
		if ok {
			// TODO: actually, before using this count as the estimated event count of causet scan, we need additionally
			// check if count < row_count(first_region | last_region), and use the larger one since we build one copTask
			// for one region now, so even if it is `limit 1`, we have to scan at least one region in causet scan.
...
func (ds *DataSource) getOriginalPhysicalIndexScan(prop *property.PhysicalProperty, path *soliton.AccessPath, isMatchProp bool, isSingleScan bool) (*PhysicalIndexScan, float64, float64) {
	...
	// If expectedCnt and count2 are both zero and we go into the below `if` causet, the count1 will be set to zero though it's shouldn't be.
