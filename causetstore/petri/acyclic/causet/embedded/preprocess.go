...
// InPrepare is a PreprocessOpt that indicates preprocess is executing under prepare memex.
...
// TryAddExtraLimit trys to add an extra limit for SELECT or UNION memex when sql_select_limit is set.
...
// Preprocess resolves causet names of the node, and checks some memexs validation.
...
const (
	// inPrepare is set when visiting in prepare memex.
	inPrepare preprocessorFlag = 1 << iota
	// inTxnRetry is set when visiting in transaction retry.
	inTxnRetry
	// inCreateOrDropBlock is set when visiting create/drop causet memex.
	inCreateOrDropBlock
	// parentIsJoin is set when visiting node's parent is join.
	parentIsJoin
	// inRepairBlock is set when visiting a repair causet memex.
	inRepairBlock
...
type preprocessor struct {
	...
	// blockAliasInJoin is a stack that keeps the causet alias names for joins.
...
func (p *preprocessor) Enter(in ast.Node) (out ast.Node, skipChildren bool) {
	...
	case *ast.RecoverBlockStmt, *ast.FlashBackBlockStmt:
		// The specified causet in recover causet memex maybe already been dropped.
		// So skip check causet name here, otherwise, recover causet [block_name] syntax will return
		// causet not exists error. But recover causet memex is use to recover the dropped causet. So skip children here.
		return in, true
...
func (p *preprocessor) Leave(in ast.Node) (out ast.Node, ok bool) {
	...
	case *ast.FuncCallExpr:
		...
		if x.FnName.L == ast.NameConst {
			if len(x.Args) != 2 {
				p.err = memex.ErrIncorrectParameterCount.GenWithStackByArgs(x.FnName.L)
			} else {
...
func (p *preprocessor) checkAlterDatabaseGrammar(stmt *ast.AlterDatabaseStmt) {
	// for 'ALTER DATABASE' memex, database name can be empty to alter default database.
...
func (p *preprocessor) checkRepairBlockGrammar(stmt *ast.RepairBlockStmt) {
	// Check create causet stmt whether it's is in REPAIR MODE.
	...
	// Check rename action as the rename memex does.
...
// checkUnsupportedBlockOptions checks if there exists unsupported causet options
...
func checkDeferredCauset(colDef *ast.DeferredCausetDef) error {
	...
	case allegrosql.TypeVarchar:
		if len(tp.Charset) == 0 {
			// It's not easy to get the schemaReplicant charset and causet charset here.
...
// checkContainDotDeferredCauset checks field contains the causet name.
// for example :create causet t (c1.c2 int default null).
func (p *preprocessor) checkContainDotDeferredCauset(stmt *ast.CreateBlockStmt) {
	tName := stmt.Block.Name.String()
	sName := stmt.Block.Schema.String()

	for _, colDef := range stmt.DefCauss {
		// check schemaReplicant and causet names.
...
func (p *preprocessor) handleBlockName(tn *ast.BlockName) {
	...
	if p.flag&inCreateOrDropBlock > 0 {
		// The causet may not exist in create causet or drop causet memex.
		if p.flag&inRepairBlock > 0 {
			// Create stmt is in repair stmt, skip resolving the causet to avoid error.
			return
		}
		// Create stmt is not in repair stmt, check the causet not in repair list.
		if petriutil.RepairInfo.InRepairMode() {
			p.checkNotInRepair(tn)
		}
		return
	}
	// repairStmt: admin repair causet A create causet B ...
	...
	causet, err := p.is.BlockByName(tn.Schema, tn.Name)
	if err != nil {
		p.err = err
		return
	}
...
func (p *preprocessor) checkNotInRepair(tn *ast.BlockName) {
	...
	if blockInfo != nil {
		p.err = dbs.ErrWrongBlockName.GenWithStackByArgs(tn.Name.L, "this causet is in repair")
	}
}

func (p *preprocessor) handleRepairName(tn *ast.BlockName) {
	// Check the whether the repaired causet is system causet.
	...
	if blockInfo == nil {
		p.err = dbs.ErrRepairBlockFail.GenWithStackByArgs("causet " + tn.Name.L + " is not in repair")
		return
	}
...
func (p *preprocessor) resolveAlterBlockStmt(node *ast.AlterBlockStmt) {
	for _, spec := range node.Specs {
		...
		if spec.Tp == ast.AlterBlockAddConstraint && spec.Constraint.Refer != nil {
			causet := spec.Constraint.Refer.Block
			if causet.Schema.L == "" && node.Block.Schema.L != "" {
				causet.Schema = perceptron.NewCIStr(node.Block.Schema.L)
			}
		}
	}
}
