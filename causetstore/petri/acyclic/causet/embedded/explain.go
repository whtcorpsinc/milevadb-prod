...
// Include `PhysicalBlockScan`, `PhysicalIndexScan`, `PointGetCauset`, `BatchPointScan` and `PhysicalMemBlock`.
// ExplainInfo = AccessObject + OperatorInfo
type dataAccesser interface {

	// AccessObject return plan's `causet`, `partition` and `index`.
	AccessObject() string
...
func (p *PhysicalIndexScan) OperatorInfo(normalized bool) string {
	...
	if len(p.rangeInfo) > 0 {
		...
		if normalized {
			fmt.Fprintf(buffer, "range: decided by %s, ", memex.SortedExplainNormalizedExpressionList(p.AccessCondition))
		} else {
...
func (p *PhysicalBlockReader) ExplainInfo() string {
	return "data:" + p.blockCauset.ExplainID().String()
}
...
func (p *PhysicalBlockReader) accessObject(sctx stochastikctx.Context) string {
	ts := p.BlockCausets[0].(*PhysicalBlockScan)
	...
	if !ok {
		return "partition causet not found" + strconv.FormatInt(ts.Block.ID, 10)
	}
...
func partitionAccessObject(sctx stochastikctx.Context, tbl causet.PartitionedBlock, pi *perceptron.PartitionInfo, partBlock *PartitionInfo) string {
...
func (p *PhysicalIndexReader) ExplainInfo() string {
	return "index:" + p.indexCauset.ExplainID().String()
}
...
func (p *PhysicalIndexReader) accessObject(sctx stochastikctx.Context) string {
	ts := p.IndexCausets[0].(*PhysicalIndexScan)
	pi := ts.Block.GetPartitionInfo()
	if pi == nil || !sctx.GetStochastikVars().UseDynamicPartitionPrune() {
		return ""
	}
	...
	tmp, ok := is.BlockByID(ts.Block.ID)
	if !ok {
		fmt.Fprintf(&buffer, "partition causet not found: %d", ts.Block.ID)
		return buffer.String()
	}
...
func (p *PhysicalUnionScan) ExplainInfo() string {
	return string(memex.SortedExplainExpressionList(p.Conditions))
}
...
func (p *PhysicalSelection) ExplainNormalizedInfo() string {
	return string(memex.SortedExplainNormalizedExpressionList(p.Conditions))
}

// ExplainInfo implements Causet interface.
func (p *PhysicalProjection) ExplainInfo() string {
	return memex.ExplainExpressionList(p.Exprs, p.schemaReplicant)
}

// ExplainNormalizedInfo implements Causet interface.
func (p *PhysicalProjection) ExplainNormalizedInfo() string {
	return string(memex.SortedExplainNormalizedExpressionList(p.Exprs))
}
...
func (p *basePhysicalAgg) explainInfo(normalized bool) string {
	sortedExplainExpressionList := memex.SortedExplainExpressionList
	if normalized {
		sortedExplainExpressionList = memex.SortedExplainNormalizedExpressionList
	}
...
func (p *PhysicalIndexJoin) explainInfo(normalized bool) string {
	...
	if len(p.OuterJoinKeys) > 0 {
		fmt.Fprintf(buffer, ", outer key:%s",
			memex.ExplainDeferredCausetList(p.OuterJoinKeys))
	}
	if len(p.InnerJoinKeys) > 0 {
		fmt.Fprintf(buffer, ", inner key:%s",
			memex.ExplainDeferredCausetList(p.InnerJoinKeys))
	}
...
func (p *PhysicalHashJoin) explainInfo(normalized bool) string {
	...
	if len(p.EqualConditions) > 0 {
		if normalized {
			fmt.Fprintf(buffer, ", equal:%s", memex.SortedExplainNormalizedScalarFuncList(p.EqualConditions))
		} else {
	...
	if len(p.LeftConditions) > 0 {
		if normalized {
			fmt.Fprintf(buffer, ", left cond:%s", memex.SortedExplainNormalizedExpressionList(p.LeftConditions))
		} else {
...
func (p *PhysicalMergeJoin) explainInfo(normalized bool) string {
	...
	if len(p.LeftJoinKeys) > 0 {
		fmt.Fprintf(buffer, ", left key:%s",
			memex.ExplainDeferredCausetList(p.LeftJoinKeys))
	}
	if len(p.RightJoinKeys) > 0 {
		fmt.Fprintf(buffer, ", right key:%s",
			memex.ExplainDeferredCausetList(p.RightJoinKeys))
	}
	if len(p.LeftConditions) > 0 {
		if normalized {
			fmt.Fprintf(buffer, ", left cond:%s", memex.SortedExplainNormalizedExpressionList(p.LeftConditions))
...
func (p *PhysicalWindow) formatFrameBound(buffer *bytes.Buffer, bound *FrameBound) {
	...
	if bound.UnBounded {
		...
		sf := bound.CalcFuncs[0].(*memex.ScalarFunction)
...
func formatWindowFuncDescs(buffer *bytes.Buffer, descs []*aggregation.WindowFuncDesc, schemaReplicant *memex.Schema) *bytes.Buffer {
...
func (p *LogicalJoin) ExplainInfo() string {
	...
	if len(p.LeftConditions) > 0 {
		fmt.Fprintf(buffer, ", left cond:%s",
			memex.SortedExplainExpressionList(p.LeftConditions))
	}
	if len(p.RightConditions) > 0 {
		fmt.Fprintf(buffer, ", right cond:%s",
			memex.SortedExplainExpressionList(p.RightConditions))
	}
	if len(p.OtherConditions) > 0 {
		fmt.Fprintf(buffer, ", other cond:%s",
			memex.SortedExplainExpressionList(p.OtherConditions))
	}
	return buffer.String()
}
...
func (p *LogicalAggregation) ExplainInfo() string {
	...
	if len(p.GroupByItems) > 0 {
		fmt.Fprintf(buffer, "group by:%s, ",
			memex.SortedExplainExpressionList(p.GroupByItems))
	}
	if len(p.AggFuncs) > 0 {
...
func (p *LogicalUnionScan) ExplainInfo() string {
	...
	fmt.Fprintf(buffer, "conds:%s",
		memex.SortedExplainExpressionList(p.conditions))
...
// MetricBlockTimeFormat is the time format for metric causet explain and format.
