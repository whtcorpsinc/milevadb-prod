...
func (s *testUnitTestSuit) rewriteSimpleExpr(str string, schemaReplicant *memex.Schema, names types.NameSlice) ([]memex.Expression, error) {
	if str == "" {
		return nil, nil
	}
	filters, err := memex.ParseSimpleExprsWithNames(s.ctx, str, schemaReplicant, names)
	if err != nil {
		return nil, err
	}
...
func (s *testUnitTestSuit) TestIndexJoinAnalyzeLookUpFilters(c *C) {
	...
	dsSchema := memex.NewSchema()
	var dsNames types.NameSlice
	dsSchema.Append(&memex.DeferredCauset{
		UniqueID: s.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
	...
	outerChildSchema := memex.NewSchema()
	var outerChildNames types.NameSlice
	outerChildSchema.Append(&memex.DeferredCauset{
		UniqueID: s.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
		RetType:  types.NewFieldType(allegrosql.TypeLonglong),
	})
	...
	joinNode.SetSchema(memex.MergeSchema(dsSchema, outerChildSchema))
	path := &soliton.AccessPath{
		IdxDefCauss:    append(make([]*memex.DeferredCauset, 0, 4), dsSchema.DeferredCausets...),
	...
	tests := []struct {
		innerKeys       []*memex.DeferredCauset
		...
		{
			innerKeys:       []*memex.DeferredCauset{dsSchema.DeferredCausets[0], dsSchema.DeferredCausets[2]},
			pushedDownConds: "",
			otherConds:      "",
			ranges:          "[[NULL,NULL]]",
		...
		{
			innerKeys:       []*memex.DeferredCauset{dsSchema.DeferredCausets[2]},
			pushedDownConds: "a = 1",
			otherConds:      "",
			ranges:          "[]",
			idxOff2KeyOff:   "[]",
		...
		// Keys are continuous.
		{
			innerKeys:       []*memex.DeferredCauset{dsSchema.DeferredCausets[1]},
			pushedDownConds: "a = 1",
			otherConds:      "",
