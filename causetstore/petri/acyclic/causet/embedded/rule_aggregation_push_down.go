...
func (a *aggregationPushDownSolver) getAggFuncChildIdx(aggFunc *aggregation.AggFuncDesc, schemaReplicant *memex.Schema) int {
	fromLeft, fromRight := false, false
	var defcaus []*memex.DeferredCauset
	defcaus = memex.ExtractDeferredCausetsFromExpressions(defcaus, aggFunc.Args, nil)
...
func (a *aggregationPushDownSolver) collectGbyDefCauss(agg *LogicalAggregation, join *LogicalJoin) (leftGbyDefCauss, rightGbyDefCauss []*memex.DeferredCauset) {
	leftChild := join.children[0]
	ctx := agg.ctx
	...
	for _, eqFunc := range join.EqualConditions {
		leftGbyDefCauss = a.addGbyDefCaus(ctx, leftGbyDefCauss, eqFunc.GetArgs()[0].(*memex.DeferredCauset))
		rightGbyDefCauss = a.addGbyDefCaus(ctx, rightGbyDefCauss, eqFunc.GetArgs()[1].(*memex.DeferredCauset))
	}
	for _, leftCond := range join.LeftConditions {
		defcaus := memex.ExtractDeferredCausets(leftCond)
		leftGbyDefCauss = a.addGbyDefCaus(ctx, leftGbyDefCauss, defcaus...)
	}
	for _, rightCond := range join.RightConditions {
		defcaus := memex.ExtractDeferredCausets(rightCond)
		rightGbyDefCauss = a.addGbyDefCaus(ctx, rightGbyDefCauss, defcaus...)
	}
	for _, otherCond := range join.OtherConditions {
		defcaus := memex.ExtractDeferredCausets(otherCond)
		for _, col := range defcaus {
			if leftChild.Schema().Contains(col) {
...
func (a *aggregationPushDownSolver) splitAggFuncsAndGbyDefCauss(agg *LogicalAggregation, join *LogicalJoin) (valid bool,
	...
	leftGbyDefCauss, rightGbyDefCauss []*memex.DeferredCauset) {
...
func (a *aggregationPushDownSolver) addGbyDefCaus(ctx stochastikctx.Context, gbyDefCauss []*memex.DeferredCauset, defcaus ...*memex.DeferredCauset) []*memex.DeferredCauset {
...
func (a *aggregationPushDownSolver) decompose(ctx stochastikctx.Context, aggFunc *aggregation.AggFuncDesc, schemaReplicant *memex.Schema) ([]*aggregation.AggFuncDesc, *memex.Schema) {
	...
	aggFunc.Args = memex.DeferredCauset2Exprs(schemaReplicant.DeferredCausets[schemaReplicant.Len()-len(result):])
...
func (a *aggregationPushDownSolver) tryToPushDownAgg(aggFuncs []*aggregation.AggFuncDesc, gbyDefCauss []*memex.DeferredCauset, join *LogicalJoin, childIdx int, aggHints aggHintInfo, blockOffset int) (_ LogicalCauset, err error) {
	...
	tmpSchema := memex.NewSchema(gbyDefCauss...)
	...
	if len(agg.GroupByItems) == 0 {
		agg.GroupByItems = []memex.Expression{&memex.Constant{
...
func (a *aggregationPushDownSolver) makeNewAgg(ctx stochastikctx.Context, aggFuncs []*aggregation.AggFuncDesc, gbyDefCauss []*memex.DeferredCauset, aggHints aggHintInfo, blockOffset int) (*LogicalAggregation, error) {
	agg := LogicalAggregation{
		GroupByItems:    memex.DeferredCauset2Exprs(gbyDefCauss),
	...
	schemaReplicant := memex.NewSchema(make([]*memex.DeferredCauset, 0, aggLen)...)
	...
	for _, gbyDefCaus := range gbyDefCauss {
		firstRow, err := aggregation.NewAggFuncDesc(agg.ctx, ast.AggFuncFirstRow, []memex.Expression{gbyDefCaus}, false)
		if err != nil {
			return nil, err
		}
...
func (a *aggregationPushDownSolver) pushAggCrossUnion(agg *LogicalAggregation, unionSchema *memex.Schema, unionChild LogicalCauset) (LogicalCauset, error) {
	ctx := agg.ctx
	newAgg := LogicalAggregation{
		AggFuncs:     make([]*aggregation.AggFuncDesc, 0, len(agg.AggFuncs)),
		GroupByItems: make([]memex.Expression, 0, len(agg.GroupByItems)),
		aggHints:     agg.aggHints,
	}.Init(ctx, agg.blockOffset)
	newAgg.SetSchema(agg.schemaReplicant.Clone())
	for _, aggFunc := range agg.AggFuncs {
		newAggFunc := aggFunc.Clone()
		newArgs := make([]memex.Expression, 0, len(newAggFunc.Args))
		for _, arg := range newAggFunc.Args {
			newArgs = append(newArgs, memex.DeferredCausetSubstitute(arg, unionSchema, memex.DeferredCauset2Exprs(unionChild.Schema().DeferredCausets)))
		}
		newAggFunc.Args = newArgs
		newAgg.AggFuncs = append(newAgg.AggFuncs, newAggFunc)
	}
	for _, gbyExpr := range agg.GroupByItems {
		newExpr := memex.DeferredCausetSubstitute(gbyExpr, unionSchema, memex.DeferredCauset2Exprs(unionChild.Schema().DeferredCausets))
		newAgg.GroupByItems = append(newAgg.GroupByItems, newExpr)
		// TODO: if there is a duplicated first_row function, we can delete it.
		firstRow, err := aggregation.NewAggFuncDesc(agg.ctx, ast.AggFuncFirstRow, []memex.Expression{gbyExpr}, false)
		if err != nil {
			return nil, err
		}
	...
	tmpSchema := memex.NewSchema(newAgg.groupByDefCauss...)
	...
	// this will cause error during interlock phase.
...
func (a *aggregationPushDownSolver) optimize(ctx context.Context, p LogicalCauset) (LogicalCauset, error) {
...
func (a *aggregationPushDownSolver) tryAggPushDownForUnion(union *LogicalUnionAll, agg *LogicalAggregation) error {
	...
	newChildren := make([]LogicalCauset, 0, len(union.Children()))
	...
	union.SetSchema(memex.NewSchema(newChildren[0].Schema().DeferredCausets...))
	union.SetChildren(newChildren...)
	return nil
}

// aggPushDown tries to push down aggregate functions to join paths.
func (a *aggregationPushDownSolver) aggPushDown(p LogicalCauset) (_ LogicalCauset, err error) {
		...
		if proj != nil {
			...
			if join, ok1 := child.(*LogicalJoin); ok1 && a.checkValidJoin(join) && p.SCtx().GetStochastikVars().AllowAggPushDown {
				if valid, leftAggFuncs, rightAggFuncs, leftGbyDefCauss, rightGbyDefCauss := a.splitAggFuncsAndGbyDefCauss(agg, join); valid {
					var lChild, rChild LogicalCauset
				...
				for i, gbyItem := range agg.GroupByItems {
					agg.GroupByItems[i] = memex.DeferredCausetSubstitute(gbyItem, proj.schemaReplicant, proj.Exprs)
				}
				agg.collectGroupByDeferredCausets()
				for _, aggFunc := range agg.AggFuncs {
					newArgs := make([]memex.Expression, 0, len(aggFunc.Args))
					for _, arg := range aggFunc.Args {
						newArgs = append(newArgs, memex.DeferredCausetSubstitute(arg, proj.schemaReplicant, proj.Exprs))
					}
