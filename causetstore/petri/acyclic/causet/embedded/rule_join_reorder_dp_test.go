...
func (s *testJoinReorderDPSuite) SetUpTest(c *C) {
	s.ctx = MockContext()
	s.ctx.GetStochastikVars().CausetID = -1
}
...
func (mj mockLogicalJoin) init(ctx stochastikctx.Context) *mockLogicalJoin {
	mj.baseLogicalCauset = newBaseLogicalCauset(ctx, "MockLogicalJoin", &mj, 0)
	return &mj
}

func (mj *mockLogicalJoin) recursiveDeriveStats(_ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if mj.stats == nil {
...
func (s *testJoinReorderDPSuite) newMockJoin(lChild, rChild LogicalCauset, eqConds []*memex.ScalarFunction, _ []memex.Expression) LogicalCauset {
	retJoin := mockLogicalJoin{}.init(s.ctx)
	retJoin.schemaReplicant = memex.MergeSchema(lChild.Schema(), rChild.Schema())
...
func (s *testJoinReorderDPSuite) newDataSource(name string, count int) LogicalCauset {
	ds := DataSource{}.Init(s.ctx, 0)
	tan := perceptron.NewCIStr(name)
	ds.BlockAsName = &tan
	ds.schemaReplicant = memex.NewSchema()
	s.ctx.GetStochastikVars().CausetDeferredCausetID++
	ds.schemaReplicant.Append(&memex.DeferredCauset{
		UniqueID: s.ctx.GetStochastikVars().CausetDeferredCausetID,
...
func (s *testJoinReorderDPSuite) planToString(plan LogicalCauset) string {
	switch x := plan.(type) {
...
func (s *testJoinReorderDPSuite) TestDPReorderTPCHQ5(c *C) {
	...
	joinGroups := make([]LogicalCauset, 0, 6)
	...
	var eqConds []memex.Expression
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[0].Schema().DeferredCausets[0], joinGroups[1].Schema().DeferredCausets[0]))
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[1].Schema().DeferredCausets[0], joinGroups[2].Schema().DeferredCausets[0]))
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[2].Schema().DeferredCausets[0], joinGroups[3].Schema().DeferredCausets[0]))
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[0].Schema().DeferredCausets[0], joinGroups[3].Schema().DeferredCausets[0]))
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[2].Schema().DeferredCausets[0], joinGroups[4].Schema().DeferredCausets[0]))
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[3].Schema().DeferredCausets[0], joinGroups[4].Schema().DeferredCausets[0]))
	eqConds = append(eqConds, memex.NewFunctionInternal(s.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), joinGroups[4].Schema().DeferredCausets[0], joinGroups[5].Schema().DeferredCausets[0]))
...
func (s *testJoinReorderDPSuite) TestDPReorderAllCartesian(c *C) {
	joinGroup := make([]LogicalCauset, 0, 4)
