...
type indexHintInfo struct {
	...
	// a Select memex, we will generate a warning for it.
	matched bool
}
...
func (info *blockHintInfo) ifPreferAsLocalInBCJoin(p LogicalCauset, blockOffset int) bool {
...
func (info *blockHintInfo) matchBlockName(blocks []*hintBlockInfo, hintBlocks []hintBlockInfo) bool {
	...
	for _, causet := range blocks {
		for i, curEntry := range hintBlocks {
			...
			if curEntry.dbName.L == causet.dbName.L && curEntry.tblName.L == causet.tblName.L && causet.selectOffset == curEntry.selectOffset {
...
func restore2BlockHint(hintBlocks ...hintBlockInfo) string {
	...
	for i, causet := range hintBlocks {
		buffer.WriteString(causet.tblName.L)
		if len(causet.partitions) > 0 {
			buffer.WriteString(" PARTITION(")
			for j, partition := range causet.partitions {
				if j > 0 {
					buffer.WriteString(", ")
				}
...
func extractUnmatchedBlocks(hintBlocks []hintBlockInfo) []string {
	...
	for _, causet := range hintBlocks {
		if !causet.matched {
			blockNames = append(blockNames, causet.tblName.O)
		}
	}
	return blockNames
}
...
var clauseMsg = map[clauseCode]string{
	...
	groupByClause:       "group memex",
	showStatement:       "show memex",
...
// CausetBuilder builds Causet from an ast.Node.
// It just builds the ast node straightforwardly.
type CausetBuilder struct {
	...
	// rewriterPool stores the memexRewriter we have created to reuse it if it has been released.
	// rewriterCounter counts how many rewriter is being used.
	rewriterPool    []*memexRewriter
	rewriterCounter int

	windowSpecs     map[string]*ast.WindowSpec
	inUFIDelateStmt bool
	inDeleteStmt    bool
	// inStraightJoin represents whether the current "SELECT" memex has
...
// GetVisitInfo gets the visitInfo of the CausetBuilder.
func (b *CausetBuilder) GetVisitInfo() []visitInfo {
	return b.visitInfo
}

// GetDBBlockInfo gets the accessed dbs and blocks info.
func (b *CausetBuilder) GetDBBlockInfo() []stmtctx.BlockEntry {
	...
	for _, v := range b.visitInfo {
		tbl := &stmtctx.BlockEntry{EDB: v.EDB, Block: v.causet}
...
// GetOptFlag gets the optFlag of the CausetBuilder.
func (b *CausetBuilder) GetOptFlag() uint64 {
	return b.optFlag
}

func (b *CausetBuilder) getSelectOffset() int {
...
func (b *CausetBuilder) pushSelectOffset(offset int) {
	b.selectOffset = append(b.selectOffset, offset)
}

func (b *CausetBuilder) popSelectOffset() {
	b.selectOffset = b.selectOffset[:len(b.selectOffset)-1]
}

// NewCausetBuilder creates a new CausetBuilder.
func NewCausetBuilder(sctx stochastikctx.Context, is schemareplicant.SchemaReplicant, processor *hint.BlockHintProcessor) *CausetBuilder {
	if processor == nil {
		sctx.GetStochastikVars().CausetAppendSelectBlockAsName = nil
	} else {
		sctx.GetStochastikVars().CausetAppendSelectBlockAsName = make([]ast.HintBlock, processor.MaxSelectStmtOffset()+1)
	}
	return &CausetBuilder{
		ctx:           sctx,
		is:            is,
...
// Build builds the ast node to a Causet.
func (b *CausetBuilder) Build(ctx context.Context, node ast.Node) (Causet, error) {
	...
	case *ast.InterDircuteStmt:
		return b.buildInterDircute(ctx, x)
	...
	case *ast.CreateBindingStmt:
		return b.buildCreateBindCauset(x)
	case *ast.DropBindingStmt:
		return b.buildDropBindCauset(x)
	case *ast.ChangeStmt:
...
func (b *CausetBuilder) buildSetConfig(ctx context.Context, v *ast.SetConfigStmt) (Causet, error) {
	...
	mockBlockCauset := LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
	expr, _, err := b.rewrite(ctx, v.Value, mockBlockCauset, nil, true)
	return &SetConfig{Name: v.Name, Type: v.Type, Instance: v.Instance, Value: expr}, err
}

func (b *CausetBuilder) buildChange(v *ast.ChangeStmt) (Causet, error) {
	exe := &Change{
		ChangeStmt: v,
	}
	return exe, nil
...
func (b *CausetBuilder) buildInterDircute(ctx context.Context, v *ast.InterDircuteStmt) (Causet, error) {
	vars := make([]memex.Expression, 0, len(v.UsingVars))
	...
	exe := &InterDircute{Name: v.Name, UsingVars: vars, InterDircID: v.InterDircID}
	if v.BinaryArgs != nil {
...
func (b *CausetBuilder) buildDo(ctx context.Context, v *ast.DoStmt) (Causet, error) {
	var p LogicalCauset
	dual := LogicalBlockDual{RowCount: 1}.Init(b.ctx, b.getSelectOffset())
	dual.SetSchema(memex.NewSchema())
	p = dual
	proj := LogicalProjection{Exprs: make([]memex.Expression, 0, len(v.Exprs))}.Init(b.ctx, b.getSelectOffset())
	proj.names = make([]*types.FieldName, len(v.Exprs))
	schemaReplicant := memex.NewSchema(make([]*memex.DeferredCauset, 0, len(v.Exprs))...)
...
func (b *CausetBuilder) buildSet(ctx context.Context, v *ast.SetStmt) (Causet, error) {
	p := &Set{}
	for _, vars := range v.Variables {
		...
		if _, ok := vars.Value.(*ast.DefaultExpr); !ok {
			if cn, ok2 := vars.Value.(*ast.DeferredCausetNameExpr); ok2 && cn.Name.Block.L == "" {
				// Convert column name memex to string value memex.
				char, col := b.ctx.GetStochastikVars().GetCharsetInfo()
				vars.Value = ast.NewValueExpr(cn.Name.Name.O, char, col)
			}
			mockBlockCauset := LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
			var err error
			assign.Expr, _, err = b.rewrite(ctx, vars.Value, mockBlockCauset, nil, true)
			if err != nil {
				return nil, err
			}
		} else {
		...
		if vars.ExtendValue != nil {
			assign.ExtendValue = &memex.Constant{
...
func (b *CausetBuilder) buildDropBindCauset(v *ast.DropBindingStmt) (Causet, error) {
	p := &ALLEGROSQLBindCauset{
...
func (b *CausetBuilder) buildCreateBindCauset(v *ast.CreateBindingStmt) (Causet, error) {
...
func (b *CausetBuilder) detectSelectAgg(sel *ast.SelectStmt) bool {
	if sel.GroupBy != nil {
		return true
	}
...
func (b *CausetBuilder) detectSelectWindow(sel *ast.SelectStmt) bool {
	for _, f := range sel.Fields.Fields {
...
func getPossibleAccessPaths(ctx stochastikctx.Context, blockHints *blockHintInfo, indexHints []*ast.IndexHint, tbl causet.Block, dbName, tblName perceptron.CIStr) ([]*soliton.AccessPath, error) {
	tblInfo := tbl.Meta()
...
func (b *CausetBuilder) buildSelectLock(src LogicalCauset, dagger *ast.SelectLockInfo) *LogicalLock {
...
func (b *CausetBuilder) buildPrepare(x *ast.PrepareStmt) Causet {
	p := &Prepare{
		Name: x.Name,
	}
...
func (b *CausetBuilder) buildAdmin(ctx context.Context, as *ast.AdminStmt) (Causet, error) {
	var ret Causet
	var err error
	switch as.Tp {
	...
	case ast.AdminFlushBindings:
		return &ALLEGROSQLBindCauset{ALLEGROSQLBindOp: OpFlushBindings}, nil
	case ast.AdminCaptureBindings:
		return &ALLEGROSQLBindCauset{ALLEGROSQLBindOp: OpCaptureBindings}, nil
	case ast.AdminEvolveBindings:
		return &ALLEGROSQLBindCauset{ALLEGROSQLBindOp: OpEvolveBindings}, nil
	case ast.AdminReloadBindings:
		return &ALLEGROSQLBindCauset{ALLEGROSQLBindOp: OpReloadBindings}, nil
...
// getGenExprs gets generated memexs map.
func (b *CausetBuilder) getGenExprs(ctx context.Context, dbName perceptron.CIStr, tbl causet.Block, idx *perceptron.IndexInfo, exprDefCauss *memex.Schema, names types.NameSlice) (
	map[perceptron.BlockDeferredCausetID]memex.Expression, error) {
	tblInfo := tbl.Meta()
	genExprsMap := make(map[perceptron.BlockDeferredCausetID]memex.Expression)
	exprs := make([]memex.Expression, 0, len(tbl.DefCauss()))
	genExprIdxs := make([]perceptron.BlockDeferredCausetID, len(tbl.DefCauss()))
	mockBlockCauset := LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
	mockBlockCauset.SetSchema(exprDefCauss)
	mockBlockCauset.names = names
	for i, colExpr := range mockBlockCauset.Schema().DeferredCausets {
		col := tbl.DefCauss()[i]
		var expr memex.Expression
		expr = colExpr
		if col.IsGenerated() && !col.GeneratedStored {
			var err error
			expr, _, err = b.rewrite(ctx, col.GeneratedExpr, mockBlockCauset, nil, true)
			if err != nil {
	...
	// Re-iterate memexs to handle those virtual generated columns that refers to the other generated columns.
	for i, expr := range exprs {
		exprs[i] = memex.DeferredCausetSubstitute(expr, mockBlockCauset.Schema(), exprs)
...
func (b *CausetBuilder) buildPhysicalIndexLookUpReader(ctx context.Context, dbName perceptron.CIStr, tbl causet.Block, idx *perceptron.IndexInfo) (Causet, error) {
	tblInfo := tbl.Meta()
	physicalID, isPartition := getPhysicalID(tbl)
	fullExprDefCauss, _, err := memex.BlockInfo2SchemaAndNames(b.ctx, dbName, tblInfo)
	if err != nil {
		return nil, err
	}
	...
	idxDefCauss, idxDefCausLens := memex.IndexInfo2PrefixDefCauss(idxDefCausInfos, idxDefCausSchema.DeferredCausets, idx)
...
func getIndexDefCaussSchema(tblInfo *perceptron.BlockInfo, idx *perceptron.IndexInfo, allDefCausSchema *memex.Schema) *memex.Schema {
	schemaReplicant := memex.NewSchema(make([]*memex.DeferredCauset, 0, len(idx.DeferredCausets))...)
...
func getPhysicalID(t causet.Block) (physicalID int64, isPartition bool) {
	tblInfo := t.Meta()
	if tblInfo.GetPartitionInfo() != nil {
		pid := t.(causet.PhysicalBlock).GetPhysicalID()
		return pid, true
	}
	return tblInfo.ID, false
}

func tryGetPkExtraDeferredCauset(sv *variable.StochastikVars, tblInfo *perceptron.BlockInfo) (*perceptron.DeferredCausetInfo, *memex.DeferredCauset, bool) {
	if tblInfo.IsCommonHandle || tblInfo.PKIsHandle {
		return nil, nil, false
	}
	info := perceptron.NewExtraHandleDefCausInfo()
	expDefCaus := &memex.DeferredCauset{
		RetType:  types.NewFieldType(allegrosql.TypeLonglong),
		UniqueID: sv.AllocCausetDeferredCausetID(),
		ID:       perceptron.ExtraHandleID,
	}
	return info, expDefCaus, true
}

func tryGetCommonHandleDefCauss(t causet.Block, allDefCausSchema *memex.Schema) ([]*causet.DeferredCauset, []*memex.DeferredCauset, bool) {
	tblInfo := t.Meta()
	if !tblInfo.IsCommonHandle {
		return nil, nil, false
	}
	pk := blocks.FindPrimaryIndex(tblInfo)
	commonHandleDefCauss, _ := memex.IndexInfo2DefCauss(tblInfo.DeferredCausets, allDefCausSchema.DeferredCausets, pk)
	commonHandelDefCausInfos := blocks.TryGetCommonPkDeferredCausets(t)
	return commonHandelDefCausInfos, commonHandleDefCauss, true
}

func tryGetPkHandleDefCaus(tblInfo *perceptron.BlockInfo, allDefCausSchema *memex.Schema) (*perceptron.DeferredCausetInfo, *memex.DeferredCauset, bool) {
...
func (b *CausetBuilder) buildPhysicalIndexLookUpReaders(ctx context.Context, dbName perceptron.CIStr, tbl causet.Block, indices []causet.Index) ([]Causet, []*perceptron.IndexInfo, error) {
	tblInfo := tbl.Meta()
	// get index information
	indexInfos := make([]*perceptron.IndexInfo, 0, len(tblInfo.Indices))
	indexLookUpReaders := make([]Causet, 0, len(tblInfo.Indices))
	for _, idx := range indices {
		...
		if idxInfo.State != perceptron.StatePublic {
			logutil.Logger(context.Background()).Info("build physical index lookup reader, the index isn't public",
				zap.String("index", idxInfo.Name.O), zap.Stringer("state", idxInfo.State), zap.String("causet", tblInfo.Name.O))
			continue
		}
		...
		if pi := tbl.Meta().GetPartitionInfo(); pi != nil {
			for _, def := range pi.Definitions {
				t := tbl.(causet.PartitionedBlock).GetPartition(def.ID)
...
func (b *CausetBuilder) buildAdminCheckBlock(ctx context.Context, as *ast.AdminStmt) (*CheckBlock, error) {
	tblName := as.Blocks[0]
	...
	if as.Tp == ast.AdminChecHoTTex {
		...
		p.ChecHoTTex = true
		readerCausets, indexInfos, err = b.buildPhysicalIndexLookUpReaders(ctx, tblName.Schema, tbl, []causet.Index{idx})
	} else {
		readerCausets, indexInfos, err = b.buildPhysicalIndexLookUpReaders(ctx, tblName.Schema, tbl, tbl.Indices())
	}
	if err != nil {
		return nil, errors.Trace(err)
	}
	readers := make([]*PhysicalIndexLookUpReader, 0, len(readerCausets))
	for _, plan := range readerCausets {
...
func (b *CausetBuilder) buildChecHoTTexSchema(tn *ast.BlockName, indexName string) (*memex.Schema, types.NameSlice, error) {
	schemaReplicant := memex.NewSchema()
...
func BuildHandleDefCaussForAnalyze(ctx stochastikctx.Context, tblInfo *perceptron.BlockInfo) HandleDefCauss {
	...
	case tblInfo.PKIsHandle:
		...
		handleDefCauss = &IntHandleDefCauss{col: &memex.DeferredCauset{
			ID:      pkDefCaus.ID,
...
func getPhysicalIDsAndPartitionNames(tblInfo *perceptron.BlockInfo, partitionNames []perceptron.CIStr) ([]int64, []string, error) {
	...
	for _, name := range partitionNames {
		...
		if !found {
			return nil, nil, fmt.Errorf("can not found the specified partition name %s in the causet definition", name.O)
		}
	}
	return ids, names, nil
}

func (b *CausetBuilder) buildAnalyzeBlock(as *ast.AnalyzeBlockStmt, opts map[ast.AnalyzeOptionType]uint64) (Causet, error) {
	p := &Analyze{Opts: opts}
...
func (b *CausetBuilder) buildAnalyzeIndex(as *ast.AnalyzeBlockStmt, opts map[ast.AnalyzeOptionType]uint64) (Causet, error) {
	p := &Analyze{Opts: opts}
...
func (b *CausetBuilder) buildAnalyzeAllIndex(as *ast.AnalyzeBlockStmt, opts map[ast.AnalyzeOptionType]uint64) (Causet, error) {
	p := &Analyze{Opts: opts}
	tblInfo := as.BlockNames[0].BlockInfo
	physicalIDs, names, err := getPhysicalIDsAndPartitionNames(tblInfo, as.PartitionNames)
	if err != nil {
...
func (b *CausetBuilder) buildAnalyze(as *ast.AnalyzeBlockStmt) (Causet, error) {
...
func buildShowNextRowID() (*memex.Schema, types.NameSlice) {
...
func buildShowDBSFields() (*memex.Schema, types.NameSlice) {
...
func buildRecoverIndexFields() (*memex.Schema, types.NameSlice) {
...
func buildCleanupIndexFields() (*memex.Schema, types.NameSlice) {
...
func buildShowDBSJobsFields() (*memex.Schema, types.NameSlice) {
...
func buildBlockRegionsSchema() (*memex.Schema, types.NameSlice) {
	schemaReplicant := newDeferredCausetsWithNames(11)
...
func buildSplitRegionsSchema() (*memex.Schema, types.NameSlice) {
	schemaReplicant := newDeferredCausetsWithNames(2)
...
func buildShowDBSJobQueriesFields() (*memex.Schema, types.NameSlice) {
	schemaReplicant := newDeferredCausetsWithNames(1)
	schemaReplicant.Append(buildDeferredCausetWithName("", "QUERY", allegrosql.TypeVarchar, 256))
	return schemaReplicant.col2Schema(), schemaReplicant.names
}

func buildShowSlowSchema() (*memex.Schema, types.NameSlice) {
...
func buildCancelDBSJobsFields() (*memex.Schema, types.NameSlice) {
	schemaReplicant := newDeferredCausetsWithNames(2)
...
func buildBRIESchema() (*memex.Schema, types.NameSlice) {
	longlongSize, _ := allegrosql.GetDefaultFieldLengthAndDecimal(allegrosql.TypeLonglong)
	...
	schemaReplicant.Append(buildDeferredCausetWithName("", "InterDircution Time", allegrosql.TypeDatetime, datetimeSize))
	return schemaReplicant.col2Schema(), schemaReplicant.names
}

func buildShowTelemetrySchema() (*memex.Schema, types.NameSlice) {
	schemaReplicant := newDeferredCausetsWithNames(1)
...
func buildDeferredCausetWithName(blockName, name string, tp byte, size int) (*memex.DeferredCauset, *types.FieldName) {
...
func newDeferredCausetsWithNames(cap int) *columnsWithNames {
	return &columnsWithNames{
		defcaus: make([]*memex.DeferredCauset, 0, 2),
		names:   make(types.NameSlice, 0, 2),
	}
}

func (cwn *columnsWithNames) Append(col *memex.DeferredCauset, name *types.FieldName) {
	cwn.defcaus = append(cwn.defcaus, col)
	cwn.names = append(cwn.names, name)
}

func (cwn *columnsWithNames) col2Schema() *memex.Schema {
	return memex.NewSchema(cwn.defcaus...)
}

// splitWhere split a where memex to a list of AND conditions.
...
func (b *CausetBuilder) buildShow(ctx context.Context, show *ast.ShowStmt) (Causet, error) {
	p := LogicalShow{
	...
	case ast.ShowCreateBlock, ast.ShowCreateSequence:
		...
		if causet, err := b.is.BlockByName(show.Block.Schema, show.Block.Name); err == nil {
			isView = causet.Meta().IsView()
			isSequence = causet.Meta().IsSequence()
		}
	...
	if np != p {
		...
		proj := LogicalProjection{Exprs: make([]memex.Expression, 0, fieldsLen)}.Init(b.ctx, 0)
		schemaReplicant := memex.NewSchema(make([]*memex.DeferredCauset, 0, fieldsLen)...)
		for _, col := range p.schemaReplicant.DeferredCausets {
			proj.Exprs = append(proj.Exprs, col)
			newDefCaus := col.Clone().(*memex.DeferredCauset)
			newDefCaus.UniqueID = b.ctx.GetStochastikVars().AllocCausetDeferredCausetID()
	...
	if show.Tp == ast.ShowVariables || show.Tp == ast.ShowStatus {
		...
		orderByDefCaus := np.Schema().DeferredCausets[0].Clone().(*memex.DeferredCauset)
		sort := LogicalSort{
...
func (b *CausetBuilder) buildSimple(node ast.StmtNode) (Causet, error) {
	...
	case *ast.KillStmt:
		// If you have the SUPER privilege, you can kill all threads and memexs.
		// Otherwise, you can kill only your own threads and memexs.
...
func (b *CausetBuilder) getDefaultValue(col *causet.DeferredCauset) (*memex.Constant, error) {
	var (
		value types.Causet
		err   error
	)
	if col.DefaultIsExpr && col.DefaultExpr != nil {
		value, err = causet.EvalDefCausDefaultExpr(b.ctx, col.ToInfo(), col.DefaultExpr)
	} else {
		value, err = causet.GetDefCausDefaultValue(b.ctx, col.ToInfo())
	}
	if err != nil {
		return nil, err
	}
	return &memex.Constant{Value: value, RetType: &col.FieldType}, nil
}

func (b *CausetBuilder) findDefaultValue(defcaus []*causet.DeferredCauset, name *ast.DeferredCausetName) (*memex.Constant, error) {
...
// memexs respectively. onDups indicates which columns are in on-duplicate list.
func (b *CausetBuilder) resolveGeneratedDeferredCausets(ctx context.Context, columns []*causet.DeferredCauset, onDups map[string]struct{}, mockCauset LogicalCauset) (igc InsertGeneratedDeferredCausets, err error) {
	for _, column := range columns {
		...
		idx, err := memex.FindFieldName(mockCauset.OutputNames(), columnName)
		if err != nil {
			return igc, err
		}
		colExpr := mockCauset.Schema().DeferredCausets[idx]

		expr, _, err := b.rewrite(ctx, column.GeneratedExpr, mockCauset, nil, true)
		if err != nil {
			return igc, err
		}
		...
		for dep := range column.Dependences {
			if _, ok := onDups[dep]; ok {
				assign := &memex.Assignment{DefCaus: colExpr, DefCausName: column.Name, Expr: expr}
...
func (b *CausetBuilder) buildInsert(ctx context.Context, insert *ast.InsertStmt) (Causet, error) {
	...
	schemaReplicant, names, err := memex.BlockInfo2SchemaAndNames(b.ctx, tn.Schema, blockInfo)
	if err != nil {
		return nil, err
	}
	blockInCauset, ok := b.is.BlockByID(blockInfo.ID)
	if !ok {
		return nil, errors.Errorf("Can't get causet %s.", blockInfo.Name.O)
	}
	...
	if blockInfo.GetPartitionInfo() != nil && len(insert.PartitionNames) != 0 {
		...
		pt := blockInCauset.(causet.PartitionedBlock)
		insertCauset.Block = blocks.NewPartitionBlockithGivenSets(pt, givenPartitionSets)
	...
	mockBlockCauset := LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
	mockBlockCauset.SetSchema(insertCauset.blockSchema)
	mockBlockCauset.names = insertCauset.blockDefCausNames

	checkRefDeferredCauset := func(n ast.Node) ast.Node {
		if insertCauset.NeedFillDefaultValue {
			return n
		}
		switch n.(type) {
		case *ast.DeferredCausetName, *ast.DeferredCausetNameExpr:
			insertCauset.NeedFillDefaultValue = true
		}
		return n
	}
	...
	if len(insert.Setlist) > 0 {
		...
		err := b.buildSetValuesOfInsert(ctx, insert, insertCauset, mockBlockCauset, checkRefDeferredCauset)
		if err != nil {
			return nil, err
		}
	} else if len(insert.Lists) > 0 {
		// Branch for `INSERT ... VALUES ...`.
		err := b.buildValuesListOfInsert(ctx, insert, insertCauset, mockBlockCauset, checkRefDeferredCauset)
		if err != nil {
			return nil, err
		}
	} else {
		// Branch for `INSERT ... SELECT ...`.
		err := b.buildSelectCausetOfInsert(ctx, insert, insertCauset)
		if err != nil {
			return nil, err
		}
	}

	mockBlockCauset.SetSchema(insertCauset.Schema4OnDuplicate)
	mockBlockCauset.names = insertCauset.names4OnDuplicate

	onDupDefCausSet, err := insertCauset.resolveOnDuplicate(insert.OnDuplicate, blockInfo, func(node ast.ExprNode) (memex.Expression, error) {
		return b.rewriteInsertOnDuplicateUFIDelate(ctx, node, mockBlockCauset, insertCauset)
	})
	if err != nil {
		return nil, err
	}
	...
	mockBlockCauset.schemaReplicant = insertCauset.blockSchema
	mockBlockCauset.names = insertCauset.blockDefCausNames
	insertCauset.GenDefCauss, err = b.resolveGeneratedDeferredCausets(ctx, insertCauset.Block.DefCauss(), onDupDefCausSet, mockBlockCauset)
	if err != nil {
		return nil, err
	}

	err = insertCauset.ResolveIndices()
	return insertCauset, err
}

func (p *Insert) resolveOnDuplicate(onDup []*ast.Assignment, tblInfo *perceptron.BlockInfo, yield func(ast.ExprNode) (memex.Expression, error)) (map[string]struct{}, error) {
	onDupDefCausSet := make(map[string]struct{}, len(onDup))
	colMap := make(map[string]*causet.DeferredCauset, len(p.Block.DefCauss()))
	for _, col := range p.Block.DefCauss() {
		colMap[col.Name.L] = col
	}
	for _, assign := range onDup {
		// Check whether the column to be uFIDelated exists in the source causet.
		idx, err := memex.FindFieldName(p.blockDefCausNames, assign.DeferredCauset)
		if err != nil {
			return nil, err
		} else if idx < 0 {
		...
		p.OnDuplicate = append(p.OnDuplicate, &memex.Assignment{
...
func (b *CausetBuilder) getAffectDefCauss(insertStmt *ast.InsertStmt, insertCauset *Insert) (affectedValuesDefCauss []*causet.DeferredCauset, err error) {
	if len(insertStmt.DeferredCausets) > 0 {
		...
		affectedValuesDefCauss, missingDefCausName = causet.FindDefCauss(insertCauset.Block.VisibleDefCauss(), colName, insertCauset.Block.Meta().PKIsHandle)
		...
		affectedValuesDefCauss = insertCauset.Block.VisibleDefCauss()
	}
	return affectedValuesDefCauss, nil
}

func (b *CausetBuilder) buildSetValuesOfInsert(ctx context.Context, insert *ast.InsertStmt, insertCauset *Insert, mockBlockCauset *LogicalBlockDual, checkRefDeferredCauset func(n ast.Node) ast.Node) error {
	blockInfo := insertCauset.Block.Meta()
	colNames := make([]string, 0, len(insert.Setlist))
	exprDefCauss := make([]*memex.DeferredCauset, 0, len(insert.Setlist))
	for _, assign := range insert.Setlist {
		idx, err := memex.FindFieldName(insertCauset.blockDefCausNames, assign.DeferredCauset)
		if err != nil {
			return err
		}
		if idx < 0 {
		...
		exprDefCauss = append(exprDefCauss, insertCauset.blockSchema.DeferredCausets[idx])
	}

	// Check whether the column to be uFIDelated is the generated column.
	tDefCauss, missingDefCausName := causet.FindDefCauss(insertCauset.Block.VisibleDefCauss(), colNames, blockInfo.PKIsHandle)
	if missingDefCausName != "" {
	...
	insertCauset.AllAssignmentsAreConstant = true
	for i, assign := range insert.Setlist {
		...
		usingCauset := mockBlockCauset
		if _, ok := assign.Expr.(*ast.SubqueryExpr); ok {
			usingCauset = LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
		}
		expr, _, err := b.rewriteWithPreprocess(ctx, assign.Expr, usingCauset, nil, nil, true, checkRefDeferredCauset)
		if err != nil {
			return err
		}
		if insertCauset.AllAssignmentsAreConstant {
			_, isConstant := expr.(*memex.Constant)
			insertCauset.AllAssignmentsAreConstant = isConstant
		}

		insertCauset.SetList = append(insertCauset.SetList, &memex.Assignment{
			DefCaus:     exprDefCauss[i],
			DefCausName: perceptron.NewCIStr(colNames[i]),
			Expr:        expr,
		})
	}
	insertCauset.Schema4OnDuplicate = insertCauset.blockSchema
	insertCauset.names4OnDuplicate = insertCauset.blockDefCausNames
	return nil
}

func (b *CausetBuilder) buildValuesListOfInsert(ctx context.Context, insert *ast.InsertStmt, insertCauset *Insert, mockBlockCauset *LogicalBlockDual, checkRefDeferredCauset func(n ast.Node) ast.Node) error {
	affectedValuesDefCauss, err := b.getAffectDefCauss(insert, insertCauset)
	if err != nil {
		return err
	}

	// If value_list and col_list are empty and we have a generated column, we can still write data to this causet.
	...
	insertCauset.AllAssignmentsAreConstant = true
	totalBlockDefCauss := insertCauset.Block.DefCauss()
	for i, valuesItem := range insert.Lists {
		...
		exprList := make([]memex.Expression, 0, len(valuesItem))
		for j, valueItem := range valuesItem {
			...
			case *ast.DefaultExpr:
				if col.IsGenerated() {
					if x.Name != nil {
						return ErrBadGeneratedDeferredCauset.GenWithStackByArgs(col.Name.O, insertCauset.Block.Meta().Name.O)
					}
			...
			default:
				...
				expr, _, err = b.rewriteWithPreprocess(ctx, valueItem, usingCauset, nil, nil, true, checkRefDeferredCauset)
			}
			if err != nil {
				return err
			}
		...
		insertCauset.Lists = append(insertCauset.Lists, exprList)
	}
	insertCauset.Schema4OnDuplicate = insertCauset.blockSchema
	insertCauset.names4OnDuplicate = insertCauset.blockDefCausNames
	return nil
...
func (b *CausetBuilder) buildSelectCausetOfInsert(ctx context.Context, insert *ast.InsertStmt, insertCauset *Insert) error {
	affectedValuesDefCauss, err := b.getAffectDefCauss(insert, insertCauset)
	if err != nil {
		return err
	}
	selectCauset, err := b.Build(ctx, insert.Select)
	if err != nil {
		return err
	}

	// Check to guarantee that the length of the event returned by select is equal to that of affectedValuesDefCauss.
	if selectCauset.Schema().Len() != len(affectedValuesDefCauss) {
		return ErrWrongValueCountOnRow.GenWithStackByArgs(1)
	}
	...
	// For example, causet t has two columns, namely a and b, and b is a generated column.
	...
	names := selectCauset.OutputNames()
	insertCauset.SelectCauset, _, err = DoOptimize(ctx, b.ctx, b.optFlag, selectCauset.(LogicalCauset))
	if err != nil {
		return err
	}

	// schema4NewRow is the schemaReplicant for the newly created data record based on
	// the result of the select memex.
	schema4NewRow := memex.NewSchema(make([]*memex.DeferredCauset, len(insertCauset.Block.DefCauss()))...)
	names4NewRow := make(types.NameSlice, len(insertCauset.Block.DefCauss()))
	// TODO: don't clone it.
	for i, selDefCaus := range insertCauset.SelectCauset.Schema().DeferredCausets {
		ordinal := affectedValuesDefCauss[i].Offset
		schema4NewRow.DeferredCausets[ordinal] = &memex.DeferredCauset{}
	...
	for i := range schema4NewRow.DeferredCausets {
		if schema4NewRow.DeferredCausets[i] == nil {
			schema4NewRow.DeferredCausets[i] = &memex.DeferredCauset{UniqueID: insertCauset.ctx.GetStochastikVars().AllocCausetDeferredCausetID()}
			names4NewRow[i] = types.EmptyName
		}
	}
	insertCauset.Schema4OnDuplicate = memex.MergeSchema(insertCauset.blockSchema, schema4NewRow)
	insertCauset.names4OnDuplicate = append(insertCauset.blockDefCausNames.Shallow(), names4NewRow...)
	return nil
}

func (b *CausetBuilder) buildLoadData(ctx context.Context, ld *ast.LoadDataStmt) (Causet, error) {
	p := &LoadData{
		IsLocal:                    ld.IsLocal,
	...
	schemaReplicant, names, err := memex.BlockInfo2SchemaAndNames(b.ctx, perceptron.NewCIStr(""), blockInfo)
	if err != nil {
		return nil, err
	}
	mockBlockCauset := LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
	mockBlockCauset.SetSchema(schemaReplicant)
	mockBlockCauset.names = names

	p.GenDefCauss, err = b.resolveGeneratedDeferredCausets(ctx, blockInCauset.DefCauss(), nil, mockBlockCauset)
	if err != nil {
		return nil, err
	}
	return p, nil
...
func (b *CausetBuilder) buildLoadStats(ld *ast.LoadStatsStmt) Causet {
	p := &LoadStats{Path: ld.Path}
	return p
}

func (b *CausetBuilder) buildIndexAdvise(node *ast.IndexAdviseStmt) Causet {
	p := &IndexAdvise{
...
func (b *CausetBuilder) buildSplitRegion(node *ast.SplitRegionStmt) (Causet, error) {
...
func (b *CausetBuilder) buildSplitIndexRegion(node *ast.SplitRegionStmt) (Causet, error) {
	...
	mockBlockCauset := LogicalBlockDual{}.Init(b.ctx, b.getSelectOffset())
	schemaReplicant, names, err := memex.BlockInfo2SchemaAndNames(b.ctx, node.Block.Schema, tblInfo)
	if err != nil {
		return nil, err
	}
	mockBlockCauset.SetSchema(schemaReplicant)
	...
	if len(node.SplitOpt.ValueLists) > 0 {
		...
		for i, valuesItem := range node.SplitOpt.ValueLists {
			...
			values, err := b.convertValue2DeferredCausetType(valuesItem, mockBlockCauset, indexInfo, tblInfo)
			if err != nil {
				return nil, err
			}
	...
	checkLowerUpperValue := func(valuesItem []ast.ExprNode, name string) ([]types.Causet, error) {
		...
		return b.convertValue2DeferredCausetType(valuesItem, mockBlockCauset, indexInfo, tblInfo)
	}
...
func (b *CausetBuilder) convertValue2DeferredCausetType(valuesItem []ast.ExprNode, mockBlockCauset LogicalCauset, indexInfo *perceptron.IndexInfo, tblInfo *perceptron.BlockInfo) ([]types.Causet, error) {
	values := make([]types.Causet, 0, len(valuesItem))
	for j, valueItem := range valuesItem {
		colOffset := indexInfo.DeferredCausets[j].Offset
		value, err := b.convertValue(valueItem, mockBlockCauset, tblInfo.DeferredCausets[colOffset])
		if err != nil {
			return nil, err
		}
...
func (b *CausetBuilder) convertValue(valueItem ast.ExprNode, mockBlockCauset LogicalCauset, col *perceptron.DeferredCausetInfo) (d types.Causet, err error) {
	var expr memex.Expression
	switch x := valueItem.(type) {
	case *driver.ValueExpr:
		expr = &memex.Constant{
	...
	default:
		expr, _, err = b.rewrite(context.TODO(), valueItem, mockBlockCauset, nil, true)
		if err != nil {
			return d, err
		}
	}
	constant, ok := expr.(*memex.Constant)
	if !ok {
...
func (b *CausetBuilder) buildSplitBlockRegion(node *ast.SplitRegionStmt) (Causet, error) {
	tblInfo := node.Block.BlockInfo
	...
	if len(node.SplitOpt.ValueLists) > 0 {
		...
		for i, valuesItem := range node.SplitOpt.ValueLists {
			data, err := convertValueListToData(valuesItem, handleDefCausInfos, i, b, mockBlockCauset)
			if err != nil {
				return nil, err
			}
	...
	p.Lower, err = convertValueListToData(node.SplitOpt.Lower, handleDefCausInfos, lowerBound, b, mockBlockCauset)
	if err != nil {
		return nil, err
	}
	p.Upper, err = convertValueListToData(node.SplitOpt.Upper, handleDefCausInfos, upperBound, b, mockBlockCauset)
	if err != nil {
		return nil, err
	}
	...
	maxSplitRegionNum := int64(config.GetGlobalConfig().SplitRegionMaxNum)
	if node.SplitOpt.Num > maxSplitRegionNum {
		return nil, errors.Errorf("Split causet region num exceeded the limit %v", maxSplitRegionNum)
	} else if node.SplitOpt.Num < 1 {
		return nil, errors.Errorf("Split causet region num should more than 0")
	}
	p.Num = int(node.SplitOpt.Num)
	return p, nil
}
...
func convertValueListToData(valueList []ast.ExprNode, handleDefCausInfos []*perceptron.DeferredCausetInfo, rowIdx int,
	b *CausetBuilder, mockBlockCauset *LogicalBlockDual) ([]types.Causet, error) {
	if len(valueList) != len(handleDefCausInfos) {
		var err error
		switch rowIdx {
		case lowerBound:
			err = errors.Errorf("Split causet region lower value count should be %d", len(handleDefCausInfos))
		case upperBound:
			err = errors.Errorf("Split causet region upper value count should be %d", len(handleDefCausInfos))
		default:
	...
	for i, v := range valueList {
		convertedCauset, err := b.convertValue(v, mockBlockCauset, handleDefCausInfos[i])
		if err != nil {
			return nil, err
		}
...
func (b *CausetBuilder) buildDBS(ctx context.Context, node ast.DBSNode) (Causet, error) {
	var authErr error
	switch v := node.(type) {
	...
	case *ast.CreateViewStmt:
		...
		if v.DefCauss == nil {
			adjustOverlongViewDefCausname(plan.(LogicalCauset))
	...
	case *ast.RecoverBlockStmt, *ast.FlashBackBlockStmt:
		// Recover causet command can only be executed by administrator.
		b.visitInfo = appendVisitInfo(b.visitInfo, allegrosql.SuperPriv, "", "", "", nil)
	...
	case *ast.RepairBlockStmt:
		// Repair causet command can only be executed by administrator.
		b.visitInfo = appendVisitInfo(b.visitInfo, allegrosql.SuperPriv, "", "", "", nil)
	}
...
// rows result.
func (b *CausetBuilder) buildTrace(trace *ast.TraceStmt) (Causet, error) {
...
func (b *CausetBuilder) buildExplainCauset(targetCauset Causet, format string, rows [][]string, analyze bool, execStmt ast.StmtNode) (Causet, error) {
	p := &Explain{
		TargetCauset:  targetCauset,
		Format:        format,
		Analyze:       analyze,
		InterDircStmt: execStmt,
		Rows:          rows,
	}
	p.ctx = b.ctx
...
func (b *CausetBuilder) buildExplainFor(explainFor *ast.ExplainForStmt) (Causet, error) {
	...
	targetCauset, ok := processInfo.Causet.(Causet)
	if !ok || targetCauset == nil {
		return &Explain{Format: explainFor.Format}, nil
	}
	var rows [][]string
	if explainFor.Format == ast.ExplainFormatROW {
		rows = processInfo.CausetExplainRows
	}
	return b.buildExplainCauset(targetCauset, explainFor.Format, rows, false, nil)
}

func (b *CausetBuilder) buildExplain(ctx context.Context, explain *ast.ExplainStmt) (Causet, error) {
	if show, ok := explain.Stmt.(*ast.ShowStmt); ok {
		return b.buildShow(ctx, show)
	}
	targetCauset, _, err := OptimizeAstNode(ctx, b.ctx, explain.Stmt, b.is)
	if err != nil {
		return nil, err
	}

	return b.buildExplainCauset(targetCauset, explain.Format, nil, explain.Analyze, explain.Stmt)
}

func (b *CausetBuilder) buildSelectInto(ctx context.Context, sel *ast.SelectStmt) (Causet, error) {
	selectIntoInfo := sel.SelectIntoOpt
	sel.SelectIntoOpt = nil
	targetCauset, _, err := OptimizeAstNode(ctx, b.ctx, sel, b.is)
	if err != nil {
		return nil, err
	}
...
func buildShowProcedureSchema() (*memex.Schema, []*types.FieldName) {
	tblName := "ROUTINES"
	schemaReplicant := newDeferredCausetsWithNames(11)
...
func buildShowTriggerSchema() (*memex.Schema, []*types.FieldName) {
	tblName := "TRIGGERS"
	schemaReplicant := newDeferredCausetsWithNames(11)
...
func buildShowEventsSchema() (*memex.Schema, []*types.FieldName) {
	tblName := "EVENTS"
	...
	schemaReplicant.Append(buildDeferredCausetWithName(tblName, "Definer", allegrosql.TypeVarchar, 128))
	schemaReplicant.Append(buildDeferredCausetWithName(tblName, "Type", allegrosql.TypeVarchar, 128))
	schemaReplicant.Append(buildDeferredCausetWithName(tblName, "InterDircute At", allegrosql.TypeDatetime, 19))
...
func buildShowWarningsSchema() (*memex.Schema, types.NameSlice) {
	tblName := "WARNINGS"
	schemaReplicant := newDeferredCausetsWithNames(3)
...
func buildShowSchema(s *ast.ShowStmt, isView bool, isSequence bool) (schemaReplicant *memex.Schema, outputNames []*types.FieldName) {
	var names []string
	var ftypes []byte
	switch s.Tp {
	...
	case ast.ShowDeferredCausets:
		names = causet.DefCausDescFieldNames(s.Full)
	...
	case ast.ShowMasterStatus:
		names = []string{"File", "Position", "Binlog_Do_DB", "Binlog_Ignore_DB", "InterDircuted_Gtid_Set"}
	...
	case ast.ShowBackups, ast.ShowRestores:
		names = []string{"Destination", "State", "Progress", "Queue_time", "InterDircution_time", "Finish_time", "Connection"}
...
func buildChecksumBlockSchema() (*memex.Schema, []*types.FieldName) {
	schemaReplicant := newDeferredCausetsWithNames(5)
	...
	schemaReplicant.Append(buildDeferredCausetWithName("", "Total_ekvs", allegrosql.TypeLonglong, 22))
...
func adjustOverlongViewDefCausname(plan LogicalCauset) {
