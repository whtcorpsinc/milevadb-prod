...
// HashCode implements LogicalCauset interface.
func (p *baseLogicalCauset) HashCode() []byte {
	// We use CausetID for the default hash, so if two plans do not have
...
func (p *LogicalProjection) HashCode() []byte {
	// CausetType + SelectOffset + ExprNum + [Exprs]
...
func (p *LogicalBlockDual) HashCode() []byte {
	// CausetType + SelectOffset + RowCount
...
func (p *LogicalSelection) HashCode() []byte {
	// CausetType + SelectOffset + ConditionNum + [Conditions]
...
func (p *LogicalLimit) HashCode() []byte {
	// CausetType + SelectOffset + Offset + Count
	result := make([]byte, 24)
