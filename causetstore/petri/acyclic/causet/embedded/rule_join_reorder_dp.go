...
type joinReorderDPSolver struct {
	*baseSingleGroupJoinOrderSolver
	newJoin func(lChild, rChild LogicalCauset, eqConds []*memex.ScalarFunction, otherConds []memex.Expression) LogicalCauset
}

type joinGroupEqEdge struct {
	nodeIDs []int
	edge    *memex.ScalarFunction
}
...
func (s *joinReorderDPSolver) solve(joinGroup []LogicalCauset, eqConds []memex.Expression) (LogicalCauset, error) {
	...
	addEqEdge := func(node1, node2 int, edgeContent *memex.ScalarFunction) {
	...
	for _, cond := range eqConds {
		sf := cond.(*memex.ScalarFunction)
		lDefCaus := sf.GetArgs()[0].(*memex.DeferredCauset)
		rDefCaus := sf.GetArgs()[1].(*memex.DeferredCauset)
	...
	for _, cond := range s.otherConds {
		defcaus := memex.ExtractDeferredCausets(cond)
		mask := uint(0)
	...
	remainedOtherConds := make([]memex.Expression, 0, len(totalNonEqEdges))
...
// dpGraph is the embedded part of this algorithm.
// It implements the traditional join reorder algorithm: DP by subset using the following formula:
//   bestCauset[S:set of node] = the best one among Join(bestCauset[S1:subset of S], bestCauset[S2: S/S1])
func (s *joinReorderDPSolver) dpGraph(visitID2NodeID, nodeID2VisitID []int, joinGroup []LogicalCauset,
	totalEqEdges []joinGroupEqEdge, totalNonEqEdges []joinGroupNonEqEdge) (LogicalCauset, error) {
	nodeCnt := uint(len(visitID2NodeID))
	bestCauset := make([]*jrNode, 1<<nodeCnt)
	// bestCauset[s] is nil can be treated as bestCost[s] = +inf.
	for i := uint(0); i < nodeCnt; i++ {
		bestCauset[1<<i] = s.curJoinGroup[visitID2NodeID[i]]
	}
	...
	for nodeBitmap := uint(1); nodeBitmap < (1 << nodeCnt); nodeBitmap++ {
		...
		for sub := (nodeBitmap - 1) & nodeBitmap; sub > 0; sub = (sub - 1) & nodeBitmap {
			...
			if bestCauset[sub] == nil || bestCauset[remain] == nil {
				continue
			}
			...
			join, err := s.newJoinWithEdge(bestCauset[sub].p, bestCauset[remain].p, usedEdges, otherConds)
			if err != nil {
				return nil, err
			}
			curCost := s.calcJoinCumCost(join, bestCauset[sub], bestCauset[remain])
			if bestCauset[nodeBitmap] == nil {
				bestCauset[nodeBitmap] = &jrNode{
					p:       join,
					cumCost: curCost,
				}
			} else if bestCauset[nodeBitmap].cumCost > curCost {
				bestCauset[nodeBitmap].p = join
				bestCauset[nodeBitmap].cumCost = curCost
			}
		}
	}
	return bestCauset[(1<<nodeCnt)-1].p, nil
}

func (s *joinReorderDPSolver) nodesAreConnected(leftMask, rightMask uint, oldPos2NewPos []int,
	totalEqEdges []joinGroupEqEdge, totalNonEqEdges []joinGroupNonEqEdge) ([]joinGroupEqEdge, []memex.Expression) {
	var (
	...
	for _, edge := range totalNonEqEdges {
		// If the result is false, means that the current group hasn't covered the columns involved in the memex.
		if edge.nodeIDMask&(leftMask|rightMask) != edge.nodeIDMask {
			continue
		}
		// Check whether this memex is only built from one side of the join.
...
func (s *joinReorderDPSolver) newJoinWithEdge(leftCauset, rightCauset LogicalCauset, edges []joinGroupEqEdge, otherConds []memex.Expression) (LogicalCauset, error) {
	var eqConds []*memex.ScalarFunction
	for _, edge := range edges {
		lDefCaus := edge.edge.GetArgs()[0].(*memex.DeferredCauset)
		rDefCaus := edge.edge.GetArgs()[1].(*memex.DeferredCauset)
		if leftCauset.Schema().Contains(lDefCaus) {
			eqConds = append(eqConds, edge.edge)
		} else {
			newSf := memex.NewFunctionInternal(s.ctx, ast.EQ, edge.edge.GetType(), rDefCaus, lDefCaus).(*memex.ScalarFunction)
			eqConds = append(eqConds, newSf)
		}
	}
	join := s.newJoin(leftCauset, rightCauset, eqConds, otherConds)
	_, err := join.recursiveDeriveStats(nil)
	return join, err
}

// Make cartesian join as bushy tree.
func (s *joinReorderDPSolver) makeBushyJoin(cartesianJoinGroup []LogicalCauset, otherConds []memex.Expression) LogicalCauset {
	for len(cartesianJoinGroup) > 1 {
		resultJoinGroup := make([]LogicalCauset, 0, len(cartesianJoinGroup))
		for i := 0; i < len(cartesianJoinGroup); i += 2 {
			...
			mergedSchema := memex.MergeSchema(cartesianJoinGroup[i].Schema(), cartesianJoinGroup[i+1].Schema())
			var usedOtherConds []memex.Expression
			otherConds, usedOtherConds = memex.FilterOutInPlace(otherConds, func(expr memex.Expression) bool {
...
func findNodeIndexInGroup(group []LogicalCauset, col *memex.DeferredCauset) (int, error) {
...
func (s *joinReorderDPSolver) newJoinWithConds(leftCauset, rightCauset LogicalCauset, eqConds []*memex.ScalarFunction, otherConds []memex.Expression) LogicalCauset {
	join := s.newCartesianJoin(leftCauset, rightCauset)
