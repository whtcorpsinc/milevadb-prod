...
func (r *PushSelDownBlockScan) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	ts := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalBlockScan)
	...
	newTblScan := causetembedded.LogicalBlockScan{
		Source:         ts.Source,
	...
	newSel := causetembedded.LogicalSelection{Conditions: remained}.Init(sel.SCtx(), sel.SelectBlockOffset())
...
func (r *PushSelDownIndexScan) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	is := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalIndexScan)
	...
	if is.AccessConds != nil {
		...
		conditions = make([]memex.Expression, len(sel.Conditions)+len(is.AccessConds))
	...
	newIs := causetembedded.LogicalIndexScan{
		Source:             is.Source,
	...
	newSel := causetembedded.LogicalSelection{Conditions: res.RemainedConds}.Init(sel.SCtx(), sel.SelectBlockOffset())
	selExpr := memo.NewGroupExpr(newSel)
...
func (r *PushSelDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	sg := old.Children[0].GetExpr().ExprNode.(*causetembedded.EinsteinDBSingleGather)
	childGroup := old.Children[0].Children[0].Group
	var pushed, remained []memex.Expression
	sctx := sg.SCtx()
	pushed, remained = memex.PushDownExprs(sctx.GetStochastikVars().StmtCtx, sel.Conditions, sctx.GetClient(), ekv.EinsteinDB)
	if len(pushed) == 0 {
		return nil, false, false, nil
	}
	pushedSel := causetembedded.LogicalSelection{Conditions: pushed}.Init(sctx, sel.SelectBlockOffset())
	...
	//       pruning or skyline pruning, we need to make a copy of the EinsteinDBSingleGather here.
	...
	remainedSel := causetembedded.LogicalSelection{Conditions: remained}.Init(sel.SCtx(), sel.SelectBlockOffset())
...
// EnumeratePaths converts DataSource to causet scan and index scans.
...
func (r *EnumeratePaths) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	ds := old.GetExpr().ExprNode.(*causetembedded.DataSource)
...
func (r *PushAggDownGather) Match(expr *memo.ExprIter) bool {
	...
	agg := expr.GetExpr().ExprNode.(*causetembedded.LogicalAggregation)
	...
	return causetembedded.CheckAggCanPushCop(agg.SCtx(), agg.AggFuncs, agg.GroupByItems, ekv.EinsteinDB)
}
...
func (r *PushAggDownGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	agg := old.GetExpr().ExprNode.(*causetembedded.LogicalAggregation)
	aggSchema := old.GetExpr().Group.Prop.Schema
	gather := old.Children[0].GetExpr().ExprNode.(*causetembedded.EinsteinDBSingleGather)
	...
	gbyItems := make([]memex.Expression, len(agg.GroupByItems))
	copy(gbyItems, agg.GroupByItems)

	partialPref, finalPref, funcMap := causetembedded.BuildFinalModeAggregation(agg.SCtx(),
		&causetembedded.AggInfo{
			AggFuncs:     aggFuncs,
	...
	partialPref.AggFuncs =
		causetembedded.RemoveUnnecessaryFirstRow(agg.SCtx(), finalPref.AggFuncs, finalPref.GroupByItems, partialPref.AggFuncs, partialPref.GroupByItems, partialPref.Schema, funcMap)

	partialAgg := causetembedded.LogicalAggregation{
	...
	finalAgg := causetembedded.LogicalAggregation{
...
func (r *PushSelDownSort) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	sort := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalSort)
	childGroup := old.Children[0].GetExpr().Children[0]
...
func (r *PushSelDownProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	proj := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalProjection)
	...
	canBePushed := make([]memex.Expression, 0, len(sel.Conditions))
	canNotBePushed := make([]memex.Expression, 0, len(sel.Conditions))
	for _, cond := range sel.Conditions {
		if !memex.HasGetSetVarFunc(cond) {
			canBePushed = append(canBePushed, memex.DeferredCausetSubstitute(cond, projSchema, proj.Exprs))
		} else {
	...
	newBottomSel := causetembedded.LogicalSelection{Conditions: canBePushed}.Init(sel.SCtx(), sel.SelectBlockOffset())
	...
	newTopSel := causetembedded.LogicalSelection{Conditions: canNotBePushed}.Init(sel.SCtx(), sel.SelectBlockOffset())
...
func (r *PushSelDownAggregation) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	agg := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalAggregation)
	aggSchema := old.Children[0].Prop.Schema
	var pushedExprs []memex.Expression
	var remainedExprs []memex.Expression
	exprsOriginal := make([]memex.Expression, 0, len(agg.AggFuncs))
	for _, aggFunc := range agg.AggFuncs {
		exprsOriginal = append(exprsOriginal, aggFunc.Args[0])
	}
	groupByDeferredCausets := memex.NewSchema(agg.GetGroupByDefCauss()...)
	for _, cond := range sel.Conditions {
		switch cond.(type) {
		case *memex.Constant:
	...
	sctx := sel.SCtx()
	childGroup := old.Children[0].GetExpr().Children[0]
	pushedSel := causetembedded.LogicalSelection{Conditions: pushedExprs}.Init(sctx, sel.SelectBlockOffset())
	...
	remainedSel := causetembedded.LogicalSelection{Conditions: remainedExprs}.Init(sctx, sel.SelectBlockOffset())
...
func (r *PushSelDownWindow) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	window := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalWindow)
	...
	partitionDefCaussSchema := memex.NewSchema(window.GetPartitionByDefCauss()...)

	for _, cond := range sel.Conditions {
		if memex.ExprFromSchema(cond, partitionDefCaussSchema) {
...
func (r *TransformLimitToTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetembedded.LogicalLimit)
	sort := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalSort)
	childGroup := old.Children[0].GetExpr().Children[0]
	topN := causetembedded.LogicalTopN{
...
func (r *PushLimitDownProjection) Match(expr *memo.ExprIter) bool {
	proj := expr.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalProjection)
	for _, expr := range proj.Exprs {
		if memex.HasAssignSetVarFunc(expr) {
			return false
		}
...
func (r *PushLimitDownUnionAll) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetembedded.LogicalLimit)
	unionAll := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalUnionAll)
	unionAllSchema := old.Children[0].Group.Prop.Schema

	newLimit := causetembedded.LogicalLimit{
...
func buildChildSelectionGroup(
	oldSel *causetembedded.LogicalSelection,
	conditions []memex.Expression,
	childGroup *memo.Group) *memo.Group {
	if len(conditions) == 0 {
		return childGroup
	}
	newSel := causetembedded.LogicalSelection{Conditions: conditions}.Init(oldSel.SCtx(), oldSel.SelectBlockOffset())
...
func (r *PushSelDownJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	join := joinExpr.ExprNode.(*causetembedded.LogicalJoin)
	sctx := sel.SCtx()
	...
	var leftPushCond, rightPushCond, otherCond, leftCond, rightCond, remainCond []memex.Expression
	switch join.JoinType {
	case causetembedded.SemiJoin, causetembedded.InnerJoin:
		tempCond := make([]memex.Expression, 0,
		...
		tempCond = append(tempCond, memex.ScalarFuncs2Exprs(join.EqualConditions)...)
		tempCond = append(tempCond, join.OtherConditions...)
		tempCond = append(tempCond, sel.Conditions...)
		tempCond = memex.ExtractFiltersFromDNFs(sctx, tempCond)
		tempCond = memex.PropagateConstant(sctx, tempCond)
		// Return causet dual when filter is constant false or null.
		dual := causetembedded.Conds2BlockDual(join, tempCond)
		if dual != nil {
	...
	case causetembedded.LeftOuterJoin, causetembedded.LeftOuterSemiJoin, causetembedded.AntiLeftOuterSemiJoin,
		causetembedded.RightOuterJoin:
		...
		remainCond = make([]memex.Expression, len(sel.Conditions))
		copy(remainCond, sel.Conditions)
		nullSensitive := join.JoinType == causetembedded.AntiLeftOuterSemiJoin || join.JoinType == causetembedded.LeftOuterSemiJoin
		if join.JoinType == causetembedded.RightOuterJoin {
			joinConds, remainCond = memex.PropConstOverOuterJoin(join.SCtx(), joinConds, remainCond, rightGroup.Prop.Schema, leftGroup.Prop.Schema, nullSensitive)
		} else {
			joinConds, remainCond = memex.PropConstOverOuterJoin(join.SCtx(), joinConds, remainCond, leftGroup.Prop.Schema, rightGroup.Prop.Schema, nullSensitive)
		}
		...
		// Return causet dual when filter is constant false or null.
		dual := causetembedded.Conds2BlockDual(join, remainCond)
		if dual != nil {
			return []*memo.GroupExpr{memo.NewGroupExpr(dual)}, false, true, nil
		}
		if join.JoinType == causetembedded.RightOuterJoin {
			remainCond = memex.ExtractFiltersFromDNFs(join.SCtx(), remainCond)
			...
			derivedLeftJoinCond, _ := causetembedded.DeriveOtherConditions(join, true, false)
			leftCond = append(join.LeftConditions, derivedLeftJoinCond...)
			join.LeftConditions = nil
			remainCond = append(memex.ScalarFuncs2Exprs(equalCond), otherCond...)
			...
			leftCond = leftPushCond
			// Handle join conditions, only derive left join condition, because right join condition cannot be pushed down
			_, derivedRightJoinCond := causetembedded.DeriveOtherConditions(join, false, true)
	...
	leftCond = memex.RemoveDupExprs(sctx, leftCond)
	rightCond = memex.RemoveDupExprs(sctx, rightCond)
	...
	if len(remainCond) > 0 {
		newSel := causetembedded.LogicalSelection{Conditions: remainCond}.Init(sctx, sel.SelectBlockOffset())
...
func (r *EliminateProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// Promote the children group's memex.
...
func (r *MergeAdjacentProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	proj := old.GetExpr().ExprNode.(*causetembedded.LogicalProjection)
	childGroup := old.Children[0].Group
	child := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalProjection)
	if causetembedded.ExprsHasSideEffects(child.Exprs) {
		return nil, false, false, nil
	}

	replace := make(map[string]*memex.DeferredCauset)
	for i, defCaus := range childGroup.Prop.Schema.DeferredCausets {
		if defCausOrigin, ok := child.Exprs[i].(*memex.DeferredCauset); ok {
			replace[string(defCaus.HashCode(nil))] = defCausOrigin
		}
	}

	newProj := causetembedded.LogicalProjection{Exprs: make([]memex.Expression, len(proj.Exprs))}.Init(proj.SCtx(), proj.SelectBlockOffset())
	newProj.SetSchema(old.GetExpr().Group.Prop.Schema)
	for i, expr := range proj.Exprs {
		newExpr := expr.Clone()
		causetembedded.ResolveExprAndReplace(newExpr, replace)
		newProj.Exprs[i] = causetembedded.ReplaceDeferredCausetOfExpr(newExpr, child, childGroup.Prop.Schema)
	}
...
func (r *PushTopNDownOuterJoin) Match(expr *memo.ExprIter) bool {
	...
	join := expr.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalJoin)
	switch join.JoinType {
	case causetembedded.LeftOuterJoin, causetembedded.LeftOuterSemiJoin, causetembedded.AntiLeftOuterSemiJoin, causetembedded.RightOuterJoin:
		return true
	default:
		return false
	}
...
func pushTopNDownOuterJoinToChild(topN *causetembedded.LogicalTopN, outerGroup *memo.Group) *memo.Group {
	...
	newTopN := causetembedded.LogicalTopN{
...
func (r *PushTopNDownOuterJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	topN := old.GetExpr().ExprNode.(*causetembedded.LogicalTopN)
	joinExpr := old.Children[0].GetExpr()
	join := joinExpr.ExprNode.(*causetembedded.LogicalJoin)
...
func (r *PushTopNDownProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	for _, by := range topN.ByItems {
		newTopN.ByItems = append(newTopN.ByItems, &soliton.ByItems{
			Expr: memex.DeferredCausetSubstitute(by.Expr, old.Children[0].Group.Prop.Schema, proj.Exprs),
			Desc: by.Desc,
		})
	}
...
func (r *PushTopNDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	particalTopN := causetembedded.LogicalTopN{
		ByItems: topN.ByItems,
		Count:   topN.Count + topN.Offset,
	}.Init(topN.SCtx(), topN.SelectBlockOffset())
...
func (r *MergeAdjacentTopN) Match(expr *memo.ExprIter) bool {
	topN := expr.GetExpr().ExprNode.(*causetembedded.LogicalTopN)
	child := expr.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalTopN)
...
func (r *MergeAdjacentTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	topN := old.GetExpr().ExprNode.(*causetembedded.LogicalTopN)
	child := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalTopN)
	childGroups := old.Children[0].GetExpr().Children

	if child.Count <= topN.Offset {
		blockDual := causetembedded.LogicalBlockDual{RowCount: 0}.Init(child.SCtx(), child.SelectBlockOffset())
...
func (r *MergeAggregationProjection) Match(old *memo.ExprIter) bool {
	proj := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalProjection)
	if causetembedded.ExprsHasSideEffects(proj.Exprs) {
		return false
	}
	return true
}
...
func (r *MergeAggregationProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	olPosetDagg := old.GetExpr().ExprNode.(*causetembedded.LogicalAggregation)
	proj := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalProjection)
	projSchema := old.Children[0].GetExpr().Schema()

	groupByItems := make([]memex.Expression, len(olPosetDagg.GroupByItems))
	for i, item := range olPosetDagg.GroupByItems {
		groupByItems[i] = memex.DeferredCausetSubstitute(item, projSchema, proj.Exprs)
	}
	...
	for i, aggFunc := range olPosetDagg.AggFuncs {
		...
		newArgs := make([]memex.Expression, len(aggFunc.Args))
		for j, arg := range aggFunc.Args {
			newArgs[j] = memex.DeferredCausetSubstitute(arg, projSchema, proj.Exprs)
		}
		aggFuncs[i].Args = newArgs
	}

	newAgg := causetembedded.LogicalAggregation{
...
func (r *EliminateSingleMaxMin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// If there's no defCausumn in f.GetArgs()[0], we still need limit and read data from real causet because the result should be NULL if the input is empty.
	if len(memex.ExtractDeferredCausets(f.Args[0])) > 0 {
		...
		if !allegrosql.HasNotNullFlag(f.Args[0].GetType().Flag) {
			sel := causetembedded.LogicalSelection{}.Init(ctx, agg.SelectBlockOffset())
		...
		top1 := causetembedded.LogicalTopN{
			ByItems: byItems,
			Count:   1,
		...
		li := causetembedded.LogicalLimit{Count: 1}.Init(ctx, agg.SelectBlockOffset())
...
func (r *MergeAdjacentSelection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	child := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	childGroups := old.Children[0].GetExpr().Children

	conditions := make([]memex.Expression, 0, len(sel.Conditions)+len(child.Conditions))
	conditions = append(conditions, sel.Conditions...)
	conditions = append(conditions, child.Conditions...)
	newSel := causetembedded.LogicalSelection{Conditions: conditions}.Init(sel.SCtx(), sel.SelectBlockOffset())
	newSelExpr := memo.NewGroupExpr(newSel)
...
func (r *MergeAdjacentLimit) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetembedded.LogicalLimit)
	child := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalLimit)
	childGroups := old.Children[0].GetExpr().Children
...
func (r *TransformLimitToBlockDual) Match(expr *memo.ExprIter) bool {
	limit := expr.GetExpr().ExprNode.(*causetembedded.LogicalLimit)
...
func (r *TransformLimitToBlockDual) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetembedded.LogicalLimit)
	blockDual := causetembedded.LogicalBlockDual{RowCount: 0}.Init(limit.SCtx(), limit.SelectBlockOffset())
	blockDual.SetSchema(old.GetExpr().Schema())
	blockDualExpr := memo.NewGroupExpr(blockDual)
	return []*memo.GroupExpr{blockDualExpr}, true, true, nil
}
...
func (r *PushLimitDownOuterJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetembedded.LogicalLimit)
	join := old.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalJoin)
	joinSchema := old.Children[0].Group.Prop.Schema
	leftGroup := old.Children[0].GetExpr().Children[0]
	rightGroup := old.Children[0].GetExpr().Children[1]
...
func (r *PushLimitDownOuterJoin) pushLimitDownOuterJoinToChild(limit *causetembedded.LogicalLimit, outerGroup *memo.Group) *memo.Group {
	newLimit := causetembedded.LogicalLimit{
		Count: limit.Count + limit.Offset,
	}.Init(limit.SCtx(), limit.SelectBlockOffset())
...
func (r *PushLimitDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	particalLimit := causetembedded.LogicalLimit{
		Count: limit.Count + limit.Offset,
	}.Init(limit.SCtx(), limit.SelectBlockOffset())
...
func (*outerJoinEliminator) isInnerJoinKeysContainUniqueKey(innerGroup *memo.Group, joinKeys *memex.Schema) (bool, error) {
...
func (r *EliminateOuterJoinBelowAggregation) Match(expr *memo.ExprIter) bool {
	joinType := expr.Children[0].GetExpr().ExprNode.(*causetembedded.LogicalJoin).JoinType
	return joinType == causetembedded.LeftOuterJoin || joinType == causetembedded.RightOuterJoin
}
...
func (r *EliminateOuterJoinBelowAggregation) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// only when agg only use the defCausumns from outer causet can eliminate outer join.
	if !causetembedded.IsDefCaussAllFromOuterBlock(agg.GetUsedDefCauss(), outerUniqueIDs) {
		return nil, false, false, nil
	}
	// outer join elimination with duplicate agnostic aggregate functions.
	_, aggDefCauss := causetembedded.GetDupAgnosticAggDefCauss(agg, nil)
...
func (r *EliminateOuterJoinBelowProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// only when proj only use the defCausumns from outer causet can eliminate outer join.
	if !causetembedded.IsDefCaussAllFromOuterBlock(proj.GetUsedDefCauss(), outerUniqueIDs) {
		return nil, false, false, nil
	}
...
func (r *TransformAggregateCaseToSelection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	newSel := causetembedded.LogicalSelection{Conditions: newConditions}.Init(agg.SCtx(), agg.SelectBlockOffset())
	newSelExpr := memo.NewGroupExpr(newSel)
...
func (r *TransformAggregateCaseToSelection) transform(agg *causetembedded.LogicalAggregation) (ok bool, newConditions []memex.Expression, newAggFuncs []*aggregation.AggFuncDesc) {
	aggFuncDesc := agg.AggFuncs[0]
	aggFuncName := aggFuncDesc.Name
	ctx := agg.SCtx()

	caseFunc := aggFuncDesc.Args[0].(*memex.ScalarFunction)
	...
	var nullFlip = caseArgsNum == 3 && caseArgs[1].Equal(ctx, memex.NewNull()) && !caseArgs[2].Equal(ctx, memex.NewNull())
	// `case when a>0 then 0 else a end` should be converted to `case when !(a>0) then a else 0 end`.
	var zeroFlip = !nullFlip && caseArgsNum == 3 && caseArgs[1].Equal(ctx, memex.NewZero())

	var outputIdx int
	if nullFlip || zeroFlip {
		outputIdx = 2
		newConditions = []memex.Expression{memex.NewFunctionInternal(ctx, ast.UnaryNot, types.NewFieldType(allegrosql.TypeTiny), conditionFromCase)}
	} else {
		outputIdx = 1
		newConditions = memex.SplitCNFItems(conditionFromCase)
	}
	...
	if aggFuncDesc.HasDistinct {
		...
		if aggFuncName == ast.AggFuncCount && r.isOnlyOneNotNull(ctx, caseArgs, caseArgsNum, outputIdx) {
			...
			newAggFuncDesc.Args = []memex.Expression{caseArgs[outputIdx]}
	...
	switch {
	case r.allowsSelection(aggFuncName) && (caseArgsNum == 2 || caseArgs[3-outputIdx].Equal(ctx, memex.NewNull())), // Case A1
		aggFuncName == ast.AggFuncSum && caseArgsNum == 3 && caseArgs[3-outputIdx].Equal(ctx, memex.NewZero()): // Case A2
		newAggFuncDesc := aggFuncDesc.Clone()
		newAggFuncDesc.Args = []memex.Expression{caseArgs[outputIdx]}
		return true, newConditions, []*aggregation.AggFuncDesc{newAggFuncDesc}
	default:
...
func (r *TransformAggregateCaseToSelection) isOnlyOneNotNull(ctx stochastikctx.Context, args []memex.Expression, argsNum int, outputIdx int) bool {
	return !args[outputIdx].Equal(ctx, memex.NewNull()) && (argsNum == 2 || args[3-outputIdx].Equal(ctx, memex.NewNull()))
}

// TransformAggregateCaseToSelection only support `case when cond then var end` and `case when cond then var1 else var2 end`.
func (r *TransformAggregateCaseToSelection) isTwoOrThreeArgCase(expr memex.Expression) bool {
	scalarFunc, ok := expr.(*memex.ScalarFunction)
	if !ok {
		return false
	}
...
func (r *TransformAggToProj) Match(expr *memo.ExprIter) bool {
	...
	schemaByGroupby := memex.NewSchema(agg.GetGroupByDefCauss()...)
...
func (r *TransformAggToProj) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	agg := old.GetExpr().ExprNode.(*causetembedded.LogicalAggregation)
	if ok, proj := causetembedded.ConvertAggToProj(agg, old.GetExpr().Schema()); ok {
		newProjExpr := memo.NewGroupExpr(proj)
...
// The reason why we need this rule is that, TopNInterlockingDirectorate in MilevaDB does not support ScalarFunction
...
func (r *InjectProjectionBelowTopN) Match(expr *memo.ExprIter) bool {
	...
	for _, item := range topN.ByItems {
		if _, ok := item.Expr.(*memex.ScalarFunction); ok {
			return true
		}
	}
	return false
...
func (r *InjectProjectionBelowTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	topProjExprs := make([]memex.Expression, oldTopNSchema.Len())
	for i := range oldTopNSchema.DeferredCausets {
		topProjExprs[i] = oldTopNSchema.DeferredCausets[i]
	}
	topProj := causetembedded.LogicalProjection{
		Exprs: topProjExprs,
	}.Init(topN.SCtx(), topN.SelectBlockOffset())
	topProj.SetSchema(oldTopNSchema)

	// Construct bottom Projection.
	bottomProjExprs := make([]memex.Expression, 0, oldTopNSchema.Len()+len(topN.ByItems))
	bottomProjSchema := make([]*memex.DeferredCauset, 0, oldTopNSchema.Len()+len(topN.ByItems))
	...
	for _, item := range topN.ByItems {
		...
		newDefCaus := &memex.DeferredCauset{
			UniqueID: topN.SCtx().GetStochastikVars().AllocCausetDeferredCausetID(),
	...
	bottomProj := causetembedded.LogicalProjection{
		Exprs: bottomProjExprs,
	}.Init(topN.SCtx(), topN.SelectBlockOffset())
	newSchema := memex.NewSchema(bottomProjSchema...)
...
// The reason why we need this rule is that, AggInterlockingDirectorate in MilevaDB does not support ScalarFunction
...
func (r *InjectProjectionBelowAgg) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	for i := 0; !hasScalarFunc && i < len(agg.GroupByItems); i++ {
		_, isScalarFunc := agg.GroupByItems[i].(*memex.ScalarFunction)
		hasScalarFunc = hasScalarFunc || isScalarFunc
	}
	if !hasScalarFunc {
		return nil, false, false, nil
	...
	projSchemaDefCauss := make([]*memex.DeferredCauset, 0, len(copyFuncs)+len(agg.GroupByItems))
		...
		for i, arg := range f.Args {
			...
			default:
				...
				newArg := &memex.DeferredCauset{
					UniqueID: agg.SCtx().GetStochastikVars().AllocCausetDeferredCausetID(),
					RetType:  arg.GetType(),
				}
	...
	newGroupByItems := make([]memex.Expression, len(agg.GroupByItems))
	...
	proj := causetembedded.LogicalProjection{
		Exprs: projExprs,
	}.Init(agg.SCtx(), agg.SelectBlockOffset())
	projSchema := memex.NewSchema(projSchemaDefCauss...)
...
func (r *TransformApplyToJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	apply := old.GetExpr().ExprNode.(*causetembedded.LogicalApply)
...
func (r *TransformApplyToJoin) extractCorDeferredCausetsBySchema(innerGroup *memo.Group, outerSchema *memex.Schema) []*memex.CorrelatedDeferredCauset {
	corDefCauss := r.extractCorDeferredCausetsFromGroup(innerGroup)
	return causetembedded.ExtractCorDeferredCausetsBySchema(corDefCauss, outerSchema, false)
}

func (r *TransformApplyToJoin) extractCorDeferredCausetsFromGroup(g *memo.Group) []*memex.CorrelatedDeferredCauset {
...
func (r *PullSelectionUpApply) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	sel := old.Children[1].GetExpr().ExprNode.(*causetembedded.LogicalSelection)
	...
	newApply := causetembedded.LogicalApply{
