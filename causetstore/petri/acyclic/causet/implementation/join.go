...
func (impl *HashJoinImpl) CalcCost(outCount float64, children ...memo.Implementation) float64 {
	hashJoin := impl.plan.(*causetembedded.PhysicalHashJoin)
...
func NewHashJoinImpl(hashJoin *causetembedded.PhysicalHashJoin) *HashJoinImpl {
...
func (impl *MergeJoinImpl) CalcCost(outCount float64, children ...memo.Implementation) float64 {
	mergeJoin := impl.plan.(*causetembedded.PhysicalMergeJoin)
	// The children here are only used to calculate the cost.
...
func NewMergeJoinImpl(mergeJoin *causetembedded.PhysicalMergeJoin) *MergeJoinImpl {
