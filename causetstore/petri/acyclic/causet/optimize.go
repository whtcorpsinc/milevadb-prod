...
func GetPreparedStmt(stmt *ast.InterDircuteStmt, vars *variable.StochastikVars) (ast.StmtNode, error) {
	...
	if stmt.Name != "" {
		if execID, ok = vars.PreparedStmtNameToID[stmt.Name]; !ok {
			return nil, causetembedded.ErrStmtNotFound
		}
	}
...
func Optimize(ctx context.Context, sctx stochastikctx.Context, node ast.Node, is schemareplicant.SchemaReplicant) (causetembedded.Causet, types.NameSlice, error) {
	...
	if _, isolationReadContainEinsteinDB := sessVars.IsolationReadEngines[ekv.EinsteinDB]; isolationReadContainEinsteinDB {
		var fp causetembedded.Causet
		if fpv, ok := sctx.Value(causetembedded.PointCausetKey).(causetembedded.PointCausetVal); ok {
			// point plan is already tried in a multi-memex query.
			fp = fpv.Causet
		} else {
			fp = causetembedded.TryFastCauset(sctx, node)
		}
		if fp != nil {
			if !useMaxTS(sctx, fp) {
	...
	bestCausetHint := causetembedded.GenHintsFromPhysicalCauset(bestCauset)
	...
	var bestCausetAmongHints causetembedded.Causet
	...
	if sctx.GetStochastikVars().EvolveCausetBaselines && binding == nil &&
		!originHints.ContainBlockHint(causetembedded.HintReadFromStorage) &&
		!bindRecord.Bindings[0].Hint.ContainBlockHint(causetembedded.HintReadFromStorage) {
...
func optimize(ctx context.Context, sctx stochastikctx.Context, node ast.Node, is schemareplicant.SchemaReplicant) (causetembedded.Causet, types.NameSlice, float64, error) {
	// build logical plan
	...
	builder := causetembedded.NewCausetBuilder(sctx, is, hintProcessor)
	...
	if pm := privilege.GetPrivilegeManager(sctx); pm != nil {
		if err := causetembedded.CheckPrivilege(activeRoles, pm, builder.GetVisitInfo()); err != nil {
			return nil, nil, 0, err
		}
	}

	if err := causetembedded.CheckBlockLock(sctx, is, builder.GetVisitInfo()); err != nil {
		return nil, nil, 0, err
	}

	// Handle the execute memex.
	if execCauset, ok := p.(*causetembedded.InterDircute); ok {
	...
	logic, isLogicalCauset := p.(causetembedded.LogicalCauset)
	if !isLogicalCauset {
	...
	beginOpt := time.Now()
	finalCauset, cost, err := causetembedded.DoOptimize(ctx, sctx, builder.GetOptFlag(), logic)
...
func extractSelectAndNormalizeDigest(stmtNode ast.StmtNode) (*ast.SelectStmt, string, string) {
	...
	case *ast.ExplainStmt:
		switch x.Stmt.(type) {
		case *ast.SelectStmt:
			causetembedded.EraseLastSemicolon(x)
...
func useMaxTS(ctx stochastikctx.Context, p causetembedded.Causet) bool {
	if !causetembedded.IsAutoCommitTxn(ctx) {
		return false
	}

	v, ok := p.(*causetembedded.PointGetCauset)
...
func OptimizeInterDircStmt(ctx context.Context, sctx stochastikctx.Context,
	execAst *ast.InterDircuteStmt, is schemareplicant.SchemaReplicant) (causetembedded.Causet, error) {
	defer trace.StartRegion(ctx, "Optimize").End()
	var err error
	builder := causetembedded.NewCausetBuilder(sctx, is, nil)
...
func init() {
	causetembedded.OptimizeAstNode = Optimize
}
