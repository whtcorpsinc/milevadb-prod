...
func (spacetime hashMeta) Value() []byte {
	buf := make([]byte, 8)
	binary.BigEndian.PutUint64(buf[0:8], uint64(spacetime.FieldCount))
	return buf
}

func (spacetime hashMeta) IsEmpty() bool {
	return spacetime.FieldCount <= 0
}
...
func (t *TxStructure) HSet(key []byte, field []byte, value []byte) error {
	...
	return t.uFIDelateHash(key, field, func([]byte) ([]byte, error) {
		return value, nil
	})
}
...
func (t *TxStructure) HInc(key []byte, field []byte, step int64) (int64, error) {
	...
	base := int64(0)
	err := t.uFIDelateHash(key, field, func(oldValue []byte) ([]byte, error) {
		if oldValue != nil {
			var err error
...
func (t *TxStructure) uFIDelateHash(key []byte, field []byte, fn func(oldValue []byte) ([]byte, error)) error {
	dataKey := t.encodeHashDataKey(key, field)
	...
	spacetimeKey := t.encodeHashMetaKey(key)
	spacetime, err := t.loadHashMeta(spacetimeKey)
	if err != nil {
		return errors.Trace(err)
	}

	if oldValue == nil {
		spacetime.FieldCount++
		if err = t.readWriter.Set(spacetimeKey, spacetime.Value()); err != nil {
			return errors.Trace(err)
		}
	}
...
func (t *TxStructure) HLen(key []byte) (int64, error) {
	...
	return spacetime.FieldCount, nil
}
...
func (t *TxStructure) HClear(key []byte) error {
	...
	return errors.Trace(t.readWriter.Delete(spacetimeKey))
}
...
func (t *TxStructure) loadHashMeta(spacetimeKey []byte) (hashMeta, error) {
	...
	spacetime := hashMeta{FieldCount: 0}
	if v == nil {
	...
	spacetime.FieldCount = int64(binary.BigEndian.Uint64(v[0:8]))
	return spacetime, nil
}
