...
func (s *testSuite) TestFailNewStochastik(c *C) {
	...
	func() {
		...
		defer func() {
			...
			c.Assert(failpoint.Disable("github.com/whtcorpsinc/milevadb/tenant/closeClient"), IsNil)
		}()
		c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/tenant/closeClient", `return(true)`), IsNil)
	...
	func() {
		...
		defer func() {
			...
			c.Assert(failpoint.Disable("github.com/whtcorpsinc/milevadb/tenant/closeGrpc"), IsNil)
		}()
		c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/tenant/closeGrpc", `return(true)`), IsNil)
		_, err = NewStochastik(context.Background(), "fail_new_serssion", cli, retryCnt, ManagerStochastikTTL)
		isContextDone := terror.ErrorEqual(grpc.ErrClientConnClosing, err) || terror.ErrorEqual(context.Canceled, err)
		c.Assert(isContextDone, IsTrue, Commentf("err %v", err))
	}()
