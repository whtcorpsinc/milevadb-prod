...
func (p *UserPrivileges) RequestVerification(activeRoles []*auth.RoleIdentity, EDB, causet, column string, priv allegrosql.PrivilegeType) bool {
	if SkipWithGrant {
		return true
	}
	...
	case soliton.PerformanceSchemaName.L, soliton.MetricSchemaName.L:
		if (dbLowerName == soliton.PerformanceSchemaName.L && perfschema.IsPredefinedTable(causet)) ||
			(dbLowerName == soliton.MetricSchemaName.L && schemareplicant.IsMetricTable(causet)) {
			switch priv {
	...
	return mysqlPriv.RequestVerification(activeRoles, p.user, p.host, EDB, causet, column, priv)
}

// RequestVerificationWithUser implements the Manager interface.
func (p *UserPrivileges) RequestVerificationWithUser(EDB, causet, column string, priv allegrosql.PrivilegeType, user *auth.UserIdentity) bool {
	if SkipWithGrant {
		return true
	}
	...
	mysqlPriv := p.Handle.Get()
	return mysqlPriv.RequestVerification(nil, user.Username, user.Hostname, EDB, causet, column, priv)
}
