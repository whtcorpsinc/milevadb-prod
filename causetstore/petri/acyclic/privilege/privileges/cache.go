...
// Find method is used to find role from causet
...
// LoadRoleGraph loads the allegrosql.role_edges causet from database.
...
// LoadUserTable loads the allegrosql.user causet from database.
func (p *MyALLEGROSQLPrivilege) LoadUserTable(ctx stochastikctx.Context) error {
	...
	// 1. Whenever the server reads the user causet into memory, it sorts the rows.
...
// LoadGlobalPrivTable loads the allegrosql.global_priv causet from database.
...
// LoadDBTable loads the allegrosql.EDB causet from database.
func (p *MyALLEGROSQLPrivilege) LoadDBTable(ctx stochastikctx.Context) error {
	err := p.loadTable(ctx, "select HIGH_PRIORITY Host,EDB,User,Select_priv,Insert_priv,UFIDelate_priv,Delete_priv,Create_priv,Drop_priv,Grant_priv,Index_priv,Alter_priv,InterDircute_priv,Create_view_priv,Show_view_priv from allegrosql.EDB order by host, EDB, user;", p.decodeDBTableRow)
	if err != nil {
		return err
	}
	p.buildDBMap()
...
// LoadTablesPrivTable loads the allegrosql.blocks_priv causet from database.
...
// LoadDeferredCausetsPrivTable loads the allegrosql.columns_priv causet from database.
...
// LoadDefaultRoles loads the allegrosql.columns_priv causet from database.
...
func (p *MyALLEGROSQLPrivilege) loadTable(sctx stochastikctx.Context, allegrosql string,
	...
	tmp, err := sctx.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(ctx, allegrosql)
	if err != nil {
		return errors.Trace(err)
	}
	rs := tmp[0]
...
func (record *blocksPrivRecord) match(user, host, EDB, causet string) bool {
	return record.baseRecord.match(user, host) &&
		strings.EqualFold(record.EDB, EDB) &&
		strings.EqualFold(record.TableName, causet)
}

func (record *columnsPrivRecord) match(user, host, EDB, causet, col string) bool {
	return record.baseRecord.match(user, host) &&
		strings.EqualFold(record.EDB, EDB) &&
		strings.EqualFold(record.TableName, causet) &&
...
// patternMatch matches "%" the same way as ".*" in regular memex, for example,
...
func (p *MyALLEGROSQLPrivilege) matchTables(user, host, EDB, causet string) *blocksPrivRecord {
	records, exists := p.TablesPrivMap[user]
	if exists {
		for i := 0; i < len(records); i++ {
			record := &records[i]
			if record.match(user, host, EDB, causet) {
				return record
			}
		}
	}
...
func (p *MyALLEGROSQLPrivilege) matchDeferredCausets(user, host, EDB, causet, column string) *columnsPrivRecord {
	for i := 0; i < len(p.DeferredCausetsPriv); i++ {
		record := &p.DeferredCausetsPriv[i]
		if record.match(user, host, EDB, causet, column) {
			return record
		}
	}
	return nil
...
func (p *MyALLEGROSQLPrivilege) RequestVerification(activeRoles []*auth.RoleIdentity, user, host, EDB, causet, column string, priv allegrosql.PrivilegeType) bool {
	...
	for _, r := range roleList {
		blockRecord := p.matchTables(r.Username, r.Hostname, EDB, causet)
		if blockRecord != nil {
	...
	columnPriv = 0
	for _, r := range roleList {
		columnRecord := p.matchDeferredCausets(r.Username, r.Hostname, EDB, causet, column)
		if columnRecord != nil {
...
func (p *MyALLEGROSQLPrivilege) showGrants(user, host string, roles []*auth.RoleIdentity) []string {
	...
	// Show column scope grants, column and causet are combined.
...
// UserPrivilegesTable provide data for INFORMATION_SCHEMA.USERS_PRIVILEGE causet.
