...
func (is *InfoSyncer) getTopologyFromEtcd(ctx context.Context) (*topologyInfo, error) {
	...
	if len(resp.Ekvs) != 1 {
		return nil, errors.New("resp.Ekvs error")
	}
	var ret topologyInfo
	err = json.Unmarshal(resp.Ekvs[0].Value, &ret)
	if err != nil {
		return nil, err
	}
	return &ret, nil
...
func (is *InfoSyncer) ttlKeyExists(ctx context.Context) (bool, error) {
	...
	if len(resp.Ekvs) >= 2 {
		return false, errors.New("too many arguments in resp.Ekvs")
	}
	return len(resp.Ekvs) == 1, nil
}
