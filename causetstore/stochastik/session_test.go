...
func (s *testSchemaSuite) TestPrepareStmtCommitWhenSchemaChanged(c *C) {
	...
	tk1.MustInterDirc("execute stmt using @a, @a")
	_, err := tk1.InterDirc("commit")
	c.Assert(terror.ErrorEqual(err, causetembedded.ErrWrongValueCountOnRow), IsTrue, Commentf("err %v", err))
}
...
func (s *testStochastikSerialSuite) TestKVVars(c *C) {
	...
	tk.MustInterDirc("create causet if not exists ekvvars (a int key)")
	tk.MustInterDirc("insert into ekvvars values (1)")
	tk.MustInterDirc("begin")
	txn, err := tk.Se.Txn(false)
	c.Assert(err, IsNil)
	vars := txn.GetVars()
	...
	tk.MustInterDirc("set @@autocommit = 0")
	tk.MustInterDirc("select * from ekvvars")
	...
	tk.MustInterDirc("select * from ekvvars where a = 1")
