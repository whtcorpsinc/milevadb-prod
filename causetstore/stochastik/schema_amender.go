...
type amendOp interface {
	genMutations(ctx context.Context, sctx stochastikctx.Context, commitMutations einsteindb.CommitterMutations, ekvMap *rowEkvMap,
...
func (a *amendOperationDeleteOldIndex) genMutations(ctx context.Context, sctx stochastikctx.Context,
	commitMutations einsteindb.CommitterMutations, ekvMap *rowEkvMap, resAddMutations *einsteindb.CommitterMutations) error {
	for i, key := range commitMutations.GetKeys() {
		...
		err := a.processRowKey(ctx, sctx, key, ekvMap.oldRowEkvMap, resAddMutations)
		if err != nil {
			return err
		}
	}
...
func (a *amendOperationAddNewIndex) genMutations(ctx context.Context, sctx stochastikctx.Context, commitMutations einsteindb.CommitterMutations,
	ekvMap *rowEkvMap, resAddMutations *einsteindb.CommitterMutations) error {
	for i, key := range commitMutations.GetKeys() {
		keyOp := commitMutations.GetOps()[i]
		if blockcodec.IsIndexKey(key) || blockcodec.DecodeTableID(key) != a.info.tblInfoAtCommit.Meta().ID {
			continue
		...
		err := a.processRowKey(ctx, sctx, key, ekvMap.newRowEkvMap, resAddMutations)
		if err != nil {
			return err
		}
	}
...
func (a *amendOperationAddIndexInfo) genIndexKeyValue(ctx context.Context, sctx stochastikctx.Context, ekvMap map[string][]byte,
	key []byte, ekvHandle ekv.Handle, keyOnly bool) ([]byte, []byte, error) {
	chk := a.chk
	chk.Reset()
	val, ok := ekvMap[string(key)]
	if !ok {
	...
	err := interlock.DecodeRowValToChunk(sctx, a.schemaAndCausetDecoder.schemaReplicant, a.tblInfoAtStart.Meta(), ekvHandle, val, chk, a.schemaAndCausetDecoder.causetDecoder)
	if err != nil {
	...
	newIdxKey, distinct, err := blockcodec.GenIndexKey(sctx.GetStochastikVars().StmtCtx,
		a.tblInfoAtCommit.Meta(), a.indexInfoAtCommit.Meta(), a.tblInfoAtCommit.Meta().ID, idxVals, ekvHandle, nil)
	if err != nil {
	...
	newIdxVal, err := blockcodec.GenIndexValue(sctx.GetStochastikVars().StmtCtx, a.tblInfoAtCommit.Meta(),
		a.indexInfoAtCommit.Meta(), containsNonBinaryString, distinct, false, idxVals, ekvHandle)
	if err != nil {
...
func (a *amendOperationAddNewIndex) processRowKey(ctx context.Context, sctx stochastikctx.Context, key []byte,
	ekvMap map[string][]byte, resAddMutations *einsteindb.CommitterMutations) error {
	ekvHandle, err := blockcodec.DecodeRowKey(key)
	if err != nil {
		logutil.Logger(ctx).Error("decode key error", zap.String("key", hex.EncodeToString(key)), zap.Error(err))
		return errors.Trace(err)
	}

	newIdxKey, newIdxValue, err := a.info.genIndexKeyValue(ctx, sctx, ekvMap, key, ekvHandle, false)
	if err != nil {
		return errors.Trace(err)
	}
...
func (a *amendOperationDeleteOldIndex) processRowKey(ctx context.Context, sctx stochastikctx.Context, key []byte,
	oldValEkvMap map[string][]byte, resAddMutations *einsteindb.CommitterMutations) error {
	ekvHandle, err := blockcodec.DecodeRowKey(key)
	if err != nil {
		logutil.Logger(ctx).Error("decode key error", zap.String("key", hex.EncodeToString(key)), zap.Error(err))
		return errors.Trace(err)
	...
	newIdxKey, emptyVal, err := a.info.genIndexKeyValue(ctx, sctx, oldValEkvMap, key, ekvHandle, true)
	if err != nil {
		return errors.Trace(err)
	}
...
type rowEkvMap struct {
	oldRowEkvMap map[string][]byte
	newRowEkvMap map[string][]byte
}

func (s *SchemaAmender) prepareEkvMap(ctx context.Context, commitMutations einsteindb.CommitterMutations, info *amendDefCauslector) (*rowEkvMap, error) {
	...
	newValEkvMap, err := txn.BatchGet(ctx, addKeys)
	if err != nil {
		logutil.Logger(ctx).Warn("amend failed to batch get ekv new keys", zap.Error(err))
		return nil, errors.Trace(err)
	}
	if len(newValEkvMap) != len(addKeys) {
		logutil.Logger(ctx).Error("amend failed to batch get results invalid",
			zap.Int("addKeys len", len(addKeys)), zap.Int("newValEkvMap", len(newValEkvMap)))
		return nil, errors.Errorf("add keys has %v values but result ekvMap has %v", len(addKeys), len(newValEkvMap))
	}
	...
	oldValEkvMap, err := snapshot.BatchGet(ctx, removeKeys)
	if err != nil {
	...
	res := &rowEkvMap{
		oldRowEkvMap: oldValEkvMap,
		newRowEkvMap: newValEkvMap,
	}
	return res, nil
}
...
func (s *SchemaAmender) genAllAmendMutations(ctx context.Context, commitMutations einsteindb.CommitterMutations,
	...
	rowEkvMap, err := s.prepareEkvMap(ctx, commitMutations, info)
	if err != nil {
		return nil, err
	}
	...
	for _, amendOps := range info.tblAmendOpMap {
		for _, curOp := range amendOps {
			err := curOp.genMutations(ctx, s.sess, commitMutations, rowEkvMap, &resultNewMutations)
			if err != nil {
				return nil, err
			}
		}
