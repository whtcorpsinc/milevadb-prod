...
type stochastik struct {
	...
	currentCauset causetembedded.Causet
...
func (s *stochastik) cleanRetryInfo() {
	...
	planCacheEnabled := causetembedded.PreparedCausetCacheEnabled()
	...
	if planCacheEnabled {
		...
		if preparedPointer, ok := s.stochastikVars.PreparedStmts[firstStmtID]; ok {
			...
			if ok {
				...
				cacheKey = causetembedded.NewPSTMTCausetCacheKey(s.stochastikVars, firstStmtID, preparedAst.SchemaVersion)
			}
		}
	}
	for i, stmtID := range retryInfo.DroppedPreparedStmtIDs {
		if planCacheEnabled {
			if i > 0 && preparedAst != nil {
				causetembedded.SetPstmtIDSchemaVersion(cacheKey, stmtID, preparedAst.SchemaVersion, s.stochastikVars.IsolationReadEngines)
			}
...
func (s *stochastik) PreparedCausetCache() *ekvcache.SimpleLRUCache {
...
func (s *stochastik) FieldList(blockName string) ([]*ast.ResultField, error) {
	...
	if pm != nil && s.stochastikVars.User != nil {
		if !pm.RequestVerification(s.stochastikVars.ActiveRoles, dbName.O, tName.O, "", allegrosql.AllPrivMask) {
			...
			return nil, causetembedded.ErrTableaccessDenied.GenWithStackByArgs("SELECT", u, h, blockName)
		}
	}
...
func (s *stochastik) SetProcessInfo(allegrosql string, t time.Time, command byte, maxInterDircutionTime uint64) {
	...
	pi := soliton.ProcessInfo{
		...
		CausetExplainRows:     causetembedded.GetExplainRowsForCauset(s.currentCauset),
		Time:                  t,
		State:                 s.Status(),
		Info:                  allegrosql,
		CurTxnStartTS:         curTxnStartTS,
		StmtCtx:               s.stochastikVars.StmtCtx,
		StatsInfo:             causetembedded.GetStatsInfo,
...
func (s *stochastik) preparedStmtInterDirc(ctx context.Context,
	stmtID uint32, prepareStmt *causetembedded.CachedPrepareStmt, args []types.Causet) (sqlexec.RecordSet, error) {
...
func (s *stochastik) IsCachedInterDircOk(ctx context.Context, preparedStmt *causetembedded.CachedPrepareStmt) (bool, error) {
	...
	case *causetembedded.PointGetCauset:
		ok = true
	case *causetembedded.UFIDelate:
		pointUFIDelate := prepared.CachedCauset.(*causetembedded.UFIDelate)
		_, ok = pointUFIDelate.SelectCauset.(*causetembedded.PointGetCauset)
		if !ok {
...
func (s *stochastik) InterDircutePreparedStmt(ctx context.Context, stmtID uint32, args []types.Causet) (sqlexec.RecordSet, error) {
	...
	if !ok {
		err = causetembedded.ErrStmtNotFound
		logutil.Logger(ctx).Error("prepared memex not found", zap.Uint32("stmtID", stmtID))
		return nil, err
	}
	preparedStmt, ok := preparedPointer.(*causetembedded.CachedPrepareStmt)
	if !ok {
...
func (s *stochastik) DropPreparedStmt(stmtID uint32) error {
	...
	if _, ok := vars.PreparedStmts[stmtID]; !ok {
		return causetembedded.ErrStmtNotFound
	}
...
func createStochastikWithOpt(causetstore ekv.CausetStorage, opt *Opt) (*stochastik, error) {
	...
	if causetembedded.PreparedCausetCacheEnabled() {
		if opt != nil && opt.PreparedCausetCache != nil {
			...
			s.preparedCausetCache = ekvcache.NewSimpleLRUCache(causetembedded.PreparedCausetCacheCapacity,
				causetembedded.PreparedCausetCacheMemoryGuardRatio, causetembedded.PreparedCausetCacheMaxMemory.Load())
		}
	}
