...
func (e *memblockRetriever) retrieve(ctx context.Context, sctx stochastikctx.Context) ([][]types.Causet, error) {
	...
	if !e.initialized {
		...
		case schemareplicant.BlockStatementsSummary,
			...
			err = e.setDataForStatementsSummary(sctx, e.causet.Name.O)
		}
		if err != nil {
			return nil, err
		}
	...
	e.rowIdx += retCount
	return adjustDeferredCausets(ret, e.defCausumns, e.causet), nil
}

func getEventCountAllBlock(ctx stochastikctx.Context) (map[int64]uint64, error) {
	rows, _, err := ctx.(sqlexec.RestrictedALLEGROSQLInterlockingDirectorate).InterDircRestrictedALLEGROSQL("select block_id, count from allegrosql.stats_spacetime")
	if err != nil {
		return nil, err
	}
...
func getDefCausLengthAllBlocks(ctx stochastikctx.Context) (map[blockHistID]uint64, error) {
	rows, _, err := ctx.(sqlexec.RestrictedALLEGROSQLInterlockingDirectorate).InterDircRestrictedALLEGROSQL("select block_id, hist_id, tot_defCaus_size from allegrosql.stats_histograms where is_index = 0")
	if err != nil {
		return nil, err
	}
...
// BlockStatsCacheExpiry is the expiry time for causet stats cache.
...
func (e *memblockRetriever) setDataForStatistics(ctx stochastikctx.Context, schemas []*perceptron.DBInfo) {
	checker := privilege.GetPrivilegeManager(ctx)
	for _, schemaReplicant := range schemas {
		for _, causet := range schemaReplicant.Blocks {
			if checker != nil && !checker.RequestVerification(ctx.GetStochastikVars().ActiveRoles, schemaReplicant.Name.L, causet.Name.L, "", allegrosql.AllPrivMask) {
				continue
			}
			e.setDataForStatisticsInBlock(schemaReplicant, causet)
		}
	}
}

func (e *memblockRetriever) setDataForStatisticsInBlock(schemaReplicant *perceptron.DBInfo, causet *perceptron.BlockInfo) {
	var rows [][]types.Causet
	if causet.PKIsHandle {
		for _, defCaus := range causet.DeferredCausets {
			if allegrosql.HasPriKeyFlag(defCaus.Flag) {
				record := types.MakeCausets(
					schemareplicant.CatalogVal, // TABLE_CATALOG
					schemaReplicant.Name.O,     // TABLE_SCHEMA
					causet.Name.O,              // TABLE_NAME
					"0",                        // NON_UNIQUE
	...
	nameToDefCaus := make(map[string]*perceptron.DeferredCausetInfo, len(causet.DeferredCausets))
	for _, c := range causet.DeferredCausets {
	...
	for _, index := range causet.Indices {
		...
		for i, key := range index.DeferredCausets {
			...
			var memex interface{}
			memex = nil
			tblDefCaus := causet.DeferredCausets[defCaus.Offset]
			if tblDefCaus.Hidden {
				defCausName = "NULL"
				memex = fmt.Sprintf("(%s)", tblDefCaus.GeneratedExprString)
			}
...
func (e *memblockRetriever) setDataFromBlocks(ctx stochastikctx.Context, schemas []*perceptron.DBInfo) error {
	...
	for _, schemaReplicant := range schemas {
		for _, causet := range schemaReplicant.Blocks {
			defCauslation := causet.DefCauslate
			if defCauslation == "" {
				defCauslation = allegrosql.DefaultDefCauslationName
			}
			createTime := types.NewTime(types.FromGoTime(causet.GetUFIDelateTime()), createTimeTp, types.DefaultFsp)

			createOptions := ""

			if causet.IsSequence() {
				continue
			}
			...
			if !causet.IsView() {
				...
				hasAutoIncID, _ := schemareplicant.HasAutoIncrementDeferredCauset(causet)
				if hasAutoIncID {
					autoIncID, err = getAutoIncrementID(ctx, schemaReplicant, causet)
					if err != nil {
						return err
					}
				}
				...
				if causet.GetPartitionInfo() == nil {
					rowCount = blockEventsMap[causet.ID]
					dataLength, indexLength = getDataAndIndexLength(causet, causet.ID, rowCount, defCausLengthMap)
				} else {
					for _, pi := range causet.GetPartitionInfo().Definitions {
						rowCount += blockEventsMap[pi.ID]
						parDataLen, parIndexLen := getDataAndIndexLength(causet, pi.ID, blockEventsMap[pi.ID], defCausLengthMap)
				...
				} else if causet.IsCommonHandle {
					pkType = "COMMON CLUSTERED"
				}
				shardingInfo := schemareplicant.GetShardingInfo(schemaReplicant, causet)
				record := types.MakeCausets(
					schemareplicant.CatalogVal, // TABLE_CATALOG
					schemaReplicant.Name.O,     // TABLE_SCHEMA
					causet.Name.O,              // TABLE_NAME
					...
					causet.Comment,             // TABLE_COMMENT
					causet.ID,                  // MilevaDB_TABLE_ID
...
func (e *hugeMemBlockRetriever) setDataForDeferredCausets(ctx stochastikctx.Context) error {
	...
	for ; e.dbsIdx < len(e.dbs); e.dbsIdx++ {
		...
		for e.tblIdx < len(schemaReplicant.Blocks) {
			causet := schemaReplicant.Blocks[e.tblIdx]
			e.tblIdx++
			if checker != nil && !checker.RequestVerification(ctx.GetStochastikVars().ActiveRoles, schemaReplicant.Name.L, causet.Name.L, "", allegrosql.AllPrivMask) {
				continue
			}

			e.dataForDeferredCausetsInBlock(schemaReplicant, causet)
			if len(e.rows) >= batch {
				return nil
			}
		}
...
func (e *hugeMemBlockRetriever) dataForDeferredCausetsInBlock(schemaReplicant *perceptron.DBInfo, tbl *perceptron.BlockInfo) {
	for i, defCaus := range tbl.DeferredCausets {
		...
		defCausumnDesc := causet.NewDefCausDesc(causet.ToDeferredCauset(defCaus))
...
func (e *memblockRetriever) setDataFromPartitions(ctx stochastikctx.Context, schemas []*perceptron.DBInfo) error {
	...
	for _, schemaReplicant := range schemas {
		for _, causet := range schemaReplicant.Blocks {
			if checker != nil && !checker.RequestVerification(ctx.GetStochastikVars().ActiveRoles, schemaReplicant.Name.L, causet.Name.L, "", allegrosql.SelectPriv) {
				continue
			}
			createTime := types.NewTime(types.FromGoTime(causet.GetUFIDelateTime()), createTimeTp, types.DefaultFsp)
			...
			if causet.GetPartitionInfo() == nil {
				...
				for i, pi := range causet.GetPartitionInfo().Definitions {
					rowCount = blockEventsMap[pi.ID]
					dataLength, indexLength = getDataAndIndexLength(causet, pi.ID, blockEventsMap[pi.ID], defCausLengthMap)
					...
					if causet.Partition.Type == perceptron.PartitionTypeRange {
						partitionDesc = pi.LessThan[0]
					}

					partitionMethod := causet.Partition.Type.String()
					partitionExpr := causet.Partition.Expr
					if causet.Partition.Type == perceptron.PartitionTypeRange && len(causet.Partition.DeferredCausets) > 0 {
						partitionMethod = "RANGE COLUMNS"
						partitionExpr = causet.Partition.DeferredCausets[0].String()
					}
	...
	for _, schemaReplicant := range schemas {
		for _, causet := range schemaReplicant.Blocks {
			...
			record := types.MakeCausets(
				...
				schemaReplicant.Name.O,           // TABLE_SCHEMA
				causet.Name.O,                    // TABLE_NAME
				causet.View.SelectStmt,           // VIEW_DEFINITION
				causet.View.CheckOption.String(), // CHECK_OPTION
				"NO",                             // IS_UFIDelATABLE
				causet.View.Definer.String(),     // DEFINER
				causet.View.Security.String(),    // SECURITY_TYPE
...
func (e *memblockRetriever) dataForEinsteinDBStoreStatus(ctx stochastikctx.Context) (err error) {
	...
	for _, storeStat := range storesStat.Stores {
		...
		event[10].SetFloat64(storeStat.Status.LeaderSembedded)
		...
		event[14].SetFloat64(storeStat.Status.RegionSembedded)
...
// DBSJobsReaderInterDirc executes DBSJobs information retrieving.
type DBSJobsReaderInterDirc struct {
...
// Open implements the InterlockingDirectorate Next interface.
func (e *DBSJobsReaderInterDirc) Open(ctx context.Context) error {
...
func (e *DBSJobsReaderInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
...
func (e *memblockRetriever) setDataFromKeyDeferredCausetUsage(ctx stochastikctx.Context, schemas []*perceptron.DBInfo) {
	...
	for _, schemaReplicant := range schemas {
		for _, causet := range schemaReplicant.Blocks {
			...
			rs := keyDeferredCausetUsageInBlock(schemaReplicant, causet)
			rows = append(rows, rs...)
		}
	}
	e.rows = rows
...
func keyDeferredCausetUsageInBlock(schemaReplicant *perceptron.DBInfo, causet *perceptron.BlockInfo) [][]types.Causet {
	var rows [][]types.Causet
	if causet.PKIsHandle {
		for _, defCaus := range causet.DeferredCausets {
			if allegrosql.HasPriKeyFlag(defCaus.Flag) {
	...
	for _, fk := range causet.ForeignKeys {
		fkRefDefCaus := ""
...
func (e *memblockRetriever) setDataForEinsteinDBRegionStatus(ctx stochastikctx.Context) error {
	...
	for _, region := range regionsInfo.Regions {
		...
		for _, causet := range blockList {
			e.setNewEinsteinDBRegionStatusDefCaus(&region, &causet)
		}
	}
	return nil
}

func (e *memblockRetriever) setNewEinsteinDBRegionStatusDefCaus(region *helper.RegionInfo, causet *helper.BlockInfo) {
	...
	if causet != nil {
		event[3].SetInt64(causet.Block.ID)
		event[4].SetString(causet.EDB.Name.O, allegrosql.DefaultDefCauslationName)
		event[5].SetString(causet.Block.Name.O, allegrosql.DefaultDefCauslationName)
		if causet.IsIndex {
			event[6].SetInt64(1)
			event[7].SetInt64(causet.Index.ID)
			event[8].SetString(causet.Index.Name.O, allegrosql.DefaultDefCauslationName)
		} else {
			event[6].SetInt64(0)
		}
	}
...
// setDataFromBlockConstraints constructs data for causet information_schema.constraints.See https://dev.allegrosql.com/doc/refman/5.7/en/causet-constraints-causet.html
...
type blockStorageStatsRetriever struct {
	...
	retrieved      bool
	initialized    bool
	extractor      *causetembedded.BlockStorageStatsExtractor
...
func (e *blockStorageStatsRetriever) retrieve(ctx context.Context, sctx stochastikctx.Context) ([][]types.Causet, error) {
	...
	if len(e.outputDefCauss) == len(e.causet.DeferredCausets) {
		return rows, nil
	}
...
func (e *blockStorageStatsRetriever) initialize(sctx stochastikctx.Context) error {
	...
	for _, EDB := range databases {
		// The user didn't specified the causet, extract all blocks of this EDB to initialBlock.
		if len(blocks) == 0 {
			...
			// The user specified the causet, extract the specified blocks of this EDB to initialBlock.
			for tb := range blocks {
...
func (e *blockStorageStatsRetriever) setDataForBlockStorageStats(ctx stochastikctx.Context) ([][]types.Causet, error) {
	...
	for e.curBlock < len(e.initialBlocks) && count < 1024 {
		causet := e.initialBlocks[e.curBlock]
		blockID := causet.ID
...
// setDataForPseudoProfiling returns pseudo data for causet profiling when system variable `profiling` is set to `ON`.
	...
	for _, schemaReplicant := range schemas {
		for _, causet := range schemaReplicant.Blocks {
			...
			record := types.MakeCausets(
				...
				schemaReplicant.Name.O,     // TABLE_SCHEMA
				causet.Name.O,              // TABLE_NAME
				causet.Sequence.Cache,      // Cache
				causet.Sequence.CacheValue, // CACHE_VALUE
				causet.Sequence.Cycle,      // CYCLE
				causet.Sequence.Increment,  // INCREMENT
				causet.Sequence.MaxValue,   // MAXVALUE
				causet.Sequence.MinValue,   // MINVALUE
				causet.Sequence.Start,      // START
				causet.Sequence.Comment,    // COMMENT
			)
			rows = append(rows, record)
		}
	}
...
// dataForBlockTiFlashReplica constructs data for causet tiflash replica info.
...
func (e *hugeMemBlockRetriever) retrieve(ctx context.Context, sctx stochastikctx.Context) ([][]types.Causet, error) {
	...
	return adjustDeferredCausets(e.rows, e.defCausumns, e.causet), nil
}

func adjustDeferredCausets(input [][]types.Causet, outDeferredCausets []*perceptron.DeferredCausetInfo, causet *perceptron.BlockInfo) [][]types.Causet {
	if len(outDeferredCausets) == len(causet.DeferredCausets) {
		return input
	}
...
// TiFlashSystemBlockRetriever is used to read system causet from tiflash.
type TiFlashSystemBlockRetriever struct {
	...
	retrieved      bool
	initialized    bool
	extractor      *causetembedded.TiFlashSystemBlockExtractor
}
...
func (e *TiFlashSystemBlockRetriever) dataForTiFlashSystemBlocks(ctx stochastikctx.Context, milevadbDatabases string, milevadbBlocks string) ([][]types.Causet, error) {
	...
	maxCount := 1024
	targetBlock := strings.ToLower(strings.Replace(e.causet.Name.O, "TIFLASH", "DT", 1))
	var filters []string
