...
func (e *SortInterDirc) Close() error {
...
func (e *SortInterDirc) Open(ctx context.Context) error {
	e.fetched = false
	e.Idx = 0

	// To avoid duplicated initialization for TopNInterDirc.
	if e.memTracker == nil {
...
func (e *SortInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	req.Reset()
	if !e.fetched {
		e.initCompareFuncs()
...
func (e *SortInterDirc) externalSorting(req *chunk.Chunk) (err error) {
...
func (e *SortInterDirc) fetchEventChunks(ctx context.Context) error {
	fields := retTypes(e)
...
func (e *SortInterDirc) initCompareFuncs() {
...
func (e *SortInterDirc) buildKeyDeferredCausets() {
	e.keyDeferredCausets = make([]int, 0, len(e.ByItems))
	for _, by := range e.ByItems {
		defCaus := by.Expr.(*memex.DeferredCauset)
		e.keyDeferredCausets = append(e.keyDeferredCausets, defCaus.Index)
	}
}

func (e *SortInterDirc) lessEvent(rowI, rowJ chunk.Event) bool {
...
// TopNInterDirc implements a Top-N algorithm and it is built from a SELECT memex with ORDER BY and LIMIT.
...
// Open implements the InterlockingDirectorate Open interface.
func (e *TopNInterDirc) Open(ctx context.Context) error {
	e.memTracker = memory.NewTracker(e.id, -1)
	e.memTracker.AttachTo(e.ctx.GetStochastikVars().StmtCtx.MemTracker)
...
// Next implements the InterlockingDirectorate Next interface.
func (e *TopNInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	req.Reset()
	if !e.fetched {
...
func (e *TopNInterDirc) loadChunksUntilTotalLimit(ctx context.Context) error {
...
func (e *TopNInterDirc) executeTopN(ctx context.Context) error {
	heap.Init(e.chkHeap)
...
func (e *TopNInterDirc) processChildChk(childEventChk *chunk.Chunk) error {
...
func (e *TopNInterDirc) doCompaction() error {
