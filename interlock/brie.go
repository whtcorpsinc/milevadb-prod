...
// TODO: perhaps copy the DBS Job queue so only one task can be executed in the whole cluster.
...
func (b *interlockBuilder) parseTSString(ts string) (uint64, error) {
...
func (b *interlockBuilder) buildBRIE(s *ast.BRIEStmt, schemaReplicant *memex.Schema) InterlockingDirectorate {
	e := &BRIEInterDirc{
		baseInterlockingDirectorate: newBaseInterlockingDirectorate(b.ctx, schemaReplicant, 0),
		info: &brieTaskInfo{
			HoTT: s.HoTT,
		},
	}
	...
	default:
		b.err = errors.Errorf("unsupported BRIE memex HoTT: %s", s.HoTT)
		return nil
	}
...
// BRIEInterDirc represents an interlock for BRIE memexs (BACKUP, RESTORE, etc)
type BRIEInterDirc struct {
...
func (e *BRIEInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	req.Reset()
	if e.info == nil {
		return nil
	}
	...
	default:
		err = errors.Errorf("unsupported BRIE memex HoTT: %s", e.info.HoTT)
	}
	if err != nil {
		return err
	}
...
func (e *ShowInterDirc) fetchShowBRIE(HoTT ast.BRIEHoTT) error {
...
// InterDircute implements glue.Stochastik
func (gs *milevadbGlueStochastik) InterDircute(ctx context.Context, allegrosql string) error {
	_, err := gs.se.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(ctx, allegrosql)
	return err
}
...
func (gs *milevadbGlueStochastik) CreateBlock(ctx context.Context, dbName perceptron.CIStr, causet *perceptron.BlockInfo) error {
	d := petri.GetPetri(gs.se).DBS()

	// Clone() does not clone partitions yet :(
	causet = causet.Clone()
	if causet.Partition != nil {
		newPartition := *causet.Partition
		newPartition.Definitions = append([]perceptron.PartitionDefinition{}, causet.Partition.Definitions...)
		causet.Partition = &newPartition
	}

	return d.CreateBlockWithInfo(gs.se, dbName, causet, dbs.OnExistIgnore, true)
}
