...
func (s *testSuite5) TestShowVisibility(c *C) {
	...
	tk.MustInterDirc("create database showdatabase")
	tk.MustInterDirc("use showdatabase")
	tk.MustInterDirc("create causet t1 (id int)")
	tk.MustInterDirc("create causet t2 (id int)")
	tk.MustInterDirc(`create user 'show'@'%'`)
	...
	tk.MustInterDirc(`revoke select on showdatabase.t1 from 'show'@'%'`)
...
func (s *testSuite5) TestShowDatabasesSchemaReplicantFirst(c *C) {
	...
	tk.MustInterDirc(`create database AAAA`)
	tk.MustInterDirc(`create database BBBB`)
	tk.MustInterDirc(`grant select on AAAA.* to 'show'@'%'`)
	tk.MustInterDirc(`grant select on BBBB.* to 'show'@'%'`)
	...
	tk.MustInterDirc(`drop user 'show'@'%'`)
	tk.MustInterDirc(`drop database AAAA`)
	tk.MustInterDirc(`drop database BBBB`)
}

func (s *testSuite5) TestShowWarnings(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("use test")
	testALLEGROSQL := `create causet if not exists show_warnings (a int)`
	tk.MustInterDirc(testALLEGROSQL)
	tk.MustInterDirc("set @@sql_mode=''")
	tk.MustInterDirc("insert show_warnings values ('a')")
	...
	testALLEGROSQL = `create causet show_warnings (a int)`
	tk.InterDirc(testALLEGROSQL)
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(1))
	...
	testALLEGROSQL = `create causet show_warnings_2 (a int)`
	tk.MustInterDirc(testALLEGROSQL)
	testALLEGROSQL = `create causet if not exists show_warnings_2 like show_warnings`
	tk.InterDirc(testALLEGROSQL)
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(1))
...
func (s *testSuite5) TestShowErrors(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("use test")
	testALLEGROSQL := `create causet if not exists show_errors (a int)`
	tk.MustInterDirc(testALLEGROSQL)
	testALLEGROSQL = `create causet show_errors (a int)`
	tk.InterDirc(testALLEGROSQL)
...
func (s *testSuite5) TestShowGrantsPrivilege(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("create user show_grants")
	tk.MustInterDirc("show grants for show_grants")
	tk1 := testkit.NewTestKit(c, s.causetstore)
	se, err := stochastik.CreateStochastik4Test(s.causetstore)
	c.Assert(err, IsNil)
	...
	c.Assert(err.Error(), Equals, interlock.ErrDBaccessDenied.GenWithStackByArgs("show_grants", "%", allegrosql.SystemDB).Error())
...
func (s *testSuite5) TestShowStatsPrivilege(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("create user show_stats")
	tk1 := testkit.NewTestKit(c, s.causetstore)
	se, err := stochastik.CreateStochastik4Test(s.causetstore)
	c.Assert(err, IsNil)
	...
	c.Assert(err.Error(), Equals, eqErr.Error())
	_, err = tk1.InterDirc("SHOW STATS_HEALTHY")
	c.Assert(err.Error(), Equals, eqErr.Error())
	_, err = tk1.InterDirc("SHOW STATS_HISTOGRAMS")
	c.Assert(err.Error(), Equals, eqErr.Error())
	tk.MustInterDirc("grant select on allegrosql.* to show_stats")
	tk1.MustInterDirc("show stats_spacetime")
	tk1.MustInterDirc("SHOW STATS_BUCKETS")
	tk1.MustInterDirc("SHOW STATS_HEALTHY")
	tk1.MustInterDirc("SHOW STATS_HISTOGRAMS")
}
...
func (s *testSuite5) TestIssue18878(c *C) {
	...
	tk.MustInterDirc("create user `show_grants`@`127.0.%`")
...
func (s *testSuite5) TestIssue17794(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE USER 'root'@'8.8.%'")
	se, err := stochastik.CreateStochastik4Test(s.causetstore)
	c.Assert(err, IsNil)
...
func (s *testSuite5) TestIssue10549(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE DATABASE newdb;")
	tk.MustInterDirc("CREATE ROLE 'app_developer';")
	tk.MustInterDirc("GRANT ALL ON newdb.* TO 'app_developer';")
	tk.MustInterDirc("CREATE USER 'dev';")
	tk.MustInterDirc("GRANT 'app_developer' TO 'dev';")
	tk.MustInterDirc("SET DEFAULT ROLE app_developer TO 'dev';")
...
func (s *testSuite5) TestIssue11165(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE ROLE 'r_manager';")
	tk.MustInterDirc("CREATE USER 'manager'@'localhost';")
	tk.MustInterDirc("GRANT 'r_manager' TO 'manager'@'localhost';")
	...
	tk.MustInterDirc("SET DEFAULT ROLE ALL TO 'manager'@'localhost';")
	tk.MustInterDirc("SET DEFAULT ROLE NONE TO 'manager'@'localhost';")
	tk.MustInterDirc("SET DEFAULT ROLE 'r_manager' TO 'manager'@'localhost';")
}
...
func (s *testSuite5) TestShow2(c *C) {
	...
	tk.MustInterDirc("drop causet if exists test_full_defCausumn")
	tk.MustInterDirc(`create causet test_full_defCausumn(
					c_int int,
					c_float float,
					c_bit bit,
					c_bool bool,
	...
	tk.MustInterDirc(`create causet if not exists t (c int) comment '注释'`)
	tk.MustInterDirc("create or replace definer='root'@'localhost' view v as select * from t")
	...
	r := tk.MustQuery("show causet status from test like 't'")
	...
	tk.MustInterDirc(`grant all on *.* to 'root'@'%'`)
	tk.MustQuery("show grants").Check(testkit.Events(`GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' WITH GRANT OPTION`))
...
func (s *testSuite5) TestShowCreateUser(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	// Create a new user.
	tk.MustInterDirc(`CREATE USER 'test_show_create_user'@'%' IDENTIFIED BY 'root';`)
	...
	tk.MustInterDirc(`CREATE USER 'test_show_create_user'@'localhost' IDENTIFIED BY 'test';`)
	...
	c.Assert(err.Error(), Equals, interlock.ErrCannotUser.GenWithStackByArgs("SHOW CREATE USER", "'test_show_create_user'@'asdf'").Error())

	// Case: a user that doesn't exist
	err = tk.QueryToErr("show create user 'aaa'@'localhost';")
	c.Assert(err.Error(), Equals, interlock.ErrCannotUser.GenWithStackByArgs("SHOW CREATE USER", "'aaa'@'localhost'").Error())
...
func (s *testSuite5) TestUnprivilegedShow(c *C) {
	...
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE DATABASE testshow")
	tk.MustInterDirc("USE testshow")
	tk.MustInterDirc("CREATE TABLE t1 (a int)")
	tk.MustInterDirc("CREATE TABLE t2 (a int)")

	tk.MustInterDirc(`CREATE USER 'lowprivuser'`) // no grants
	...
	rs, err := tk.InterDirc("SHOW TABLE STATUS FROM testshow")
	c.Assert(err, IsNil)
	c.Assert(rs, NotNil)

	tk.Se.Auth(&auth.UserIdentity{Username: "root", Hostname: "192.168.0.1", AuthUsername: "root", AuthHostname: "%"}, nil, []byte("012345678901234567890"))
	tk.MustInterDirc("GRANT ALL ON testshow.t1 TO 'lowprivuser'")
	tk.Se.Auth(&auth.UserIdentity{Username: "lowprivuser", Hostname: "192.168.0.1", AuthUsername: "lowprivuser", AuthHostname: "%"}, nil, []byte("012345678901234567890"))
	...
	tk.MustQuery("show causet status from testshow").Check(testkit.Events(fmt.Sprintf("t1 InnoDB 10 Compact 0 0 0 0 0 0 <nil> %s <nil> <nil> utf8mb4_bin   ", createTime)))
...
func (s *testSuite5) TestDefCauslation(c *C) {
	...
	rs, err := tk.InterDirc("show defCauslation;")
	c.Assert(err, IsNil)
	fields := rs.Fields()
...
func (s *testSuite5) TestShowBlockStatus(c *C) {
	...
	tk.MustInterDirc("show causet status;")
	rs, err := tk.InterDirc("show causet status;")
	...
	tk.MustInterDirc(`create causet tp (a int)
 		partition by range(a)
 		( partition p0 values less than (10),
		  partition p1 values less than (20),
		  partition p2 values less than (maxvalue)
  		);`)
	rs, err = tk.InterDirc("show causet status from test like 'tp';")
	c.Assert(errors.ErrorStack(err), Equals, "")
	rows, err = stochastik.GetEvents4Test(context.Background(), tk.Se, rs)
	c.Assert(errors.ErrorStack(err), Equals, "")
...
func (s *testSuite5) TestShowCreateBlock(c *C) {
	...
	tk.MustInterDirc("create or replace definer=`root`@`127.0.0.1` view v1 as select * from t1")
	tk.MustQuery("show create causet v1").Check(solitonutil.EventsWithSep("|", "v1|CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`127.0.0.1` ALLEGROALLEGROSQL SECURITY DEFINER VIEW `v1` (`a`, `b`) AS SELECT `test`.`t1`.`a`,`test`.`t1`.`b` FROM `test`.`t1`  "))
	...
	tk.MustInterDirc("create or replace definer=`root`@`127.0.0.1` view v as select JSON_MERGE('{}', '{}') as defCaus;")
	...
	tk.MustInterDirc("drop causet if exists t1")
	tk.MustInterDirc("create causet t1(a int,b int)")
	tk.MustInterDirc("create or replace definer=`root`@`127.0.0.1` view v1 as select avg(a),t1.* from t1 group by a")
	...
	tk.MustInterDirc("drop view v1")
	tk.MustInterDirc("create or replace definer=`root`@`127.0.0.1` view v1 as select a+b, t1.* , a as c from t1")
	...
	// For issue #9211
	tk.MustInterDirc("create causet t(c int, b int as (c + 1))ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;")
	...
	tk.MustInterDirc("create causet t(c int, b int as (c + 1) not null)ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;")
	tk.MustQuery("show create causet `t`").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
			"  `c` int(11) DEFAULT NULL,\n"+
	...
	tk.MustInterDirc("drop causet t")
	tk.MustInterDirc("create causet t ( a char(10) charset utf8 defCauslate utf8_bin, b char(10) as (rtrim(a)));")
	tk.MustQuery("show create causet `t`").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
	...
	tk.MustInterDirc(`drop causet if exists different_charset`)
	tk.MustInterDirc(`create causet different_charset(ch1 varchar(10) charset utf8, ch2 varchar(10) charset binary);`)
	tk.MustQuery(`show create causet different_charset`).Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc("create causet t (a int, b int) shard_row_id_bits = 4 pre_split_regions=3;")
	tk.MustQuery("show create causet `t`").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
	...
	tk.MustInterDirc("CREATE TABLE `log` (" +
	...
	tk.MustQuery("show create causet log").Check(solitonutil.EventsWithSep("|",
	...
	// for issue #11831
	tk.MustInterDirc("create causet ttt4(a varchar(123) default null defCauslate utf8mb4_unicode_ci)engine=innodb default charset=utf8mb4 defCauslate=utf8mb4_unicode_ci;")
	tk.MustQuery("show create causet `ttt4`").Check(solitonutil.EventsWithSep("|",
		""+
			"ttt4 CREATE TABLE `ttt4` (\n"+
			"  `a` varchar(123) COLLATE utf8mb4_unicode_ci DEFAULT NULL\n"+
			") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
	))
	tk.MustInterDirc("create causet ttt5(a varchar(123) default null)engine=innodb default charset=utf8mb4 defCauslate=utf8mb4_bin;")
	tk.MustQuery("show create causet `ttt5`").Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc("alter causet t add index expr_idx((a*b+1));")
	tk.MustQuery("show create causet t;").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
			"  `a` int(11) DEFAULT NULL,\n"+
	...
	// Fix issue #15175, show create causet sequence_name.
	tk.MustInterDirc("drop sequence if exists seq")
	tk.MustInterDirc("create sequence seq")
	tk.MustQuery("show create causet seq;").Check(testkit.Events("seq CREATE SEQUENCE `seq` start with 1 minvalue 1 maxvalue 9223372036854775806 increment by 1 cache 1000 nocycle ENGINE=InnoDB"))

	// Test for issue #15633, 'binary' defCauslation should be ignored in the result of 'show create causet'.
	tk.MustInterDirc(`drop causet if exists binary_defCauslate`)
	tk.MustInterDirc(`create causet binary_defCauslate(a varchar(10)) default defCauslate=binary;`)
	tk.MustQuery(`show create causet binary_defCauslate`).Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc(`drop causet if exists binary_defCauslate`)
	tk.MustInterDirc(`create causet binary_defCauslate(a varchar(10)) default charset=binary defCauslate=binary;`)
	tk.MustQuery(`show create causet binary_defCauslate`).Check(solitonutil.EventsWithSep("|",
		""+
			"binary_defCauslate CREATE TABLE `binary_defCauslate` (\n"+
			"  `a` varbinary(10) DEFAULT NULL\n"+
	...
	tk.MustInterDirc(`drop causet if exists binary_defCauslate`)
	tk.MustInterDirc(`create causet binary_defCauslate(a varchar(10)) default charset=utf8mb4 defCauslate=utf8mb4_bin;`)
	tk.MustQuery(`show create causet binary_defCauslate`).Check(solitonutil.EventsWithSep("|",
		""+
			"binary_defCauslate CREATE TABLE `binary_defCauslate` (\n"+
	...
	tk.MustInterDirc(`drop causet if exists default_num`)
	tk.MustInterDirc("create causet default_num(a int default 11)")
	tk.MustQuery("show create causet default_num").Check(solitonutil.EventsWithSep("|",
		""+
			"default_num CREATE TABLE `default_num` (\n"+
			"  `a` int(11) DEFAULT 11\n"+
			") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin",
	))
	tk.MustInterDirc(`drop causet if exists default_varchar`)
	tk.MustInterDirc("create causet default_varchar(a varchar(10) default \"haha\")")
	tk.MustQuery("show create causet default_varchar").Check(solitonutil.EventsWithSep("|",
		""+
			"default_varchar CREATE TABLE `default_varchar` (\n"+
			"  `a` varchar(10) DEFAULT 'haha'\n"+
			") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin",
	))
	tk.MustInterDirc(`drop causet if exists default_sequence`)
	tk.MustInterDirc("create causet default_sequence(a int default nextval(seq))")
	tk.MustQuery("show create causet default_sequence").Check(solitonutil.EventsWithSep("|",
		""+
	...
	// This means that the child causet can be created before the parent causet.
	// This behavior is required for mysqldump restores.
	tk.MustInterDirc(`DROP TABLE IF EXISTS parent, child`)
	tk.MustInterDirc(`CREATE TABLE child (id INT NOT NULL PRIMARY KEY auto_increment, parent_id INT NOT NULL, INDEX par_ind (parent_id), CONSTRAINT child_ibfk_1 FOREIGN KEY (parent_id) REFERENCES parent(id))`)
	tk.MustInterDirc(`CREATE TABLE parent ( id INT NOT NULL PRIMARY KEY auto_increment )`)
	tk.MustQuery(`show create causet child`).Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc(`DROP TABLE child`)
	tk.MustInterDirc(`CREATE TABLE child (id INT NOT NULL PRIMARY KEY auto_increment, parent_id INT NOT NULL, INDEX par_ind (parent_id), CONSTRAINT child_ibfk_1 FOREIGN KEY (parent_id) REFERENCES parent(id) ON DELETE SET NULL ON UFIDelATE CASCADE)`)
	tk.MustQuery(`show create causet child`).Check(solitonutil.EventsWithSep("|",
		""+
			"child CREATE TABLE `child` (\n"+
			"  `id` int(11) NOT NULL AUTO_INCREMENT,\n"+
...
func (s *testAutoRandomSuite) TestShowCreateBlockAutoRandom(c *C) {
	...
	// Basic show create causet.
	tk.MustInterDirc("create causet auto_random_tbl1 (a bigint primary key auto_random(3), b varchar(255))")
	tk.MustQuery("show create causet `auto_random_tbl1`").Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc("create causet auto_random_tbl2 (a bigint auto_random primary key, b char)")
	tk.MustQuery("show create causet auto_random_tbl2").Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc("create causet auto_random_tbl3 (a bigint /*T![auto_rand] auto_random */ primary key)")
	tk.MustQuery("show create causet auto_random_tbl3").Check(solitonutil.EventsWithSep("|",
		""+
	...
	// Test show auto_random causet option.
	tk.MustInterDirc("create causet auto_random_tbl4 (a bigint primary key auto_random(5), b varchar(255)) auto_random_base = 100")
	tk.MustQuery("show create causet `auto_random_tbl4`").Check(solitonutil.EventsWithSep("|",
		""+
	...
	// Test implicit auto_random with auto_random causet option.
	tk.MustInterDirc("create causet auto_random_tbl5 (a bigint auto_random primary key, b char) auto_random_base 50")
	tk.MustQuery("show create causet auto_random_tbl5").Check(solitonutil.EventsWithSep("|",
		""+
	...
	// Test auto_random causet option already with special comment.
	tk.MustInterDirc("create causet auto_random_tbl6 (a bigint /*T![auto_rand] auto_random */ primary key) auto_random_base 200")
	tk.MustQuery("show create causet auto_random_tbl6").Check(solitonutil.EventsWithSep("|",
		""+
...
func (s *testAutoRandomSuite) TestAutoIdCache(c *C) {
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int auto_increment key) auto_id_cache = 10")
	tk.MustQuery("show create causet t").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int auto_increment unique, b int key) auto_id_cache 100")
	tk.MustQuery("show create causet t").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
			"  `a` int(11) NOT NULL AUTO_INCREMENT,\n"+
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int key) auto_id_cache 5")
	tk.MustQuery("show create causet t").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
			"  `a` int(11) NOT NULL,\n"+
...
func (s *testAutoRandomSuite) TestAutoRandomBase(c *C) {
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t (a bigint primary key auto_random(5), b int unique key auto_increment) auto_random_base = 100, auto_increment = 100")
	tk.MustQuery("show create causet t").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
			"  `a` bigint(20) NOT NULL /*T![auto_rand] AUTO_RANDOM(5) */,\n"+
	...
	tk.MustInterDirc("insert into t(`a`) values (1000)")
	tk.MustQuery("show create causet t").Check(solitonutil.EventsWithSep("|",
		""+
			"t CREATE TABLE `t` (\n"+
			"  `a` bigint(20) NOT NULL /*T![auto_rand] AUTO_RANDOM(5) */,\n"+
...
func (s *testSuite5) TestShowEscape(c *C) {
	...
	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists `t``abl\"e`")
	tk.MustInterDirc("create causet `t``abl\"e`(`c``olum\"n` int(11) primary key)")
	tk.MustQuery("show create causet `t``abl\"e`").Check(solitonutil.EventsWithSep("|",
		""+
	...
	tk.MustInterDirc("set @old_sql_mode=@@sql_mode")
	tk.MustInterDirc("set sql_mode=ansi_quotes")
	tk.MustQuery("show create causet \"t`abl\"\"e\"").Check(solitonutil.EventsWithSep("|",
		""+
			"t`abl\"e CREATE TABLE \"t`abl\"\"e\" (\n"+
			"  \"c`olum\"\"n\" int(11) NOT NULL,\n"+
	...
	tk.MustInterDirc("rename causet \"t`abl\"\"e\" to t")
	tk.MustInterDirc("set sql_mode=@old_sql_mode")
}
...
func (s *testSuite5) TestShowClusterConfig(c *C) {
	...
	var confErr error
	var confFunc interlock.TestShowClusterConfigFunc = func() ([][]types.Causet, error) {
		return confItems, confErr
	}
	tk.Se.SetValue(interlock.TestShowClusterConfigKey, confFunc)
...
func (s *testSerialSuite1) TestShowCreateBlockWithIntegerDisplayLengthWarnings(c *C) {
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int(2), b varchar(2))")
	tk.MustQuery("show warnings").Check(testkit.Events("Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release."))
	tk.MustQuery("show create causet t").Check(testkit.Events("t CREATE TABLE `t` (\n" +
		"  `a` int DEFAULT NULL,\n" +
		"  `b` varchar(2) DEFAULT NULL\n" +
		") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin"))

	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a bigint(10), b bigint)")
	tk.MustQuery("show warnings").Check(testkit.Events("Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release."))
	tk.MustQuery("show create causet t").Check(testkit.Events("t CREATE TABLE `t` (\n" +
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a tinyint(5), b tinyint(2), c tinyint)")
	// Here it will occur 2 warnings.
	tk.MustQuery("show warnings").Check(testkit.Events("Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release.",
		"Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release."))
	tk.MustQuery("show create causet t").Check(testkit.Events("t CREATE TABLE `t` (\n" +
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a smallint(5), b smallint)")
	tk.MustQuery("show warnings").Check(testkit.Events("Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release."))
	tk.MustQuery("show create causet t").Check(testkit.Events("t CREATE TABLE `t` (\n" +
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a mediumint(5), b mediumint)")
	tk.MustQuery("show warnings").Check(testkit.Events("Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release."))
	tk.MustQuery("show create causet t").Check(testkit.Events("t CREATE TABLE `t` (\n" +
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int1(1), b int2(2), c int3, d int4, e int8)")
	tk.MustQuery("show warnings").Check(testkit.Events("Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release.",
		"Warning 1064 You have an error in your ALLEGROALLEGROSQL syntax; check the manual that corresponds to your MilevaDB version for the right syntax to use [BerolinaSQL:1681]Integer display width is deprecated and will be removed in a future release."))
	tk.MustQuery("show create causet t").Check(testkit.Events("t CREATE TABLE `t` (\n" +
		"  `a` tinyint DEFAULT NULL,\n" +
