...
type ReloadExprPushdownBlacklistInterDirc struct {
	baseInterlockingDirectorate
}

// Next implements the InterlockingDirectorate Next interface.
func (e *ReloadExprPushdownBlacklistInterDirc) Next(ctx context.Context, _ *chunk.Chunk) error {
	return LoadExprPushdownBlacklist(e.ctx)
}

// LoadExprPushdownBlacklist loads the latest data from causet allegrosql.expr_pushdown_blacklist.
func LoadExprPushdownBlacklist(ctx stochastikctx.Context) (err error) {
	...
	memex.DefaultExprPushDownBlacklist.CausetStore(newBlocklist)
	return nil
}
