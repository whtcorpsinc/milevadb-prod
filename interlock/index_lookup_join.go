...
var _ InterlockingDirectorate = &IndexLookUpJoin{}
...
func (e *IndexLookUpJoin) Open(ctx context.Context) error {
	...
	// IndexLookUpJoin need to rebuild interlock (the dataReaderBuilder) during
...
func (ow *outerWorker) buildTask(ctx context.Context) (*lookUpJoinTask, error) {
	task := &lookUpJoinTask{
		...
		outerResult: newList(ow.interlock),
	...
	if ow.filter != nil {
		...
		for i := 0; i < numChks; i++ {
			...
			task.outerMatch[i], err = memex.VectorizedFilter(ow.ctx, ow.filter, chunk.NewIterator4Chunk(chk), outerMatch)
			if err != nil {
				return task, err
			}
		}
...
func (iw *innerWorker) fetchInnerResults(ctx context.Context, task *lookUpJoinTask, lookUpContent []*indexJoinLookUpContent) error {
	...
	innerInterDirc, err := iw.readerBuilder.buildInterlockingDirectorateForIndexJoin(ctx, lookUpContent, iw.indexRanges, iw.keyOff2IdxOff, iw.nextDefCausCompareFilters)
	if err != nil {
		return err
	}
	defer terror.Call(innerInterDirc.Close)
	innerResult := chunk.NewList(retTypes(innerInterDirc), iw.ctx.GetStochastikVars().MaxChunkSize, iw.ctx.GetStochastikVars().MaxChunkSize)
	...
	for {
		...
		err := Next(ctx, innerInterDirc, iw.interlockChk)
		if err != nil {
			return err
		}
		if iw.interlockChk.NumEvents() == 0 {
			break
		}
		innerResult.Add(iw.interlockChk)
		iw.interlockChk = newFirstChunk(innerInterDirc)
	}
