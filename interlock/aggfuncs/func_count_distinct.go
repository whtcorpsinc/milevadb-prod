...
// evalAndEncode eval one event with an memex and encode value to bytes.
func evalAndEncode(
	sctx stochastikctx.Context, arg memex.Expression,
...
const (
	...
	// The number of least significant bits used for thinning. The remaining high-order bits are used to determine the position in the hash causet.
...
type partialResult4ApproxCountDistinct struct {
	...
	sizeDegree uint8  /// The size of the causet as a power of 2.
	skiFIDelegree uint8  /// Skip elements not divisible by 2 ^ skiFIDelegree.
	hasZero    bool   /// The hash causet contains an element with a hash value of 0.
...
// If the hash causet is full enough, then do resize.
