...
func buildWindowTesterWithArgs(funcName string, tp byte, args []memex.Expression, orderByDefCauss int, numEvents int, results ...interface{}) windowTest {
	pt := windowTest{
	...
	if funcName != ast.WindowFuncNtile {
		pt.args = append(pt.args, &memex.DeferredCauset{RetType: pt.dataType, Index: 0})
	}
	pt.args = append(pt.args, args...)
	if orderByDefCauss > 0 {
		pt.orderByDefCauss = append(pt.orderByDefCauss, &memex.DeferredCauset{RetType: pt.dataType, Index: 0})
	}
...
func buildWindowTester(funcName string, tp byte, constantArg uint64, orderByDefCauss int, numEvents int, results ...interface{}) windowTest {
	...
	if constantArg > 0 {
		pt.args = append(pt.args, &memex.Constant{Value: types.NewUintCauset(constantArg)})
	}
	if orderByDefCauss > 0 {
		pt.orderByDefCauss = append(pt.orderByDefCauss, &memex.DeferredCauset{RetType: pt.dataType, Index: 0})
	}
...
func buildWindowMemTesterWithArgs(funcName string, tp byte, args []memex.Expression, orderByDefCauss int, numEvents int, allocMemDelta int64, uFIDelateMemDeltaGens uFIDelateMemDeltaGens) windowMemTest {
