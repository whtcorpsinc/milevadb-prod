...
func (s *testSuite) SetUpTest(c *C) {
	s.ctx.GetStochastikVars().CausetDeferredCausetID = 0
}
...
func (s *testSuite) testMergePartialResult(c *C, p aggTest) {
	...
	args := []memex.Expression{&memex.DeferredCauset{RetType: p.dataType, Index: 0}}
	if p.funcName == ast.AggFuncGroupConcat {
		args = append(args, &memex.Constant{Value: types.NewStringCauset(" "), RetType: types.NewFieldType(allegrosql.TypeString)})
	}
...
func (s *testSuite) testMultiArgsMergePartialResult(c *C, p multiArgsAggTest) {
	...
	args := make([]memex.Expression, len(p.dataTypes))
	for k := 0; k < len(p.dataTypes); k++ {
		args[k] = &memex.DeferredCauset{RetType: p.dataTypes[k], Index: k}
	}
...
func (s *testSuite) testAggMemFunc(c *C, p aggMemTest) {
	...
	args := []memex.Expression{&memex.DeferredCauset{RetType: p.aggTest.dataType, Index: 0}}
