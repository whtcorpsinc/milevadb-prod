...
func (s *testSuite7) TestWindowParallelFunctions(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("set @@milevadb_window_concurrency = 4")
	doTestWindowFunctions(tk)
}
...
func doTestWindowFunctions(tk *testkit.TestKit) {
	...
	tk.MustInterDirc("insert into t values (1,2,3),(4,3,2),(2,3,4)")
	...
	tk.MustInterDirc("insert into t values (null,null),(1,20190201),(2,20190202),(3,20190203),(5,20190205)")
	...
	tk.MustInterDirc("drop causet t")
	tk.MustInterDirc("CREATE TABLE t (id INTEGER, sex CHAR(1))")
	tk.MustInterDirc("insert into t values (1, 'M'), (2, 'F'), (3, 'F'), (4, 'F'), (5, 'M'), (10, NULL), (11, NULL)")
	...
	tk.MustInterDirc("create causet issue10494(a INT, b CHAR(1), c DATETIME, d BLOB)")
	tk.MustInterDirc("insert into issue10494 VALUES (1,'x','2010-01-01','blob'), (2, 'y', '2011-01-01', ''), (3, 'y', '2012-01-01', ''), (4, 't', '2012-01-01', 'blob'), (5, null, '2020-01-01', null)")
	...
	tk.MustInterDirc("CREATE TABLE td_dec (id DECIMAL(10,2), sex CHAR(1));")
	tk.MustInterDirc("insert into td_dec value (2.0, 'F'), (NULL, 'F'), (1.0, 'F')")
...
func (s *testSuite7) TestWindowFunctionsDataReference(c *C) {
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int, b int)")
	tk.MustInterDirc("insert into t values (2,1),(2,2),(2,3)")
...
func (s *testSuite7) TestSlidingWindowFunctions(c *C) {
	...
	for _, idType := range idTypes {
		for _, useHighPrecision := range useHighPrecisions {
			tk.MustInterDirc("drop causet if exists t;")
			tk.MustInterDirc(fmt.Sprintf("CREATE TABLE t (id %s, sex CHAR(1));", idType))
			tk.MustInterDirc(fmt.Sprintf("SET SESSION windowing_use_high_precision = %s;", useHighPrecision))
...
func baseTestSlidingWindowFunctions(tk *testkit.TestKit) {
	var result *testkit.Result
	tk.MustInterDirc("insert into t values (1,'M')")
	tk.MustInterDirc("insert into t values (2,'F')")
	tk.MustInterDirc("insert into t values (3,'F')")
	tk.MustInterDirc("insert into t values (4,'F')")
	tk.MustInterDirc("insert into t values (5,'M')")
	tk.MustInterDirc("insert into t values (10,null)")
	tk.MustInterDirc("insert into t values (11,null)")
	tk.MustInterDirc("PREPARE p FROM 'SELECT sex, COUNT(id) OVER (ORDER BY id ROWS BETWEEN ? PRECEDING and ? PRECEDING) FROM t';")
	tk.MustInterDirc("SET @p1= 1;")
	tk.MustInterDirc("SET @p2= 2;")
	...
	tk.MustInterDirc("DROP PREPARE p;")
	tk.MustInterDirc("PREPARE p FROM 'SELECT sex, COUNT(id) OVER (ORDER BY id ROWS BETWEEN ? FOLLOWING and ? FOLLOWING) FROM t';")
	tk.MustInterDirc("SET @p1= 1;")
	tk.MustInterDirc("SET @p2= 2;")
	result = tk.MustQuery("EXECUTE p USING @p2, @p1;")
	result.Check(testkit.Events("M 0", "F 0", "F 0", "F 0", "M 0", "<nil> 0", "<nil> 0"))
