...
type InsertValues struct {
	...
	insertDeferredCausets []*causet.DeferredCauset
	...
	// Other memexs like `insert select from` don't guarantee consecutive autoID.
...
// initInsertDeferredCausets sets the explicitly specified defCausumns of an insert memex. There are three cases:
// There are three types of insert memexs:
...
func (e *InsertValues) initInsertDeferredCausets() error {
	...
	if len(e.SetList) > 0 {
		...
		defcaus, missingDefCausName = causet.FindDefCauss(blockDefCauss, defCausumns, e.Block.Meta().PKIsHandle)
	...
	for _, defCaus := range defcaus {
		...
		if defCaus.Name.L == perceptron.ExtraHandleName.L {
			if !e.ctx.GetStochastikVars().AllowWriteEventID {
				return errors.Errorf("insert, uFIDelate and replace memexs for _milevadb_rowid are not supported.")
			}
...
func (e *InsertValues) lazilyInitDefCausDefaultValBuf() (ok bool) {
	...
	// only if values count of insert memex is more than one, use defCausDefaultVals to causetstore
...
func (e *InsertValues) processSetList() error {
	if len(e.SetList) > 0 {
		...
		l := make([]memex.Expression, 0, len(e.SetList))
		for _, v := range e.SetList {
			l = append(l, v.Expr)
		}
...
func (e *InsertValues) handleErr(defCaus *causet.DeferredCauset, val *types.Causet, rowIdx int, err error) error {
	if err == nil {
		return nil
	}
	...
	if types.ErrDataTooLong.Equal(err) {
		...
		err = causet.ErrTruncatedWrongValueForField.GenWithStackByArgs(types.TypeStr(defCausTp), valStr, defCausName, rowIdx+1)
	}
...
func (e *InsertValues) evalEvent(ctx context.Context, list []memex.Expression, rowIdx int) ([]types.Causet, error) {
	...
	// For memexs like `insert into t set a = b + 1`.
	if e.hasRefDefCauss {
	...
	for i, expr := range list {
		...
		val1, err := causet.CastValue(e.ctx, val, e.insertDeferredCausets[i].ToInfo(), false, false)
		if err = e.handleErr(e.insertDeferredCausets[i], &val, rowIdx, err); err != nil {
			return nil, err
		}

		offset := e.insertDeferredCausets[i].Offset
		val1.Copy(&event[offset])
...
func (e *InsertValues) fastEvalEvent(ctx context.Context, list []memex.Expression, rowIdx int) ([]types.Causet, error) {
	rowLen := len(e.Block.DefCauss())
	if e.hasExtraHandle {
		rowLen++
	}
...
func (e *InsertValues) setValueForRefDeferredCauset(event []types.Causet, hasValue []bool) error {
	for i, c := range e.Block.DefCauss() {
		...
		} else if causet.ErrNoDefaultValue.Equal(err) {
			event[i] = causet.GetZeroValue(c.ToInfo())
...
func insertEventsFromSelect(ctx context.Context, base insertCommon) error {
	...
	e := base.insertCommon()
	selectInterDirc := e.children[0]
	fields := retTypes(selectInterDirc)
	chk := newFirstChunk(selectInterDirc)
	iter := chunk.NewIterator4Chunk(chk)
	rows := make([][]types.Causet, 0, chk.Capacity())

	sessVars := e.ctx.GetStochastikVars()
	if !sessVars.StrictALLEGROSQLMode {
		// If StrictALLEGROSQLMode is disabled and it is a insert-select memex, it also handle BadNullAsWarning.
	...
	memTracker := e.memTracker
	for {
		err := Next(ctx, selectInterDirc, chk)
		if err != nil {
			return err
		}
		if chk.NumEvents() == 0 {
...
// The input values from these two memexs are datums instead of
// memexs which are used in `insert into set x=y`.
func (e *InsertValues) getEvent(ctx context.Context, vals []types.Causet) ([]types.Causet, error) {
	...
	for i, v := range vals {
		casted, err := causet.CastValue(e.ctx, v, e.insertDeferredCausets[i].ToInfo(), false, false)
...
func (e *InsertValues) getDefCausDefaultValue(idx int, defCaus *causet.DeferredCauset) (d types.Causet, err error) {
	...
	if defCaus.DefaultIsExpr && defCaus.DefaultExpr != nil {
		defaultVal, err = causet.EvalDefCausDefaultExpr(e.ctx, defCaus.ToInfo(), defCaus.DefaultExpr)
	} else {
		defaultVal, err = causet.GetDefCausDefaultValue(e.ctx, defCaus.ToInfo())
	}
	if err != nil {
...
// fillDefCausValue fills the defCausumn value if it is not set in the insert memex.
func (e *InsertValues) fillDefCausValue(ctx context.Context, causet types.Causet, idx int, defCausumn *causet.DeferredCauset, hasValue bool) (types.Causet,
	error) {
...
func (e *InsertValues) fillEvent(ctx context.Context, event []types.Causet, hasValue []bool) ([]types.Causet, error) {
	gDefCauss := make([]*causet.DeferredCauset, 0)
	for i, c := range e.Block.DefCauss() {
		var err error
	...
	for i, gDefCaus := range gDefCauss {
		...
		event[defCausIdx], err = causet.CastValue(e.ctx, val, gDefCaus.ToInfo(), false, false)
		if err != nil {
			return nil, err
		}
...
func (e *InsertValues) isAutoNull(ctx context.Context, d types.Causet, defCaus *causet.DeferredCauset) bool {
	var err error
	var recordID int64
	if !d.IsNull() {
...
func (e *InsertValues) lazyAdjustAutoIncrementCauset(ctx context.Context, rows [][]types.Causet) ([][]types.Causet, error) {
	...
	for i := 0; i < length; i++ {
		...
		if autoCauset.IsNull() || e.ctx.GetStochastikVars().ALLEGROSQLMode&allegrosql.ModeNoAutoValueOnZero == 0 {
			...
			min, increment, err := causet.AllocBatchAutoIncrementValue(ctx, e.Block, e.ctx, cnt)
			...
			for j := 0; j < cnt; j++ {
				...
				d, err := causet.CastValue(e.ctx, d, defCaus.ToInfo(), false, false)
				if err != nil {
					return nil, err
				}
		...
		// the value of d is adjusted by auto ID, so we need to cast it again.
		autoCauset, err = causet.CastValue(e.ctx, autoCauset, defCaus.ToInfo(), false, false)
		if err != nil {
			return nil, err
		}
		rows[i][defCausIdx] = autoCauset
...
func (e *InsertValues) adjustAutoIncrementCauset(ctx context.Context, d types.Causet, hasValue bool, c *causet.DeferredCauset) (types.Causet, error) {
	retryInfo := e.ctx.GetStochastikVars().RetryInfo
	if retryInfo.Retrying {
		id, err := retryInfo.GetCurrAutoIncrementID()
		if err != nil {
	...
	// Change value 0 to auto id, if NoAutoValueOnZero ALLEGROALLEGROSQL mode is not set.
	if d.IsNull() || e.ctx.GetStochastikVars().ALLEGROSQLMode&allegrosql.ModeNoAutoValueOnZero == 0 {
		recordID, err = causet.AllocAutoIncrementValue(ctx, e.Block, e.ctx)
	...
	// the value of d is adjusted by auto ID, so we need to cast it again.
	casted, err := causet.CastValue(e.ctx, d, c.ToInfo(), false, false)
	if err != nil {
		return types.Causet{}, err
	}
	return casted, nil
...
func (e *InsertValues) adjustAutoRandomCauset(ctx context.Context, d types.Causet, hasValue bool, c *causet.DeferredCauset) (types.Causet, error) {
	retryInfo := e.ctx.GetStochastikVars().RetryInfo
	if retryInfo.Retrying {
...
func (e *InsertValues) batchCheckAndInsert(ctx context.Context, rows [][]types.Causet, addRecord func(ctx context.Context, event []types.Causet) error) error {
	...
	for i, r := range toBeCheckedEvents {
		...
		// There may be duplicate keys inside the insert memex.
		if !skip {
			e.ctx.GetStochastikVars().StmtCtx.AddCopiedEvents(1)
...
func (e *InsertValues) addRecordWithAutoIDHint(ctx context.Context, event []types.Causet, reserveAutoIDCount int) (err error) {
	...
	if reserveAutoIDCount > 0 {
		_, err = e.Block.AddRecord(e.ctx, event, causet.WithCtx(ctx), causet.WithReserveAutoIDHint(reserveAutoIDCount))
	} else {
		_, err = e.Block.AddRecord(e.ctx, event, causet.WithCtx(ctx))
	}
