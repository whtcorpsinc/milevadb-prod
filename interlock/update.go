...
type UFIDelateInterDirc struct {
...
func (e *UFIDelateInterDirc) canNotUFIDelate(handle types.Causet) bool {
	return handle.IsNull()
}

// Next implements the InterlockingDirectorate Next interface.
func (e *UFIDelateInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	req.Reset()
	if !e.drained {
...
func (e *UFIDelateInterDirc) uFIDelateEvents(ctx context.Context) (int, error) {
	fields := retTypes(e.children[0])
	defcausInfo := make([]*causet.DeferredCauset, len(fields))
	for _, content := range e.tblDefCausPosInfos {
		tbl := e.tblID2block[content.TblID]
...
func (e *UFIDelateInterDirc) handleErr(defCausName perceptron.CIStr, rowIdx int, err error) error {
	if err == nil {
		return nil
	}
...
func (e *UFIDelateInterDirc) fastComposeNewEvent(rowIdx int, oldEvent []types.Causet, defcaus []*causet.DeferredCauset) ([]types.Causet, error) {
	...
	for _, assign := range e.OrderedList {
		...
		con := assign.Expr.(*memex.Constant)
		...
		if defcaus[assign.DefCaus.Index] != nil {
			val, err = causet.CastValue(e.ctx, val, defcaus[assign.DefCaus.Index].DeferredCausetInfo, false, false)
			if err = e.handleErr(assign.DefCausName, rowIdx, err); err != nil {
				return nil, err
			}
		}

		val.Copy(&newEventData[assign.DefCaus.Index])
	}
	return newEventData, nil
}

func (e *UFIDelateInterDirc) composeNewEvent(rowIdx int, oldEvent []types.Causet, defcaus []*causet.DeferredCauset) ([]types.Causet, error) {
	newEventData := types.CloneEvent(oldEvent)
...
func (e *UFIDelateInterDirc) Close() error {
	e.setMessage()
...
func (e *UFIDelateInterDirc) Open(ctx context.Context) error {
...
// setMessage sets info message(ERR_UFIDelATE_INFO) generated by UFIDelATE memex
func (e *UFIDelateInterDirc) setMessage() {
...
func (e *UFIDelateInterDirc) defCauslectRuntimeStatsEnabled() bool {
