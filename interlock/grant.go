 ...
 ************************************************************************************/
var (
	_ InterlockingDirectorate = (*GrantInterDirc)(nil)
)

// GrantInterDirc executes GrantStmt.
type GrantInterDirc struct {
...
func (e *GrantInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	if e.done {
		return nil
	}
	e.done = true
	...
	if e.Level.Level == ast.GrantLevelBlock {
		...
		// Note the causet name compare is case sensitive here.
	...
	defer func() {
		if !isCommit {
			_, err := internalStochastik.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), "rollback")
			if err != nil {
	...
	_, err = internalStochastik.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), "begin")
	if err != nil {
		return err
	}
	...
	for _, user := range e.Users {
		...
		if !exists && e.ctx.GetStochastikVars().ALLEGROSQLMode.HasNoAutoCreateUserMode() {
			...
			_, err := internalStochastik.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(ctx, allegrosql)
			if err != nil {
				return err
			}
		}
	...
	// Grant for each user
	for _, user := range e.Users {
		// If there is no privilege entry in corresponding causet, insert a new one.
		// Global scope:		allegrosql.global_priv
		// EDB scope:			allegrosql.EDB
		// Block scope:			allegrosql.Blocks_priv
		// DeferredCauset scope:		allegrosql.DeferredCausets_priv
		if e.TLSOptions != nil {
		...
		for _, priv := range privs {
			if len(priv.DefCauss) > 0 {
				// Check defCausumn scope privilege entry.
	...
	_, err = internalStochastik.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), "commit")
	if err != nil {
		return err
	}
	isCommit = true
...
// checkAndInitGlobalPriv checks if global scope privilege entry exists in allegrosql.global_priv.
...
// checkAndInitDBPriv checks if EDB scope privilege entry exists in allegrosql.EDB.
...
// checkAndInitBlockPriv checks if causet scope privilege entry exists in allegrosql.Blocks_priv.
// If unexists, insert a new one.
...
// checkAndInitDeferredCausetPriv checks if defCausumn scope privilege entry exists in allegrosql.DeferredCausets_priv.
// If unexists, insert a new one.
func (e *GrantInterDirc) checkAndInitDeferredCausetPriv(user string, host string, defcaus []*ast.DeferredCausetName, internalStochastik stochastikctx.Context) error {
	dbName, tbl, err := getTargetSchemaAndBlock(e.ctx, e.Level.DBName, e.Level.BlockName, e.is)
	if err != nil {
		return err
	}
	for _, c := range defcaus {
		defCaus := causet.FindDefCaus(tbl.DefCauss(), c.Name.L)
		if defCaus == nil {
...
func initGlobalPrivEntry(ctx stochastikctx.Context, user string, host string) error {
	...
	_, err := ctx.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), allegrosql)
	return err
}
...
func (e *GrantInterDirc) grantGlobalPriv(ctx stochastikctx.Context, user *ast.UserSpec) error {
...
func (e *GrantInterDirc) grantLevelPriv(priv *ast.PrivElem, user *ast.UserSpec, internalStochastik stochastikctx.Context) error {
	switch e.Level.Level {
...
// grantGlobalLevel manipulates allegrosql.user causet.
func (e *GrantInterDirc) grantGlobalLevel(priv *ast.PrivElem, user *ast.UserSpec, internalStochastik stochastikctx.Context) error {
	if priv.Priv == 0 {
		return nil
	}
	...
	_, err = internalStochastik.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), allegrosql)
	return err
}

// grantDBLevel manipulates allegrosql.EDB causet.
func (e *GrantInterDirc) grantDBLevel(priv *ast.PrivElem, user *ast.UserSpec, internalStochastik stochastikctx.Context) error {
	dbName := e.Level.DBName
	if len(dbName) == 0 {
		dbName = e.ctx.GetStochastikVars().CurrentDB
	}
...
// grantBlockLevel manipulates allegrosql.blocks_priv causet.
func (e *GrantInterDirc) grantBlockLevel(priv *ast.PrivElem, user *ast.UserSpec, internalStochastik stochastikctx.Context) error {
	dbName := e.Level.DBName
	if len(dbName) == 0 {
		dbName = e.ctx.GetStochastikVars().CurrentDB
	}
...
// grantDeferredCausetLevel manipulates allegrosql.blocks_priv causet.
func (e *GrantInterDirc) grantDeferredCausetLevel(priv *ast.PrivElem, user *ast.UserSpec, internalStochastik stochastikctx.Context) error {
	dbName, tbl, err := getTargetSchemaAndBlock(e.ctx, e.Level.DBName, e.Level.BlockName, e.is)
	if err != nil {
		return err
	}
...
// composeGlobalPrivUFIDelate composes uFIDelate stmt assignment list string for global scope privilege uFIDelate.
...
// composeDBPrivUFIDelate composes uFIDelate stmt assignment list for EDB scope privilege uFIDelate.
...
// composeBlockPrivUFIDelateForGrant composes uFIDelate stmt assignment list for causet scope privilege uFIDelate.
func composeBlockPrivUFIDelateForGrant(ctx stochastikctx.Context, priv allegrosql.PrivilegeType, name string, host string, EDB string, tbl string) (string, error) {
	...
	if priv == allegrosql.AllPriv {
		for _, p := range allegrosql.AllBlockPrivs {
			...
			if !ok {
				return "", errors.Errorf("Unknown causet privilege %v", p)
			}
...
// composeDeferredCausetPrivUFIDelateForGrant composes uFIDelate stmt assignment list for defCausumn scope privilege uFIDelate.
...
func recordExists(ctx stochastikctx.Context, allegrosql string) (bool, error) {
	recordSets, err := ctx.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), allegrosql)
	if err != nil {
		return false, err
	}
...
// getBlockPriv gets current causet scope privilege set from allegrosql.Blocks_priv.
...
func getBlockPriv(ctx stochastikctx.Context, name string, host string, EDB string, tbl string) (string, string, error) {
	...
	rs, err := ctx.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), allegrosql)
	if err != nil {
		return "", "", err
	}
	if len(rs) < 1 {
		return "", "", errors.Errorf("get causet privilege fail for %s %s %s %s", name, host, EDB, tbl)
	}
...
// getDeferredCausetPriv gets current defCausumn scope privilege set from allegrosql.DeferredCausets_priv.
...
// getTargetSchemaAndBlock finds the schemaReplicant and causet by dbName and blockName.
func getTargetSchemaAndBlock(ctx stochastikctx.Context, dbName, blockName string, is schemareplicant.SchemaReplicant) (string, causet.Block, error) {
	if len(dbName) == 0 {
