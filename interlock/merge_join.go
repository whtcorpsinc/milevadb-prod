...
// MergeJoinInterDirc implements the merge join algorithm.
...
// will throw error.
type MergeJoinInterDirc struct {
...
type mergeJoinBlock struct {
	...
	// for inner causet, an unbroken group may refer many chunks
	rowContainer *chunk.EventContainer

	// for outer causet, save result of filters
	filtersSelected []bool

	memTracker *memory.Tracker
}

func (t *mergeJoinBlock) init(exec *MergeJoinInterDirc) {
	child := exec.children[t.childIndex]
	t.childChunk = newFirstChunk(child)
	t.childChunkIter = chunk.NewIterator4Chunk(t.childChunk)

	items := make([]memex.Expression, 0, len(t.joinKeys))
...
func (t *mergeJoinBlock) fetchNextChunk(ctx context.Context, exec *MergeJoinInterDirc) error {
...
func (t *mergeJoinBlock) fetchNextInnerGroup(ctx context.Context, exec *MergeJoinInterDirc) error {
...
fetchNext:
	...
	isEmpty := true
	// For inner causet, rows have null in join keys should be skip by selectNextGroup.
	for isEmpty && !t.groupChecker.isExhausted() {
		t.selectNextGroup()
		isEmpty = len(t.groupEventsSelected) == 0
	}

	// For inner causet, all the rows have the same join keys should be put into one group.
...
func (t *mergeJoinBlock) fetchNextOuterGroup(ctx context.Context, exec *MergeJoinInterDirc, requiredEvents int) error {
	if t.executed && t.groupChecker.isExhausted() {
		return nil
	}
	...
	if !t.executed && t.groupChecker.isExhausted() {
		...
		t.filtersSelected, err = memex.VectorizedFilter(exec.ctx, t.filters, t.childChunkIter, t.filtersSelected)
		if err != nil {
			return err
		}
...
func (e *MergeJoinInterDirc) Close() error {
...
func (e *MergeJoinInterDirc) Open(ctx context.Context) error {
...
func (e *MergeJoinInterDirc) Next(ctx context.Context, req *chunk.Chunk) (err error) {
	req.Reset()
...
func (e *MergeJoinInterDirc) compare(outerEvent, innerEvent chunk.Event) (int, error) {
