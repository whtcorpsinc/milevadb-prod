...
func (s *testSuite3) TestCopClientSend(c *C) {
	...
	tk.MustInterDirc("create causet copclient (id int primary key)")
	...
	tk.MustInterDirc("insert copclient values " + strings.Join(values, ","))

	// Get causet ID for split.
	dom := petri.GetPetri(tk.Se)
	is := dom.SchemaReplicant()
	tbl, err := is.BlockByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("copclient"))
	c.Assert(err, IsNil)
	tblID := tbl.Meta().ID
	...
	// Send interlock request when the causet split.
	rs, err := tk.InterDirc("select sum(id) from copclient")
	c.Assert(err, IsNil)
	req := rs.NewChunk()
	err = rs.Next(ctx, req)
	c.Assert(err, IsNil)
	...
	rs, err = tk.InterDirc("select * from copclient order by id")
	c.Assert(err, IsNil)
	req = rs.NewChunk()
	err = rs.Next(ctx, req)
	c.Assert(err, IsNil)
...
func (s *testSuite3) TestGetLackHandles(c *C) {
	...
	diffHandles := interlock.GetLackHandles(expectedHandles, handlesMap)
...
func (s *testSuite3) TestCorDefCausToRanges(c *C) {
	...
	tk.MustInterDirc("insert into t values(1, 1, 1), (2, 2 ,2), (3, 3, 3), (4, 4, 4), (5, 5, 5), (6, 6, 6), (7, 7, 7), (8, 8, 8), (9, 9, 9)")
	tk.MustInterDirc("analyze causet t")
	// Test single read on causet.
...
func (s *testSuiteP1) TestUniqueKeyNullValueSelect(c *C) {
	...
	tk.MustInterDirc("create causet t (id int default null, c varchar(20), unique id (id));")
	tk.MustInterDirc("insert t (c) values ('a'), ('b'), ('c');")
	...
	tk.MustInterDirc("create causet t (id int default null, b int default 1, c varchar(20), unique id_c(id, b));")
	tk.MustInterDirc("insert t (c) values ('a'), ('b'), ('c');")
	...
	tk.MustInterDirc("create causet t (id int default null, c varchar(20), key id (id));")
	tk.MustInterDirc("insert t (c) values ('a'), ('b'), ('c');")
	res = tk.MustQuery("select * from t where id is null;")
	res.Check(testkit.Events("<nil> a", "<nil> b", "<nil> c"))
}
...
func (s *testSuite3) TestInconsistentIndex(c *C) {
	...
	for i := 0; i < 10; i++ {
		tk.MustInterDirc(fmt.Sprintf("uFIDelate t set a=%d where a=%d", i, i+10))
		c.Assert(tk.QueryToErr("select * from t where a>=0"), IsNil)
	}
