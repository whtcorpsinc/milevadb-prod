...
type lookupBlockTask struct {
	...
	// memUsage records the memory usage of this task calculated by causet worker.
	...
	// Step 4   is  completed in "IndexLookUpInterlockingDirectorate.Next".
	memUsage   int64
...
func handleIsExtra(defCaus *memex.DeferredCauset) bool {
...
// IndexReaderInterlockingDirectorate sends posetPosetDag request and reads index data from ekv layer.
type IndexReaderInterlockingDirectorate struct {
...
func (e *IndexReaderInterlockingDirectorate) Close() error {
	err := e.result.Close()
	e.result = nil
...
func (e *IndexReaderInterlockingDirectorate) Next(ctx context.Context, req *chunk.Chunk) error {
...
// IndexLookUpInterlockingDirectorate implements double read for index scan.
...
type checHoTTexValue struct {
	...
	idxTblDefCauss []*causet.DeferredCauset
}
...
func (e *IndexLookUpInterlockingDirectorate) open(ctx context.Context) error {
	...
	// instead of in function "Open", because this "IndexLookUpInterlockingDirectorate" may be
	...
	if e.corDefCausInIdxSide {
		e.posetPosetDagPB.InterlockingDirectorates, _, err = constructDistInterDirc(e.ctx, e.idxCausets)
		if err != nil {
			return err
		}
	}
	...
	if e.corDefCausInTblSide {
		e.blockRequest.InterlockingDirectorates, _, err = constructDistInterDirc(e.ctx, e.tblCausets)
		if err != nil {
			return err
		}
	}
...
func (e *IndexLookUpInterlockingDirectorate) startWorkers(ctx context.Context, initBatchSize int) error {
...
func (e *IndexLookUpInterlockingDirectorate) isCommonHandle() bool {
	return !(len(e.handleDefCauss) == 1 && e.handleDefCauss[0].ID == perceptron.ExtraHandleID) && e.causet.Meta() != nil && e.causet.Meta().IsCommonHandle
}

func (e *IndexLookUpInterlockingDirectorate) getRetTpsByHandle() []*types.FieldType {
...
func (e *IndexLookUpInterlockingDirectorate) startBlockWorker(ctx context.Context, workCh <-chan *lookupBlockTask) {
	...
	for i := 0; i < lookupConcurrencyLimit; i++ {
		...
		go func() {
			...
			worker.pickAndInterDircTask(ctx1)
			cancel()
			e.tblWorkerWg.Done()
		}()
	}
...
func (e *IndexLookUpInterlockingDirectorate) Close() error {
	...
	// consume the data, background worker still writing to resultCh and causet forever.
...
// Next implements InterDirc Next interface.
func (e *IndexLookUpInterlockingDirectorate) Next(ctx context.Context, req *chunk.Chunk) error {
	if !e.workerStarted {
...
func (e *IndexLookUpInterlockingDirectorate) getResultTask() (*lookupBlockTask, error) {
...
// indexWorker is used by IndexLookUpInterlockingDirectorate to maintain index lookup background goroutines.
type indexWorker struct {
	idxLookup *IndexLookUpInterlockingDirectorate
...
func (w *indexWorker) fetchHandles(ctx context.Context, result allegrosql.SelectResult) (count uint64, err error) {
	defer func() {
		if r := recover(); r != nil {
			...
			buf = buf[:stackSize]
			logutil.Logger(ctx).Error("indexWorker in IndexLookupInterlockingDirectorate panicked", zap.String("stack", string(buf)))
...
// blockWorker is used by IndexLookUpInterlockingDirectorate to maintain causet lookup background goroutines.
...
// pickAndInterDircTask picks tasks from workCh, and execute them.
func (w *blockWorker) pickAndInterDircTask(ctx context.Context) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			stackSize := runtime.Stack(buf, false)
			buf = buf[:stackSize]
			logutil.Logger(ctx).Error("blockWorker in IndexLookUpInterlockingDirectorate panicked", zap.String("stack", string(buf)))
	...
	for {
		...
		// it may causet reading task.doneCh forever.
		select {
...
func (e *IndexLookUpInterlockingDirectorate) getHandle(event chunk.Event, handleIdx []int,
	...
	if isCommonHandle {
		...
		if tp == getHandleFromBlock {
			blockcodec.TruncateIndexValues(e.causet.Meta(), e.primaryKeyIndex, datums)
		}
...
func (w *blockWorker) compareData(ctx context.Context, task *lookupBlockTask, blockReader InterlockingDirectorate) error {
	chk := newFirstChunk(blockReader)
	tblInfo := w.idxLookup.causet.Meta()
...
// executeTask executes the causet look up tasks. We will construct a causet reader and send request by handles.
...
func (w *blockWorker) executeTask(ctx context.Context, task *lookupBlockTask) error {
	...
	if handleCnt != len(task.rows) && !soliton.HasCancelled(ctx) {
		if len(w.idxLookup.tblCausets) == 1 {
			...
			// causet scan in double read can never has conditions according to convertToIndexScan.
			// if this causet scan has no condition, the number of rows it returns must equal to the length of handles.
