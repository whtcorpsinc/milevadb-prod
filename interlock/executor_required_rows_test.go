...
func newRequiredEventsDataSource(ctx stochastikctx.Context, totalEvents int, expectedEventsRet []int) *requiredEventsDataSource {
	...
	return &requiredEventsDataSource{baseInterDirc, totalEvents, 0, ctx, expectedEventsRet, 0, defaultGenerator}
}
...
func (s *testInterDircSuite) TestLimitRequiredEvents(c *C) {
	...
	for _, testCase := range testCases {
		...
		exec := buildLimitInterDirc(sctx, ds, testCase.limitOffset, testCase.limitCount)
...
func buildLimitInterDirc(ctx stochastikctx.Context, src InterlockingDirectorate, offset, count int) InterlockingDirectorate {
	n := mathutil.Min(count, ctx.GetStochastikVars().MaxChunkSize)
	base := newBaseInterlockingDirectorate(ctx, src.Schema(), 0, src)
	base.initCap = n
...
func (s *testInterDircSuite) TestSortRequiredEvents(c *C) {
	maxChunkSize := defaultCtx().GetStochastikVars().MaxChunkSize
	testCases := []struct {
		totalEvents      int
		groupBy        []int
	...
	for _, testCase := range testCases {
		...
		exec := buildSortInterDirc(sctx, byItems, ds)
		c.Assert(exec.Open(ctx), IsNil)
		chk := newFirstChunk(exec)
		for i := range testCase.requiredEvents {
...
func (s *testInterDircSuite) TestTopNRequiredEvents(c *C) {
	maxChunkSize := defaultCtx().GetStochastikVars().MaxChunkSize
	testCases := []struct {
		totalEvents      int
		topNOffset     int
	...
	for _, testCase := range testCases {
		...
		exec := buildTopNInterDirc(sctx, testCase.topNOffset, testCase.topNCount, byItems, ds)
		c.Assert(exec.Open(ctx), IsNil)
		chk := newFirstChunk(exec)
		for i := range testCase.requiredEvents {
			chk.SetRequiredEvents(testCase.requiredEvents[i], maxChunkSize)
...
func (s *testInterDircSuite) TestSelectionRequiredEvents(c *C) {
	...
	for _, testCase := range testCases {
		...
		if testCase.gen == nil {
			...
			f, err := memex.NewFunction(
				sctx, ast.EQ, types.NewFieldType(byte(types.ETInt)), ds.Schema().DeferredCausets[1], &memex.Constant{
		...
		exec := buildSelectionInterDirc(sctx, filters, ds)
		c.Assert(exec.Open(ctx), IsNil)
		chk := newFirstChunk(exec)
		for i := range testCase.requiredEvents {
			chk.SetRequiredEvents(testCase.requiredEvents[i], maxChunkSize)
...
func (s *testInterDircSuite) TestProjectionUnparallelRequiredEvents(c *C) {
	maxChunkSize := defaultCtx().GetStochastikVars().MaxChunkSize
	testCases := []struct {
		totalEvents      int
		requiredEvents   []int
	...
	for _, testCase := range testCases {
		...
		ctx := context.Background()
		ds := newRequiredEventsDataSource(sctx, testCase.totalEvents, testCase.expectedEventsDS)
		exprs := make([]memex.Expression, 0, len(ds.Schema().DeferredCausets))
		if len(exprs) == 0 {
			for _, defCaus := range ds.Schema().DeferredCausets {
				exprs = append(exprs, defCaus)
			}
		}
		exec := buildProjectionInterDirc(sctx, exprs, ds, 0)
		c.Assert(exec.Open(ctx), IsNil)
		chk := newFirstChunk(exec)
		for i := range testCase.requiredEvents {
			chk.SetRequiredEvents(testCase.requiredEvents[i], maxChunkSize)
...
func (s *testInterDircSuite) TestProjectionParallelRequiredEvents(c *C) {
	...
	for _, testCase := range testCases {
		...
		exec := buildProjectionInterDirc(sctx, exprs, ds, testCase.numWorkers)
		c.Assert(exec.Open(ctx), IsNil)
		chk := newFirstChunk(exec)
		for i := range testCase.requiredEvents {
			chk.SetRequiredEvents(testCase.requiredEvents[i], maxChunkSize)
...
func (s *testInterDircSuite) TestStreamAggRequiredEvents(c *C) {
	maxChunkSize := defaultCtx().GetStochastikVars().MaxChunkSize
	testCases := []struct {
		totalEvents      int
		aggFunc        string
	...
	for _, testCase := range testCases {
		...
		aggFunc, err := aggregation.NewAggFuncDesc(sctx, testCase.aggFunc, []memex.Expression{childDefCauss[0]}, true)
		c.Assert(err, IsNil)
		aggFuncs := []*aggregation.AggFuncDesc{aggFunc}
		exec := buildStreamAggInterlockingDirectorate(sctx, ds, schemaReplicant, aggFuncs, groupBy)
		c.Assert(exec.Open(ctx), IsNil)
		chk := newFirstChunk(exec)
		for i := range testCase.requiredEvents {
			chk.SetRequiredEvents(testCase.requiredEvents[i], maxChunkSize)
...
func (s *testInterDircSuite) TestMergeJoinRequiredEvents(c *C) {
	...
	for _, joinType := range joinTypes {
		...
		exec := buildMergeJoinInterDirc(ctx, joinType, innerSrc, outerSrc)
...
func genTestChunk4VecGroupChecker(chkEvents []int, sameNum int) (expr []memex.Expression, inputs []*chunk.Chunk) {
	...
	expr = make([]memex.Expression, 1)
	expr[0] = &memex.DeferredCauset{
...
func buildMergeJoinInterDirc(ctx stochastikctx.Context, joinType causetcore.JoinType, innerSrc, outerSrc InterlockingDirectorate) InterlockingDirectorate {
	...
	j.CompareFuncs = make([]memex.CompareFunc, 0, len(j.LeftJoinKeys))
	for i := range j.LeftJoinKeys {
		j.CompareFuncs = append(j.CompareFuncs, memex.GetCmpFunction(nil, j.LeftJoinKeys[i], j.RightJoinKeys[i]))
	}
...
type mockCauset struct {
	MockPhysicalCauset
	exec InterlockingDirectorate
}

func (mp *mockCauset) GetInterlockingDirectorate() InterlockingDirectorate {
	return mp.exec
}

func (mp *mockCauset) Schema() *memex.Schema {
	return mp.exec.Schema()
}

func (s *testInterDircSuite) TestVecGroupCheckerDATARACE(c *C) {
	ctx := mock.NewContext()

	mTypes := []byte{allegrosql.TypeVarString, allegrosql.TypeNewDecimal, allegrosql.TypeJSON}
	for _, mType := range mTypes {
		exprs := make([]memex.Expression, 1)
		exprs[0] = &memex.DeferredCauset{
