...
// TraceInterDirc represents a root interlock of trace query.
type TraceInterDirc struct {
	...
	builder *interlockBuilder
	format  string
}

// Next executes real query and defCauslects span later.
func (e *TraceInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	req.Reset()
	if e.exhausted {
		return nil
	}
	se, ok := e.ctx.(sqlexec.ALLEGROSQLInterlockingDirectorate)
	if !ok {
		e.exhausted = true
		return nil
	}
...
func (e *TraceInterDirc) nextTraceLog(ctx context.Context, se sqlexec.ALLEGROSQLInterlockingDirectorate, req *chunk.Chunk) error {
...
func (e *TraceInterDirc) nextEventJSON(ctx context.Context, se sqlexec.ALLEGROSQLInterlockingDirectorate, req *chunk.Chunk) error {
...
func (e *TraceInterDirc) executeChild(ctx context.Context, se sqlexec.ALLEGROSQLInterlockingDirectorate) {
	recordSets, err := se.InterDircute(ctx, e.stmtNode.Text())
	if len(recordSets) == 0 {
		if err != nil {
			var errCode uint16
