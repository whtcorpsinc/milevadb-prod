...
func (s *testschemaReplicantBlockSuite) TestSchemataBlocks(c *C) {
	...
	tk.MustInterDirc("create user schemata_tester")
	schemataTester := testkit.NewTestKit(c, s.causetstore)
	schemataTester.MustInterDirc("use information_schema")
	...
	tk.MustInterDirc("CREATE ROLE r_mysql_priv;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON allegrosql.* TO r_mysql_priv;")
	tk.MustInterDirc("GRANT r_mysql_priv TO schemata_tester;")
	schemataTester.MustInterDirc("set role r_mysql_priv")
...
func (s *testschemaReplicantBlockSuite) TestBlockIDAndIndexID(c *C) {
	...
	tk.MustInterDirc("create causet test.t (a int, b int, primary key(a), key k1(b))")
...
func (s *testschemaReplicantBlockSuite) TestSchemataCharacterSet(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE DATABASE `foo` DEFAULT CHARACTER SET = 'utf8mb4'")
	...
	tk.MustInterDirc("drop database `foo`")
}

func (s *testschemaReplicantBlockSuite) TestViews(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE DEFINER='root'@'localhost' VIEW test.v1 AS SELECT 1")
...
func (s *testschemaReplicantBlockSuite) TestDBSJobs(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("create database if not exists test_dbs_jobs")
	...
	tk.MustInterDirc("use test_dbs_jobs")
	...
	tk.MustQuery("select db_name, block_name, job_type from information_schema.DBS_JOBS where block_name = 't'").Check(
		testkit.Events("test_dbs_jobs t create causet"))

	tk.MustQuery("select job_type from information_schema.DBS_JOBS group by job_type having job_type = 'create causet'").Check(
		testkit.Events("create causet"))

	// Test the START_TIME and END_TIME field.
	tk.MustQuery("select distinct job_type from information_schema.DBS_JOBS where job_type = 'create causet' and start_time > str_to_date('20190101','%Y%m%d%H%i%s')").Check(
		testkit.Events("create causet"))

	// Test the privilege of new user for information_schema.DBS_JOBS.
	tk.MustInterDirc("create user DBS_JOBS_tester")
	DBSJobsTester := testkit.NewTestKit(c, s.causetstore)
	DBSJobsTester.MustInterDirc("use information_schema")
	...
	DBSJobsTester.MustQuery("select DB_NAME, TABLE_NAME from information_schema.DBS_JOBS where DB_NAME = 'test_dbs_jobs' and TABLE_NAME = 't';").Check(
		[][]interface{}{})
	tk.MustInterDirc("CREATE ROLE r_priv;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON test_dbs_jobs.* TO r_priv;")
	tk.MustInterDirc("GRANT r_priv TO DBS_JOBS_tester;")
	DBSJobsTester.MustInterDirc("set role r_priv")
	DBSJobsTester.MustQuery("select DB_NAME, TABLE_NAME from information_schema.DBS_JOBS where DB_NAME = 'test_dbs_jobs' and TABLE_NAME = 't';").Check(
...
func (s *testschemaReplicantBlockSuite) TestKeyDeferredCausetUsage(c *C) {
	...
	tk.MustQuery("select * from information_schema.KEY_COLUMN_USAGE where TABLE_NAME='stats_spacetime' and COLUMN_NAME='block_id';").Check(
		testkit.Events("def allegrosql tbl def allegrosql stats_spacetime block_id 1 <nil> <nil> <nil> <nil>"))

	//test the privilege of new user for information_schema.block_constraints
	tk.MustInterDirc("create user key_defCausumn_tester")
	keyDeferredCausetTester := testkit.NewTestKit(c, s.causetstore)
	keyDeferredCausetTester.MustInterDirc("use information_schema")
	...
	tk.MustInterDirc("CREATE ROLE r_stats_spacetime ;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON allegrosql.stats_spacetime TO r_stats_spacetime;")
	tk.MustInterDirc("GRANT r_stats_spacetime TO key_defCausumn_tester;")
	keyDeferredCausetTester.MustInterDirc("set role r_stats_spacetime")
	c.Assert(len(keyDeferredCausetTester.MustQuery("select * from information_schema.KEY_COLUMN_USAGE where TABLE_NAME='stats_spacetime';").Events()), Greater, 0)
}

func (s *testschemaReplicantBlockSuite) TestUserPrivileges(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	//test the privilege of new user for information_schema.block_constraints
	tk.MustInterDirc("create user constraints_tester")
	constraintsTester := testkit.NewTestKit(c, s.causetstore)
	constraintsTester.MustInterDirc("use information_schema")
	...
	//test the privilege of user with privilege of allegrosql.gc_delete_range for information_schema.block_constraints
	tk.MustInterDirc("CREATE ROLE r_gc_delete_range ;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON allegrosql.gc_delete_range TO r_gc_delete_range;")
	tk.MustInterDirc("GRANT r_gc_delete_range TO constraints_tester;")
	constraintsTester.MustInterDirc("set role r_gc_delete_range")
	...
	tk.MustInterDirc("create user tester1")
	tk1 := testkit.NewTestKit(c, s.causetstore)
	tk1.MustInterDirc("use information_schema")
	...
	tk.MustInterDirc("create user tester2")
	tk.MustInterDirc("CREATE ROLE r_defCausumns_priv;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON allegrosql.defCausumns_priv TO r_defCausumns_priv;")
	tk.MustInterDirc("GRANT r_defCausumns_priv TO tester2;")
	tk2 := testkit.NewTestKit(c, s.causetstore)
	tk2.MustInterDirc("use information_schema")
	c.Assert(tk2.Se.Auth(&auth.UserIdentity{
		Username: "tester2",
		Hostname: "127.0.0.1",
	}, nil, nil), IsTrue)
	tk2.MustInterDirc("set role r_defCausumns_priv")
	...
	tk.MustInterDirc("create user tester3")
	tk.MustInterDirc("CREATE ROLE r_all_priv;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON allegrosql.* TO r_all_priv;")
	tk.MustInterDirc("GRANT r_all_priv TO tester3;")
	tk3 := testkit.NewTestKit(c, s.causetstore)
	tk3.MustInterDirc("use information_schema")
	c.Assert(tk3.Se.Auth(&auth.UserIdentity{
		Username: "tester3",
		Hostname: "127.0.0.1",
	}, nil, nil), IsTrue)
	tk3.MustInterDirc("set role r_all_priv")
...
func (s *testschemaReplicantBlockSerialSuite) TestDataForBlockStatsField(c *C) {
	...
	oldExpiryTime := interlock.BlockStatsCacheExpiry
	interlock.BlockStatsCacheExpiry = 0
	defer func() { interlock.BlockStatsCacheExpiry = oldExpiryTime }()
	do := s.dom
	h := do.StatsHandle()
	h.Clear()
	...
	tk.MustInterDirc("create causet t (c int, d int, e char(5), index idx(e))")
	...
	tk.MustInterDirc(`insert into t(c, d, e) values(1, 2, "c"), (2, 3, "d"), (3, 4, "e")`)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	tk.MustQuery("select block_rows, avg_row_length, data_length, index_length from information_schema.blocks where block_name='t'").Check(
		testkit.Events("3 18 54 6"))
	tk.MustInterDirc(`insert into t(c, d, e) values(4, 5, "f")`)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	tk.MustQuery("select block_rows, avg_row_length, data_length, index_length from information_schema.blocks where block_name='t'").Check(
		testkit.Events("4 18 72 8"))
	tk.MustInterDirc("delete from t where c >= 3")
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	tk.MustQuery("select block_rows, avg_row_length, data_length, index_length from information_schema.blocks where block_name='t'").Check(
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc(`CREATE TABLE t (a int, b int, c varchar(5), primary key(a), index idx(c)) PARTITION BY RANGE (a) (PARTITION p0 VALUES LESS THAN (6), PARTITION p1 VALUES LESS THAN (11), PARTITION p2 VALUES LESS THAN (16))`)
	h.HandleDBSEvent(<-h.DBSEventCh())
	tk.MustInterDirc(`insert into t(a, b, c) values(1, 2, "c"), (7, 3, "d"), (12, 4, "e")`)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	tk.MustQuery("select block_rows, avg_row_length, data_length, index_length from information_schema.blocks where block_name='t'").Check(
		testkit.Events("3 18 54 6"))
...
func (s *testschemaReplicantBlockSerialSuite) TestPartitionsBlock(c *C) {
	...
	tk.MustInterDirc("USE test;")
	tk.MustInterDirc("DROP TABLE IF EXISTS `test_partitions`;")
	tk.MustInterDirc(`CREATE TABLE test_partitions (a int, b int, c varchar(5), primary key(a), index idx(c)) PARTITION BY RANGE (a) (PARTITION p0 VALUES LESS THAN (6), PARTITION p1 VALUES LESS THAN (11), PARTITION p2 VALUES LESS THAN (16));`)
	err := h.HandleDBSEvent(<-h.DBSEventCh())
	c.Assert(err, IsNil)
	tk.MustInterDirc(`insert into test_partitions(a, b, c) values(1, 2, "c"), (7, 3, "d"), (12, 4, "e");`)
	...
	// Test for causet has no partitions.
	tk.MustInterDirc("DROP TABLE IF EXISTS `test_partitions_1`;")
	tk.MustInterDirc(`CREATE TABLE test_partitions_1 (a int, b int, c varchar(5), primary key(a), index idx(c));`)
	err = h.HandleDBSEvent(<-h.DBSEventCh())
	c.Assert(err, IsNil)
	tk.MustInterDirc(`insert into test_partitions_1(a, b, c) values(1, 2, "c"), (7, 3, "d"), (12, 4, "e");`)
	c.Assert(h.DumpStatsDeltaToKV(handle.DumpAll), IsNil)
	c.Assert(h.UFIDelate(is), IsNil)
	...
	tk.MustInterDirc("DROP TABLE `test_partitions`;")

	tk.MustInterDirc(`CREATE TABLE test_partitions1 (id int, b int, c varchar(5), primary key(id), index idx(c)) PARTITION BY RANGE COLUMNS(id) (PARTITION p0 VALUES LESS THAN (6), PARTITION p1 VALUES LESS THAN (11), PARTITION p2 VALUES LESS THAN (16));`)
	...
	tk.MustInterDirc("DROP TABLE test_partitions1")
}
...
func (s *testschemaReplicantBlockSuite) TestForAnalyzeStatus(c *C) {
	...
	statistics.ClearHistoryJobs()
	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists analyze_test")
	tk.MustInterDirc("create causet analyze_test (a int, b int, index idx(a))")
	tk.MustInterDirc("insert into analyze_test values (1,2),(3,4)")
	...
	tk.MustInterDirc("analyze causet analyze_test")
	...
	tk.MustInterDirc("create user analyze_tester")
	analyzeTester := testkit.NewTestKit(c, s.causetstore)
	analyzeTester.MustInterDirc("use information_schema")
	...
	tk.MustInterDirc("create causet t1 (a int, b int, index idx(a))")
	tk.MustInterDirc("insert into t1 values (1,2),(3,4)")
	tk.MustInterDirc("analyze causet t1")
	tk.MustInterDirc("CREATE ROLE r_t1 ;")
	tk.MustInterDirc("GRANT ALL PRIVILEGES ON test.t1 TO r_t1;")
	tk.MustInterDirc("GRANT r_t1 TO analyze_tester;")
	analyzeTester.MustInterDirc("set role r_t1")
...
func (s *testschemaReplicantBlockSerialSuite) TestForBlockTiFlashReplica(c *C) {
	...
	tk.MustInterDirc("alter causet t set tiflash replica 2 location labels 'a','b';")
...
func (s *testschemaReplicantBlockSuite) TestSequences(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("CREATE SEQUENCE test.seq maxvalue 10000000")
	...
	tk.MustInterDirc("DROP SEQUENCE test.seq")
	tk.MustInterDirc("CREATE SEQUENCE test.seq start = -1 minvalue -1 maxvalue 10 increment 1 cache 10")
	...
	tk.MustInterDirc("CREATE SEQUENCE test.seq2 start = -9 minvalue -10 maxvalue 10 increment -1 cache 15")
...
func (s *testschemaReplicantBlockSuite) TestBlocksPKType(c *C) {
	...
	tk.MustInterDirc("create causet t_int (a int primary key, b int)")
	...
	tk.MustInterDirc("create causet t_implicit (a varchar(64) primary key, b int)")
	...
	tk.MustInterDirc("create causet t_common (a varchar(64) primary key, b int)")
