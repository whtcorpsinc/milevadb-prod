...
// It preserves the order of the outer causet and support batch lookup.
//
...
func (e *IndexLookUpMergeJoin) Open(ctx context.Context) error {
	...
	// IndexLookMergeUpJoin need to rebuild interlock (the dataReaderBuilder) during
	// executing. However `interlock.Next()` is lazy evaluation when the RecordSet
	// result is drained.
	// Lazy evaluation means the saved stochastik context may change during interlock's
	// building and its running.
	// A specific sequence for example:
	//
	// e := buildInterlockingDirectorate()   // txn at build time
	// recordSet := runStmt(e)
...
func (e *IndexLookUpMergeJoin) newInnerMergeWorker(taskCh chan *lookUpMergeJoinTask, workID int) *innerMergeWorker {
	...
	if e.lastDefCausHelper != nil {
		...
		nextCwf.TmpConstant = make([]*memex.Constant, len(e.lastDefCausHelper.TmpConstant))
		for i := range e.lastDefCausHelper.TmpConstant {
			nextCwf.TmpConstant[i] = &memex.Constant{RetType: nextCwf.TargetDefCaus.RetType}
		}
...
func (omw *outerMergeWorker) buildTask(ctx context.Context) (*lookUpMergeJoinTask, error) {
	task := &lookUpMergeJoinTask{
		results:     make(chan *indexMergeJoinResult, numResChkHold),
		outerResult: chunk.NewList(omw.rowTypes, omw.interlock.base().initCap, omw.interlock.base().maxChunkSize),
	}
	...
	for requiredEvents > 0 {
		execChk := newFirstChunk(omw.interlock)
		err := Next(ctx, omw.interlock, execChk)
		if err != nil {
			return task, err
		}
...
func (imw *innerMergeWorker) handleTask(ctx context.Context, task *lookUpMergeJoinTask) (err error) {
	...
	if imw.outerMergeCtx.filter != nil {
		...
		for i := 0; i < numOuterChks; i++ {
			...
			outerMatch[i], err = memex.VectorizedFilter(imw.ctx, imw.outerMergeCtx.filter, chunk.NewIterator4Chunk(chk), outerMatch[i])
			if err != nil {
				return err
			}
		}
	...
	imw.innerInterDirc, err = imw.readerBuilder.buildInterlockingDirectorateForIndexJoin(ctx, dLookUpKeys, imw.indexRanges, imw.keyOff2IdxOff, imw.nextDefCausCompareFilters)
	if err != nil {
		return err
	}
	defer terror.Call(imw.innerInterDirc.Close)
...
// fetchNextInnerResult defCauslects a chunk of inner results from inner child interlock.
func (imw *innerMergeWorker) fetchNextInnerResult(ctx context.Context, task *lookUpMergeJoinTask) (beginEvent chunk.Event, err error) {
	task.innerResult = chunk.NewChunkWithCapacity(retTypes(imw.innerInterDirc), imw.ctx.GetStochastikVars().MaxChunkSize)
	err = Next(ctx, imw.innerInterDirc, task.innerResult)
