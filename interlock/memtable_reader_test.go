...
func (s *testMemBlockReaderSuite) TestMetricBlockData(c *C) {
	fpName := "github.com/whtcorpsinc/milevadb/interlock/mockMetricsPromData"
	...
	tk.MustInterDirc("use metrics_schema")
	...
	for _, cas := range cases {
		rs, err := tk.Se.InterDircute(ctx, cas.allegrosql)
		c.Assert(err, IsNil)
...
func (s *testMemBlockReaderSuite) TestMilevaDBClusterConfig(c *C) {
	...
	fpName := "github.com/whtcorpsinc/milevadb/interlock/mockClusterConfigServerInfo"
...
func (s *testMemBlockReaderSuite) TestMilevaDBClusterLog(c *C) {
	...
	fpName := "github.com/whtcorpsinc/milevadb/interlock/mockClusterLogServerInfo"
	fpExpr := strings.Join(servers, ";")
	c.Assert(failpoint.Enable(fpName, fmt.Sprintf(`return("%s")`, fpExpr)), IsNil)
	defer func() { c.Assert(failpoint.Disable(fpName), IsNil) }()
...
func (s *testMemBlockReaderSuite) TestMilevaDBClusterLogError(c *C) {
	...
	rs, err := tk.InterDirc("select * from information_schema.cluster_log")
	c.Assert(err, IsNil)
	...
	rs, err = tk.InterDirc("select * from information_schema.cluster_log where time>='2020/08/26 06:18:13.011'")
	c.Assert(err, IsNil)
	_, err = stochastik.ResultSetToStringSlice(context.Background(), tk.Se, rs)
	c.Assert(err, NotNil)
	...
	rs, err = tk.InterDirc("select * from information_schema.cluster_log where time>='2020/08/26 06:18:13.011' and time<'2020/08/26 16:18:13.011'")
	c.Assert(err, IsNil)
	_, err = stochastik.ResultSetToStringSlice(context.Background(), tk.Se, rs)
	c.Assert(err, NotNil)
