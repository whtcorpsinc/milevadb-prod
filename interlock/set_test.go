...
func (s *testSuite5) TestSetVar(c *C) {
	...
	_, err = tk.InterDirc(errTestALLEGROSQL)
	c.Assert(err, NotNil)
	...
	// For issue 4302
	testALLEGROSQL = "use test;drop causet if exists x;create causet x(a int);insert into x value(1);"
	tk.MustInterDirc(testALLEGROSQL)
	...
	// Set default
	// {ScopeGlobal | ScopeStochastik, "low_priority_uFIDelates", "OFF"},
	// For global var
	tk.MustQuery(`select @@global.low_priority_uFIDelates;`).Check(testkit.Events("0"))
	tk.MustInterDirc(`set @@global.low_priority_uFIDelates="ON";`)
	tk.MustQuery(`select @@global.low_priority_uFIDelates;`).Check(testkit.Events("1"))
	tk.MustInterDirc(`set @@global.low_priority_uFIDelates=DEFAULT;`) // It will be set to compiled-in default value.
	tk.MustQuery(`select @@global.low_priority_uFIDelates;`).Check(testkit.Events("0"))
	// For stochastik
	tk.MustQuery(`select @@stochastik.low_priority_uFIDelates;`).Check(testkit.Events("0"))
	tk.MustInterDirc(`set @@global.low_priority_uFIDelates="ON";`)
	tk.MustInterDirc(`set @@stochastik.low_priority_uFIDelates=DEFAULT;`) // It will be set to global var value.
	...
	tk.MustInterDirc("set character_set_results = NULL")
	tk.MustQuery("select @@character_set_results").Check(testkit.Events(""))

	tk.MustInterDirc("set @@stochastik.dbs_slow_threshold=12345")
	...
	tk.MustInterDirc("set stochastik dbs_slow_threshold=\"54321\"")
	...
	tk.MustInterDirc("SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED")
	...
	// error
	_, err = tk.InterDirc("SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@stochastik.tx_isolation").Check(testkit.Events("READ-COMMITTED"))
	tk.MustQuery("select @@stochastik.transaction_isolation").Check(testkit.Events("READ-COMMITTED"))
	// Fails
	_, err = tk.InterDirc("SET GLOBAL TRANSACTION ISOLATION LEVEL SERIALIZABLE")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	...
	tk.MustInterDirc("SET SESSION tx_isolation = 'READ-COMMITTED'")
	tk.MustQuery("select @@stochastik.tx_isolation").Check(testkit.Events("READ-COMMITTED"))
	tk.MustQuery("select @@stochastik.transaction_isolation").Check(testkit.Events("READ-COMMITTED"))

	_, err = tk.InterDirc("SET SESSION tx_isolation = 'READ-UNCOMMITTED'")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@stochastik.tx_isolation").Check(testkit.Events("READ-COMMITTED"))
	tk.MustQuery("select @@stochastik.transaction_isolation").Check(testkit.Events("READ-COMMITTED"))

	// fails
	_, err = tk.InterDirc("SET SESSION transaction_isolation = 'SERIALIZABLE'")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@stochastik.tx_isolation").Check(testkit.Events("READ-COMMITTED"))
	tk.MustQuery("select @@stochastik.transaction_isolation").Check(testkit.Events("READ-COMMITTED"))

	// fails
	_, err = tk.InterDirc("SET GLOBAL transaction_isolation = 'SERIALIZABLE'")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@global.tx_isolation").Check(testkit.Events("REPEATABLE-READ"))
	tk.MustQuery("select @@global.transaction_isolation").Check(testkit.Events("REPEATABLE-READ"))

	_, err = tk.InterDirc("SET GLOBAL transaction_isolation = 'READ-UNCOMMITTED'")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@global.tx_isolation").Check(testkit.Events("REPEATABLE-READ"))
	tk.MustQuery("select @@global.transaction_isolation").Check(testkit.Events("REPEATABLE-READ"))

	_, err = tk.InterDirc("SET GLOBAL tx_isolation = 'SERIALIZABLE'")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@global.tx_isolation").Check(testkit.Events("REPEATABLE-READ"))
	tk.MustQuery("select @@global.transaction_isolation").Check(testkit.Events("REPEATABLE-READ"))

	tk.MustInterDirc("SET SESSION tx_read_only = 1")
	tk.MustInterDirc("SET SESSION tx_read_only = 0")
	tk.MustQuery("select @@stochastik.tx_read_only").Check(testkit.Events("0"))
	tk.MustQuery("select @@stochastik.transaction_read_only").Check(testkit.Events("0"))

	tk.MustInterDirc("SET GLOBAL tx_read_only = 1")
	tk.MustInterDirc("SET GLOBAL tx_read_only = 0")
	...
	tk.MustInterDirc("SET SESSION transaction_read_only = 1")
	tk.MustInterDirc("SET SESSION transaction_read_only = 0")
	tk.MustQuery("select @@stochastik.tx_read_only").Check(testkit.Events("0"))
	tk.MustQuery("select @@stochastik.transaction_read_only").Check(testkit.Events("0"))
	...
	tk.MustInterDirc("SET GLOBAL transaction_read_only = 1")
	tk.MustInterDirc("SET GLOBAL transaction_read_only = 0")
	tk.MustQuery("select @@global.tx_read_only").Check(testkit.Events("0"))
	tk.MustQuery("select @@global.transaction_read_only").Check(testkit.Events("0"))
	...
	tk.MustInterDirc("set @@milevadb_general_log = 1")
	tk.MustInterDirc("set @@milevadb_general_log = 0")

	tk.MustInterDirc("set @@milevadb_pprof_sql_cpu = 1")
	tk.MustInterDirc("set @@milevadb_pprof_sql_cpu = 0")
	...
	tk.MustInterDirc(`set milevadb_force_priority = "abc"`)
	tk.MustQuery(`select @@milevadb_force_priority;`).Check(testkit.Events("NO_PRIORITY"))
	_, err = tk.InterDirc(`set global milevadb_force_priority = ""`)
	c.Assert(err, NotNil)
	...
	tk.MustInterDirc("set global milevadb_constraint_check_in_place = 0")
	tk.MustQuery(`select @@global.milevadb_constraint_check_in_place;`).Check(testkit.Events("0"))

	tk.MustInterDirc("set milevadb_batch_commit = 0")
	...
	_, err = tk.InterDirc("set global milevadb_batch_commit = 0")
	c.Assert(err, NotNil)
	_, err = tk.InterDirc("set global milevadb_batch_commit = 2")
	c.Assert(err, NotNil)

	// test skip isolation level check: init
	tk.MustInterDirc("SET GLOBAL milevadb_skip_isolation_level_check = 0")
	tk.MustInterDirc("SET SESSION milevadb_skip_isolation_level_check = 0")
	tk.MustInterDirc("SET GLOBAL TRANSACTION ISOLATION LEVEL READ COMMITTED")
	tk.MustInterDirc("SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED")
	...
	_, err = tk.InterDirc("SET SESSION TRANSACTION ISOLATION LEVEL SERIALIZABLE")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))
	tk.MustQuery("select @@stochastik.tx_isolation").Check(testkit.Events("READ-COMMITTED"))
	tk.MustQuery("select @@stochastik.transaction_isolation").Check(testkit.Events("READ-COMMITTED"))
	...
	tk.MustInterDirc("SET GLOBAL milevadb_skip_isolation_level_check = 1")
	tk.MustInterDirc("SET SESSION milevadb_skip_isolation_level_check = 1")
	tk.MustInterDirc("SET SESSION TRANSACTION ISOLATION LEVEL SERIALIZABLE")
	...
	tk.MustInterDirc("SET GLOBAL milevadb_skip_isolation_level_check = 0")
	tk.MustInterDirc("SET SESSION milevadb_skip_isolation_level_check = 1")
	tk.MustInterDirc("SET GLOBAL TRANSACTION ISOLATION LEVEL READ UNCOMMITTED")
	tk.MustQuery("show warnings").Check(testkit.Events(
	...
	tk.MustInterDirc("SET GLOBAL transaction_isolation='REPEATABLE-READ'") // should reset tx_isolation back to rr before reset milevadb_skip_isolation_level_check
	tk.MustInterDirc("SET GLOBAL milevadb_skip_isolation_level_check = 0")
	tk.MustInterDirc("SET SESSION milevadb_skip_isolation_level_check = 0")

	tk.MustInterDirc("set global read_only = 0")
	tk.MustQuery("select @@global.read_only;").Check(testkit.Events("0"))
	tk.MustInterDirc("set global read_only = off")
	tk.MustQuery("select @@global.read_only;").Check(testkit.Events("0"))
	tk.MustInterDirc("set global read_only = 1")
	tk.MustQuery("select @@global.read_only;").Check(testkit.Events("1"))
	tk.MustInterDirc("set global read_only = on")
	tk.MustQuery("select @@global.read_only;").Check(testkit.Events("1"))
	_, err = tk.InterDirc("set global read_only = abc")
	c.Assert(err, NotNil)
	...
	tk.MustInterDirc("set milevadb_wait_split_region_finish = 1")
	tk.MustQuery(`select @@stochastik.milevadb_wait_split_region_finish;`).Check(testkit.Events("1"))
	tk.MustInterDirc("set milevadb_wait_split_region_finish = 0")
	...
	tk.MustInterDirc("set global milevadb_scatter_region = 0")
	tk.MustQuery(`select @@global.milevadb_scatter_region;`).Check(testkit.Events("0"))
	_, err = tk.InterDirc("set stochastik milevadb_scatter_region = 0")
	c.Assert(err, NotNil)
	_, err = tk.InterDirc(`select @@stochastik.milevadb_scatter_region;`)
	c.Assert(err, NotNil)
	...
	_, err = tk.InterDirc("set milevadb_wait_split_region_timeout = 0")
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "milevadb_wait_split_region_timeout(0) cannot be smaller than 1")
	tk.MustQuery(`select @@stochastik.milevadb_wait_split_region_timeout;`).Check(testkit.Events("1"))

	tk.MustInterDirc("set stochastik milevadb_backoff_weight = 3")
	tk.MustQuery("select @@stochastik.milevadb_backoff_weight;").Check(testkit.Events("3"))
	tk.MustInterDirc("set stochastik milevadb_backoff_weight = 20")
	tk.MustQuery("select @@stochastik.milevadb_backoff_weight;").Check(testkit.Events("20"))
	_, err = tk.InterDirc("set stochastik milevadb_backoff_weight = -1")
	c.Assert(err, NotNil)
	_, err = tk.InterDirc("set global milevadb_backoff_weight = 0")
	c.Assert(err, NotNil)
	tk.MustInterDirc("set global milevadb_backoff_weight = 10")
	tk.MustQuery("select @@global.milevadb_backoff_weight;").Check(testkit.Events("10"))

	tk.MustInterDirc("set @@milevadb_expensive_query_time_threshold=70")
	...
	tk.MustInterDirc("set @@milevadb_store_limit = 100")
	...
	tk.MustInterDirc("set @@milevadb_store_limit = 0")

	tk.MustInterDirc("set global milevadb_store_limit = 100")
	tk.MustQuery("select @@milevadb_store_limit;").Check(testkit.Events("0"))
	...
	tk.MustInterDirc("set global milevadb_enable_change_defCausumn_type = 1")
	tk.MustQuery("select @@milevadb_enable_change_defCausumn_type;").Check(testkit.Events("1"))
	tk.MustInterDirc("set global milevadb_enable_change_defCausumn_type = off")
	tk.MustQuery("select @@milevadb_enable_change_defCausumn_type;").Check(testkit.Events("0"))

	tk.MustQuery("select @@stochastik.milevadb_metric_query_step;").Check(testkit.Events("60"))
	tk.MustInterDirc("set @@stochastik.milevadb_metric_query_step = 120")
	_, err = tk.InterDirc("set @@stochastik.milevadb_metric_query_step = 9")
	c.Assert(err, NotNil)
	...
	tk.MustInterDirc("set @@stochastik.milevadb_metric_query_range_duration = 120")
	_, err = tk.InterDirc("set @@stochastik.milevadb_metric_query_range_duration = 9")
	c.Assert(err, NotNil)
	...
	tk.MustInterDirc("set global milevadb_slow_log_masking = 1")
	tk.MustQuery(`select @@global.milevadb_slow_log_masking;`).Check(testkit.Events("1"))
	tk.MustInterDirc("set global milevadb_slow_log_masking = 0")
	tk.MustQuery(`select @@global.milevadb_slow_log_masking;`).Check(testkit.Events("0"))
	_, err = tk.InterDirc("set stochastik milevadb_slow_log_masking = 0")
	c.Assert(err, NotNil)
	_, err = tk.InterDirc(`select @@stochastik.milevadb_slow_log_masking;`)
	c.Assert(err, NotNil)

	tk.MustQuery("select @@milevadb_dml_batch_size;").Check(testkit.Events("0"))
	tk.MustInterDirc("set @@stochastik.milevadb_dml_batch_size = 120")
	tk.MustQuery("select @@milevadb_dml_batch_size;").Check(testkit.Events("120"))
	c.Assert(tk.InterDircToErr("set @@stochastik.milevadb_dml_batch_size = -120"), NotNil)
	c.Assert(tk.InterDircToErr("set @@global.milevadb_dml_batch_size = 200"), IsNil)  // now permitted due to MilevaDB #19809
	tk.MustQuery("select @@milevadb_dml_batch_size;").Check(testkit.Events("120")) // global only applies to new stochastik

	_, err = tk.InterDirc("set milevadb_enable_parallel_apply=-1")
...
func (s *testSuite5) TestTruncateIncorrectIntStochastikVar(c *C) {
	...
	for _, tc := range testCases {
		...
		tk.MustInterDirc(fmt.Sprintf("set @@%s = %d", name, validValue))
		tk.MustQuery(selectALLEGROSQL).Check(testkit.Events(fmt.Sprintf("%d", validValue)))

		tk.MustInterDirc(fmt.Sprintf("set @@%s = %d", name, tc.minValue-1))
		...
		tk.MustInterDirc(fmt.Sprintf("set @@%s = %d", name, tc.maxValue+1))
...
func (s *testSuite5) TestSetCharset(c *C) {
	...
	tk.MustInterDirc(`SET NAMES latin1`)
	check(
		"latin1",
		"latin1",
		"latin1",
	...
	tk.MustInterDirc(`SET NAMES default`)
	check(
		"utf8mb4",
		"utf8mb4",
		"utf8mb4",
	...
	// Issue #1523
	tk.MustInterDirc(`SET NAMES binary`)
	check(
		"binary",
		"binary",
		"binary",
	...
	tk.MustInterDirc(`SET CHARACTER SET latin1`)
	check(
		"latin1",
		"utf8mb4",
		"latin1",
	...
	tk.MustInterDirc(`SET CHARACTER SET default`)
	check(
		"utf8mb4",
		"utf8mb4",
		"utf8mb4",
...
func (s *testSuite5) TestValidateSetVar(c *C) {
	...
	_, err := tk.InterDirc("set global milevadb_allegrosql_scan_concurrency='fff';")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set global milevadb_allegrosql_scan_concurrency=-2;")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set @@milevadb_allegrosql_scan_concurrency='fff';")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set @@milevadb_allegrosql_scan_concurrency=-2;")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set @@milevadb_batch_delete='ok';")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	tk.MustInterDirc("set @@milevadb_batch_delete='On';")
	tk.MustQuery("select @@milevadb_batch_delete;").Check(testkit.Events("1"))
	tk.MustInterDirc("set @@milevadb_batch_delete='oFf';")
	tk.MustQuery("select @@milevadb_batch_delete;").Check(testkit.Events("0"))
	tk.MustInterDirc("set @@milevadb_batch_delete=1;")
	tk.MustQuery("select @@milevadb_batch_delete;").Check(testkit.Events("1"))
	tk.MustInterDirc("set @@milevadb_batch_delete=0;")
	tk.MustQuery("select @@milevadb_batch_delete;").Check(testkit.Events("0"))

	tk.MustInterDirc("set @@milevadb_opt_agg_push_down=off;")
	tk.MustQuery("select @@milevadb_opt_agg_push_down;").Check(testkit.Events("0"))

	tk.MustInterDirc("set @@milevadb_constraint_check_in_place=on;")
	...
	tk.MustInterDirc("set @@milevadb_enable_streaming=1;")
	tk.MustQuery("select @@milevadb_enable_streaming;").Check(testkit.Events("1"))

	_, err = tk.InterDirc("set @@milevadb_batch_delete=3;")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))
	...
	tk.MustInterDirc("set @@default_week_format=-1")
	...
	tk.MustInterDirc("set @@default_week_format=9")
	...
	tk.MustInterDirc("set @@global.max_connections=100001")
	...
	tk.MustInterDirc("set @@global.max_connections=-1")
	...
	_, err = tk.InterDirc("set @@global.max_connections='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@global.thread_pool_size=65")
	...
	tk.MustInterDirc("set @@global.thread_pool_size=-1")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect thread_pool_size value: '-1'"))
	result = tk.MustQuery("select @@global.thread_pool_size;")
	result.Check(testkit.Events("1"))

	_, err = tk.InterDirc("set @@global.thread_pool_size='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@global.max_allowed_packet=-1")
	...
	_, err = tk.InterDirc("set @@global.max_allowed_packet='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@global.max_connect_errors=18446744073709551615")

	tk.MustInterDirc("set @@global.max_connect_errors=-1")
	...
	_, err = tk.InterDirc("set @@global.max_connect_errors=18446744073709551616")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)
	...
	tk.MustInterDirc("set @@max_sort_length=1")
	...
	_, err = tk.InterDirc("set @@max_sort_length='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@global.block_definition_cache=399")
	...
	tk.MustInterDirc("set @@global.block_definition_cache=-1")
	...
	tk.MustInterDirc("set @@global.block_definition_cache=524289")
	...
	_, err = tk.InterDirc("set @@global.block_definition_cache='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@old_passwords=-1")
	...
	tk.MustInterDirc("set @@old_passwords=3")
	...
	_, err = tk.InterDirc("set @@old_passwords='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@tmp_block_size=-1")
	...
	tk.MustInterDirc("set @@tmp_block_size=1020")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect tmp_block_size value: '1020'"))
	result = tk.MustQuery("select @@tmp_block_size;")
	result.Check(testkit.Events("1024"))

	tk.MustInterDirc("set @@tmp_block_size=167772161")
	result = tk.MustQuery("select @@tmp_block_size;")
	result.Check(testkit.Events("167772161"))

	tk.MustInterDirc("set @@tmp_block_size=18446744073709551615")
	result = tk.MustQuery("select @@tmp_block_size;")
	result.Check(testkit.Events("18446744073709551615"))

	_, err = tk.InterDirc("set @@tmp_block_size=18446744073709551616")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	_, err = tk.InterDirc("set @@tmp_block_size='hello'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongTypeForVar), IsTrue)

	tk.MustInterDirc("set @@global.connect_timeout=1")
	...
	tk.MustInterDirc("set @@global.connect_timeout=31536000")
	result = tk.MustQuery("select @@global.connect_timeout;")
	result.Check(testkit.Events("31536000"))

	tk.MustInterDirc("set @@global.connect_timeout=31536001")
	...
	result.Check(testkit.Events("18446744073709551615"))
	tk.MustInterDirc("set @@sql_select_limit=default")
	result = tk.MustQuery("select @@sql_select_limit;")
	result.Check(testkit.Events("18446744073709551615"))

	tk.MustInterDirc("set @@sql_auto_is_null=00")
	result = tk.MustQuery("select @@sql_auto_is_null;")
	result.Check(testkit.Events("0"))

	tk.MustInterDirc("set @@sql_warnings=001")
	result = tk.MustQuery("select @@sql_warnings;")
	result.Check(testkit.Events("1"))

	tk.MustInterDirc("set @@sql_warnings=000")
	result = tk.MustQuery("select @@sql_warnings;")
	result.Check(testkit.Events("0"))

	tk.MustInterDirc("set @@global.super_read_only=-0")
	result = tk.MustQuery("select @@global.super_read_only;")
	result.Check(testkit.Events("0"))

	_, err = tk.InterDirc("set @@global.super_read_only=-1")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	tk.MustInterDirc("set @@global.innodb_status_output_locks=-1")
	result = tk.MustQuery("select @@global.innodb_status_output_locks;")
	result.Check(testkit.Events("1"))

	tk.MustInterDirc("set @@global.innodb_ft_enable_stopword=0000000")
	result = tk.MustQuery("select @@global.innodb_ft_enable_stopword;")
	result.Check(testkit.Events("0"))

	tk.MustInterDirc("set @@global.innodb_stats_on_spacetimedata=1")
	result = tk.MustQuery("select @@global.innodb_stats_on_spacetimedata;")
	result.Check(testkit.Events("1"))

	tk.MustInterDirc("set @@global.innodb_file_per_block=-50")
	result = tk.MustQuery("select @@global.innodb_file_per_block;")
	result.Check(testkit.Events("1"))

	_, err = tk.InterDirc("set @@global.innodb_ft_enable_stopword=2")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	tk.MustInterDirc("set @@query_cache_type=0")
	result = tk.MustQuery("select @@query_cache_type;")
	result.Check(testkit.Events("OFF"))

	tk.MustInterDirc("set @@query_cache_type=2")
	result = tk.MustQuery("select @@query_cache_type;")
	result.Check(testkit.Events("DEMAND"))

	tk.MustInterDirc("set @@global.sync_binlog=-1")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect sync_binlog value: '-1'"))

	tk.MustInterDirc("set @@global.sync_binlog=4294967299")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect sync_binlog value: '4294967299'"))

	tk.MustInterDirc("set @@global.flush_time=31536001")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect flush_time value: '31536001'"))

	tk.MustInterDirc("set @@global.interactive_timeout=31536001")
	...
	tk.MustInterDirc("set @@global.innodb_commit_concurrency = -1")
	...
	tk.MustInterDirc("set @@global.innodb_commit_concurrency = 1001")
	...
	tk.MustInterDirc("set @@global.innodb_lock_wait_timeout = 1073741825")
	...
	tk.MustInterDirc("set @@innodb_lock_wait_timeout = 0")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect innodb_lock_wait_timeout value: '0'"))

	tk.MustInterDirc("set @@innodb_lock_wait_timeout = 1073741825")
	tk.MustQuery("show warnings").Check(solitonutil.EventsWithSep("|", "Warning|1292|Truncated incorrect innodb_lock_wait_timeout value: '1073741825'"))

	tk.MustInterDirc("set @@global.validate_password_number_count=-1")
	...
	tk.MustInterDirc("set @@global.validate_password_length=-1")
	...
	tk.MustInterDirc("set @@global.validate_password_length=8")
	tk.MustQuery("show warnings").Check(testkit.Events())

	_, err = tk.InterDirc("set @@tx_isolation=''")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set global tx_isolation=''")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set @@transaction_isolation=''")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set global transaction_isolation=''")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))

	_, err = tk.InterDirc("set global tx_isolation='REPEATABLE-READ1'")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))
	...
	tk.MustInterDirc("SET GLOBAL milevadb_skip_isolation_level_check = 0")
	tk.MustInterDirc("SET SESSION milevadb_skip_isolation_level_check = 0")
	_, err = tk.InterDirc("set @@tx_isolation='SERIALIZABLE'")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnsupportedIsolationLevel), IsTrue, Commentf("err %v", err))

	tk.MustInterDirc("set global allow_auto_random_explicit_insert=on;")
...
func (s *testSuite5) TestSelectGlobalVar(c *C) {
	...
	tk.MustInterDirc("set @@global.max_connections=100;")
	...
	tk.MustInterDirc("set @@global.max_connections=151;")

	// test for unknown variable.
	err := tk.InterDircToErr("select @@invalid")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnknownSystemVar), IsTrue, Commentf("err %v", err))
	err = tk.InterDircToErr("select @@global.invalid")
	c.Assert(terror.ErrorEqual(err, variable.ErrUnknownSystemVar), IsTrue, Commentf("err %v", err))
}
...
func (s *testSuite5) TestSetConcurrency(c *C) {
	...
	// test default value
	tk.MustQuery("select @@milevadb_interlock_concurrency;").Check(testkit.Events(strconv.Itoa(variable.DefInterlockingDirectorateConcurrency)))
	...
	vars := tk.Se.(stochastikctx.Context).GetStochastikVars()
	c.Assert(vars.InterlockingDirectorateConcurrency, Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.IndexLookupConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.IndexLookupJoinConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.HashJoinConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.HashAggPartialConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.HashAggFinalConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.WindowConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	c.Assert(vars.ProjectionConcurrency(), Equals, variable.DefInterlockingDirectorateConcurrency)
	...
	warnTpl := "Warning 1287 '%s' is deprecated and will be removed in a future release. Please use milevadb_interlock_concurrency instead"

	checkSet := func(v string) {
		tk.MustInterDirc(fmt.Sprintf("set @@%s=1;", v))
	...
	tk.MustInterDirc(fmt.Sprintf("set @@%s=1;", variable.MilevaDBDistALLEGROSQLScanConcurrency))
	...
	tk.MustInterDirc("set @@milevadb_index_serial_scan_concurrency=4")
	tk.MustQuery("show warnings").Check(testkit.Events())
	...
	tk.MustInterDirc("set @@milevadb_index_lookup_concurrency=-1;")
	...
	tk.MustInterDirc("set @@milevadb_window_concurrency=-1;")
	...
	_, err := tk.InterDirc("set @@milevadb_interlock_concurrency=-1;")
	c.Assert(terror.ErrorEqual(err, variable.ErrWrongValueForVar), IsTrue, Commentf("err %v", err))
}
...
func (s *testSuite5) TestEnableNoopFunctionsVar(c *C) {
	...
	_, err := tk.InterDirc(`select get_lock('lock1', 2);`)
	c.Assert(terror.ErrorEqual(err, memex.ErrFunctionsNoopImpl), IsTrue, Commentf("err %v", err))
	_, err = tk.InterDirc(`select release_lock('lock1');`)
	c.Assert(terror.ErrorEqual(err, memex.ErrFunctionsNoopImpl), IsTrue, Commentf("err %v", err))
	...
	// restore to 0
	tk.MustInterDirc(`set milevadb_enable_noop_functions=0;`)
	tk.MustQuery(`select @@milevadb_enable_noop_functions;`).Check(testkit.Events("0"))
	tk.MustQuery(`select @@global.milevadb_enable_noop_functions;`).Check(testkit.Events("0"))

	_, err = tk.InterDirc(`select get_lock('lock2', 10);`)
	c.Assert(terror.ErrorEqual(err, memex.ErrFunctionsNoopImpl), IsTrue, Commentf("err %v", err))
	_, err = tk.InterDirc(`select release_lock('lock2');`)
	c.Assert(terror.ErrorEqual(err, memex.ErrFunctionsNoopImpl), IsTrue, Commentf("err %v", err))

	// set test
	_, err = tk.InterDirc(`set milevadb_enable_noop_functions='abc'`)
	c.Assert(err, NotNil)
	_, err = tk.InterDirc(`set milevadb_enable_noop_functions=11`)
	c.Assert(err, NotNil)
	tk.MustInterDirc(`set milevadb_enable_noop_functions="off";`)
	tk.MustQuery(`select @@milevadb_enable_noop_functions;`).Check(testkit.Events("0"))
	tk.MustInterDirc(`set milevadb_enable_noop_functions="on";`)
	tk.MustQuery(`select @@milevadb_enable_noop_functions;`).Check(testkit.Events("1"))
	tk.MustInterDirc(`set milevadb_enable_noop_functions=0;`)
	tk.MustQuery(`select @@milevadb_enable_noop_functions;`).Check(testkit.Events("0"))
}
...
func (s *testSuite5) TestSetClusterConfig(c *C) {
	...
	tk.Se.SetValue(interlock.TestSetConfigServerInfoKey, serverInfoFunc)

	c.Assert(tk.InterDircToErr("set config xxx log.level='info'"), ErrorMatches, "unknown type xxx")
	c.Assert(tk.InterDircToErr("set config milevadb log.level='info'"), ErrorMatches, "MilevaDB doesn't support to change configs online, please use ALLEGROALLEGROSQL variables")
	c.Assert(tk.InterDircToErr("set config '127.0.0.1:1111' log.level='info'"), ErrorMatches, "MilevaDB doesn't support to change configs online, please use ALLEGROALLEGROSQL variables")
	c.Assert(tk.InterDircToErr("set config '127.a.b.c:1234' log.level='info'"), ErrorMatches, "invalid instance 127.a.b.c:1234")
	c.Assert(tk.InterDircToErr("set config einsteindb log.level=null"), ErrorMatches, "can't set config to null")
	c.Assert(tk.InterDircToErr("set config '1.1.1.1:1111' log.level='info'"), ErrorMatches, "instance 1.1.1.1:1111 is not found in this cluster")

	httpCnt := 0
	tk.Se.SetValue(interlock.TestSetConfigHTTPHandlerKey, func(*http.Request) (*http.Response, error) {
		httpCnt++
		return &http.Response{StatusCode: http.StatusOK, Body: ioutil.NopCloser(nil)}, nil
	})
	tk.MustInterDirc("set config einsteindb log.level='info'")
	c.Assert(httpCnt, Equals, 2)

	httpCnt = 0
	tk.MustInterDirc("set config '127.0.0.1:5555' log.level='info'")
...
func (s *testSuite5) TestSetClusterConfigJSONData(c *C) {
	...
	cases := []struct {
		val    memex.Expression
		result string
		succ   bool
	}{
		{&memex.Constant{Value: types.NewIntCauset(1), RetType: tyBool}, `{"k":true}`, true},
		{&memex.Constant{Value: types.NewIntCauset(0), RetType: tyBool}, `{"k":false}`, true},
		{&memex.Constant{Value: types.NewIntCauset(2333), RetType: types.NewFieldType(allegrosql.TypeLong)}, `{"k":2333}`, true},
		{&memex.Constant{Value: types.NewFloat64Causet(23.33), RetType: types.NewFieldType(allegrosql.TypeDouble)}, `{"k":23.33}`, true},
		{&memex.Constant{Value: types.NewStringCauset("abcd"), RetType: types.NewFieldType(allegrosql.TypeString)}, `{"k":"abcd"}`, true},
		{&memex.Constant{Value: types.NewDecimalCauset(&d), RetType: types.NewFieldType(allegrosql.TypeNewDecimal)}, `{"k":123.456}`, true},
		{&memex.Constant{Value: types.NewCauset(nil), RetType: types.NewFieldType(allegrosql.TypeLonglong)}, "", false},
		{&memex.Constant{RetType: types.NewFieldType(allegrosql.TypeJSON)}, "", false}, // unsupported type
		{nil, "", false},
	}

	ctx := mock.NewContext()
	for _, t := range cases {
		result, err := interlock.ConvertConfigItem2JSON(ctx, "k", t.val)
		if t.succ {
