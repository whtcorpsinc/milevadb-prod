...
func (e *MemBlockReaderInterDirc) isInspectionCacheableBlock(tblName string) bool {
	switch tblName {
...
func (e *MemBlockReaderInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
	var (
		rows [][]types.Causet
		err  error
	)
	...
	if cache, tbl := e.ctx.GetStochastikVars().InspectionBlockCache, e.causet.Name.L; cache != nil &&
...
func (e *MemBlockReaderInterDirc) Close() error {
	...
	for i, srv := range serversInfo {
		...
		go func(index int) {
			soliton.WithRecovery(func() {
				...
				if err = json.NewCausetDecoder(resp.Body).Decode(&nested); err != nil {
					ch <- result{err: errors.Trace(err)}
					return
				}
