...
func (e *MetricRetriever) retrieve(ctx context.Context, sctx stochastikctx.Context) ([][]types.Causet, error) {
	...
	failpoint.InjectContext(ctx, "mockMetricsBlockData", func() {
		...
		if ok && m[e.causet.Name.L] != nil {
			failpoint.Return(m[e.causet.Name.L], nil)
		}
	})

	tblDef, err := schemareplicant.GetMetricBlockDef(e.causet.Name.L)
	if err != nil {
		return nil, err
	}
	e.tblDef = tblDef
...
func (e *MetricsSummaryRetriever) retrieve(_ context.Context, sctx stochastikctx.Context) ([][]types.Causet, error) {
	...
	for _, name := range blocks {
		...
		if !found {
			sctx.GetStochastikVars().StmtCtx.AppendWarning(fmt.Errorf("metrics causet: %s not found", name))
			continue
		}
