...
// SplitIndexRegionInterDirc represents a split index regions interlock.
type SplitIndexRegionInterDirc struct {
...
func (e *SplitIndexRegionInterDirc) Open(ctx context.Context) (err error) {
	e.splitIdxKeys, err = e.getSplitIdxKeys()
	return err
}

// Next implements the InterlockingDirectorate Next interface.
func (e *SplitIndexRegionInterDirc) Next(ctx context.Context, chk *chunk.Chunk) error {
	chk.Reset()
	if e.done {
		return nil
	}
...
func (e *SplitIndexRegionInterDirc) splitIndexRegion(ctx context.Context) error {
	...
	if err != nil {
		logutil.BgLogger().Warn("split causet index region failed",
			zap.String("causet", e.blockInfo.Name.L),
...
func (e *SplitIndexRegionInterDirc) getSplitIdxKeys() ([][]byte, error) {
...
func (e *SplitIndexRegionInterDirc) getSplitIdxKeysFromValueList() (keys [][]byte, err error) {
	...
	// Split for all causet partitions.
	...
	// Split for specified causet partitions.
...
func (e *SplitIndexRegionInterDirc) getSplitIdxPhysicalKeysFromValueList(physicalID int64, keys [][]byte) ([][]byte, error) {
...
func (e *SplitIndexRegionInterDirc) getSplitIdxPhysicalStartAndOtherIdxKeys(physicalID int64, keys [][]byte) [][]byte {
...
func (e *SplitIndexRegionInterDirc) getSplitIdxKeysFromBound() (keys [][]byte, err error) {
	pi := e.blockInfo.GetPartitionInfo()
	if pi == nil {
...
func (e *SplitIndexRegionInterDirc) getSplitIdxPhysicalKeysFromBound(physicalID int64, keys [][]byte) ([][]byte, error) {
	keys = e.getSplitIdxPhysicalStartAndOtherIdxKeys(physicalID, keys)
	index := blocks.NewIndex(physicalID, e.blockInfo, e.indexInfo)
...
// SplitBlockRegionInterDirc represents a split causet regions interlock.
type SplitBlockRegionInterDirc struct {
	baseInterlockingDirectorate
...
// Open implements the InterlockingDirectorate Open interface.
func (e *SplitBlockRegionInterDirc) Open(ctx context.Context) (err error) {
	e.splitKeys, err = e.getSplitBlockKeys()
	return err
}

// Next implements the InterlockingDirectorate Next interface.
func (e *SplitBlockRegionInterDirc) Next(ctx context.Context, chk *chunk.Chunk) error {
	chk.Reset()
	if e.done {
		return nil
	}
...
func (e *SplitBlockRegionInterDirc) splitBlockRegion(ctx context.Context) error {
	causetstore := e.ctx.GetStore()
	s, ok := causetstore.(ekv.SplitblockStore)
	if !ok {
		return nil
	...
	if err != nil {
		logutil.BgLogger().Warn("split causet region failed",
			zap.String("causet", e.blockInfo.Name.L),
			zap.Error(err))
	}
	e.splitRegions = len(regionIDs)
...
func (e *SplitBlockRegionInterDirc) getSplitBlockKeys() ([][]byte, error) {
	if len(e.valueLists) > 0 {
...
func (e *SplitBlockRegionInterDirc) getSplitBlockKeysFromValueList() ([][]byte, error) {
	var keys [][]byte
	pi := e.blockInfo.GetPartitionInfo()
	if pi == nil {
...
func (e *SplitBlockRegionInterDirc) getSplitBlockPhysicalKeysFromValueList(physicalID int64, keys [][]byte) ([][]byte, error) {
...
func (e *SplitBlockRegionInterDirc) getSplitBlockKeysFromBound() ([][]byte, error) {
	var keys [][]byte
	pi := e.blockInfo.GetPartitionInfo()
	if pi == nil {
		keys = make([][]byte, 0, e.num)
...
func (e *SplitBlockRegionInterDirc) calculateIntBoundValue() (lowerValue int64, step int64, err error) {
	isUnsigned := false
	...
	if isUnsigned {
		...
		if upperRecordID <= lowerRecordID {
			return 0, 0, errors.Errorf("Split causet `%s` region lower value %v should less than the upper value %v", e.blockInfo.Name, lowerRecordID, upperRecordID)
		}
	...
	if step < minRegionStepValue {
		return 0, 0, errors.Errorf("Split causet `%s` region step value should more than %v, step %v is invalid", e.blockInfo.Name, minRegionStepValue, step)
	}
	return lowerValue, step, nil
}

func (e *SplitBlockRegionInterDirc) getSplitBlockPhysicalKeysFromBound(physicalID int64, keys [][]byte) ([][]byte, error) {
	recordPrefix := blockcodec.GenBlockRecordPrefix(physicalID)
	...
	if lowerHandle.Compare(upperHandle) >= 0 {
		...
		upperStr, err2 := datumSliceToString(e.upper)
		if err1 != nil || err2 != nil {
			return nil, errors.Errorf("Split causet `%v` region lower value %v should less than the upper value %v",
...
type regionKeyCausetDecoder struct {
...
func (d *regionKeyCausetDecoder) decodeRegionKey(key []byte) string {
