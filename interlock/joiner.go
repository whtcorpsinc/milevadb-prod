...
type joiner interface {
	...
	// It's used when the join is an outer join and the hash causet is built
	...
	//   1. it can not pass the filter on the outer causet side.
...
// makeShallowJoinEvent shallow copies `inner` and `outer` into `shallowEvent`.
...
func (j *baseJoiner) makeShallowJoinEvent(isRightJoin bool, inner, outer chunk.Event) {
	...
	j.shallowEvent.ShallowCopyPartialEvent(0, inner)
	j.shallowEvent.ShallowCopyPartialEvent(inner.Len(), outer)
}
...
func (j *baseJoiner) filter(
	...
	var err error
	j.selected, err = memex.VectorizedFilter(j.ctx, j.conditions, chunk.NewIterator4Chunk(input), j.selected)
	if err != nil {
		return false, err
	}
	...
	return chunk.CopySelectedJoinEventsWithSameOuterEvents(input, innerDefCausOffset, innerDefCausLen, outerDefCausOffset, outerDefCausLen, j.selected, output)
}
...
func (j *baseJoiner) filterAndCheckOuterEventStatus(
	...
	var err error
	j.selected, j.isNull, err = memex.VectorizedFilterConsiderNull(j.ctx, j.conditions, chunk.NewIterator4Chunk(input), j.selected, j.isNull)
	if err != nil {
		return nil, err
	}
	...
	// Batch copies selected rows to output chunk.
	_, err = chunk.CopySelectedJoinEventsDirect(input, j.selected, output)
...
func (j *baseJoiner) Clone() baseJoiner {
	base := baseJoiner{
		ctx:          j.ctx,
		conditions:   make([]memex.Expression, 0, len(j.conditions)),
...
func (j *semiJoiner) tryToMatchInners(outer chunk.Event, inners chunk.Iterator, chk *chunk.Chunk) (matched bool, hasNull bool, err error) {
	...
	for inner := inners.Current(); inner != inners.End(); inner = inners.Next() {
		...
		matched, _, err = memex.EvalBool(j.ctx, j.conditions, j.shallowEvent.ToEvent())
		if err != nil {
...
func (j *antiSemiJoiner) tryToMatchInners(outer chunk.Event, inners chunk.Iterator, chk *chunk.Chunk) (matched bool, hasNull bool, err error) {
	...
	for inner := inners.Current(); inner != inners.End(); inner = inners.Next() {
		...
		matched, isNull, err := memex.EvalBool(j.ctx, j.conditions, j.shallowEvent.ToEvent())
		if err != nil {
			return false, false, err
		}
		if matched {
