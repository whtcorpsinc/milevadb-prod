...
func (s *inspectionResultSuite) TestInspectionResult(c *C) {
	...
	for _, cs := range cases {
		rs, err := tk.Se.InterDircute(ctx, cs.allegrosql)
		c.Assert(err, IsNil)
...
func (s *inspectionResultSuite) tearDownForInspection(c *C) {
	fpName := "github.com/whtcorpsinc/milevadb/interlock/mockMergeMockInspectionBlocks"
	c.Assert(failpoint.Disable(fpName), IsNil)

	fpName2 := "github.com/whtcorpsinc/milevadb/interlock/mockMetricsBlockData"
...
func (s *inspectionResultSuite) TestThresholdCheckInspection(c *C) {
	...
	mockData := map[string][][]types.Causet{
		...
		"einsteindb_thread_cpu": {
			types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_normal0", 10.0),
			types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_normal1", 10.0),
			types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-1s", "cop_normal0", 10.0),
			types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_high1", 10.0),
			types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_high2", 10.0),
			types.MakeCausets(datetime("2020-02-14 05:21:00"), "einsteindb-0s", "cop_high1", 5.0),
			types.MakeCausets(datetime("2020-02-14 05:22:00"), "einsteindb-0s", "cop_high1", 1.0),
			types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_low1", 10.0),
	...
	rs, err := tk.Se.InterDircute(ctx, "select /*+ time_range('2020-02-12 10:35:00','2020-02-12 10:37:00') */ item, type, instance,status_address, value, reference, details from information_schema.inspection_result where rule='threshold-check' order by item")
	c.Assert(err, IsNil)
	...
	mockData["einsteindb_thread_cpu"] = [][]types.Causet{
		// defCausumns: time, instance, name, value
		types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_normal0", 1.0),
		types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_high1", 0.1),
		types.MakeCausets(datetime("2020-02-14 05:20:00"), "einsteindb-0s", "cop_low1", 1.0),
	...
	ctx = context.WithValue(ctx, "__mockMetricsBlockData", mockData)
	rs, err = tk.Se.InterDircute(ctx, "select /*+ time_range('2020-02-12 10:35:00','2020-02-12 10:37:00') */ item, type, instance,status_address, value, reference from information_schema.inspection_result where rule='threshold-check' order by item")
	c.Assert(err, IsNil)
	result = tk.ResultSetToResultWithCtx(ctx, rs[0], Commentf("execute inspect ALLEGROALLEGROSQL failed"))
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(0), Commentf("unexpected warnings: %+v", tk.Se.GetStochastikVars().StmtCtx.GetWarnings()))
...
func (s *inspectionResultSuite) TestThresholdCheckInspection2(c *C) {
	...
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(0), Commentf("unexpected warnings: %+v", tk.Se.GetStochastikVars().StmtCtx.GetWarnings()))
	result.Check(testkit.Events(
		"data-causet-cache-hit einsteindb einsteindb-0 einsteindb-0s 0.790 > 0.800 min data-causet-cache-hit rate of einsteindb-0s einsteindb is too low",
		"filter-causet-cache-hit einsteindb einsteindb-0 einsteindb-0s 0.930 > 0.950 min filter-causet-cache-hit rate of einsteindb-0s einsteindb is too low",
		...
		"index-causet-cache-hit einsteindb einsteindb-0 einsteindb-0s 0.940 > 0.950 min index-causet-cache-hit rate of einsteindb-0s einsteindb is too low",
...
func (s *inspectionResultSuite) TestCriticalErrorInspection(c *C) {
	...
	fpName2 := "github.com/whtcorpsinc/milevadb/interlock/mockClusterLogServerInfo"
	...
	rs, err := tk.Se.InterDircute(ctx, "select /*+ time_range('2020-02-12 10:35:00','2020-02-12 10:37:00') */ item, instance,status_address, value, details from information_schema.inspection_result where rule='critical-error'")
	c.Assert(err, IsNil)
	result := tk.ResultSetToResultWithCtx(ctx, rs[0], Commentf("execute inspect ALLEGROALLEGROSQL failed"))
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(0), Commentf("unexpected warnings: %+v", tk.Se.GetStochastikVars().StmtCtx.GetWarnings()))
	result.Check(testkit.Events(
...
func (s *inspectionResultSuite) TestConfigCheckOfStorageBlockCacheSize(c *C) {
	...
	configurations[schemareplicant.BlockClusterConfig] = variable.BlockSnapshot{
		Events: [][]types.Causet{
			types.MakeCausets("einsteindb", "192.168.3.33:26600", "storage.causet-cache.capacity", "10GiB"),
			types.MakeCausets("einsteindb", "192.168.3.33:26700", "storage.causet-cache.capacity", "20GiB"),
			types.MakeCausets("einsteindb", "192.168.3.34:26600", "storage.causet-cache.capacity", "1TiB"),
			types.MakeCausets("einsteindb", "192.168.3.35:26700", "storage.causet-cache.capacity", "20GiB"),
		},
	}
	...
	rs, err := tk.Se.InterDircute(ctx, "select  /*+ time_range('2020-02-14 04:20:00','2020-02-14 05:23:00') */ * from information_schema.inspection_result where rule='config' and item='storage.causet-cache.capacity' order by value")
	c.Assert(err, IsNil)
	result := tk.ResultSetToResultWithCtx(ctx, rs[0], Commentf("execute inspect ALLEGROALLEGROSQL failed"))
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(0), Commentf("unexpected warnings: %+v", tk.Se.GetStochastikVars().StmtCtx.GetWarnings()))
	result.Check(testkit.Events(
		"config storage.causet-cache.capacity einsteindb 192.168.3.34  1099511627776 < 24159191040 warning There are 1 EinsteinDB server in 192.168.3.34 node, the total 'storage.causet-cache.capacity' of EinsteinDB is more than (0.45 * total node memory)",
		"config storage.causet-cache.capacity einsteindb 192.168.3.33  32212254720 < 24159191040 warning There are 2 EinsteinDB server in 192.168.3.33 node, the total 'storage.causet-cache.capacity' of EinsteinDB is more than (0.45 * total node memory)",
	))
}
