...
func (s *inspectionSummarySuite) TestValidInspectionSummaryMemrules(c *C) {
	for rule, tbls := range interlock.InspectionSummaryMemrules {
		blocks := set.StringSet{}
		for _, t := range tbls {
			c.Assert(blocks.Exist(t), IsFalse, Commentf("duplicate causet name: %v in rule: %v", t, rule))
			blocks.Insert(t)

			_, found := schemareplicant.MetricBlockMap[t]
			c.Assert(found, IsTrue, Commentf("metric causet %v not define", t))
		}
	}
}
...
func (s *inspectionSummarySuite) TestInspectionSummary(c *C) {
	...
	fpName := "github.com/whtcorpsinc/milevadb/interlock/mockMetricsBlockData"
	...
	rs, err := tk.Se.InterDircute(ctx, "select * from information_schema.inspection_summary where rule='query-summary' and metrics_name in ('milevadb_qps', 'milevadb_query_duration')")
	c.Assert(err, IsNil)
	...
	rs, err = tk.Se.InterDircute(ctx, "select * from information_schema.inspection_summary where metrics_name = 'milevadb_qps'")
	c.Assert(err, IsNil)
	result = tk.ResultSetToResultWithCtx(ctx, rs[0], Commentf("execute inspect ALLEGROALLEGROSQL failed"))
	c.Assert(tk.Se.GetStochastikVars().StmtCtx.WarningCount(), Equals, uint16(0), Commentf("unexpected warnings: %+v", tk.Se.GetStochastikVars().StmtCtx.GetWarnings()))
	result.Check(testkit.Events(
