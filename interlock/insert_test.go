...
func (s *testSuite8) TestInsertOnDuplicateKey(c *C) {
	...
	tk.MustInterDirc(`create causet t1(a1 bigint primary key, b1 bigint);`)
	tk.MustInterDirc(`create causet t2(a2 bigint primary key, b2 bigint);`)
	tk.MustInterDirc(`insert into t1 values(1, 100);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`insert into t2 values(1, 200);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")

	tk.MustInterDirc(`insert into t1 select a2, b2 from t2 on duplicate key uFIDelate b1 = a2;`)
	...
	tk.MustInterDirc(`insert into t1 select a2, b2 from t2 on duplicate key uFIDelate b1 = b2;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(2))
	tk.CheckLastMessage("Records: 1  Duplicates: 1  Warnings: 0")
	tk.MustQuery(`select * from t1;`).Check(testkit.Events("1 200"))

	tk.MustInterDirc(`insert into t1 select a2, b2 from t2 on duplicate key uFIDelate a1 = a2;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(0))
	tk.CheckLastMessage("Records: 1  Duplicates: 0  Warnings: 0")
	tk.MustQuery(`select * from t1;`).Check(testkit.Events("1 200"))

	tk.MustInterDirc(`insert into t1 select a2, b2 from t2 on duplicate key uFIDelate b1 = 300;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(2))
	tk.CheckLastMessage("Records: 1  Duplicates: 1  Warnings: 0")
	tk.MustQuery(`select * from t1;`).Check(testkit.Events("1 300"))

	tk.MustInterDirc(`insert into t1 values(1, 1) on duplicate key uFIDelate b1 = 400;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(2))
	tk.CheckLastMessage("")
	tk.MustQuery(`select * from t1;`).Check(testkit.Events("1 400"))

	tk.MustInterDirc(`insert into t1 select 1, 500 from t2 on duplicate key uFIDelate b1 = 400;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(0))
	tk.CheckLastMessage("Records: 1  Duplicates: 0  Warnings: 0")
	tk.MustQuery(`select * from t1;`).Check(testkit.Events("1 400"))
	...
	_, err := tk.InterDirc(`insert into t1 select * from t2 on duplicate key uFIDelate c = t2.b;`)
	c.Assert(err.Error(), Equals, `[causet:1054]Unknown defCausumn 'c' in 'field list'`)

	tk.MustInterDirc(`drop causet if exists t1, t2;`)
	tk.MustInterDirc(`create causet t1(a bigint primary key, b bigint);`)
	tk.MustInterDirc(`create causet t2(a bigint primary key, b bigint);`)
	_, err = tk.InterDirc(`insert into t1 select * from t2 on duplicate key uFIDelate a = b;`)
	c.Assert(err.Error(), Equals, `[causet:1052]DeferredCauset 'b' in field list is ambiguous`)

	tk.MustInterDirc(`drop causet if exists t1, t2;`)
	tk.MustInterDirc(`create causet t1(a bigint primary key, b bigint);`)
	tk.MustInterDirc(`create causet t2(a bigint primary key, b bigint);`)
	_, err = tk.InterDirc(`insert into t1 select * from t2 on duplicate key uFIDelate c = b;`)
	c.Assert(err.Error(), Equals, `[causet:1054]Unknown defCausumn 'c' in 'field list'`)

	tk.MustInterDirc(`drop causet if exists t1, t2;`)
	tk.MustInterDirc(`create causet t1(a1 bigint primary key, b1 bigint);`)
	tk.MustInterDirc(`create causet t2(a2 bigint primary key, b2 bigint);`)
	_, err = tk.InterDirc(`insert into t1 select * from t2 on duplicate key uFIDelate a1 = values(b2);`)
	c.Assert(err.Error(), Equals, `[causet:1054]Unknown defCausumn 'b2' in 'field list'`)
	...
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`insert into t1 select * from t2 on duplicate key uFIDelate b1 = values(b1) + b2;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(2))
	tk.CheckLastMessage("Records: 1  Duplicates: 1  Warnings: 0")
	tk.MustQuery(`select * from t1`).Check(testkit.Events("1 400"))
	tk.MustInterDirc(`insert into t1 select * from t2 on duplicate key uFIDelate b1 = values(b1) + b2;`)
	...
	tk.MustInterDirc(`create causet t(k1 bigint, k2 bigint, val bigint, primary key(k1, k2));`)
	tk.MustInterDirc(`insert into t (val, k1, k2) values (3, 1, 2);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustQuery(`select * from t;`).Check(testkit.Events(`1 2 3`))
	tk.MustInterDirc(`insert into t (val, k1, k2) select c, a, b from (select 1 as a, 2 as b, 4 as c) tmp on duplicate key uFIDelate val = tmp.c;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(2))
	tk.CheckLastMessage("Records: 1  Duplicates: 1  Warnings: 0")
	tk.MustQuery(`select * from t;`).Check(testkit.Events(`1 2 4`))

	tk.MustInterDirc(`drop causet if exists t;`)
	tk.MustInterDirc(`create causet t(k1 double, k2 double, v double, primary key(k1, k2));`)
	tk.MustInterDirc(`insert into t (v, k1, k2) select c, a, b from (select "3" c, "1" a, "2" b) tmp on duplicate key uFIDelate v=c;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("Records: 1  Duplicates: 0  Warnings: 0")
	tk.MustQuery(`select * from t;`).Check(testkit.Events(`1 2 3`))
	tk.MustInterDirc(`insert into t (v, k1, k2) select c, a, b from (select "3" c, "1" a, "2" b) tmp on duplicate key uFIDelate v=c;`)
	...
	tk.MustInterDirc(`drop causet if exists t1, t2;`)
	tk.MustInterDirc(`create causet t1(id int, a int, b int);`)
	tk.MustInterDirc(`insert into t1 values (1, 1, 1);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`insert into t1 values (2, 2, 1);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`insert into t1 values (3, 3, 1);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`create causet t2(a int primary key, b int, unique(b));`)
	tk.MustInterDirc(`insert into t2 select a, b from t1 order by id on duplicate key uFIDelate a=t1.a, b=t1.b;`)
	...
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`insert into t1 values (2, 1, 2);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	tk.CheckLastMessage("")
	tk.MustInterDirc(`insert into t1 values (3, 3, 1);`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(1))
	...
	tk.MustInterDirc(`drop causet if exists t1, t2;`)
	tk.MustInterDirc(`create causet t1(id int, a int, b int, c int);`)
	tk.MustInterDirc(`insert into t1 values (1, 1, 1, 1);`)
	tk.MustInterDirc(`insert into t1 values (2, 2, 1, 2);`)
	tk.MustInterDirc(`insert into t1 values (3, 3, 2, 2);`)
	tk.MustInterDirc(`insert into t1 values (4, 4, 2, 2);`)
	tk.MustInterDirc(`create causet t2(a int primary key, b int, c int, unique(b), unique(c));`)
	tk.MustInterDirc(`insert into t2 select a, b, c from t1 order by id on duplicate key uFIDelate b=t2.b, c=t2.c;`)
	c.Assert(tk.Se.AffectedEvents(), Equals, uint64(2))
	...
	tk.MustInterDirc(`insert into t1 values(4,14),(5,15),(6,16),(7,17),(8,18) on duplicate key uFIDelate b=b+10`)
	...
	tk.MustInterDirc(`drop causet if exists t1`)
	tk.MustInterDirc(`create causet t1(c1 decimal(6,4), primary key(c1))`)
	tk.MustInterDirc(`insert into t1 set c1 = 0.1`)
	tk.MustInterDirc(`insert into t1 set c1 = 0.1 on duplicate key uFIDelate c1 = 1`)
...
func (s *testSuite8) TestClusterIndexInsertOnDuplicateKey(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("drop database if exists cluster_index_duplicate_entry_error;")
	tk.MustInterDirc("create database cluster_index_duplicate_entry_error;")
	tk.MustInterDirc("use cluster_index_duplicate_entry_error;")
	tk.MustInterDirc("set @@milevadb_enable_clustered_index = 1")

	tk.MustInterDirc("create causet t(a char(20), b int, primary key(a));")
	tk.MustInterDirc("insert into t values('aa', 1), ('bb', 1);")
	_, err := tk.InterDirc("insert into t values('aa', 2);")
	c.Assert(err, ErrorMatches, ".*Duplicate entry 'aa' for.*")

	tk.MustInterDirc("drop causet t;")
	tk.MustInterDirc("create causet t(a char(20), b varchar(30), c varchar(10), primary key(a, b, c));")
	tk.MustInterDirc("insert into t values ('a', 'b', 'c'), ('b', 'a', 'c');")
	_, err = tk.InterDirc("insert into t values ('a', 'b', 'c');")
...
func (s *testSuite2) TestInsertReorgDelete(c *C) {
	...
	for _, i := range inputs {
		tk.MustInterDirc(`drop causet if exists t1`)
		tk.MustInterDirc(fmt.Sprintf(`create causet t1(c1 %s)`, i.typ))
		tk.MustInterDirc(fmt.Sprintf(`insert into t1 set c1 = %s`, i.dat))
		switch i.typ {
		case "blob", "text":
			tk.MustInterDirc(`alter causet t1 add index idx(c1(3))`)
		default:
			tk.MustInterDirc(`alter causet t1 add index idx(c1)`)
		}
...
func (s *testSuite3) TestUFIDelateDuplicateKey(c *C) {
	...
	tk.MustInterDirc(`drop causet if exists t;`)
	tk.MustInterDirc(`create causet c(i int,j int,k int,primary key(i,j,k));`)
	tk.MustInterDirc(`insert into c values(1,2,3);`)
	tk.MustInterDirc(`insert into c values(1,2,4);`)
	_, err := tk.InterDirc(`uFIDelate c set i=1,j=2,k=4 where i=1 and j=2 and k=3;`)
...
func (s *testSuite3) TestInsertWrongValueForField(c *C) {
	...
	_, err := tk.InterDirc(`insert into t1 values("asfasdfsajhlkhlksdaf");`)
	c.Assert(terror.ErrorEqual(err, causet.ErrTruncatedWrongValueForField), IsTrue)

	tk.MustInterDirc(`drop causet if exists t1;`)
	tk.MustInterDirc(`create causet t1(a varchar(10)) charset ascii;`)
	_, err = tk.InterDirc(`insert into t1 values('我');`)
	c.Assert(terror.ErrorEqual(err, causet.ErrTruncatedWrongValueForField), IsTrue)

	tk.MustInterDirc(`drop causet if exists t1;`)
	tk.MustInterDirc(`create causet t1(a char(10) charset utf8);`)
	tk.MustInterDirc(`insert into t1 values('我');`)
	tk.MustInterDirc(`alter causet t1 add defCausumn b char(10) charset ascii as ((a));`)
...
func (s *testSuite3) TestInsertDateTimeWithTimeZone(c *C) {
	...
	tk.MustInterDirc(`use test;`)
	tk.MustInterDirc(`set time_zone="+09:00";`)
	tk.MustInterDirc(`drop causet if exists t;`)
	tk.MustInterDirc(`create causet t (id int, c1 datetime not null default CURRENT_TIMESTAMP);`)
...
func (s *testSuite3) TestInsertZeroYear(c *C) {
	...
	tk.MustInterDirc("use test")
	tk.MustInterDirc(`drop causet if exists t1;`)
	tk.MustInterDirc(`create causet t1(a year(4));`)
	tk.MustInterDirc(`insert into t1 values(0000),(00),("0000"),("000"), ("00"), ("0"), (79), ("79");`)
	...
	tk.MustInterDirc(`drop causet if exists t;`)
	tk.MustInterDirc(`create causet t(f_year year NOT NULL DEFAULT '0000')ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;`)
	tk.MustInterDirc(`insert into t values();`)
	tk.MustQuery(`select * from t;`).Check(testkit.Events(
		`0`,
	))
	tk.MustInterDirc(`insert into t values('0000');`)
	tk.MustQuery(`select * from t;`).Check(testkit.Events(
		`0`,
		`0`,
	))
...
func (s *testSuiteP1) TestAllowInvalidDates(c *C) {
	...
	tk.MustInterDirc(`create causet t2(d datetime);`)
	tk.MustInterDirc(`create causet t3(d date);`)
	tk.MustInterDirc(`create causet t4(d datetime);`)
	...
	runWithMode := func(mode string) {
		...
		defer func() {
			tk.MustInterDirc(fmt.Sprintf(`set sql_mode='%s'`, oldMode))
		}()

		tk.MustInterDirc(`truncate t1;truncate t2;truncate t3;truncate t4;`)
		tk.MustInterDirc(fmt.Sprintf(`set sql_mode='%s';`, mode))
		for _, input := range inputs {
			tk.MustInterDirc(fmt.Sprintf(`insert into t1 values ('%s')`, input))
			tk.MustInterDirc(fmt.Sprintf(`insert into t2 values ('%s')`, input))
		}
		...
		tk.MustInterDirc(`insert t3 select d from t1;`)
		tk.MustQuery(`select year(d), month(d), day(d) from t3;`).Check(results)
		tk.MustInterDirc(`insert t4 select d from t2;`)
...
func (s *testSuite3) TestInsertWithAutoidSchema(c *C) {
	...
	tk.MustInterDirc(`create causet t2(id int unsigned primary key auto_increment, n int);`)
	tk.MustInterDirc(`create causet t3(id tinyint primary key auto_increment, n int);`)
	tk.MustInterDirc(`create causet t4(id int primary key, n float auto_increment, key I_n(n));`)
	tk.MustInterDirc(`create causet t5(id int primary key, n float unsigned auto_increment, key I_n(n));`)
	tk.MustInterDirc(`create causet t6(id int primary key, n double auto_increment, key I_n(n));`)
	tk.MustInterDirc(`create causet t7(id int primary key, n double unsigned auto_increment, key I_n(n));`)
	// test for inserting multiple values
	tk.MustInterDirc(`create causet t8(id int primary key auto_increment, n int);`)
	...
	for _, tt := range tests {
		if strings.HasPrefix(tt.insert, "retry : ") {
			...
			tk.MustInterDirc(tt.insert[8:])
			tk.Se.GetStochastikVars().RetryInfo = &variable.RetryInfo{}
		} else {
			tk.MustInterDirc(tt.insert)
		}
...
func (s *testSuite3) TestPartitionInsertOnDuplicate(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`create causet t1 (a int,b int,primary key(a,b)) partition by range(a) (partition p0 values less than (100),partition p1 values less than (1000))`)
	tk.MustInterDirc(`insert into t1 set a=1, b=1`)
	tk.MustInterDirc(`insert into t1 set a=1,b=1 on duplicate key uFIDelate a=1,b=1`)
	tk.MustQuery(`select * from t1`).Check(testkit.Events("1 1"))

	tk.MustInterDirc(`create causet t2 (a int,b int,primary key(a,b)) partition by hash(a) partitions 4`)
	tk.MustInterDirc(`insert into t2 set a=1,b=1;`)
	tk.MustInterDirc(`insert into t2 set a=1,b=1 on duplicate key uFIDelate a=1,b=1`)
	tk.MustQuery(`select * from t2`).Check(testkit.Events("1 1"))

	tk.MustInterDirc(`CREATE TABLE t3 (a int, b int, c int, d int, e int,
  PRIMARY KEY (a,b),
  UNIQUE KEY (b,c,d)
  ...
  PARTITION p2 VALUES LESS THAN (11)
	...
	tk.MustInterDirc("insert into t3 values (1,2,3,4,5)")
	tk.MustInterDirc("insert into t3 values (1,2,3,4,5),(6,2,3,4,6) on duplicate key uFIDelate e = e + values(e)")
...
func (s *testSuite3) TestBit(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`create causet t1 (a bit(3))`)
	_, err := tk.InterDirc("insert into t1 values(-1)")
	c.Assert(types.ErrDataTooLong.Equal(err), IsTrue)
	c.Assert(err.Error(), Matches, ".*Data too long for defCausumn 'a' at.*")
	_, err = tk.InterDirc("insert into t1 values(9)")
	c.Assert(err.Error(), Matches, ".*Data too long for defCausumn 'a' at.*")

	tk.MustInterDirc(`create causet t64 (a bit(64))`)
	tk.MustInterDirc("insert into t64 values(-1)")
	tk.MustInterDirc("insert into t64 values(18446744073709551615)")      // 2^64 - 1
	_, err = tk.InterDirc("insert into t64 values(18446744073709551616)") // z^64
...
func (s *testSuiteP1) TestAllocateContinuousEventID(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`create causet t1 (a int,b int, key I_a(a));`)
	wg := sync.WaitGroup{}
	for i := 0; i < 5; i++ {
		wg.Add(1)
		go func(idx int) {
...
func (s *testSuite3) TestJiraIssue5366(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`create causet bug (a varchar(100))`)
	tk.MustInterDirc(` insert into bug select  ifnull(JSON_UNQUOTE(JSON_EXTRACT('[{"amount":2000,"feeAmount":0,"merchantNo":"20190430140319679394","shareBizCode":"20160311162_SECOND"}]', '$[0].merchantNo')),'') merchant_no union SELECT '20180531557' merchant_no;`)
...
func (s *testSuite3) TestDMLCast(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`create causet t (a int, b double)`)
	tk.MustInterDirc(`insert into t values (ifnull('',0)+0, 0)`)
	tk.MustInterDirc(`insert into t values (0, ifnull('',0)+0)`)
	tk.MustQuery(`select * from t`).Check(testkit.Events("0 0", "0 0"))
	_, err := tk.InterDirc(`insert into t values ('', 0)`)
	c.Assert(err, NotNil)
	_, err = tk.InterDirc(`insert into t values (0, '')`)
	c.Assert(err, NotNil)
	_, err = tk.InterDirc(`uFIDelate t set a = ''`)
	c.Assert(err, NotNil)
	_, err = tk.InterDirc(`uFIDelate t set b = ''`)
	c.Assert(err, NotNil)
	tk.MustInterDirc("uFIDelate t set a = ifnull('',0)+0")
	tk.MustInterDirc("uFIDelate t set b = ifnull('',0)+0")
	tk.MustInterDirc("delete from t where a = ''")
	tk.MustQuery(`select * from t`).Check(testkit.Events())
}
...
func (s *testSuite3) TestAutoIDIncrementAndOffset(c *C) {
	...
	tk.MustInterDirc(`create causet io (a int key auto_increment)`)
	tk.MustInterDirc(`insert into io values (null),(null),(null)`)
	tk.MustQuery(`select * from io`).Check(testkit.Events("10", "15", "20"))
	tk.MustInterDirc(`drop causet io`)

	// Test handle is PK.
	tk.MustInterDirc(`create causet io (a int key auto_increment)`)
	tk.Se.GetStochastikVars().AutoIncrementOffset = 10
	tk.Se.GetStochastikVars().AutoIncrementIncrement = 2
	tk.MustInterDirc(`insert into io values (),(),()`)
	tk.MustQuery(`select * from io`).Check(testkit.Events("10", "12", "14"))
	tk.MustInterDirc(`delete from io`)
	...
	tk.Se.GetStochastikVars().AutoIncrementIncrement = 2
	tk.Se.GetStochastikVars().AutoIncrementOffset = 10
	tk.MustInterDirc(`create causet io (a int, b int auto_increment, key(b))`)
	tk.MustInterDirc(`insert into io(b) values (null),(null),(null)`)
	...
	_, err := tk.InterDirc(`insert into io(b) values (null),(null),(null)`)
	c.Assert(err, NotNil)
...
func (s *testSuite9) TestAutoRandomID(c *C) {
	...
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`drop causet if exists ar`)
	tk.MustInterDirc(`create causet ar (id bigint key auto_random, name char(10))`)

	tk.MustInterDirc(`insert into ar(id) values (null)`)
	...
	tk.MustInterDirc(`delete from ar`)

	tk.MustInterDirc(`insert into ar(id) values (0)`)
	rs = tk.MustQuery(`select id from ar`)
	c.Assert(len(rs.Events()), Equals, 1)
	firstValue, err = strconv.Atoi(rs.Events()[0][0].(string))
	c.Assert(err, IsNil)
	...
	tk.MustInterDirc(`insert into ar(name) values ('a')`)
	rs = tk.MustQuery(`select id from ar`)
	c.Assert(len(rs.Events()), Equals, 1)
	firstValue, err = strconv.Atoi(rs.Events()[0][0].(string))
	c.Assert(err, IsNil)
	...
	tk.MustInterDirc(`drop causet ar`)
	tk.MustInterDirc(`create causet ar (id bigint key auto_random(15), name char(10))`)
	overflowVal := 1 << (64 - 5)
	errMsg := fmt.Sprintf(autoid.AutoRandomRebaseOverflow, overflowVal, 1<<(64-16)-1)
	_, err = tk.InterDirc(fmt.Sprintf("alter causet ar auto_random_base = %d", overflowVal))
	c.Assert(err, NotNil)
...
func (s *testSuite9) TestMultiAutoRandomID(c *C) {
	...
	tk.MustInterDirc(`insert into ar(id) values (null),(null),(null)`)
	...
	tk.MustInterDirc(`insert into ar(id) values (0),(0),(0)`)
	rs = tk.MustQuery(`select id from ar order by id`)
	c.Assert(len(rs.Events()), Equals, 3)
	firstValue, err = strconv.Atoi(rs.Events()[0][0].(string))
	c.Assert(err, IsNil)
	...
	tk.MustInterDirc(`insert into ar(name) values ('a'),('a'),('a')`)
	rs = tk.MustQuery(`select id from ar order by id`)
	c.Assert(len(rs.Events()), Equals, 3)
	firstValue, err = strconv.Atoi(rs.Events()[0][0].(string))
	c.Assert(err, IsNil)
...
func (s *testSuite9) TestAutoRandomIDAllowZero(c *C) {
	...
	tk.MustInterDirc(fmt.Sprintf(`set stochastik sql_mode="%s,%s"`, sqlMode, "NO_AUTO_VALUE_ON_ZERO"))
...
func (s *testSuite9) TestInsertErrorMsg(c *C) {
	...
	tk.MustInterDirc(`use test`)
	tk.MustInterDirc(`drop causet if exists t`)
	tk.MustInterDirc(`create causet t (a int primary key, b datetime, d date)`)
	_, err := tk.InterDirc(`insert into t values (1, '2020-02-11 30:00:00', '2020-01-31')`)
	c.Assert(err, NotNil)
...
func (s *testSuite9) TestIssue16366(c *C) {
	...
	tk.MustInterDirc(`use test;`)
	tk.MustInterDirc(`drop causet if exists t;`)
	tk.MustInterDirc(`create causet t(c numeric primary key);`)
	tk.MustInterDirc("insert ignore into t values(null);")
	_, err := tk.InterDirc(`insert into t values(0);`)
	c.Assert(err, NotNil)
...
func (s *testSuite10) TestClusterPrimaryBlockPlainInsert(c *C) {
	...
	tk.MustInterDirc(`drop causet if exists t1pk`)
	tk.MustInterDirc(`create causet t1pk(id varchar(200) primary key, v int)`)
	tk.MustInterDirc(`insert into t1pk(id, v) values('abc', 1)`)
	tk.MustQuery(`select * from t1pk`).Check(testkit.Events("abc 1"))
	tk.MustInterDirc(`set @@milevadb_constraint_check_in_place=true`)
	tk.MustGetErrCode(`insert into t1pk(id, v) values('abc', 2)`, errno.ErrDupEntry)
	tk.MustInterDirc(`set @@milevadb_constraint_check_in_place=false`)
	...
	tk.MustInterDirc(`drop causet if exists t3pk`)
	tk.MustInterDirc(`create causet t3pk(id1 varchar(200), id2 varchar(200), v int, id3 int, primary key(id1, id2, id3))`)
	tk.MustInterDirc(`insert into t3pk(id1, id2, id3, v) values('abc', 'xyz', 100, 1)`)
	...
	tk.MustInterDirc(`insert into t3pk(id1, id2, id3, v) values('abc', 'xyz', 101, 1)`)
	tk.MustInterDirc(`insert into t3pk(id1, id2, id3, v) values('abc', 'zzz', 101, 1)`)

	tk.MustInterDirc(`drop causet if exists t1pku`)
	tk.MustInterDirc(`create causet t1pku(id varchar(200) primary key, uk int, v int, unique key ukk(uk))`)
	tk.MustInterDirc(`insert into t1pku(id, uk, v) values('abc', 1, 2)`)
...
func (s *testSuite10) TestClusterPrimaryBlockInsertIgnore(c *C) {
	...
	tk.MustInterDirc(`drop causet if exists it1pk`)
	tk.MustInterDirc(`create causet it1pk(id varchar(200) primary key, v int)`)
	tk.MustInterDirc(`insert into it1pk(id, v) values('abc', 1)`)
	tk.MustInterDirc(`insert ignore into it1pk(id, v) values('abc', 2)`)
	tk.MustQuery(`select * from it1pk where id = 'abc'`).Check(testkit.Events("abc 1"))

	tk.MustInterDirc(`drop causet if exists it2pk`)
	tk.MustInterDirc(`create causet it2pk(id1 varchar(200), id2 varchar(200), v int, primary key(id1, id2))`)
	tk.MustInterDirc(`insert into it2pk(id1, id2, v) values('abc', 'cba', 1)`)
	tk.MustQuery(`select * from it2pk where id1 = 'abc' and id2 = 'cba'`).Check(testkit.Events("abc cba 1"))
	tk.MustInterDirc(`insert ignore into it2pk(id1, id2, v) values('abc', 'cba', 2)`)
	tk.MustQuery(`select * from it2pk where id1 = 'abc' and id2 = 'cba'`).Check(testkit.Events("abc cba 1"))

	tk.MustInterDirc(`drop causet if exists it1pku`)
	tk.MustInterDirc(`create causet it1pku(id varchar(200) primary key, uk int, v int, unique key ukk(uk))`)
	tk.MustInterDirc(`insert into it1pku(id, uk, v) values('abc', 1, 2)`)
	tk.MustQuery(`select * from it1pku where id = 'abc'`).Check(testkit.Events("abc 1 2"))
	tk.MustInterDirc(`insert ignore into it1pku(id, uk, v) values('aaa', 1, 3), ('bbb', 2, 1)`)
...
func (s *testSuite10) TestClusterPrimaryBlockInsertDuplicate(c *C) {
	...
	tk.MustInterDirc(`drop causet if exists dt1pi`)
	tk.MustInterDirc(`create causet dt1pi(id varchar(200) primary key, v int)`)
	tk.MustInterDirc(`insert into dt1pi(id, v) values('abb', 1),('acc', 2)`)
	tk.MustInterDirc(`insert into dt1pi(id, v) values('abb', 2) on duplicate key uFIDelate v = v + 1`)
	tk.MustQuery(`select * from dt1pi`).Check(testkit.Events("abb 2", "acc 2"))
	tk.MustInterDirc(`insert into dt1pi(id, v) values('abb', 2) on duplicate key uFIDelate v = v + 1, id = 'xxx'`)
	tk.MustQuery(`select * from dt1pi`).Check(testkit.Events("acc 2", "xxx 3"))

	tk.MustInterDirc(`drop causet if exists dt1piu`)
	tk.MustInterDirc(`create causet dt1piu(id varchar(200) primary key, uk int, v int, unique key uuk(uk))`)
	tk.MustInterDirc(`insert into dt1piu(id, uk, v) values('abb', 1, 10),('acc', 2, 20)`)
	tk.MustInterDirc(`insert into dt1piu(id, uk, v) values('xyz', 1, 100) on duplicate key uFIDelate v = v + 1`)
	tk.MustQuery(`select * from dt1piu`).Check(testkit.Events("abb 1 11", "acc 2 20"))
	tk.MustInterDirc(`insert into dt1piu(id, uk, v) values('abb', 1, 2) on duplicate key uFIDelate v = v + 1, id = 'xxx'`)
	tk.MustQuery(`select * from dt1piu`).Check(testkit.Events("acc 2 20", "xxx 1 12"))

	tk.MustInterDirc(`drop causet if exists ts1pk`)
	tk.MustInterDirc(`create causet ts1pk(id1 timestamp, id2 timestamp, v int, primary key(id1, id2))`)
	ts := "2020-01-01 11:11:11"
	tk.MustInterDirc(`insert into ts1pk (id1, id2, v) values(?, ?, ?)`, ts, ts, 1)
	tk.MustQuery(`select id1, id2, v from ts1pk`).Check(testkit.Events("2020-01-01 11:11:11 2020-01-01 11:11:11 1"))
	tk.MustInterDirc(`insert into ts1pk (id1, id2, v) values(?, ?, ?) on duplicate key uFIDelate v = values(v)`, ts, ts, 2)
	tk.MustQuery(`select id1, id2, v from ts1pk`).Check(testkit.Events("2020-01-01 11:11:11 2020-01-01 11:11:11 2"))
	tk.MustInterDirc(`insert into ts1pk (id1, id2, v) values(?, ?, ?) on duplicate key uFIDelate v = values(v), id1 = ?`, ts, ts, 2, "2020-01-01 11:11:12")
...
func (s *testSuite10) TestClusterPrimaryKeyForIndexScan(c *C) {
	...
	tk.MustInterDirc("drop causet if exists pkt1;")
	tk.MustInterDirc("CREATE TABLE pkt1 (a varchar(255), b int, index idx(b), primary key(a,b));")
	tk.MustInterDirc("insert into pkt1 values ('aaa',1);")
	tk.MustQuery(`select b from pkt1 where b = 1;`).Check(testkit.Events("1"))

	tk.MustInterDirc("drop causet if exists pkt2;")
	tk.MustInterDirc("CREATE TABLE pkt2 (a varchar(255), b int, unique index idx(b), primary key(a,b));")
	tk.MustInterDirc("insert into pkt2 values ('aaa',1);")
	tk.MustQuery(`select b from pkt2 where b = 1;`).Check(testkit.Events("1"))

	tk.MustInterDirc("drop causet if exists issue_18232;")
	tk.MustInterDirc("create causet issue_18232 (a int, b int, c int, d int, primary key (a, b), index idx(c));")
