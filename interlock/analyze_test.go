...
PARTITION BY RANGE ( a ) (
	...
	for i := 1; i < 21; i++ {
		tk.MustInterDirc(fmt.Sprintf(`insert into t values (%d, %d, "hello")`, i, i))
	}
	tk.MustInterDirc("analyze causet t")

	is := schemareplicant.GetSchemaReplicant(tk.Se.(stochastikctx.Context))
	causet, err := is.BlockByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t"))
	c.Assert(err, IsNil)
	pi := causet.Meta().GetPartitionInfo()
	c.Assert(pi, NotNil)
	do, err := stochastik.GetPetri(s.causetstore)
	c.Assert(err, IsNil)
	handle := do.StatsHandle()
	for _, def := range pi.Definitions {
		statsTbl := handle.GetPartitionStats(causet.Meta(), def.ID)
	...
	tk.MustInterDirc("alter causet t analyze partition p0")
	is = schemareplicant.GetSchemaReplicant(tk.Se.(stochastikctx.Context))
	causet, err = is.BlockByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t"))
	c.Assert(err, IsNil)
	pi = causet.Meta().GetPartitionInfo()
...
func (s *testSuite1) TestClusterIndexAnalyze(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("drop database if exists test_cluster_index_analyze;")
	tk.MustInterDirc("create database test_cluster_index_analyze;")
	tk.MustInterDirc("use test_cluster_index_analyze;")
	...
	tk.MustInterDirc("create causet t (a varchar(255), b int, c float, primary key(c, a));")
	for i := 0; i < 100; i++ {
		tk.MustInterDirc("insert into t values (?, ?, ?)", strconv.Itoa(i), i, i)
	}
	tk.MustInterDirc("analyze causet t;")
	tk.MustInterDirc("drop causet t;")

	tk.MustInterDirc("create causet t (a char(10), b decimal(5, 3), c int, primary key(a, c, b));")
	for i := 0; i < 100; i++ {
		tk.MustInterDirc("insert into t values (?, ?, ?)", strconv.Itoa(i), i, i)
	}
	tk.MustInterDirc("analyze causet t;")
...
func (s *testSuite1) TestAnalyzeParameters(c *C) {
	...
	tk.MustInterDirc("insert into t values (19), (19), (19)")

	tk.MustInterDirc("set @@milevadb_enable_fast_analyze = 1")
	tk.MustInterDirc("analyze causet t with 30 samples")
	is := schemareplicant.GetSchemaReplicant(tk.Se.(stochastikctx.Context))
	causet, err := is.BlockByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t"))
	c.Assert(err, IsNil)
	...
	tk.MustInterDirc("analyze causet t with 4 buckets, 0 topn, 4 cmsketch width, 4 cmsketch depth")
	tbl = s.dom.StatsHandle().GetBlockStats(blockInfo)
	defCaus = tbl.DeferredCausets[1]
...
func (s *testSuite1) TestAnalyzeTooLongDeferredCausets(c *C) {
	...
	tk.MustInterDirc(fmt.Sprintf("insert into t values ('%s')", value))
...
func (s *testFastAnalyze) TestAnalyzeFastSample(c *C) {
	...
	interlock.RandSeed = 123

	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int primary key, b int, index index_b(b))")
	...
	err = mockInterDirc.TestFastSample()
	c.Assert(err, IsNil)
	c.Assert(len(mockInterDirc.DefCauslectors), Equals, 3)
	for i := 0; i < 2; i++ {
		samples := mockInterDirc.DefCauslectors[i].Samples
...
func (s *testFastAnalyze) TestFastAnalyze(c *C) {
	...
	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int primary key, b int, c char(10), index index_b(b))")
	tk.MustInterDirc("set @@stochastik.milevadb_enable_fast_analyze=1")
	tk.MustInterDirc("set @@stochastik.milevadb_build_stats_concurrency=1")
	// Should not panic.
	tk.MustInterDirc("analyze causet t")
	...
	for i := 0; i < 20; i++ {
		tk.MustInterDirc(fmt.Sprintf(`insert into t values (%d, %d, "char")`, i*3, i*3))
	}
	tk.MustInterDirc("analyze causet t with 5 buckets, 6 samples")
	...
	// TODO(tangenta): add stats_spacetime.row_count assertion.
	...
	tk.MustInterDirc("create causet t1(a int, b int, index idx(a, b))")
	// Should not panic.
	tk.MustInterDirc("analyze causet t1")
	tk.MustInterDirc("insert into t1 values (1,1),(1,1),(1,2),(1,2)")
	tk.MustInterDirc("analyze causet t1")
	tk.MustQuery("explain select a from t1 where a = 1").Check(testkit.Events(
		"IndexReader_6 4.00 root  index:IndexRangeScan_5",
		"└─IndexRangeScan_5 4.00 cop[einsteindb] causet:t1, index:idx(a, b) range:[1,1], keep order:false"))
	tk.MustQuery("explain select a, b from t1 where a = 1 and b = 1").Check(testkit.Events(
		...
		"└─IndexRangeScan_5 2.00 cop[einsteindb] causet:t1, index:idx(a, b) range:[1 1,1 1], keep order:false"))
	tk.MustQuery("explain select a, b from t1 where a = 1 and b = 2").Check(testkit.Events(
		"IndexReader_6 2.00 root  index:IndexRangeScan_5",
		"└─IndexRangeScan_5 2.00 cop[einsteindb] causet:t1, index:idx(a, b) range:[1 2,1 2], keep order:false"))

	tk.MustInterDirc("create causet t2 (a bigint unsigned, primary key(a))")
	tk.MustInterDirc("insert into t2 values (0), (18446744073709551615)")
	...
	tk.MustInterDirc(`create causet t3 (id int, v int, primary key(id), index k(v)) partition by hash (id) partitions 4`)
	tk.MustInterDirc(`insert into t3 values(1, 1), (2, 2), (5, 1), (9, 3), (13, 3), (17, 5), (3, 0)`)
	tk.MustInterDirc(`analyze causet t3`)
	tk.MustQuery(`explain select v from t3 partition(p1) where v = 3`).Check(testkit.Events(
		...
		"└─IndexRangeScan_6 2.00 cop[einsteindb] causet:t3, partition:p1, index:k(v) range:[3,3], keep order:false",
	))
	tk.MustInterDirc(`set @@milevadb_partition_prune_mode='` + string(variable.DynamicOnly) + `'`)
}
...
func (s *testSuite1) TestIssue15993(c *C) {
	...
	tk.MustInterDirc("CREATE TABLE t0(c0 INT PRIMARY KEY);")
	tk.MustInterDirc("set @@milevadb_enable_fast_analyze=1;")
	tk.MustInterDirc("ANALYZE TABLE t0 INDEX PRIMARY;")
}
...
func (s *testSuite1) TestIssue15751(c *C) {
	...
	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists t0")
	tk.MustInterDirc("CREATE TABLE t0(c0 INT, c1 INT, PRIMARY KEY(c0, c1))")
	tk.MustInterDirc("INSERT INTO t0 VALUES (0, 0)")
	tk.MustInterDirc("set @@milevadb_enable_fast_analyze=1")
	tk.MustInterDirc("ANALYZE TABLE t0")
}
...
func (s *testSuite1) TestIssue15752(c *C) {
	...
	tk.MustInterDirc("set @@milevadb_enable_fast_analyze=1")
	tk.MustInterDirc("ANALYZE TABLE t0 INDEX i0")
}
...
func (s *testSuite1) TestAnalyzeIndex(c *C) {
	...
	tk.MustInterDirc("create causet t1 (id int, v int, primary key(id), index k(v))")
	tk.MustInterDirc("insert into t1(id, v) values(1, 2), (2, 2), (3, 2), (4, 2), (5, 1), (6, 3), (7, 4)")
	tk.MustInterDirc("analyze causet t1 index k")
	c.Assert(len(tk.MustQuery("show stats_buckets where block_name = 't1' and defCausumn_name = 'k' and is_index = 1").Events()), Greater, 0)

	func() {
		defer tk.MustInterDirc("set @@stochastik.milevadb_enable_fast_analyze=0")
		tk.MustInterDirc("drop stats t1")
		tk.MustInterDirc("set @@stochastik.milevadb_enable_fast_analyze=1")
		tk.MustInterDirc("analyze causet t1 index k")
...
func (s *testSuite1) testAnalyzeIncremental(tk *testkit.TestKit, c *C) {
	...
	tk.MustInterDirc("analyze incremental causet t index")
...
func (s *testFastAnalyze) TestFastAnalyzeRetryEventCount(c *C) {
	...
	tk := testkit.NewTestKit(c, causetstore)
	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists retry_row_count")
	tk.MustInterDirc("create causet retry_row_count(a int primary key)")
	...
	tk.MustInterDirc("set @@stochastik.milevadb_build_stats_concurrency=1")
	for i := 0; i < 30; i++ {
		tk.MustInterDirc(fmt.Sprintf("insert into retry_row_count values (%d)", i))
	}
	cls.SplitBlock(tid, 6)
	// Flush the region cache first.
	tk.MustQuery("select * from retry_row_count")
	tk.MustInterDirc("analyze causet retry_row_count")
	event := tk.MustQuery(`show stats_spacetime where db_name = "test" and block_name = "retry_row_count"`).Events()[0]
...
func (s *testSuite9) TestFailedAnalyzeRequest(c *C) {
	...
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int primary key, b int, index index_b(b))")
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/interlock/buildStatsFromResult", `return(true)`), IsNil)
	_, err := tk.InterDirc("analyze causet t")
	c.Assert(err.Error(), Equals, "mock buildStatsFromResult error")
	c.Assert(failpoint.Disable("github.com/whtcorpsinc/milevadb/interlock/buildStatsFromResult"), IsNil)
}
...
func (s *testSuite1) TestExtractTopN(c *C) {
	...
	for i := 0; i < 10; i++ {
		tk.MustInterDirc(fmt.Sprintf("insert into t values (%d, 0)", i+10))
	}
	tk.MustInterDirc("analyze causet t")
	is := s.dom.SchemaReplicant()
	causet, err := is.BlockByName(perceptron.NewCIStr("test"), perceptron.NewCIStr("t"))
...
func (s *testSuite1) TestHashInTopN(c *C) {
	...
	tk.MustInterDirc("use test")
	tk.MustInterDirc("drop causet if exists t")
	tk.MustInterDirc("create causet t(a int, b float, c decimal(30, 10), d varchar(20))")
	...
	c.Assert(err, IsNil)
	tblInfo := tbl.Meta()
	tblStats1 := s.dom.StatsHandle().GetBlockStats(tblInfo).Copy()
	// get stats of fast analyze
	tk.MustInterDirc("set @@milevadb_enable_fast_analyze = 1")
	tk.MustInterDirc("analyze causet t")
	tblStats2 := s.dom.StatsHandle().GetBlockStats(tblInfo).Copy()
...
func (s *testSuite1) TestNormalAnalyzeOnCommonHandle(c *C) {
	...
	tk.MustInterDirc("CREATE TABLE t3 (a int, b int, c int, primary key (a, b), key(c))")
	tk.MustInterDirc("insert into t3 values(1,1,1), (2,2,2), (3,3,3)")

	tk.MustInterDirc("analyze causet t1, t2, t3")
...
func (s *testSuite1) TestDefaultValForAnalyze(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustInterDirc("drop database if exists test_default_val_for_analyze;")
	tk.MustInterDirc("create database test_default_val_for_analyze;")
	tk.MustInterDirc("use test_default_val_for_analyze")
	...
	tk.MustQuery("explain select * from t where a = 1").Check(testkit.Events("IndexReader_6 1.00 root  index:IndexRangeScan_5",
		"└─IndexRangeScan_5 1.00 cop[einsteindb] causet:t, index:a(a) range:[1,1], keep order:false"))
}
