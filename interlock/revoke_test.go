...
func (s *testSuite1) TestRevokeGlobal(c *C) {
	...
	_, err := tk.InterDirc(`REVOKE ALL PRIVILEGES ON *.* FROM 'nonexistuser'@'host'`)
	c.Assert(err, NotNil)
	...
	tk.MustInterDirc(grantPrivALLEGROSQL)
...
func (s *testSuite1) TestRevokeDBScope(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	// Create a new user.
	tk.MustInterDirc(`CREATE USER 'testDBRevoke'@'localhost' IDENTIFIED BY '123';`)
	tk.MustInterDirc(`GRANT ALL ON test.* TO 'testDBRevoke'@'localhost';`)

	_, err := tk.InterDirc(`REVOKE ALL PRIVILEGES ON nonexistdb.* FROM 'testDBRevoke'@'localhost'`)
	c.Assert(err, NotNil)
...
func (s *testSuite1) TestRevokeBlockScope(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	// Create a new user.
	tk.MustInterDirc(`CREATE USER 'testTblRevoke'@'localhost' IDENTIFIED BY '123';`)
	tk.MustInterDirc(`CREATE TABLE test.test1(c1 int);`)
	tk.MustInterDirc(`GRANT ALL PRIVILEGES ON test.test1 TO 'testTblRevoke'@'localhost';`)

	_, err := tk.InterDirc(`REVOKE ALL PRIVILEGES ON test.nonexistblock FROM 'testTblRevoke'@'localhost'`)
	c.Assert(err, NotNil)

	// Make sure all the causet privs for new user is Y.
	...
	// Revoke all causet scope privs.
	tk.MustInterDirc("REVOKE ALL ON test.test1 FROM 'testTblRevoke'@'localhost';")
...
func (s *testSuite1) TestRevokeDeferredCausetScope(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	// Create a new user.
	tk.MustInterDirc(`CREATE USER 'testDefCausRevoke'@'localhost' IDENTIFIED BY '123';`)
	...
	for _, v := range allegrosql.AllDeferredCausetPrivs {
		...
		tk.MustInterDirc(grantALLEGROSQL)
		...
		tk.MustInterDirc(revokeALLEGROSQL)
		tk.MustQuery(checkALLEGROSQL).Check(testkit.Events(""))
	}

	// Create a new user.
	tk.MustInterDirc("CREATE USER 'testDefCaus1Revoke'@'localhost' IDENTIFIED BY '123';")
	tk.MustInterDirc("USE test;")
	// Grant all defCausumn scope privs.
	tk.MustInterDirc("GRANT ALL(c2) ON test3 TO 'testDefCaus1Revoke'@'localhost';")
	...
	tk.MustInterDirc("REVOKE ALL(c2) ON test3 FROM 'testDefCaus1Revoke'@'localhost'")
