...
// SimpleInterDirc represents simple memex interlock.
// For memexs do simple execution.
// includes `UseStmt`, 'SetStmt`, `DoStmt`,
// `BeginStmt`, `CommitStmt`, `RollbackStmt`.
// TODO: list all simple memexs.
type SimpleInterDirc struct {
...
func (e *baseInterlockingDirectorate) getSysStochastik() (stochastikctx.Context, error) {
...
func (e *baseInterlockingDirectorate) releaseSysStochastik(ctx stochastikctx.Context) {
	if ctx == nil {
		return
	}
	dom := petri.GetPetri(e.ctx)
	sysStochastikPool := dom.SysStochastikPool()
	if _, err := ctx.(sqlexec.ALLEGROSQLInterlockingDirectorate).InterDircute(context.Background(), "rollback"); err != nil {
...
func (e *SimpleInterDirc) Next(ctx context.Context, req *chunk.Chunk) (err error) {
	if e.done {
		return nil
	}
...
func (e *SimpleInterDirc) setDefaultRoleNone(s *ast.SetDefaultRoleStmt) error {
	restrictedCtx, err := e.getSysStochastik()
	if err != nil {
		return err
	}
	defer e.releaseSysStochastik(restrictedCtx)
	sqlInterlockingDirectorate := restrictedCtx.(sqlexec.ALLEGROSQLInterlockingDirectorate)
	if _, err := sqlInterlockingDirectorate.InterDircute(context.Background(), "begin"); err != nil {
		return err
	}
	for _, u := range s.UserList {
		...
		if _, err := sqlInterlockingDirectorate.InterDircute(context.Background(), allegrosql); err != nil {
			logutil.BgLogger().Error(fmt.Sprintf("Error occur when executing %s", allegrosql))
			if _, rollbackErr := sqlInterlockingDirectorate.InterDircute(context.Background(), "rollback"); rollbackErr != nil {
				return rollbackErr
			}
			return err
		}
	}
	if _, err := sqlInterlockingDirectorate.InterDircute(context.Background(), "commit"); err != nil {
		return err
	}
	return nil
}

func (e *SimpleInterDirc) setDefaultRoleRegular(s *ast.SetDefaultRoleStmt) error {
...
func (e *SimpleInterDirc) setDefaultRoleAll(s *ast.SetDefaultRoleStmt) error {
	for _, user := range s.UserList {
		exists, err := userExists(e.ctx, user.Username, user.Hostname)
		if err != nil {
			return err
...
func (e *SimpleInterDirc) setDefaultRoleForCurrentUser(s *ast.SetDefaultRoleStmt) (err error) {
	checker := privilege.GetPrivilegeManager(e.ctx)
	...
	if _, err := sqlInterlockingDirectorate.InterDircute(context.Background(), deleteALLEGROSQL); err != nil {
		logutil.BgLogger().Error(fmt.Sprintf("Error occur when executing %s", allegrosql))
		if _, rollbackErr := sqlInterlockingDirectorate.InterDircute(context.Background(), "rollback"); rollbackErr != nil {
			return rollbackErr
		}
...
func (e *SimpleInterDirc) executeSetDefaultRole(s *ast.SetDefaultRoleStmt) (err error) {
...
func (e *SimpleInterDirc) setRoleRegular(s *ast.SetRoleStmt) error {
...
func (e *SimpleInterDirc) setRoleAll(s *ast.SetRoleStmt) error {
...
func (e *SimpleInterDirc) setRoleAllExcept(s *ast.SetRoleStmt) error {
...
func (e *SimpleInterDirc) setRoleDefault(s *ast.SetRoleStmt) error {
...
func (e *SimpleInterDirc) setRoleNone(s *ast.SetRoleStmt) error {
...
func (e *SimpleInterDirc) executeSetRole(s *ast.SetRoleStmt) error {
	switch s.SetRoleOpt {
	case ast.SetRoleRegular:
...
func (e *SimpleInterDirc) dbAccessDenied(dbname string) error {
...
func (e *SimpleInterDirc) executeUse(s *ast.UseStmt) error {
...
func (e *SimpleInterDirc) executeBegin(ctx context.Context, s *ast.BeginStmt) error {
	// If BEGIN is the first memex in TxnCtx, we can reuse the existing transaction, without the
...
func (e *SimpleInterDirc) executeRevokeRole(s *ast.RevokeRoleStmt) error {
	...
	for _, user := range s.Users {
		...
		if !exists {
			if _, err := sqlInterlockingDirectorate.InterDircute(context.Background(), "rollback"); err != nil {
				return errors.Trace(err)
			}
...
func (e *SimpleInterDirc) executeCommit(s *ast.CommitStmt) {
	e.ctx.GetStochastikVars().SetStatusFlag(allegrosql.ServerStatusInTrans, false)
}

func (e *SimpleInterDirc) executeRollback(s *ast.RollbackStmt) error {
	sessVars := e.ctx.GetStochastikVars()
	logutil.BgLogger().Debug("execute rollback memex", zap.Uint64("conn", sessVars.ConnectionID))
...
func (e *SimpleInterDirc) executeCreateUser(ctx context.Context, s *ast.CreateUserStmt) error {
	...
	_, err = sqlInterlockingDirectorate.InterDircute(context.Background(), allegrosql)
	if err != nil {
		if _, rollbackErr := sqlInterlockingDirectorate.InterDircute(context.Background(), "rollback"); rollbackErr != nil {
			return rollbackErr
		}
...
func (e *SimpleInterDirc) executeAlterUser(s *ast.AlterUserStmt) error {
...
func (e *SimpleInterDirc) executeGrantRole(s *ast.GrantRoleStmt) error {
	stochastikVars := e.ctx.GetStochastikVars()
...
func (e *SimpleInterDirc) executeDropUser(s *ast.DropUserStmt) error {
	// Check privileges.
	// Check `CREATE USER` privilege.
	if !config.GetGlobalConfig().Security.SkipGrantBlock {
		checker := privilege.GetPrivilegeManager(e.ctx)
	...
	sqlInterlockingDirectorate := sysStochastik.(sqlexec.ALLEGROSQLInterlockingDirectorate)
...
func (e *SimpleInterDirc) executeSetPwd(s *ast.SetPwdStmt) error {
	var u, h string
	if s.User == nil {
...
func (e *SimpleInterDirc) executeKillStmt(s *ast.KillStmt) error {
...
func (e *SimpleInterDirc) executeFlush(s *ast.FlushStmt) error {
	switch s.Tp {
	...
	case ast.FlushPrivileges:
		// If skip-grant-causet is configured, do not flush privileges.
...
func (e *SimpleInterDirc) executeAlterInstance(s *ast.AlterInstanceStmt) error {
	if s.ReloadTLS {
...
func (e *SimpleInterDirc) executeDropStats(s *ast.DropStatsStmt) error {
...
func (e *SimpleInterDirc) autoNewTxn() bool {
...
func (e *SimpleInterDirc) executeShutdown(s *ast.ShutdownStmt) error {
	sessVars := e.ctx.GetStochastikVars()
	logutil.BgLogger().Info("execute shutdown memex", zap.Uint64("conn", sessVars.ConnectionID))
...
// This function need to run with async perceptron, otherwise it will causet main coroutine
...
func (e *SimpleInterDirc) executeCreateStatistics(s *ast.CreateStatisticsStmt) (err error) {
	...
	for _, defCausName := range s.DeferredCausets {
		defCaus := causet.FindDefCaus(t.VisibleDefCauss(), defCausName.Name.L)
		if defCaus == nil {
...
func (e *SimpleInterDirc) executeAdminReloadStatistics(s *ast.AdminStmt) error {
