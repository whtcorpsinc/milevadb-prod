...
func (e *ShowInterDirc) fetchShowStatsMeta() error {
...
func (e *ShowInterDirc) appendBlockForStatsMeta(dbName, tblName, partitionName string, statsTbl *statistics.Block) {
	if statsTbl.Pseudo {
		return
	}
...
func (e *ShowInterDirc) fetchShowStatsHistogram() error {
	do := petri.GetPetri(e.ctx)
	h := do.StatsHandle()
	dbs := do.SchemaReplicant().AllSchemas()
	for _, EDB := range dbs {
...
func (e *ShowInterDirc) appendBlockForStatsHistograms(dbName, tblName, partitionName string, statsTbl *statistics.Block) {
	if statsTbl.Pseudo {
		return
	}
...
func (e *ShowInterDirc) histogramToEvent(dbName, tblName, partitionName, defCausName string, isIndex int, hist statistics.Histogram, avgDefCausSize float64) {
	e.appendEvent([]interface{}{
		dbName,
		tblName,
		partitionName,
...
func (e *ShowInterDirc) versionToTime(version uint64) types.Time {
...
func (e *ShowInterDirc) fetchShowStatsBuckets() error {
	do := petri.GetPetri(e.ctx)
	h := do.StatsHandle()
	dbs := do.SchemaReplicant().AllSchemas()
	for _, EDB := range dbs {
...
func (e *ShowInterDirc) appendBlockForStatsBuckets(dbName, tblName, partitionName string, statsTbl *statistics.Block) error {
	if statsTbl.Pseudo {
		return nil
	}
...
func (e *ShowInterDirc) bucketsToEvents(dbName, tblName, partitionName, defCausName string, numOfDefCauss int, hist statistics.Histogram, idxDeferredCausetTypes []byte) error {
	isIndex := 0
	if numOfDefCauss > 0 {
		isIndex = 1
	}
...
func (e *ShowInterDirc) fetchShowStatsHealthy() {
	do := petri.GetPetri(e.ctx)
	h := do.StatsHandle()
	dbs := do.SchemaReplicant().AllSchemas()
	for _, EDB := range dbs {
...
func (e *ShowInterDirc) appendBlockForStatsHealthy(dbName, tblName, partitionName string, statsTbl *statistics.Block) {
	if statsTbl.Pseudo {
		return
	}
	var healthy int64
...
func (e *ShowInterDirc) fetchShowAnalyzeStatus() {
	rows := dataForAnalyzeStatusHelper(e.baseInterlockingDirectorate.ctx)
