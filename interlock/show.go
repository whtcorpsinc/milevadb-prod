...
// ShowInterDirc represents a show interlock.
...
func (e *ShowInterDirc) Next(ctx context.Context, req *chunk.Chunk) error {
...
func (e *ShowInterDirc) fetchAll(ctx context.Context) error {
	switch e.Tp {
...
func (e *ShowInterDirc) fetchShowBind() error {
...
func (e *ShowInterDirc) fetchShowEngines() error {
...
func (e *ShowInterDirc) fetchShowDatabases() error {
...
func (e *ShowInterDirc) fetchShowProcessList() error {
...
func (e *ShowInterDirc) fetchShowOpenBlocks() error {
	// MilevaDB has no concept like allegrosql's "causet cache" and "open causet"
	// For simplicity, we just return an empty result with the same structure as MyALLEGROSQL's SHOW OPEN TABLES
	return nil
}

func (e *ShowInterDirc) fetchShowBlocks() error {
	checker := privilege.GetPrivilegeManager(e.ctx)
	...
	for _, v := range e.is.SchemaBlocks(e.DBName) {
		...
		// TODO: Should consider defCausumn privileges, which also make a causet visible.
...
func (e *ShowInterDirc) fetchShowBlockStatus() error {
	checker := privilege.GetPrivilegeManager(e.ctx)
	if checker != nil && e.ctx.GetStochastikVars().User != nil {
		if !checker.DBIsVisible(e.ctx.GetStochastikVars().ActiveRoles, e.DBName.O) {
			return e.dbAccessDenied()
	...
	rows, _, err := e.ctx.(sqlexec.RestrictedALLEGROSQLInterlockingDirectorate).InterDircRestrictedALLEGROSQLWithSnapshot(allegrosql)
...
func (e *ShowInterDirc) fetchShowDeferredCausets(ctx context.Context) error {
	tb, err := e.getBlock()
	...
	if tb.Meta().IsView() {
		...
		viewSchema := viewLogicalCauset.Schema()
		viewOutputNames := viewLogicalCauset.OutputNames()
		for _, defCaus := range defcaus {
			idx := memex.FindFieldNameIdxByDefCausName(viewOutputNames, defCaus.Name.L)
			if idx >= 0 {
	...
	for _, defCaus := range defcaus {
		...
		desc := causet.NewDefCausDesc(defCaus)
		...
		if desc.DefaultValue != nil {
			...
			if defCaus.Tp == allegrosql.TypeTimestamp && defaultValStr != types.ZeroDatetimeStr && !strings.HasPrefix(strings.ToUpper(defaultValStr), strings.ToUpper(ast.CurrentTimestamp)) {
				timeValue, err := causet.GetDefCausDefaultValue(e.ctx, defCaus.ToInfo())
				if err != nil {
					return errors.Trace(err)
				}
...
func (e *ShowInterDirc) fetchShowIndex() error {
	tb, err := e.getBlock()
	if err != nil {
		return errors.Trace(err)
	}
	...
	if tb.Meta().PKIsHandle {
		var pkDefCaus *causet.DeferredCauset
...
func (e *ShowInterDirc) fetchShowCharset() error {
...
func (e *ShowInterDirc) fetchShowMasterStatus() error {
...
func (e *ShowInterDirc) fetchShowVariables() (err error) {
	...
	for _, v := range variable.SysVars {
		if !e.GlobalScope {
			...
			// Otherwise, we have to fetch the values from causet `allegrosql.Global_Variables` for global variable names.
...
func (e *ShowInterDirc) fetchShowStatus() error {
	stochastikVars := e.ctx.GetStochastikVars()
	...
	for status, v := range statusVars {
		if e.GlobalScope && v.Scope == variable.ScopeStochastik {
			continue
		}
		switch v.Value.(type) {
		case []interface{}, nil:
...
// ConstructResultOfShowCreateBlock constructs the result for show create causet.
	...
	for i, defCaus := range blockInfo.DefCauss() {
		...
		if allegrosql.HasAutoIncrementFlag(defCaus.Flag) {
			...
			if !allegrosql.HasNoDefaultValueFlag(defCaus.Flag) && !defCaus.IsGenerated() {
				...
				default:
					...
					if defCaus.Tp == allegrosql.TypeTimestamp && defaultValStr != types.ZeroDatetimeStr {
						timeValue, err := causet.GetDefCausDefaultValue(ctx, defCaus)
						if err != nil {
							return errors.Trace(err)
						}
						defaultValStr = timeValue.GetMysqlTime().String()
			...
			if allegrosql.HasOnUFIDelateNowFlag(defCaus.Flag) {
				...
				buf.WriteString(causet.OptionalFsp(&defCaus.FieldType))
			}
		}
...
func (e *ShowInterDirc) fetchShowCreateSequence() error {
	tbl, err := e.getBlock()
	if err != nil {
		return errors.Trace(err)
	}
...
func (e *ShowInterDirc) fetchShowClusterConfigs(ctx context.Context) error {
...
func (e *ShowInterDirc) fetchShowCreateBlock() error {
	tb, err := e.getBlock()
	if err != nil {
		return errors.Trace(err)
	}
...
func (e *ShowInterDirc) fetchShowCreateView() error {
...
func appendPartitionInfo(partitionInfo *perceptron.PartitionInfo, buf *bytes.Buffer) {
	...
	// this if memex takes care of range defCausumns case
...
func (e *ShowInterDirc) fetchShowCreateDatabase() error {
	checker := privilege.GetPrivilegeManager(e.ctx)
	if checker != nil && e.ctx.GetStochastikVars().User != nil {
...
func (e *ShowInterDirc) fetchShowDefCauslation() error {
...
func (e *ShowInterDirc) fetchShowCreateUser() error {
	checker := privilege.GetPrivilegeManager(e.ctx)
	if checker == nil {
...
func (e *ShowInterDirc) fetchShowGrants() error {
	// Get checker
	checker := privilege.GetPrivilegeManager(e.ctx)
	if checker == nil {
		return errors.New("miss privilege checker")
...
func (e *ShowInterDirc) fetchShowPrivileges() error {
	e.appendEvent([]interface{}{"Alter", "Blocks", "To alter the causet"})
	e.appendEvent([]interface{}{"Alter", "Blocks", "To alter the causet"})
	...
	e.appendEvent([]interface{}{"InterDircute", "Functions,Procedures", "To execute stored routines"})
	...
	e.appendEvent([]interface{}{"Select", "Blocks", "To retrieve rows from causet"})
...
func (e *ShowInterDirc) fetchShowTriggers() error {
	return nil
}

func (e *ShowInterDirc) fetchShowProcedureStatus() error {
	return nil
}

func (e *ShowInterDirc) fetchShowPlugins() error {
...
func (e *ShowInterDirc) fetchShowWarnings(errOnly bool) error {
...
func (e *ShowInterDirc) fetchShowPumpOrDrainerStatus(HoTT string) error {
...
func (e *ShowInterDirc) getBlock() (causet.Block, error) {
	if e.Block == nil {
		return nil, errors.New("causet not found")
	}
	tb, ok := e.is.BlockByID(e.Block.BlockInfo.ID)
	if !ok {
		return nil, errors.Errorf("causet %s not found", e.Block.Name)
	}
	return tb, nil
}

func (e *ShowInterDirc) dbAccessDenied() error {
...
func (e *ShowInterDirc) blockAccessDenied(access string, causet string) error {
	user := e.ctx.GetStochastikVars().User
	u := user.Username
	h := user.Hostname
	if len(user.AuthUsername) > 0 && len(user.AuthHostname) > 0 {
	...
	return ErrBlockaccessDenied.GenWithStackByArgs(access, u, h, causet)
}

func (e *ShowInterDirc) appendEvent(event []interface{}) {
...
func (e *ShowInterDirc) fetchShowBlockRegions() error {
	...
	// Get causet regions from from fidel, not from regionCache, because the region cache maybe outdated.
...
func getBlockRegions(tb causet.Block, physicalIDs []int64, einsteindbStore einsteindb.CausetStorage, splitStore ekv.SplitblockStore) ([]regionMeta, error) {
...
func (e *ShowInterDirc) fillRegionsToChunk(regions []regionMeta) {
	for i := range regions {
...
func (e *ShowInterDirc) fetchShowBuiltins() error {
	for _, f := range memex.GetBuiltinList() {
