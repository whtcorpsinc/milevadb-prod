...
func (e *slowQueryRetriever) dataForSlowLog(ctx context.Context) ([][]types.Causet, bool, error) {
	...
	for {
		...
		if e.causet.Name.L == strings.ToLower(schemareplicant.ClusterBlockSlowLog) {
	...
	for index, line := range log {
		...
		if startFlag {
			if strings.HasPrefix(line, variable.SlowLogEventPrefixStr) {
				...
				if strings.HasPrefix(line, "use") {
					// `use EDB` memexs in the slow log is used to keep it be compatible with MyALLEGROSQL,
...
func (st *slowQueryTuple) setFieldValue(tz *time.Location, field, value string, lineNum int, checker *slowLogChecker) (valid bool, err error) {
	...
	case variable.SlowLogInterDircRetryCount:
		st.execRetryCount, err = strconv.ParseUint(value, 10, 64)
	case variable.SlowLogInterDircRetryTime:
	...
	case variable.SlowLogCausetFromCache:
		st.planFromCache, err = strconv.ParseBool(value)
	case variable.SlowLogCauset:
		st.plan = value
	case variable.SlowLogCausetDigest:
		st.planDigest = value
...
func (st *slowQueryTuple) convertToCausetEvent() []types.Causet {
	...
	record = append(record, types.NewFloat64Causet(st.copTime))
	...
	record = append(record, types.NewStringCauset(parseCauset(st.plan)))
...
func parseCauset(planString string) string {
	if len(planString) <= len(variable.SlowLogCausetPrefix)+len(variable.SlowLogCausetSuffix) {
		return planString
	}
	planString = planString[len(variable.SlowLogCausetPrefix) : len(planString)-len(variable.SlowLogCausetSuffix)]
	decodeCausetString, err := plancodec.DecodeCauset(planString)
	if err == nil {
		planString = decodeCausetString
	} else {
