...
func (s *testInterDircSerialSuite) TestParseSlowLogPanic(c *C) {
	slowLogStr :=
...
use test;
select * from t;`
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/interlock/errorMockParseSlowLogPanic", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/whtcorpsinc/milevadb/interlock/errorMockParseSlowLogPanic"), IsNil)
	}()
...
func (s *testInterDircSuite) TestParseSlowLogFile(c *C) {
	slowLogStr :=
		`# Time: 2020-04-28T15:24:04.309074+08:00
# Txn_start_ts: 405888132465033227
# User@Host: root[root] @ localhost [127.0.0.1]
...
func (s *testInterDircSerialSuite) TestParseSlowLogFileSerial(c *C) {
	loc, err := time.LoadLocation("Asia/Shanghai")
	c.Assert(err, IsNil)
	ctx := mock.NewContext()
	ctx.GetStochastikVars().TimeZone = loc
...
func (s *testInterDircSuite) TestSlowLogParseTime(c *C) {
...
func (s *testInterDircSuite) TestFixParseSlowLogFile(c *C) {
	slowLog := bytes.NewBufferString(
...
func (s *testInterDircSuite) TestSlowQueryRetriever(c *C) {
