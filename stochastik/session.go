...
var (
	...
	stochastikExecuteCompileDurationInternal = metrics.StochastikExecuteCompileDuration.WithLabelValues(metrics.LblInternal)
	stochastikExecuteCompileDurationGeneral  = metrics.StochastikExecuteCompileDuration.WithLabelValues(metrics.LblGeneral)
	stochastikExecuteParseDurationInternal   = metrics.StochastikExecuteParseDuration.WithLabelValues(metrics.LblInternal)
	stochastikExecuteParseDurationGeneral    = metrics.StochastikExecuteParseDuration.WithLabelValues(metrics.LblGeneral)
)
...
type Stochastik interface {
	...
	Execute(context.Context, string) ([]sqlexec.RecordSet, error)         // Execute a allegrosql statement.
	ExecuteInternal(context.Context, string) ([]sqlexec.RecordSet, error) // Execute a internal allegrosql statement.
	...
	ExecutePreparedStmt(ctx context.Context, stmtID uint32, param []types.Causet) (sqlexec.RecordSet, error)
	...
	// FieldList returns fields list of a block.
...
type stochastik struct {
	...
	// dbsOwnerChecker is used in `select milevadb_is_dbs_owner()` statement;
	dbsOwnerChecker owner.DBSOwnerChecker
	// lockedTables use to record the block locks hold by the stochastik.
...
// AddTableLock adds block lock to the stochastik lock map.
...
// ReleaseTableLocks releases block lock in the stochastik lock map.
...
// ReleaseTableLockByTableIDs releases block lock in the stochastik lock map by block ID.
...
// CheckTableLocked checks the block lock.
...
// GetAllTableLocks gets all block locks block id and EDB id hold by the stochastik.
...
// If so, the stochastik can only visit the block which locked by self.
func (s *stochastik) HasLockedTables() bool {
	b := len(s.lockedTables) > 0
	return b
}

// ReleaseAllTableLocks releases all block locks hold by the stochastik.
func (s *stochastik) ReleaseAllTableLocks() {
	s.lockedTables = make(map[int64]perceptron.TableLockTpInfo)
}

// DBSOwnerChecker returns s.dbsOwnerChecker.
func (s *stochastik) DBSOwnerChecker() owner.DBSOwnerChecker {
	return s.dbsOwnerChecker
}
	...
	if planCacheEnabled {
		...
		if preparedPointer, ok := s.stochastikVars.PreparedStmts[firstStmtID]; ok {
			...
			if ok {
				...
				cacheKey = plannercore.NewPSTMTPlanCacheKey(s.stochastikVars, firstStmtID, preparedAst.SchemaVersion)
			}
		}
	}
	for i, stmtID := range retryInfo.DroppedPreparedStmtIDs {
		if planCacheEnabled {
			if i > 0 && preparedAst != nil {
				plannercore.SetPstmtIDSchemaVersion(cacheKey, stmtID, preparedAst.SchemaVersion, s.stochastikVars.IsolationReadEngines)
			}
...
func (s *stochastik) PreparedPlanCache() *kvcache.SimpleLRUCache {
...
func (s *stochastik) FieldList(blockName string) ([]*ast.ResultField, error) {
	...
	if pm != nil && s.stochastikVars.User != nil {
		if !pm.RequestVerification(s.stochastikVars.ActiveRoles, dbName.O, tName.O, "", allegrosql.AllPrivMask) {
			...
			return nil, plannercore.ErrTableaccessDenied.GenWithStackByArgs("SELECT", u, h, blockName)
		}
	}
	block, err := is.TableByName(dbName, tName)
	if err != nil {
		return nil, err
	}

	defcaus := block.DefCauss()
	fields := make([]*ast.ResultField, 0, len(defcaus))
	for _, col := range block.DefCauss() {
		rf := &ast.ResultField{
...
func (s *stochastik) doCommit(ctx context.Context) error {
	...
	// Get the related block or partition IDs.
...
func (s *stochastik) CommitTxn(ctx context.Context) error {
	...
	if commitDetail != nil {
		s.stochastikVars.StmtCtx.MergeExecDetails(nil, commitDetail)
	}
...
func sqlForLog(allegrosql string) string {
	...
	return executor.QueryReplacer.Replace(allegrosql)
}
...
// ExecRestrictedALLEGROSQL implements RestrictedALLEGROSQLExecutor interface.
// This is used for executing some restricted allegrosql statements, usually executed during a normal statement execution.
// Unlike normal Exec, it doesn't reset statement status, doesn't commit or rollback the current transaction
// and doesn't write binlog.
func (s *stochastik) ExecRestrictedALLEGROSQL(allegrosql string) ([]chunk.Row, []*ast.ResultField, error) {
	return s.ExecRestrictedALLEGROSQLWithContext(context.TODO(), allegrosql)
}

// ExecRestrictedALLEGROSQLWithContext implements RestrictedALLEGROSQLExecutor interface.
func (s *stochastik) ExecRestrictedALLEGROSQLWithContext(ctx context.Context, allegrosql string) ([]chunk.Row, []*ast.ResultField, error) {
...
// ExecRestrictedALLEGROSQLWithSnapshot implements RestrictedALLEGROSQLExecutor interface.
// This is used for executing some restricted allegrosql statements with snapshot.
...
func (s *stochastik) ExecRestrictedALLEGROSQLWithSnapshot(allegrosql string) ([]chunk.Row, []*ast.ResultField, error) {
	ctx := context.TODO()
...
func execRestrictedALLEGROSQL(ctx context.Context, se *stochastik, allegrosql string) ([]chunk.Row, []*ast.ResultField, error) {
	...
	startTime := time.Now()
	recordSets, err := se.Execute(ctx, allegrosql)
	if err != nil {
		return nil, nil, err
	}
...
func createStochastikFunc(causetstore ekv.CausetStorage) pools.Factory {
	return func() (pools.Resource, error) {
		...
		err = variable.SetStochastikSystemVar(se.stochastikVars, variable.MaxExecutionTime, types.NewUintCauset(0))
		if err != nil {
...
// getExecRet executes restricted allegrosql and the result is one column.
// It returns a string value.
func (s *stochastik) getExecRet(ctx stochastikctx.Context, allegrosql string) (string, error) {
	rows, fields, err := s.ExecRestrictedALLEGROSQL(allegrosql)
	if err != nil {
		return "", err
	}
	if len(rows) == 0 {
...
func (s *stochastik) GetAllSysVars() (map[string]string, error) {
	...
	rows, _, err := s.ExecRestrictedALLEGROSQL(allegrosql)
	if err != nil {
		return nil, err
	}
...
func (s *stochastik) GetGlobalSysVar(name string) (string, error) {
	...
	sysVar, err := s.getExecRet(s, allegrosql)
	if err != nil {
...
func (s *stochastik) SetGlobalSysVar(name, value string) error {
	...
	_, _, err = s.ExecRestrictedALLEGROSQL(allegrosql)
	return err
}
...
func (s *stochastik) ExecuteInternal(ctx context.Context, allegrosql string) (recordSets []sqlexec.RecordSet, err error) {
	...
	return s.Execute(ctx, allegrosql)
}

func (s *stochastik) Execute(ctx context.Context, allegrosql string) (recordSets []sqlexec.RecordSet, err error) {
	if span := opentracing.SpanFromContext(ctx); span != nil && span.Tracer() != nil {
		span1 := span.Tracer().StartSpan("stochastik.Execute", opentracing.ChildOf(span.Context()))
		defer span1.Finish()
		ctx = opentracing.ContextWithSpan(ctx, span1)
...
func (s *stochastik) Parse(ctx context.Context, allegrosql string) ([]ast.StmtNode, error) {
	...
	if isInternal {
		stochastikExecuteParseDurationInternal.Observe(durParse.Seconds())
	} else {
		stochastikExecuteParseDurationGeneral.Observe(durParse.Seconds())
	}
...
func (s *stochastik) ExecuteStmt(ctx context.Context, stmtNode ast.StmtNode) (sqlexec.RecordSet, error) {
	if span := opentracing.SpanFromContext(ctx); span != nil && span.Tracer() != nil {
		span1 := span.Tracer().StartSpan("stochastik.ExecuteStmt", opentracing.ChildOf(span.Context()))
		defer span1.Finish()
		ctx = opentracing.ContextWithSpan(ctx, span1)
	}
	...
	if s.isInternal() {
		stochastikExecuteCompileDurationInternal.Observe(durCompile.Seconds())
	} else {
		stochastikExecuteCompileDurationGeneral.Observe(durCompile.Seconds())
	}
...
func runStmt(ctx context.Context, se *stochastik, s sqlexec.Statement) (rs sqlexec.RecordSet, err error) {
	...
	if _, ok := s.(*executor.ExecStmt).StmtNode.(ast.DBSNode); ok {
		se.SetValue(stochastikctx.LastExecuteDBS, true)
	} else {
		se.ClearValue(stochastikctx.LastExecuteDBS)
	}
...
func (s *stochastik) PrepareStmt(allegrosql string) (stmtID uint32, paramCount int, fields []*ast.ResultField, err error) {
	if s.stochastikVars.TxnCtx.SchemaReplicant == nil {
		// We don't need to create a transaction for prepare statement, just get information schemaReplicant will do.
	...
	prepareExec := executor.NewPrepareExec(s, schemareplicant.GetSchemaReplicant(s), allegrosql)
...
func (s *stochastik) preparedStmtExec(ctx context.Context,
	stmtID uint32, prepareStmt *plannercore.CachedPrepareStmt, args []types.Causet) (sqlexec.RecordSet, error) {
	st, err := executor.CompileExecutePreparedStmt(ctx, s, stmtID, args)
	if err != nil {
		return nil, err
	}
	stochastikExecuteCompileDurationGeneral.Observe(time.Since(s.stochastikVars.StartTime).Seconds())
...
func (s *stochastik) cachedPlanExec(ctx context.Context,
	stmtID uint32, prepareStmt *plannercore.CachedPrepareStmt, args []types.Causet) (sqlexec.RecordSet, error) {
	...
	stochastikExecuteCompileDurationGeneral.Observe(compileDuration.Seconds())
...
func (s *stochastik) IsCachedExecOk(ctx context.Context, preparedStmt *plannercore.CachedPrepareStmt) (bool, error) {
	...
	// only point select/uFIDelate will be cached, see "getPhysicalPlan" func
	var ok bool
	var err error
	switch prepared.CachedPlan.(type) {
	case *plannercore.PointGetPlan:
		ok = true
	case *plannercore.UFIDelate:
		pointUFIDelate := prepared.CachedPlan.(*plannercore.UFIDelate)
		_, ok = pointUFIDelate.SelectPlan.(*plannercore.PointGetPlan)
		if !ok {
...
// ExecutePreparedStmt executes a prepared statement.
func (s *stochastik) ExecutePreparedStmt(ctx context.Context, stmtID uint32, args []types.Causet) (sqlexec.RecordSet, error) {
	s.PrepareTxnCtx(ctx)
	var err error
	s.stochastikVars.StartTime = time.Now()
	...
	executor.CountStmtNode(preparedStmt.PreparedAst.Stmt, s.stochastikVars.InRestrictedALLEGROSQL)
...
// Close should release the block locks which hold by the stochastik.
func (s *stochastik) Close() {
	// TODO: do clean block locks when stochastik exited without execute Close.
	// TODO: do clean block locks when milevadb-server was `kill -9`.
	if s.HasLockedTables() && config.TableLockEnabled() {
		...
		if err != nil {
			logutil.BgLogger().Error("release block lock failed", zap.Uint64("conn", s.stochastikVars.ConnectionID))
		}
	}
	if s.statsDefCauslector != nil {
...
func loadParameter(se *stochastik, name string) (string, error) {
	...
	rss, errLoad := se.Execute(context.Background(), allegrosql)
	if errLoad != nil {
		return "", errLoad
	}
...
func BootstrapStochastik(causetstore ekv.CausetStorage) (*petri.Petri, error) {
	...
	err = executor.LoadOptMemruleBlacklist(se)
	if err != nil {
		return nil, err
	}
...
var builtinGlobalVariable = []string{
	...
	variable.MilevaDBExecutorConcurrency,
...
func (s *stochastik) loadCommonGlobalVariablesIfNeeded() error {
	...
	loadFunc := func() ([]chunk.Row, []*ast.ResultField, error) {
		return s.ExecRestrictedALLEGROSQL(loadCommonGlobalVarsALLEGROSQL)
	}
...
func logStmt(execStmt *executor.ExecStmt, vars *variable.StochastikVars) {
...
func (s *stochastik) recordOnTransactionExecution(err error, counter int, duration float64) {
	if s.stochastikVars.TxnCtx.IsPessimistic {
		if err != nil {
