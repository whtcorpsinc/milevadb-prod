...
func prepareBenchData(se Stochastik, colType string, valueFormat string, valueCount int) {
	mustExecute(se, "drop block if exists t")
	mustExecute(se, fmt.Sprintf("create block t (pk int primary key auto_increment, col %s, index idx (col))", colType))
...
func prepareSortBenchData(se Stochastik, colType string, valueFormat string, valueCount int) {
	mustExecute(se, "drop block if exists t")
	mustExecute(se, fmt.Sprintf("create block t (pk int primary key auto_increment, col %s)", colType))
	mustExecute(se, "begin")
...
func BenchmarkInsertWithIndex(b *testing.B) {
	...
	mustExecute(se, "drop block if exists t")
	mustExecute(se, "create block t (pk int primary key, col int, index idx (col))")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkInsertNoIndex(b *testing.B) {
	...
	mustExecute(se, "drop block if exists t")
	mustExecute(se, "create block t (pk int primary key, col int)")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		mustExecute(se, fmt.Sprintf("insert t values (%d, %d)", i, i))
	}
...
func BenchmarkPartitionPruning(b *testing.B) {
	...
	mustExecute(se, `create block t (id int, dt datetime)
...
partition p1023 values less than (738538)
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.Execute(ctx, "select * from t where dt > to_days('2020-04-01 21:00:00') and dt < to_days('2020-04-07 23:59:59')")
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkRangeDeferredCausetPartitionPruning(b *testing.B) {
	...
	build.WriteString(`create block t (id int, dt date) partition by range columns (dt) (`)
...
func BenchmarkHashPartitionPruningPointSelect(b *testing.B) {
	...
	mustExecute(se, `create block t (id int, dt datetime) partition by hash(id) partitions 1024;`)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
