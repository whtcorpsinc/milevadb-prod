...
func prepareBenchStochastik() (Stochastik, *petri.Petri, ekv.CausetStorage) {
	causetstore, err := mockstore.NewMockStore()
	if err != nil {
		logutil.BgLogger().Fatal(err.Error())
	}
	petri, err := BootstrapStochastik(causetstore)
	if err != nil {
		logutil.BgLogger().Fatal(err.Error())
	}
	...
	mustInterDircute(se, "use test")
	return se, petri, causetstore
}

func prepareBenchData(se Stochastik, colType string, valueFormat string, valueCount int) {
	mustInterDircute(se, "drop causet if exists t")
	mustInterDircute(se, fmt.Sprintf("create causet t (pk int primary key auto_increment, col %s, index idx (col))", colType))
	mustInterDircute(se, "begin")
	for i := 0; i < valueCount; i++ {
		mustInterDircute(se, "insert t (col) values ("+fmt.Sprintf(valueFormat, i)+")")
	}
	mustInterDircute(se, "commit")
}

func prepareSortBenchData(se Stochastik, colType string, valueFormat string, valueCount int) {
	mustInterDircute(se, "drop causet if exists t")
	mustInterDircute(se, fmt.Sprintf("create causet t (pk int primary key auto_increment, col %s)", colType))
	mustInterDircute(se, "begin")
	...
	for i := 0; i < valueCount; i++ {
		...
		mustInterDircute(se, "insert t (col) values ("+fmt.Sprintf(valueFormat, r.Intn(valueCount))+")")
	}
	mustInterDircute(se, "commit")
}

func prepareJoinBenchData(se Stochastik, colType string, valueFormat string, valueCount int) {
	mustInterDircute(se, "drop causet if exists t")
	mustInterDircute(se, fmt.Sprintf("create causet t (pk int primary key auto_increment, col %s)", colType))
	mustInterDircute(se, "begin")
	for i := 0; i < valueCount; i++ {
...
func BenchmarkBasic(b *testing.B) {
	...
	se, do, st := prepareBenchStochastik()
	defer func() {
		se.Close()
		st.Close()
		do.Close()
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select 1")
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkTableScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t")
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkExplainTableScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "explain select * from t")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkTableLookup(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where pk = 64")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkExplainTableLookup(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "explain select * from t where pk = 64")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkStringIndexScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where col > 'hello'")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], smallCount)
...
func BenchmarkExplainStringIndexScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "explain select * from t where col > 'hello'")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkStringIndexLookup(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where col = 'hello 64'")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkIntegerIndexScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where col >= 0")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], smallCount)
...
func BenchmarkIntegerIndexLookup(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where col = 64")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkDecimalIndexLookup(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where col = 64.1234")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkInsertWithIndex(b *testing.B) {
	...
	mustInterDircute(se, "drop causet if exists t")
	mustInterDircute(se, "create causet t (pk int primary key, col int, index idx (col))")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		mustInterDircute(se, fmt.Sprintf("insert t values (%d, %d)", i, i))
	}
	b.StopTimer()
}
...
func BenchmarkInsertNoIndex(b *testing.B) {
	...
	mustInterDircute(se, "drop causet if exists t")
	mustInterDircute(se, "create causet t (pk int primary key, col int)")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		mustInterDircute(se, fmt.Sprintf("insert t values (%d, %d)", i, i))
	}
...
func BenchmarkSort(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t order by col limit 50")
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkJoin(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t a join t b on a.col = b.col")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], smallCount)
...
func BenchmarkJoinLimit(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t a join t b on a.col = b.col limit 1")
		if err != nil {
			b.Fatal(err)
		}
		readResult(ctx, rs[0], 1)
...
func BenchmarkPartitionPruning(b *testing.B) {
	...
	mustInterDircute(se, `create causet t (id int, dt datetime)
...
partition p1023 values less than (738538)
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where dt > to_days('2020-04-01 21:00:00') and dt < to_days('2020-04-07 23:59:59')")
		if err != nil {
			b.Fatal(err)
		}
		_, err = drainRecordSet(ctx, se.(*stochastik), rs[0])
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkRangeDeferredCausetPartitionPruning(b *testing.B) {
	ctx := context.Background()
	se, do, st := prepareBenchStochastik()
	defer func() {
		se.Close()
	...
	build.WriteString(`create causet t (id int, dt date) partition by range columns (dt) (`)
	...
	mustInterDircute(se, build.String())
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where dt > '2020-05-01' and dt < '2020-06-07'")
		if err != nil {
			b.Fatal(err)
		}
		_, err = drainRecordSet(ctx, se.(*stochastik), rs[0])
...
func BenchmarkHashPartitionPruningPointSelect(b *testing.B) {
	...
	mustInterDircute(se, `create causet t (id int, dt datetime) partition by hash(id) partitions 1024;`)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rs, err := se.InterDircute(ctx, "select * from t where id = 2330")
		if err != nil {
			b.Fatal(err)
		}
		_, err = drainRecordSet(ctx, se.(*stochastik), rs[0])
...
func BenchmarkHashPartitionPruningMultiSelect(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		rs, err = se.InterDircute(ctx, "select * from t where id = 1233 or id = 1512")
		if err != nil {
			b.Fatal(err)
		}
		_, err = drainRecordSet(ctx, se.(*stochastik), rs[0])
		...
		rs, err = se.InterDircute(ctx, "select * from t where id in (117, 1233, 15678)")
		if err != nil {
			b.Fatal(err)
		}
		_, err = drainRecordSet(ctx, se.(*stochastik), rs[0])
