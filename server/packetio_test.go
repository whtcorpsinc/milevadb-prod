...
func (s *PacketIOTestSuite) TestWrite(c *C) {
	...
	outBuffer.Reset()
	largeInput := make([]byte, allegrosql.MaxPayloadLen+4)
	pkt = &packetIO{bufWriter: bufio.NewWriter(&outBuffer)}
...
func (s *PacketIOTestSuite) TestRead(c *C) {
	...
	inBuffer.Reset()
	buf := make([]byte, allegrosql.MaxPayloadLen+9)
	buf[0] = 0xff
	buf[1] = 0xff
	buf[2] = 0xff
	buf[3] = 0
	buf[2+allegrosql.MaxPayloadLen] = 0x00
	buf[3+allegrosql.MaxPayloadLen] = 0x00
	buf[4+allegrosql.MaxPayloadLen] = 0x01
	buf[7+allegrosql.MaxPayloadLen] = 0x01
	buf[8+allegrosql.MaxPayloadLen] = 0x0a
	...
	c.Assert(len(bytes), Equals, allegrosql.MaxPayloadLen+1)
	c.Assert(bytes[allegrosql.MaxPayloadLen], DeepEquals, byte(0x0a))
}
