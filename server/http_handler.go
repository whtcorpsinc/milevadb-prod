...
func (s *Server) newEinsteinDBHandlerTool() *einsteindbHandlerTool {
	...
	if einsteindbStore, ok = causetstore.causetstore.(einsteindb.CausetStorage); !ok {
		panic("Invalid KvStore with illegal causetstore")
	}
...
func (t *einsteindbHandlerTool) getMvccByStartTs(startTS uint64, startKey, endKey ekv.Key) (*mvccKV, error) {
	...
	for {
		...
		einsteindbReq := einsteindbrpc.NewRequest(einsteindbrpc.CmdMvccGetByStartTs, &kvrpcpb.MvccGetByStartTsRequest{
			StartTs: startTS,
		})
		einsteindbReq.Context.Priority = kvrpcpb.CommandPri_Low
		kvResp, err := t.CausetStore.SendReq(bo, einsteindbReq, curRegion.Region, time.Hour)
		if err != nil {
