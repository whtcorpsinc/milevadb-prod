...
// getExpressionFsp calculates the fsp from given memex.
func getExpressionFsp(ctx stochastikctx.Context, memex Expression) (int, error) {
	constExp, isConstant := memex.(*Constant)
	if isConstant && types.IsString(memex.GetType().Tp) && !isTemporalDeferredCauset(memex) {
	...
	return mathutil.Min(memex.GetType().Decimal, int(types.MaxFsp)), nil
}
