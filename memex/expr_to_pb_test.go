...
func init() {
	fpname := "github.com/whtcorpsinc/milevadb/memex/PanicIfPbCodeUnspecified"
...
func (s *testEvaluatorSerialSuites) TestPushDownSwitcher(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/memex/PushDownTestSwitcher", `return("all")`), IsNil)
	defer func() { c.Assert(failpoint.Disable("github.com/whtcorpsinc/milevadb/memex/PushDownTestSwitcher"), IsNil) }()
	...
	// All disabled
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/memex/PushDownTestSwitcher", `return("")`), IsNil)
	...
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/milevadb/memex/PushDownTestSwitcher", fpexpr), IsNil)
	for i := range funcs {
		pbExpr := pc.ExprToPB(funcs[i])
		if !cases[i].enable {
			c.Assert(pbExpr, IsNil, Commentf("function: %s, sig: %v", cases[i].name, cases[i].sig))
...
func (s *testEvaluatorSerialSuites) TestMetadata(c *C) {
	...
	spacetimedata := new(fidelpb.InUnionMetadata)
	var err error
	...
	c.Assert(cast.(*ScalarFunction).Function.spacetimedata(), DeepEquals, &fidelpb.InUnionMetadata{InUnion: false})
	expr := pc.ExprToPB(cast)
	c.Assert(expr.Sig, Equals, fidelpb.ScalarFuncSig_CastStringAsInt)
	c.Assert(len(expr.Val), Greater, 0)
	err = proto.Unmarshal(expr.Val, spacetimedata)
	c.Assert(err, IsNil)
	c.Assert(spacetimedata.InUnion, Equals, false)
	...
	c.Assert(castInUnion.(*ScalarFunction).Function.spacetimedata(), IsNil)
	...
	c.Assert(castInUnion.(*ScalarFunction).Function.spacetimedata(), DeepEquals, &fidelpb.InUnionMetadata{InUnion: true})
	expr = pc.ExprToPB(castInUnion)
	c.Assert(expr.Sig, Equals, fidelpb.ScalarFuncSig_CastStringAsInt)
	c.Assert(len(expr.Val), Greater, 0)
	err = proto.Unmarshal(expr.Val, spacetimedata)
	c.Assert(err, IsNil)
	c.Assert(spacetimedata.InUnion, Equals, true)
}
