...
// NOTE: Control memexs optionally evaluate some branches depending on conditions, but vectorization executes all
...
var testFile = template.Must(template.New("testFile").Parse(`// Copyright 2020 WHTCORPS INC, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
func generateDotGo(fileName string) error {
	...
	for _, function := range tmplVal.Functions {
		err := function.Tmpl.InterDircute(w, function)
		if err != nil {
			return err
		}
	}
...
func generateTestDotGo(fileName string) error {
	w := new(bytes.Buffer)
	err := testFile.InterDircute(w, tmplVal)
	if err != nil {
		return err
	}
	data, err := format.Source(w.Bytes())
