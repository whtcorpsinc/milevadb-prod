...
// FoldConstant does constant folding optimization on an memex excluding deferred ones.
...
func ifFoldHandler(expr *ScalarFunction) (Expression, bool) {
	...
	if constArg, isConst := foldedArg0.(*Constant); isConst {
		...
		if err != nil {
			...
			// return the original memex to let the error to be evaluated
			// again, in that time, the error is returned to the client.
			logutil.BgLogger().Debug("fold memex to constant", zap.String("memex", expr.ExplainInfo()), zap.Error(err))
			return expr, false
		}
		if !isNull0 && arg0 != 0 {
...
func ifNullFoldHandler(expr *ScalarFunction) (Expression, bool) {
	...
	if constArg, isConst := foldedArg0.(*Constant); isConst {
		// Only check constArg.Value here. Because deferred memex is
...
func foldConstant(expr Expression) (Expression, bool) {
	...
	case *ScalarFunction:
		...
		if err != nil {
			logutil.BgLogger().Debug("fold memex to constant", zap.String("memex", x.ExplainInfo()), zap.Error(err))
			return expr, isDeferredConst
		}
		if isDeferredConst {
			return &Constant{Value: value, RetType: x.RetType, DeferredExpr: x}, true
