...
// ExpressionsToPBList converts memexs to fidelpb.Expr list for new plan.
	...
	for _, expr := range exprs {
		...
		if v == nil {
			return nil, terror.ClassOptimizer.New(allegrosql.ErrInternal, allegrosql.MyALLEGROSQLErrName[allegrosql.ErrInternal]).
				GenWithStack("memex %v cannot be pushed down", expr)
		}
...
// PbConverter supplys methods to convert MilevaDB memexs to FIDelpb.
...
func (pc PbConverter) conOrCorDefCausToPBExpr(expr Expression) *fidelpb.Expr {
	...
	if err != nil {
		logutil.BgLogger().Error("eval constant or correlated defCausumn", zap.String("memex", expr.ExplainInfo()), zap.Error(err))
		return nil
	}
...
func (pc PbConverter) defCausumnToPBExpr(defCausumn *DeferredCauset) *fidelpb.Expr {
	...
	id := defCausumn.ID
	// Zero DeferredCauset ID is not a defCausumn from causet, can not support for now.
	if id == 0 || id == -1 {
		return nil
	}
...
func (pc PbConverter) scalarFuncToPBExpr(expr *ScalarFunction) *fidelpb.Expr {
	...
	var encoded []byte
	if spacetimedata := expr.Function.spacetimedata(); spacetimedata != nil {
		var err error
		encoded, err = proto.Marshal(spacetimedata)
		if err != nil {
