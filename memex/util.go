...
// cowExprRef is a copy-on-write slice ref soliton using in `DeferredCausetSubstitute`
...
// Filter the input memexs, append the results to result.
...
// ExtractDeferredCausets extracts all defCausumns from an memex.
...
// ExtractCorDeferredCausets extracts correlated defCausumn from given memex.
...
// ExtractDeferredCausetSet extracts the different values of `UniqueId` for defCausumns in memexs.
...
// DeferredCausetSubstitute substitutes the defCausumns in filter to memexs in select fields.
...
func DeferredCausetSubstituteImpl(expr Expression, schemaReplicant *Schema, newExprs []Expression) (bool, Expression) {
	...
	case *ScalarFunction:
		...
		// cowExprRef is a copy-on-write soliton, args array allocation happens only
...
// pushNotAcrossExpr try to eliminate the NOT expr in memex tree.
// Input `not` indicates whether there's a `NOT` be pushed down.
// Output `changed` indicates whether the output memex differs from the
...
// PushDownNot pushes the `not` function down to the memex's arguments.
...
func extractFiltersFromDNF(ctx stochastikctx.Context, dnfFunc *ScalarFunction) ([]Expression, Expression) {
	...
	for _, dnfItem := range dnfItems {
		...
		// If the extracted part is just one leaf of the DNF memex. Then the value of the total DNF memex is
...
// DeriveRelaxedFiltersFromDNF given a DNF memex, derive a relaxed DNF memex which only contains defCausumns
// in specified schemaReplicant; the derived memex is a superset of original memex, i.e, any tuple satisfying
// the original memex must satisfy the derived memex. Return nil when the derived memex is universal set.
	...
	for _, dnfItem := range dnfItems {
		...
		for _, cnfItem := range cnfItems {
			if itemSF, ok := cnfItem.(*ScalarFunction); ok && itemSF.FuncName.L == ast.LogicOr {
				...
				// If relaxed memex for embedded DNF is universal set, just drop this CNF item
				continue
			}
			// This cnfItem must be simple memex now
			// If it cannot be fully covered by schemaReplicant, just drop this CNF item
			if ExprFromSchema(cnfItem, schemaReplicant) {
				newCNFItems = append(newCNFItems, cnfItem)
			}
		}
		// If this DNF item involves no defCausumn of specified schemaReplicant, the relaxed memex must be universal set
		if len(newCNFItems) == 0 {
			return nil
		}
		newDNFItems = append(newDNFItems, ComposeCNFCondition(ctx, newCNFItems...))
...
// exprStack is a stack of memexs.
type exprStack struct {
	stack []Expression
}

// pop pops an memex from the stack.
...
// popN pops n memexs from the stack.
// If n greater than stack length or n is negative, it pops all the memexs.
...
// push pushes one memex to the stack.
...
// CausetToConstant generates a Constant memex from a Causet.
func CausetToConstant(d types.Causet, tp byte) *Constant {
	return &Constant{Value: d, RetType: types.NewFieldType(tp)}
}

// ParamMarkerExpression generate a getparam function memex.
...
// GetStringFromConstant gets a string value from the Constant memex.
func GetStringFromConstant(ctx stochastikctx.Context, value Expression) (string, bool, error) {
	con, ok := value.(*Constant)
	if !ok {
		err := errors.Errorf("Not a Constant memex %+v", value)
		return "", true, err
	}
...
// GetIntFromConstant gets an interger value from the Constant memex.
...
// BuildNotNullExpr wraps up `not(isnull())` for given memex.
...
// if the plan is going to be cached, we cannot remove memexs containing `?` neither.
...
// GetUint64FromConstant gets a uint64 from constant memex.
func GetUint64FromConstant(expr Expression) (uint64, bool, bool) {
	con, ok := expr.(*Constant)
	if !ok {
		logutil.BgLogger().Warn("not a constant memex", zap.String("memex", expr.ExplainInfo()))
...
// ContainVirtualDeferredCauset checks if the memexs contain a virtual defCausumn
...
// ContainMublockConst checks if the memexs contain a lazy constant.
