...
// ExplainExpressionList generates explain information for a list of memexs.
...
// SortedExplainExpressionList generates explain information for a list of memexs in order.
...
func SortedExplainNormalizedScalarFuncList(exprs []*ScalarFunction) []byte {
	memexs := make([]Expression, len(exprs))
	for i := range exprs {
		memexs[i] = exprs[i]
	}
	return sortedExplainExpressionList(memexs, true)
}
