...
var (
	...
	_ functionClass = &milevadbIsDBSTenantFunctionClass{}
	_ functionClass = &milevadbDecodeCausetFunctionClass{}
...
type milevadbIsDBSTenantFunctionClass struct {
	baseFunctionClass
}

func (c *milevadbIsDBSTenantFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETInt)
	...
	sig := &builtinMilevaDBIsDBSTenantSig{bf}
	return sig, nil
}

type builtinMilevaDBIsDBSTenantSig struct {
	baseBuiltinFunc
}

func (b *builtinMilevaDBIsDBSTenantSig) Clone() builtinFunc {
	newSig := &builtinMilevaDBIsDBSTenantSig{}
	newSig.cloneFrom(&b.baseBuiltinFunc)
	return newSig
}

// evalInt evals a builtinMilevaDBIsDBSTenantSig.
func (b *builtinMilevaDBIsDBSTenantSig) evalInt(_ chunk.Event) (res int64, isNull bool, err error) {
	dbsTenantChecker := b.ctx.DBSTenantChecker()
...
func (c *benchmarkFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	...
	// Syntax: BENCHMARK(loop_count, memex)
...
// evalInt evals a builtinBenchmarkSig. It will execute memex repeatedly count times.
...
func decodeKey(ctx stochastikctx.Context, s string) string {
	...
	// Try decode as causet index key.
...
type milevadbDecodeCausetFunctionClass struct {
	baseFunctionClass
}

func (c *milevadbDecodeCausetFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
	...
	sig := &builtinMilevaDBDecodeCausetSig{bf}
	return sig, nil
}

type builtinMilevaDBDecodeCausetSig struct {
	baseBuiltinFunc
}

func (b *builtinMilevaDBDecodeCausetSig) Clone() builtinFunc {
	newSig := &builtinMilevaDBDecodeCausetSig{}
	newSig.cloneFrom(&b.baseBuiltinFunc)
	return newSig
}

func (b *builtinMilevaDBDecodeCausetSig) evalString(event chunk.Event) (string, bool, error) {
	planString, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", isNull, err
	}
	planTree, err := plancodec.DecodeCauset(planString)
