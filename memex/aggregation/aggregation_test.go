...
func (s *testAggFuncSuit) TestAvg(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncAvg, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
	...
	desc, err = NewAggFuncDesc(s.ctx, ast.AggFuncAvg, []memex.Expression{defCaus}, true)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestAvgFinalMode(c *C) {
	...
	ctx := mock.NewContext()
	cntDefCaus := &memex.DeferredCauset{
		Index:   0,
		RetType: types.NewFieldType(allegrosql.TypeLonglong),
	}
	sumDefCaus := &memex.DeferredCauset{
		Index:   1,
		RetType: types.NewFieldType(allegrosql.TypeNewDecimal),
	}
	aggFunc, err := NewAggFuncDesc(s.ctx, ast.AggFuncAvg, []memex.Expression{cntDefCaus, sumDefCaus}, false)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestSum(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncSum, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
	...
	desc, err = NewAggFuncDesc(s.ctx, ast.AggFuncSum, []memex.Expression{defCaus}, true)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestBitAnd(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncBitAnd, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestBitOr(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncBitOr, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestBitXor(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncBitXor, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestCount(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncCount, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
	...
	desc, err = NewAggFuncDesc(s.ctx, ast.AggFuncCount, []memex.Expression{defCaus}, true)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestConcat(c *C) {
	...
	sep := &memex.DeferredCauset{
		Index:   1,
		RetType: types.NewFieldType(allegrosql.TypeVarchar),
	}
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncGroupConcat, []memex.Expression{defCaus, sep}, false)
	c.Assert(err, IsNil)
	...
	desc, err = NewAggFuncDesc(s.ctx, ast.AggFuncGroupConcat, []memex.Expression{defCaus, sep}, true)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestFirstEvent(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncFirstEvent, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
...
func (s *testAggFuncSuit) TestMaxMin(c *C) {
	...
	ctx := mock.NewContext()
	desc, err := NewAggFuncDesc(s.ctx, ast.AggFuncMax, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
	maxFunc := desc.GetAggFunc(ctx)
	desc, err = NewAggFuncDesc(s.ctx, ast.AggFuncMin, []memex.Expression{defCaus}, false)
	c.Assert(err, IsNil)
