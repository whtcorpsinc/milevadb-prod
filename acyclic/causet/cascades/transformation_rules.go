...
func (r *PushSelDownBlockScan) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	ts := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalBlockScan)
	...
	newTblScan := causetcore.LogicalBlockScan{
		Source:      ts.Source,
	...
	newSel := causetcore.LogicalSelection{Conditions: remained}.Init(sel.SCtx(), sel.SelectBlockOffset())
...
func (r *PushSelDownIndexScan) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	is := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalIndexScan)
	...
	newIs := causetcore.LogicalIndexScan{
		Source:         is.Source,
	...
	newSel := causetcore.LogicalSelection{Conditions: res.RemainedConds}.Init(sel.SCtx(), sel.SelectBlockOffset())
	selExpr := memo.NewGroupExpr(newSel)
...
func (r *PushSelDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	sg := old.Children[0].GetExpr().ExprNode.(*causetcore.EinsteinDBSingleGather)
	...
	pushedSel := causetcore.LogicalSelection{Conditions: pushed}.Init(sctx, sel.SelectBlockOffset())
	...
	remainedSel := causetcore.LogicalSelection{Conditions: remained}.Init(sel.SCtx(), sel.SelectBlockOffset())
...
func (r *EnumeratePaths) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	ds := old.GetExpr().ExprNode.(*causetcore.DataSource)
...
func (r *PushAggDownGather) Match(expr *memo.ExprIter) bool {
	...
	agg := expr.GetExpr().ExprNode.(*causetcore.LogicalAggregation)
	...
	return causetcore.CheckAggCanPushCop(agg.SCtx(), agg.AggFuncs, agg.GroupByItems, ekv.EinsteinDB)
}
...
func (r *PushAggDownGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	agg := old.GetExpr().ExprNode.(*causetcore.LogicalAggregation)
	aggSchema := old.GetExpr().Group.Prop.Schema
	gather := old.Children[0].GetExpr().ExprNode.(*causetcore.EinsteinDBSingleGather)
	...
	partialPref, finalPref, funcMap := causetcore.BuildFinalModeAggregation(agg.SCtx(),
		&causetcore.AggInfo{
			AggFuncs:     aggFuncs,
	...
	partialPref.AggFuncs =
		causetcore.RemoveUnnecessaryFirstRow(agg.SCtx(), finalPref.AggFuncs, finalPref.GroupByItems, partialPref.AggFuncs, partialPref.GroupByItems, partialPref.Schema, funcMap)

	partialAgg := causetcore.LogicalAggregation{
	...
	finalAgg := causetcore.LogicalAggregation{
...
func (r *PushSelDownSort) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	sort := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalSort)
	childGroup := old.Children[0].GetExpr().Children[0]
...
func (r *PushSelDownProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	proj := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalProjection)
	...
	newBottomSel := causetcore.LogicalSelection{Conditions: canBePushed}.Init(sel.SCtx(), sel.SelectBlockOffset())
	...
	newTopSel := causetcore.LogicalSelection{Conditions: canNotBePushed}.Init(sel.SCtx(), sel.SelectBlockOffset())
...
func (r *PushSelDownAggregation) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	agg := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalAggregation)
	...
	sctx := sel.SCtx()
	childGroup := old.Children[0].GetExpr().Children[0]
	pushedSel := causetcore.LogicalSelection{Conditions: pushedExprs}.Init(sctx, sel.SelectBlockOffset())
	...
	remainedSel := causetcore.LogicalSelection{Conditions: remainedExprs}.Init(sctx, sel.SelectBlockOffset())
...
func (r *PushSelDownWindow) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	window := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalWindow)
...
func (r *TransformLimitToTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetcore.LogicalLimit)
	sort := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalSort)
	childGroup := old.Children[0].GetExpr().Children[0]
	topN := causetcore.LogicalTopN{
...
func (r *PushLimitDownProjection) Match(expr *memo.ExprIter) bool {
	proj := expr.Children[0].GetExpr().ExprNode.(*causetcore.LogicalProjection)
	for _, expr := range proj.Exprs {
		if memex.HasAssignSetVarFunc(expr) {
			return false
		}
...
func (r *PushLimitDownUnionAll) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetcore.LogicalLimit)
	unionAll := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalUnionAll)
	unionAllSchema := old.Children[0].Group.Prop.Schema

	newLimit := causetcore.LogicalLimit{
...
func buildChildSelectionGroup(
	oldSel *causetcore.LogicalSelection,
	conditions []memex.Expression,
	childGroup *memo.Group) *memo.Group {
	if len(conditions) == 0 {
		return childGroup
	}
	newSel := causetcore.LogicalSelection{Conditions: conditions}.Init(oldSel.SCtx(), oldSel.SelectBlockOffset())
...
func (r *PushSelDownJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	join := joinExpr.ExprNode.(*causetcore.LogicalJoin)
	sctx := sel.SCtx()
	...
	case causetcore.SemiJoin, causetcore.InnerJoin:
		...
		dual := causetcore.Conds2BlockDual(join, tempCond)
		if dual != nil {
	...
	case causetcore.LeftOuterJoin, causetcore.LeftOuterSemiJoin, causetcore.AntiLeftOuterSemiJoin,
		causetcore.RightOuterJoin:
		...
		nullSensitive := join.JoinType == causetcore.AntiLeftOuterSemiJoin || join.JoinType == causetcore.LeftOuterSemiJoin
		if join.JoinType == causetcore.RightOuterJoin {
		...
		// Return causet dual when filter is constant false or null.
		dual := causetcore.Conds2BlockDual(join, remainCond)
		if dual != nil {
			return []*memo.GroupExpr{memo.NewGroupExpr(dual)}, false, true, nil
		}
		if join.JoinType == causetcore.RightOuterJoin {
			...
			derivedLeftJoinCond, _ := causetcore.DeriveOtherConditions(join, true, false)
			...
			leftCond = leftPushCond
			// Handle join conditions, only derive left join condition, because right join condition cannot be pushed down
			_, derivedRightJoinCond := causetcore.DeriveOtherConditions(join, false, true)
	...
	if len(remainCond) > 0 {
		newSel := causetcore.LogicalSelection{Conditions: remainCond}.Init(sctx, sel.SelectBlockOffset())
...
func (r *MergeAdjacentProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	proj := old.GetExpr().ExprNode.(*causetcore.LogicalProjection)
	childGroup := old.Children[0].Group
	child := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalProjection)
	if causetcore.ExprsHasSideEffects(child.Exprs) {
		return nil, false, false, nil
	}
	...
	newProj := causetcore.LogicalProjection{Exprs: make([]memex.Expression, len(proj.Exprs))}.Init(proj.SCtx(), proj.SelectBlockOffset())
	newProj.SetSchema(old.GetExpr().Group.Prop.Schema)
	for i, expr := range proj.Exprs {
		newExpr := expr.Clone()
		causetcore.ResolveExprAndReplace(newExpr, replace)
		newProj.Exprs[i] = causetcore.ReplaceDeferredCausetOfExpr(newExpr, child, childGroup.Prop.Schema)
	}
...
func (r *PushTopNDownOuterJoin) Match(expr *memo.ExprIter) bool {
	...
	join := expr.Children[0].GetExpr().ExprNode.(*causetcore.LogicalJoin)
	switch join.JoinType {
	case causetcore.LeftOuterJoin, causetcore.LeftOuterSemiJoin, causetcore.AntiLeftOuterSemiJoin, causetcore.RightOuterJoin:
		return true
	default:
		return false
	}
...
func pushTopNDownOuterJoinToChild(topN *causetcore.LogicalTopN, outerGroup *memo.Group) *memo.Group {
	...
	newTopN := causetcore.LogicalTopN{
...
func (r *PushTopNDownOuterJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	topN := old.GetExpr().ExprNode.(*causetcore.LogicalTopN)
	joinExpr := old.Children[0].GetExpr()
	join := joinExpr.ExprNode.(*causetcore.LogicalJoin)
...
func (r *PushTopNDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	particalTopN := causetcore.LogicalTopN{
		ByItems: topN.ByItems,
		Count:   topN.Count + topN.Offset,
	}.Init(topN.SCtx(), topN.SelectBlockOffset())
...
func (r *MergeAdjacentTopN) Match(expr *memo.ExprIter) bool {
	topN := expr.GetExpr().ExprNode.(*causetcore.LogicalTopN)
	child := expr.Children[0].GetExpr().ExprNode.(*causetcore.LogicalTopN)
...
func (r *MergeAdjacentTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	topN := old.GetExpr().ExprNode.(*causetcore.LogicalTopN)
	child := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalTopN)
	childGroups := old.Children[0].GetExpr().Children

	if child.Count <= topN.Offset {
		blockDual := causetcore.LogicalBlockDual{RowCount: 0}.Init(child.SCtx(), child.SelectBlockOffset())
...
func (r *MergeAggregationProjection) Match(old *memo.ExprIter) bool {
	proj := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalProjection)
	if causetcore.ExprsHasSideEffects(proj.Exprs) {
		return false
	}
	return true
}
...
func (r *MergeAggregationProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	olPosetDagg := old.GetExpr().ExprNode.(*causetcore.LogicalAggregation)
	proj := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalProjection)
	...
	newAgg := causetcore.LogicalAggregation{
...
func (r *EliminateSingleMaxMin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	if len(memex.ExtractDeferredCausets(f.Args[0])) > 0 {
		...
		if !allegrosql.HasNotNullFlag(f.Args[0].GetType().Flag) {
			sel := causetcore.LogicalSelection{}.Init(ctx, agg.SelectBlockOffset())
		...
		li := causetcore.LogicalLimit{Count: 1}.Init(ctx, agg.SelectBlockOffset())
...
func (r *MergeAdjacentSelection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*causetcore.LogicalSelection)
	child := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalSelection)
	childGroups := old.Children[0].GetExpr().Children
	...
	newSel := causetcore.LogicalSelection{Conditions: conditions}.Init(sel.SCtx(), sel.SelectBlockOffset())
	newSelExpr := memo.NewGroupExpr(newSel)
...
func (r *MergeAdjacentLimit) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetcore.LogicalLimit)
	child := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalLimit)
	childGroups := old.Children[0].GetExpr().Children
...
func (r *TransformLimitToBlockDual) Match(expr *memo.ExprIter) bool {
	limit := expr.GetExpr().ExprNode.(*causetcore.LogicalLimit)
...
func (r *TransformLimitToBlockDual) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetcore.LogicalLimit)
	blockDual := causetcore.LogicalBlockDual{RowCount: 0}.Init(limit.SCtx(), limit.SelectBlockOffset())
	blockDual.SetSchema(old.GetExpr().Schema())
	blockDualExpr := memo.NewGroupExpr(blockDual)
	return []*memo.GroupExpr{blockDualExpr}, true, true, nil
}
...
func (r *PushLimitDownOuterJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*causetcore.LogicalLimit)
	join := old.Children[0].GetExpr().ExprNode.(*causetcore.LogicalJoin)
	joinSchema := old.Children[0].Group.Prop.Schema
	leftGroup := old.Children[0].GetExpr().Children[0]
	rightGroup := old.Children[0].GetExpr().Children[1]
...
func (r *PushLimitDownOuterJoin) pushLimitDownOuterJoinToChild(limit *causetcore.LogicalLimit, outerGroup *memo.Group) *memo.Group {
	newLimit := causetcore.LogicalLimit{
		Count: limit.Count + limit.Offset,
	}.Init(limit.SCtx(), limit.SelectBlockOffset())
...
func (r *PushLimitDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	particalLimit := causetcore.LogicalLimit{
		Count: limit.Count + limit.Offset,
	}.Init(limit.SCtx(), limit.SelectBlockOffset())
...
func (r *EliminateOuterJoinBelowAggregation) Match(expr *memo.ExprIter) bool {
	joinType := expr.Children[0].GetExpr().ExprNode.(*causetcore.LogicalJoin).JoinType
	return joinType == causetcore.LeftOuterJoin || joinType == causetcore.RightOuterJoin
}
...
func (r *EliminateOuterJoinBelowAggregation) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	if !causetcore.IsDefCaussAllFromOuterBlock(agg.GetUsedDefCauss(), outerUniqueIDs) {
		return nil, false, false, nil
	}
	// outer join elimination with duplicate agnostic aggregate functions.
	_, aggDefCauss := causetcore.GetDupAgnosticAggDefCauss(agg, nil)
...
func (r *EliminateOuterJoinBelowProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	if !causetcore.IsDefCaussAllFromOuterBlock(proj.GetUsedDefCauss(), outerUniqueIDs) {
		return nil, false, false, nil
	}
...
func (r *TransformAggregateCaseToSelection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	newSel := causetcore.LogicalSelection{Conditions: newConditions}.Init(agg.SCtx(), agg.SelectBlockOffset())
	newSelExpr := memo.NewGroupExpr(newSel)
...
func (r *TransformAggregateCaseToSelection) transform(agg *causetcore.LogicalAggregation) (ok bool, newConditions []memex.Expression, newAggFuncs []*aggregation.AggFuncDesc) {
...
func (r *TransformAggToProj) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	agg := old.GetExpr().ExprNode.(*causetcore.LogicalAggregation)
	if ok, proj := causetcore.ConvertAggToProj(agg, old.GetExpr().Schema()); ok {
		newProjExpr := memo.NewGroupExpr(proj)
...
func (r *InjectProjectionBelowTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	topProj := causetcore.LogicalProjection{
		Exprs: topProjExprs,
	}.Init(topN.SCtx(), topN.SelectBlockOffset())
	...
	bottomProj := causetcore.LogicalProjection{
...
func (r *InjectProjectionBelowAgg) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	proj := causetcore.LogicalProjection{
		Exprs: projExprs,
	}.Init(agg.SCtx(), agg.SelectBlockOffset())
...
func (r *TransformApplyToJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	apply := old.GetExpr().ExprNode.(*causetcore.LogicalApply)
...
func (r *TransformApplyToJoin) extractCorDeferredCausetsBySchema(innerGroup *memo.Group, outerSchema *memex.Schema) []*memex.CorrelatedDeferredCauset {
	...
	return causetcore.ExtractCorDeferredCausetsBySchema(corDefCauss, outerSchema, false)
}
...
func (r *PullSelectionUpApply) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	sel := old.Children[1].GetExpr().ExprNode.(*causetcore.LogicalSelection)
	...
	newApply := causetcore.LogicalApply{
