...
func (s *testCascadesSuite) TestImplGroupZeroCost(c *C) {
	...
	c.Assert(err, IsNil)
	p, _, err := causetcore.BuildLogicalCauset(context.Background(), s.sctx, stmt, s.is)
	c.Assert(err, IsNil)
	logic, ok := p.(causetcore.LogicalCauset)
	c.Assert(ok, IsTrue)
...
func (s *testCascadesSuite) TestPreparePossibleProperties(c *C) {
	...
	ds, ok := logic.Children()[0].Children()[0].(*causetcore.DataSource)
	c.Assert(ok, IsTrue)
	var defCausumnF, defCausumnA *memex.DeferredCauset
	...
	agg, ok := logic.Children()[0].(*causetcore.LogicalAggregation)
	c.Assert(ok, IsTrue)
	...
	//   EinsteinDBSingleGather_5 input:[Group#2], causet:t
	//   EinsteinDBSingleGather_9 input:[Group#3], causet:t, index:f_g
	//   EinsteinDBSingleGather_7 input:[Group#4], causet:t, index:f
	// Group#2 Schema:[test.t.a,test.t.f]
	//   BlockScan_4 causet:t, pk defCaus:test.t.a
	// Group#3 Schema:[test.t.a,test.t.f]
	//   IndexScan_8 causet:t, index:f, g
	// Group#4 Schema:[test.t.a,test.t.f]
	//   IndexScan_6 causet:t, index:f
	propMap := make(map[*memo.Group][][]*memex.DeferredCauset)
