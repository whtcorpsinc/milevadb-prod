...
func (s *testCausetNormalize) TestNormalizedCauset(c *C) {
	...
	for i, tt := range input {
		...
		p, ok := info.Causet.(core.Causet)
		c.Assert(ok, IsTrue)
		normalized, _ := core.NormalizeCauset(p)
...
func (s *testCausetNormalize) TestNormalizedCausetForDiffStore(c *C) {
	...
	for i, tt := range input {
		...
		info := tk.Se.ShowProcess()
		c.Assert(info, NotNil)
		ep, ok := info.Causet.(*core.Explain)
		c.Assert(ok, IsTrue)
		normalized, digest := core.NormalizeCauset(ep.TargetCauset)
		normalizedCauset, err := plancodec.DecodeNormalizedCauset(normalized)
		normalizedCausetRows := getCausetRows(normalizedCauset)
		c.Assert(err, IsNil)
		s.testData.OnRecord(func() {
...
func testNormalizeDigest(tk *testkit.TestKit, c *C, sql1, sql2 string, isSame bool) {
	...
	info := tk.Se.ShowProcess()
	c.Assert(info, NotNil)
	physicalCauset, ok := info.Causet.(core.PhysicalCauset)
	c.Assert(ok, IsTrue)
	normalized1, digest1 := core.NormalizeCauset(physicalCauset)
	...
	physicalCauset, ok = info.Causet.(core.PhysicalCauset)
	c.Assert(ok, IsTrue)
	normalized2, digest2 := core.NormalizeCauset(physicalCauset)
...
func (s *testCausetNormalize) TestDecodeCausetPerformance(c *C) {
	...
	info := tk.Se.ShowProcess()
	c.Assert(info, NotNil)
	p, ok := info.Causet.(core.PhysicalCauset)
	c.Assert(ok, IsTrue)
	...
	encodedCausetStr := core.EncodeCauset(p)
	start := time.Now()
