...
// ToString explains a Causet, returns description string.
func ToString(p Causet) string {
	strs, _ := toString(p, []string{}, []int{})
	return strings.Join(strs, "->")
}

func toString(in Causet, strs []string, idxs []int) ([]string, []int) {
	switch x := in.(type) {
	case LogicalCauset:
		if len(x.Children()) > 1 {
	...
	case *PhysicalBlockReader:
		str = fmt.Sprintf("BlockReader(%s)", ToString(x.blockCauset))
	case *PhysicalIndexReader:
		str = fmt.Sprintf("IndexReader(%s)", ToString(x.indexCauset))
	case *PhysicalIndexLookUpReader:
		str = fmt.Sprintf("IndexLookUp(%s, %s)", ToString(x.indexCauset), ToString(x.blockCauset))
	case *PhysicalIndexMergeReader:
		str = "IndexMergeReader(PartialCausets->["
		for i, paritalCauset := range x.partialCausets {
			if i > 0 {
				str += ", "
			}
			str += ToString(paritalCauset)
		}
		str += "], BlockCauset->" + ToString(x.blockCauset) + ")"
	...
	case *UFIDelate:
		str = fmt.Sprintf("%s->UFIDelate", ToString(x.SelectCauset))
	case *Delete:
		str = fmt.Sprintf("%s->Delete", ToString(x.SelectCauset))
	case *Insert:
		str = "Insert"
		if x.SelectCauset != nil {
			str = fmt.Sprintf("%s->Insert", ToString(x.SelectCauset))
		}
	case *LogicalWindow:
