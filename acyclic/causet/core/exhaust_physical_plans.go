...
func (p *LogicalUnionScan) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	...
	return []PhysicalCauset{us}, true
}

func getMaxSortPrefix(sortDefCauss, allDefCauss []*memex.DeferredCauset) []int {
	tmpSchema := memex.NewSchema(allDefCauss...)
...
func findMaxPrefixLen(candidates [][]*memex.DeferredCauset, keys []*memex.DeferredCauset) int {
	maxLen := 0
...
func (p *LogicalJoin) moveEqualToOtherConditions(offsets []int) []memex.Expression {
	...
	otherConds := make([]memex.Expression, len(p.OtherConditions), numOtherConds)
...
func (p *LogicalJoin) checkJoinKeyDefCauslation(leftKeys, rightKeys []*memex.DeferredCauset) bool {
...
func (p *LogicalJoin) GetMergeJoin(prop *property.PhysicalProperty, schemaReplicant *memex.Schema, statsInfo *property.StatsInfo, leftStatsInfo *property.StatsInfo, rightStatsInfo *property.StatsInfo) []PhysicalCauset {
	joins := make([]PhysicalCauset, 0, len(p.leftProperties)+1)
	...
	for _, lhsChildProperty := range p.leftProperties {
		...
		rightKeys := memex.NewSchema(rightJoinKeys...).DeferredCausetsByIndices(offsets)
...
func getNewJoinKeysByOffsets(oldJoinKeys []*memex.DeferredCauset, offsets []int) []*memex.DeferredCauset {
	newKeys := make([]*memex.DeferredCauset, 0, len(oldJoinKeys))
	for _, offset := range offsets {
...
func (p *LogicalJoin) getEnforcedMergeJoin(prop *property.PhysicalProperty, schemaReplicant *memex.Schema, statsInfo *property.StatsInfo) []PhysicalCauset {
	...
	otherConditions := make([]memex.Expression, len(p.OtherConditions), len(p.OtherConditions)+len(p.EqualConditions))
	...
	if !p.checkJoinKeyDefCauslation(leftKeys, rightKeys) {
		...
		rightKeys = nil
		newNullEQ = nil
		otherConditions = append(otherConditions, memex.ScalarFuncs2Exprs(p.EqualConditions)...)
	}
	...
	return []PhysicalCauset{enforcedPhysicalMergeJoin}
}

func (p *PhysicalMergeJoin) initCompareFuncs() {
	p.CompareFuncs = make([]memex.CompareFunc, 0, len(p.LeftJoinKeys))
	for i := range p.LeftJoinKeys {
		p.CompareFuncs = append(p.CompareFuncs, memex.GetCmpFunction(p.ctx, p.LeftJoinKeys[i], p.RightJoinKeys[i]))
	}
}
...
func (p *LogicalJoin) getHashJoins(prop *property.PhysicalProperty) []PhysicalCauset {
	if !prop.IsEmpty() { // hash join doesn't promise any orders
		return nil
	}
	joins := make([]PhysicalCauset, 0, 2)
	switch p.JoinType {
...
func (p *LogicalJoin) constructIndexJoin(
	...
	newInnerKeys := make([]*memex.DeferredCauset, 0, len(innerJoinKeys))
	newOuterKeys := make([]*memex.DeferredCauset, 0, len(outerJoinKeys))
	newIsNullEQ := make([]bool, 0, len(isNullEQ))
	newKeyOff := make([]int, 0, len(keyOff2IdxOff))
	newOtherConds := make([]memex.Expression, len(p.OtherConditions), len(p.OtherConditions)+len(p.EqualConditions))
...
func (p *LogicalJoin) constructIndexMergeJoin(
	...
	indexMergeJoins := make([]PhysicalCauset, 0, len(indexJoins))
	for _, plan := range indexJoins {
		...
		compareFuncs := make([]memex.CompareFunc, 0, len(join.OuterJoinKeys))
		outerCompareFuncs := make([]memex.CompareFunc, 0, len(join.OuterJoinKeys))
		...
		for i := range join.KeyOff2IdxOff {
			...
			compareFuncs = append(compareFuncs, memex.GetCmpFunction(p.ctx, join.OuterJoinKeys[i], join.InnerJoinKeys[i]))
			outerCompareFuncs = append(outerCompareFuncs, memex.GetCmpFunction(p.ctx, join.OuterJoinKeys[i], join.OuterJoinKeys[i]))
		}
...
func (p *LogicalJoin) constructIndexHashJoin(
	...
	indexJoins := p.constructIndexJoin(prop, outerIdx, innerTask, ranges, keyOff2IdxOff, path, compareFilters)
	indexHashJoins := make([]PhysicalCauset, 0, len(indexJoins))
	for _, plan := range indexJoins {
		join := plan.(*PhysicalIndexJoin)
...
func (p *LogicalJoin) getIndexJoinByOuterIdx(prop *property.PhysicalProperty, outerIdx int) (joins []PhysicalCauset) {
	...
	if isUnionScan {
		// The child of union scan may be union all for partition causet.
		ds, isDataSource = us.Children()[0].(*DataSource)
		if !isDataSource {
			return nil
		}
		// If one of the union scan children is a TiFlash causet, then we can't choose index join.
...
func (p *LogicalJoin) getIndexJoinBuildHelper(ds *DataSource, innerJoinKeys []*memex.DeferredCauset,
...
func (p *LogicalJoin) buildIndexJoinInner2BlockScan(
	prop *property.PhysicalProperty, ds *DataSource, innerJoinKeys, outerJoinKeys []*memex.DeferredCauset,
	outerIdx int, us *LogicalUnionScan, avgInnerRowCnt float64) (joins []PhysicalCauset) {
	...
	keyOff2IdxOff := make([]int, len(innerJoinKeys))
	newOuterJoinKeys := make([]*memex.DeferredCauset, 0)
...
func (p *LogicalJoin) buildIndexJoinInner2IndexScan(
	...
	if helper.chosenPath.Index.Unique && helper.maxUsedDefCauss == len(helper.chosenPath.FullIdxDefCauss) {
		...
		if l == 0 {
			...
			sf, ok := helper.chosenAccess[l-1].(*memex.ScalarFunction)
...
type indexJoinBuildHelper struct {
	...
	maxUsedDefCauss     int
	chosenAccess    []memex.Expression
	chosenRemained  []memex.Expression
	idxOff2KeyOff   []int
	lastDefCausManager  *DefCausWithCmpFuncManager
	chosenRanges    []*ranger.Range
	chosenPath      *soliton.AccessPath

	curPossibleUsedKeys []*memex.DeferredCauset
	curNotUsedIndexDefCauss []*memex.DeferredCauset
	curNotUsedDefCausLens   []int
	curIdxOff2KeyOff    []int
}

func (ijHelper *indexJoinBuildHelper) buildRangeDecidedByInformation(idxDefCauss []*memex.DeferredCauset, outerJoinKeys []*memex.DeferredCauset) string {
...
func (p *LogicalJoin) constructInnerBlockScanTask(
	...
	// it means the data source is a partition causet reader.
	// If the inner task need to keep order, the partition causet reader can't satisfy it.
	if keepOrder && ds.blockInfo.GetPartitionInfo() != nil {
		return nil
	}
	var ranges []*ranger.Range
	// pk is nil means the causet uses the common handle.
	if pk == nil {
	...
	if len(ts.filterCondition) > 0 {
		...
		if err != nil || selectivity <= 0 {
			logutil.BgLogger().Debug("unexpected selectivity, use selection factor", zap.Float64("selectivity", selectivity), zap.String("causet", ts.BlockAsName.L))
...
func (p *LogicalJoin) constructInnerUnionScan(us *LogicalUnionScan, reader PhysicalCauset) PhysicalCauset {
	if us == nil {
		return reader
	}
...
func (p *LogicalJoin) constructInnerIndexScanTask(
	...
	if !ds.isCoveringIndex(ds.schemaReplicant.DeferredCausets, path.FullIdxDefCauss, path.FullIdxDefCausLens, is.Block) {
		...
		// If inner cop task need keep order, the extraHandleDefCaus should be set.
		if cop.keepOrder && !ds.blockInfo.IsCommonHandle {
			cop.extraHandleDefCaus, cop.doubleReadNeedProj = ts.appendExtraHandleDefCaus(ds)
		}
		cop.blockCauset = ts
	}
	if cop.blockCauset != nil && ds.blockInfo.IsCommonHandle {
		cop.commonHandleDefCauss = ds.commonHandleDefCauss
	}
	is.initSchema(append(path.FullIdxDefCauss, ds.commonHandleDefCauss...), cop.blockCauset != nil)
	...
	// to causet size, to simply make it more expensive.
	if rowCount <= 0 {
	...
	if len(tblConds) > 0 {
		...
		if err != nil || selectivity <= 0 {
			logutil.BgLogger().Debug("unexpected selectivity, use selection factor", zap.Float64("selectivity", selectivity), zap.String("causet", ds.BlockAsName.L))
			selectivity = SelectionFactor
		}
		// rowCount is computed from result event count of join, which has already accounted the filters on DataSource,
...
type DefCausWithCmpFuncManager struct {
	TargetDefCaus         *memex.DeferredCauset
	colLength         int
	OpType            []string
	opArg             []memex.Expression
	TmpConstant       []*memex.Constant
	affectedDefCausSchema *memex.Schema
	compareFuncs      []chunk.CompareFunc
}

func (cwc *DefCausWithCmpFuncManager) appendNewExpr(opName string, arg memex.Expression, affectedDefCauss []*memex.DeferredCauset) {
	cwc.OpType = append(cwc.OpType, opName)
	cwc.opArg = append(cwc.opArg, arg)
	cwc.TmpConstant = append(cwc.TmpConstant, &memex.Constant{RetType: cwc.TargetDefCaus.RetType})
...
func (cwc *DefCausWithCmpFuncManager) BuildRangesByRow(ctx stochastikctx.Context, event chunk.Row) ([]*ranger.Range, error) {
	exprs := make([]memex.Expression, len(cwc.OpType))
	for i, opType := range cwc.OpType {
		...
		newExpr, err := memex.NewFunction(ctx, opType, types.NewFieldType(allegrosql.TypeTiny), cwc.TargetDefCaus, cwc.TmpConstant[i])
		if err != nil {
			return nil, err
		}
...
func (cwc *DefCausWithCmpFuncManager) resolveIndices(schemaReplicant *memex.Schema) (err error) {
...
func (ijHelper *indexJoinBuildHelper) resetContextForIndex(innerKeys []*memex.DeferredCauset, idxDefCauss []*memex.DeferredCauset, colLens []int) {
	tmpSchema := memex.NewSchema(innerKeys...)
	ijHelper.curIdxOff2KeyOff = make([]int, len(idxDefCauss))
	ijHelper.curNotUsedIndexDefCauss = make([]*memex.DeferredCauset, 0, len(idxDefCauss))
...
func (ijHelper *indexJoinBuildHelper) findUsefulEqAndInFilters(innerCauset *DataSource) (usefulEqOrInFilters, uselessFilters, remainingRangeCandidates []memex.Expression) {
	uselessFilters = make([]memex.Expression, 0, len(innerCauset.pushedDownConds))
	var remainedEqOrIn []memex.Expression
	...
	usefulEqOrInFilters, remainedEqOrIn, remainingRangeCandidates, _ = ranger.ExtractEqAndInCondition(
		innerCauset.ctx, innerCauset.pushedDownConds,
...
func (ijHelper *indexJoinBuildHelper) buildLastDefCausManager(nextDefCaus *memex.DeferredCauset,
	innerCauset *DataSource, cwc *DefCausWithCmpFuncManager) []memex.Expression {
	var lastDefCausAccesses []memex.Expression
loopOtherConds:
	for _, filter := range ijHelper.join.OtherConditions {
		sf, ok := filter.(*memex.ScalarFunction)
		if !ok || !(sf.FuncName.L == ast.LE || sf.FuncName.L == ast.LT || sf.FuncName.L == ast.GE || sf.FuncName.L == ast.GT) {
			continue
		}
		var funcName string
		var anotherArg memex.Expression
		if lDefCaus, ok := sf.GetArgs()[0].(*memex.DeferredCauset); ok && lDefCaus.Equal(nil, nextDefCaus) {
			anotherArg = sf.GetArgs()[1]
			funcName = sf.FuncName.L
		} else if rDefCaus, ok := sf.GetArgs()[1].(*memex.DeferredCauset); ok && rDefCaus.Equal(nil, nextDefCaus) {
			anotherArg = sf.GetArgs()[0]
			// The column manager always build memex in the form of col op arg1.
			// So we need use the symmetric one of the current function.
			funcName = symmetricOp[sf.FuncName.L]
		} else {
			continue
		}
		affectedDefCauss := memex.ExtractDeferredCausets(anotherArg)
		if len(affectedDefCauss) == 0 {
			continue
		}
		for _, col := range affectedDefCauss {
			if innerCauset.schemaReplicant.Contains(col) {
...
func (ijHelper *indexJoinBuildHelper) removeUselessEqAndInFunc(
	...
	notKeyEqAndIn []memex.Expression) (
	usefulEqAndIn, uselessOnes []memex.Expression,
) {
	ijHelper.curPossibleUsedKeys = make([]*memex.DeferredCauset, 0, len(idxDefCauss))
	for idxDefCausPos, notKeyDefCausPos := 0, 0; idxDefCausPos < len(idxDefCauss); idxDefCausPos++ {
		...
		remained := make([]memex.Expression, 0, len(notKeyEqAndIn)-notKeyDefCausPos)
...
func (ijHelper *indexJoinBuildHelper) analyzeLookUpFilters(path *soliton.AccessPath, innerCauset *DataSource, innerJoinKeys []*memex.DeferredCauset) (emptyRange bool, err error) {
	if len(path.IdxDefCauss) == 0 {
		return false, nil
	}
	accesses := make([]memex.Expression, 0, len(path.IdxDefCauss))
	ijHelper.resetContextForIndex(innerJoinKeys, path.IdxDefCauss, path.IdxDefCausLens)
	notKeyEqAndIn, remained, rangeFilterCandidates := ijHelper.findUsefulEqAndInFilters(innerCauset)
	var remainedEqAndIn []memex.Expression
	...
	lastDefCausManager := &DefCausWithCmpFuncManager{
		...
		affectedDefCausSchema: memex.NewSchema(),
	}
	lastDefCausAccess := ijHelper.buildLastDefCausManager(lastPossibleDefCaus, innerCauset, lastDefCausManager)
	// If the column manager holds no memex, then we fallback to find whether there're useful normal filters
...
func (ijHelper *indexJoinBuildHelper) uFIDelateBestChoice(ranges []*ranger.Range, path *soliton.AccessPath, accesses,
	remained []memex.Expression, lastDefCausManager *DefCausWithCmpFuncManager) {
...
func (ijHelper *indexJoinBuildHelper) buildTemplateRange(matchedKeyCnt int, eqAndInFuncs []memex.Expression, nextDefCausRange []*ranger.Range, haveExtraDefCaus bool) (ranges []*ranger.Range, emptyRange bool, err error) {
	...
	for i, j := 0, 0; j < len(eqAndInFuncs); i++ {
		...
		oneDeferredCausetRan, err := ranger.BuildDeferredCausetRange([]memex.Expression{eqAndInFuncs[j]}, sc, ijHelper.curNotUsedIndexDefCauss[j].RetType, ijHelper.curNotUsedDefCausLens[j])
		if err != nil {
...
func (p *LogicalJoin) tryToGetIndexJoin(prop *property.PhysicalProperty) (indexJoins []PhysicalCauset, canForced bool) {
	...
	var allLeftOuterJoins, allRightOuterJoins, forcedLeftOuterJoins, forcedRightOuterJoins []PhysicalCauset
	if supportLeftOuter {
		allLeftOuterJoins = p.getIndexJoinByOuterIdx(prop, 0)
		forcedLeftOuterJoins = make([]PhysicalCauset, 0, len(allLeftOuterJoins))
	...
	if supportRightOuter {
		...
		forcedRightOuterJoins = make([]PhysicalCauset, 0, len(allRightOuterJoins))
...
func (p *LogicalJoin) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	failpoint.Inject("MockOnlyEnableIndexHashJoin", func(val failpoint.Value) {
		if val.(bool) {
...
func (p *LogicalJoin) tryToGetBroadCastJoin(prop *property.PhysicalProperty) []PhysicalCauset {
	...
	// Disable broadcast join on partition causet for TiFlash.
...
func (p *LogicalJoin) tryToGetBroadCastJoinByPreferGlobalIdx(prop *property.PhysicalProperty, preferredGlobalIndex int) []PhysicalCauset {
...
func (p *LogicalProjection) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	...
	return []PhysicalCauset{proj}, true
}
...
func (lt *LogicalTopN) canPushToCop() bool {
	...
	// When we push task to interlock, finishCopTask will close the cop task and create a root task in the current implementation.
	// Thus, we can't push two different tasks to interlock now, and can only push task to interlock when the child is Datasource.

	// TODO: develop this function after supporting push several tasks to coprecessor and supporting Projection to interlock.
	_, ok := lt.children[0].(*DataSource)
	return ok
}

func (lt *LogicalTopN) getPhysTopN(prop *property.PhysicalProperty) []PhysicalCauset {
	...
	allTaskTypes := []property.TaskType{property.CopSingleReadTaskType, property.CoFIDeloubleReadTaskType}
	if !lt.limitHints.preferLimitToCop {
		allTaskTypes = append(allTaskTypes, property.RootTaskType)
	}
	ret := make([]PhysicalCauset, 0, len(allTaskTypes))
...
func (lt *LogicalTopN) getPhysLimits(prop *property.PhysicalProperty) []PhysicalCauset {
...
func (lt *LogicalTopN) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
...
// GetHashJoin is public for cascades causet.
...
func (la *LogicalApply) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	...
	var columns []*memex.DeferredCauset
	...
	return []PhysicalCauset{apply}, true
}

func (p *LogicalWindow) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	if prop.IsFlashOnlyProp() {
		return nil, true
	}
	...
	return []PhysicalCauset{window}, true
}

// exhaustPhysicalCausets is only for implementing interface. DataSource and Dual generate task in `findBestTask` directly.
func (p *baseLogicalCauset) exhaustPhysicalCausets(_ *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	panic("baseLogicalCauset.exhaustPhysicalCausets() should never be called.")
}
...
func (la *LogicalAggregation) getEnforcedStreamAggs(prop *property.PhysicalProperty) []PhysicalCauset {
	if prop.IsFlashOnlyProp() {
		return nil
	}
	_, desc := prop.AllSameOrder()
	allTaskTypes := prop.GetAllPossibleChildTaskTypes()
	enforcePosetDaggs := make([]PhysicalCauset, 0, len(allTaskTypes))
	...
	taskTypes := []property.TaskType{property.CopSingleReadTaskType, property.CoFIDeloubleReadTaskType}
	if la.HasDistinct() {
		// TODO: remove AllowDistinctAggPushDown after the cost estimation of distinct pushdown is implemented.
		// If AllowDistinctAggPushDown is set to true, we should not consider RootTask.
		if !la.canPushToCop() || !la.ctx.GetStochastikVars().AllowDistinctAggPushDown {
	...
	for _, taskTp := range taskTypes {
		...
		*copiedChildProperty = *childProp // It's ok to not deep copy the "defcaus" field.
	...
	for _, aggFunc := range la.AggFuncs {
		if aggFunc.HasDistinct {
			for _, distinctArg := range aggFunc.Args {
				if !memex.Contains(la.GroupByItems, distinctArg) {
					return false
				}
			}
		}
...
func (la *LogicalAggregation) getStreamAggs(prop *property.PhysicalProperty) []PhysicalCauset {
	...
	allTaskTypes := prop.GetAllPossibleChildTaskTypes()
	streamAggs := make([]PhysicalCauset, 0, len(la.possibleProperties)*(len(allTaskTypes)-1)+len(allTaskTypes))
	childProp := &property.PhysicalProperty{
		ExpectedCnt: math.Max(prop.ExpectedCnt*la.inputCount/la.stats.RowCount, prop.ExpectedCnt),
	}
	...
	for _, possibleChildProperty := range la.possibleProperties {
		...
		// The causet read of "CoFIDeloubleReadTaskType" can't promises the sort
...
func (la *LogicalAggregation) getHashAggs(prop *property.PhysicalProperty) []PhysicalCauset {
	if !prop.IsEmpty() {
		return nil
	}
	hashAggs := make([]PhysicalCauset, 0, len(prop.GetAllPossibleChildTaskTypes()))
	taskTypes := []property.TaskType{property.CopSingleReadTaskType, property.CoFIDeloubleReadTaskType}
...
func (la *LogicalAggregation) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
...
func (p *LogicalSelection) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	childProp := prop.Clone()
	sel := PhysicalSelection{
		Conditions: p.Conditions,
	}.Init(p.ctx, p.stats.ScaleByExpectCnt(prop.ExpectedCnt), p.blockOffset, childProp)
	return []PhysicalCauset{sel}, true
}
...
func (p *LogicalLimit) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	if !prop.IsEmpty() {
		return nil, true
	}
	// allTaskTypes := prop.GetAllPossibleChildTaskTypes()
...
func (p *LogicalLock) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	if prop.IsFlashOnlyProp() {
		return nil, true
	}
	childProp := prop.Clone()
	...
	return []PhysicalCauset{dagger}, true
}

func (p *LogicalUnionAll) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	...
	return []PhysicalCauset{ua}, true
}

func (p *LogicalPartitionUnionAll) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	uas, flagHint := p.LogicalUnionAll.exhaustPhysicalCausets(prop)
	for _, ua := range uas {
...
func (ls *LogicalSort) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	if MatchItems(prop, ls.ByItems) {
		ret := make([]PhysicalCauset, 0, 2)
...
func (p *LogicalMaxOneRow) exhaustPhysicalCausets(prop *property.PhysicalProperty) ([]PhysicalCauset, bool) {
	if !prop.IsEmpty() || prop.IsFlashOnlyProp() {
		return nil, true
	}
	mor := PhysicalMaxOneRow{}.Init(p.ctx, p.stats, p.blockOffset, &property.PhysicalProperty{ExpectedCnt: 2})
	return []PhysicalCauset{mor}, true
}
