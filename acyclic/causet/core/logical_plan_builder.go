...
const (
	...
	// HintBCJPreferLocal specifies the preferred local read causet
	...
	// HintAggToCop is hint enforce pushing aggregation to interlock.
	...
	// HintIgnoreCausetCache is a hint to enforce ignoring plan cache
	HintIgnoreCausetCache = "ignore_plan_cache"
	// HintLimitToCop is a hint enforce pushing limit or topn to interlock.
...
func (la *LogicalAggregation) collectGroupByDeferredCausets() {
	...
	for _, item := range la.GroupByItems {
		if col, ok := item.(*memex.DeferredCauset); ok {
			la.groupByDefCauss = append(la.groupByDefCauss, col)
		}
	}
}

// aggOrderByResolver is currently resolving memexs of order by clause
// in aggregate function GROUP_CONCAT.
type aggOrderByResolver struct {
	ctx       stochastikctx.Context
	err       error
	args      []ast.ExprNode
	exprDepth int // exprDepth is the depth of current memex in memex tree.
}
...
func (a *aggOrderByResolver) Enter(inNode ast.Node) (ast.Node, bool) {
	...
	case *driver.ParamMarkerExpr:
		if a.exprDepth == 1 {
			...
			// For constant uint memex in top level, it should be treated as position memex.
			if !isNull && isExpectedType {
				return memex.ConstructPositionExpr(n), true
			}
		}
	}
	return inNode, false
...
func (a *aggOrderByResolver) Leave(inNode ast.Node) (ast.Node, bool) {
	...
	case *ast.PositionExpr:
		pos, isNull, err := memex.PosFromPositionExpr(a.ctx, v)
		if err != nil {
			a.err = err
		}
		if err != nil || isNull {
...
func (b *CausetBuilder) builPosetDaggregation(ctx context.Context, p LogicalCauset, aggFuncList []*ast.AggregateFuncExpr, gbyItems []memex.Expression) (LogicalCauset, map[int]int, error) {
	...
	schema4Agg := memex.NewSchema(make([]*memex.DeferredCauset, 0, len(aggFuncList)+p.Schema().Len())...)
	names := make(types.NameSlice, 0, len(aggFuncList)+p.Schema().Len())
	// aggIdxMap maps the old index to new index after applying common aggregation functions elimination.
	aggIndexMap := make(map[int]int)

	for i, aggFunc := range aggFuncList {
		newArgList := make([]memex.Expression, 0, len(aggFunc.Args))
		...
		if !combined {
			...
			schema4Agg.Append(&memex.DeferredCauset{
				UniqueID: b.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
	...
	for i, col := range p.Schema().DeferredCausets {
		newFunc, err := aggregation.NewAggFuncDesc(b.ctx, ast.AggFuncFirstRow, []memex.Expression{col}, false)
		if err != nil {
			return nil, nil, err
		}
		plan4Agg.AggFuncs = append(plan4Agg.AggFuncs, newFunc)
		newDefCaus, _ := col.Clone().(*memex.DeferredCauset)
...
func (b *CausetBuilder) buildResultSetNode(ctx context.Context, node ast.ResultSetNode) (p LogicalCauset, err error) {
	switch x := node.(type) {
	case *ast.Join:
	...
	case *ast.BlockSource:
		...
		// `BlockName` is not a select causet, so we do not need to handle it.
		if !isBlockName && b.ctx.GetStochastikVars().CausetAppendSelectBlockAsName != nil {
			b.ctx.GetStochastikVars().CausetAppendSelectBlockAsName[p.SelectBlockOffset()] = ast.HintBlock{DBName: p.OutputNames()[0].DBName, BlockName: p.OutputNames()[0].TblName}
		}
		// Duplicate column name in one causet is not allowed.
...
func (p *LogicalJoin) pushDownConstExpr(expr memex.Expression, leftCond []memex.Expression,
	rightCond []memex.Expression, filterCond bool) ([]memex.Expression, []memex.Expression) {
	switch p.JoinType {
...
func (p *LogicalJoin) extractOnCondition(conditions []memex.Expression, deriveLeft bool,
	deriveRight bool) (eqCond []*memex.ScalarFunction, leftCond []memex.Expression,
	rightCond []memex.Expression, otherCond []memex.Expression) {
...
func (p *LogicalJoin) ExtractOnCondition(
	...
	leftSchema *memex.Schema,
	rightSchema *memex.Schema,
	deriveLeft bool,
	deriveRight bool) (eqCond []*memex.ScalarFunction, leftCond []memex.Expression,
	rightCond []memex.Expression, otherCond []memex.Expression) {
	for _, expr := range conditions {
				...
				if leftDefCaus != nil && rightDefCaus != nil {
					if deriveLeft {
						if isNullRejected(ctx, leftSchema, expr) && !allegrosql.HasNotNullFlag(leftDefCaus.RetType.Flag) {
							notNullExpr := memex.BuildNotNullExpr(ctx, leftDefCaus)
							leftCond = append(leftCond, notNullExpr)
						}
					}
					if deriveRight {
						if isNullRejected(ctx, rightSchema, expr) && !allegrosql.HasNotNullFlag(rightDefCaus.RetType.Flag) {
							notNullExpr := memex.BuildNotNullExpr(ctx, rightDefCaus)
					...
					if binop.FuncName.L == ast.EQ && !arg0.InOperand && !arg1.InOperand {
						cond := memex.NewFunctionInternal(ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), arg0, arg1)
						eqCond = append(eqCond, cond.(*memex.ScalarFunction))
						continue
					}
				}
			}
		}
		columns := memex.ExtractDeferredCausets(expr)
		...
		if allFromRight {
			...
			// Relax expr to two supersets: leftRelaxedCond and rightRelaxedCond, the memex now is
			// `expr AND leftRelaxedCond AND rightRelaxedCond`. Motivation is to push filters down to
			// children as much as possible.
			if deriveLeft {
				leftRelaxedCond := memex.DeriveRelaxedFiltersFromDNF(expr, leftSchema)
				if leftRelaxedCond != nil {
					leftCond = append(leftCond, leftRelaxedCond)
				}
			}
			if deriveRight {
				rightRelaxedCond := memex.DeriveRelaxedFiltersFromDNF(expr, rightSchema)
...
// extractBlockAlias returns causet alias of the LogicalCauset's columns.
// It will return nil when there are multiple causet alias, because the alias is only used to check if
// the logicalCauset match some optimizer hints, and hints are not expected to take effect in this case.
func extractBlockAlias(p Causet, parentOffset int) *hintBlockInfo {
	if len(p.OutputNames()) > 0 && p.OutputNames()[0].TblName.L != "" {
		...
		blockAsNames := p.SCtx().GetStochastikVars().CausetAppendSelectBlockAsName
...
func (ds *DataSource) setPreferredStoreType(hintInfo *blockHintInfo) {
	...
	if hintTbl := hintInfo.ifPreferEinsteinDB(alias); hintTbl != nil {
		...
		if ds.preferStoreType&preferEinsteinDB == 0 {
			errMsg := fmt.Sprintf("No available path for causet %s.%s with the causetstore type %s of the hint /*+ read_from_storage */, "+
				"please check the status of the causet replica and variable value of milevadb_isolation_read_engines(%v)",
				ds.DBName.O, ds.causet.Meta().Name.O, ekv.EinsteinDB.Name(), ds.ctx.GetStochastikVars().GetIsolationReadEngines())
			warning := ErrInternal.GenWithStack(errMsg)
			ds.ctx.GetStochastikVars().StmtCtx.AppendWarning(warning)
		}
	}
	if hintTbl := hintInfo.ifPreferTiFlash(alias); hintTbl != nil {
		// 1. `ds.blockInfo.Partition == nil`, which means the hint takes effect in the whole causet.
		// 2. `ds.preferStoreType != 0`, which means there's a hint hit the both EinsteinDB value and TiFlash value for causet.
		...
		if ds.preferStoreType != 0 && ds.blockInfo.Partition == nil {
			errMsg := fmt.Sprintf("CausetStorage hints are conflict, you can only specify one storage type of causet %s.%s",
		...
		if ds.preferStoreType&preferTiFlash == 0 {
			errMsg := fmt.Sprintf("No available path for causet %s.%s with the causetstore type %s of the hint /*+ read_from_storage */, "+
				"please check the status of the causet replica and variable value of milevadb_isolation_read_engines(%v)",
				ds.DBName.O, ds.causet.Meta().Name.O, ekv.TiFlash.Name(), ds.ctx.GetStochastikVars().GetIsolationReadEngines())
			warning := ErrInternal.GenWithStack(errMsg)
			ds.ctx.GetStochastikVars().StmtCtx.AppendWarning(warning)
		}
	}
...
func resetNotNullFlag(schemaReplicant *memex.Schema, start, end int) {
	for i := start; i < end; i++ {
...
func (b *CausetBuilder) buildJoin(ctx context.Context, joinNode *ast.Join) (LogicalCauset, error) {
	// We will construct a "Join" node for some memexs like "INSERT",
	...
	leftCauset, err := b.buildResultSetNode(ctx, joinNode.Left)
	if err != nil {
		return nil, err
	}

	rightCauset, err := b.buildResultSetNode(ctx, joinNode.Right)
	if err != nil {
		return nil, err
	}
	...
	joinCauset := LogicalJoin{StraightJoin: joinNode.StraightJoin || b.inStraightJoin}.Init(b.ctx, b.getSelectOffset())
	joinCauset.SetChildren(leftCauset, rightCauset)
	joinCauset.SetSchema(memex.MergeSchema(leftCauset.Schema(), rightCauset.Schema()))
	joinCauset.names = make([]*types.FieldName, leftCauset.Schema().Len()+rightCauset.Schema().Len())
	copy(joinCauset.names, leftCauset.OutputNames())
	copy(joinCauset.names[leftCauset.Schema().Len():], rightCauset.OutputNames())
	...
	case ast.LeftJoin:
		...
		joinCauset.JoinType = LeftOuterJoin
		resetNotNullFlag(joinCauset.schemaReplicant, leftCauset.Schema().Len(), joinCauset.schemaReplicant.Len())
	case ast.RightJoin:
		// right outer join need to be checked elimination
		b.optFlag = b.optFlag | flagEliminateOuterJoin
		joinCauset.JoinType = RightOuterJoin
		resetNotNullFlag(joinCauset.schemaReplicant, 0, leftCauset.Schema().Len())
	default:
		b.optFlag = b.optFlag | flagJoinReOrder
		joinCauset.JoinType = InnerJoin
	}
	...
	var (
		lRedundantSchema, rRedundantSchema *memex.Schema
		lRedundantNames, rRedundantNames   types.NameSlice
	)
	if left, ok := leftCauset.(*LogicalJoin); ok && left.redundantSchema != nil {
		lRedundantSchema = left.redundantSchema
		lRedundantNames = left.redundantNames
	}
	if right, ok := rightCauset.(*LogicalJoin); ok && right.redundantSchema != nil {
		rRedundantSchema = right.redundantSchema
		rRedundantNames = right.redundantNames
	}
	joinCauset.redundantSchema = memex.MergeSchema(lRedundantSchema, rRedundantSchema)
	joinCauset.redundantNames = make([]*types.FieldName, len(lRedundantNames)+len(rRedundantNames))
	copy(joinCauset.redundantNames, lRedundantNames)
	copy(joinCauset.redundantNames[len(lRedundantNames):], rRedundantNames)

	// Set preferred join algorithm if some join hints is specified by user.
	joinCauset.setPreferredJoinType(b.BlockHints())
	...
	if joinNode.NaturalJoin {
		err = b.buildNaturalJoin(joinCauset, leftCauset, rightCauset, joinNode)
		if err != nil {
			return nil, err
		}
	} else if joinNode.Using != nil {
		err = b.buildUsingClause(joinCauset, leftCauset, rightCauset, joinNode)
		if err != nil {
			return nil, err
		}
	} else if joinNode.On != nil {
		b.curClause = onClause
		onExpr, newCauset, err := b.rewrite(ctx, joinNode.On.Expr, joinCauset, nil, false)
		if err != nil {
			return nil, err
		}
		if newCauset != joinCauset {
			return nil, errors.New("ON condition doesn't support subqueries yet")
		}
		onCondition := memex.SplitCNFItems(onExpr)
		joinCauset.AttachOnConds(onCondition)
	} else if joinCauset.JoinType == InnerJoin {
		// If a inner join without "ON" or "USING" clause, it's a cartesian
		// product over the join blocks.
		joinCauset.cartesianJoin = true
	}

	return joinCauset, nil
}
...
// 1. coalesced common columns of "leftCauset" and "rightCauset", in the order they
//    appears in "leftCauset".
// 2. the rest columns in "leftCauset", in the order they appears in "leftCauset".
// 3. the rest columns in "rightCauset", in the order they appears in "rightCauset".
func (b *CausetBuilder) buildUsingClause(p *LogicalJoin, leftCauset, rightCauset LogicalCauset, join *ast.Join) error {
	filter := make(map[string]bool, len(join.Using))
	for _, col := range join.Using {
		filter[col.Name.L] = true
	}
	return b.coalesceCommonDeferredCausets(p, leftCauset, rightCauset, join.Tp, filter)
}
...
// 	Every column in the first (left) causet that is not a common column
// 	Every column in the second (right) causet that is not a common column
func (b *CausetBuilder) buildNaturalJoin(p *LogicalJoin, leftCauset, rightCauset LogicalCauset, join *ast.Join) error {
	return b.coalesceCommonDeferredCausets(p, leftCauset, rightCauset, join.Tp, nil)
}

// coalesceCommonDeferredCausets is used by buildUsingClause and buildNaturalJoin. The filter is used by buildUsingClause.
func (b *CausetBuilder) coalesceCommonDeferredCausets(p *LogicalJoin, leftCauset, rightCauset LogicalCauset, joinTp ast.JoinType, filter map[string]bool) error {
	lsc := leftCauset.Schema().Clone()
	rsc := rightCauset.Schema().Clone()
	...
	lNames, rNames := leftCauset.OutputNames().Shallow(), rightCauset.OutputNames().Shallow()
	...
	for i, lName := range lNames {
		for j := commonLen; j < len(rNames); j++ {
			...
			copy(lDeferredCausets[commonLen+1:i+1], lDeferredCausets[commonLen:i])
			...
			copy(rDeferredCausets[commonLen+1:j+1], rDeferredCausets[commonLen:j])
	...
	schemaDefCauss := make([]*memex.DeferredCauset, len(lDeferredCausets)+len(rDeferredCausets)-commonLen)
	copy(schemaDefCauss[:len(lDeferredCausets)], lDeferredCausets)
	copy(schemaDefCauss[len(lDeferredCausets):], rDeferredCausets[commonLen:])
	names := make(types.NameSlice, len(schemaDefCauss))
	copy(names, lNames)
	copy(names[len(lNames):], rNames[commonLen:])

	conds := make([]memex.Expression, 0, commonLen)
	for i := 0; i < commonLen; i++ {
		lc, rc := lsc.DeferredCausets[i], rsc.DeferredCausets[i]
		cond, err := memex.NewFunction(b.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), lc, rc)
		if err != nil {
			return err
		}
	...
	p.SetSchema(memex.NewSchema(schemaDefCauss...))
	p.names = names
	p.redundantSchema = memex.MergeSchema(p.redundantSchema, memex.NewSchema(rDeferredCausets[:commonLen]...))
...
func (b *CausetBuilder) buildSelection(ctx context.Context, p LogicalCauset, where ast.ExprNode, AggMapper map[*ast.AggregateFuncExpr]int) (LogicalCauset, error) {
	b.optFlag |= flagPredicatePushDown
	...
	memexs := make([]memex.Expression, 0, len(conditions))
	...
	for _, cond := range conditions {
		...
		cnfItems := memex.SplitCNFItems(expr)
		for _, item := range cnfItems {
			if con, ok := item.(*memex.Constant); ok && con.DeferredExpr == nil && con.ParamMarker == nil {
				ret, _, err := memex.EvalBool(b.ctx, memex.CNFExprs{con}, chunk.Row{})
				if err != nil || ret {
					continue
				}
			...
			memexs = append(memexs, item)
		}
	}
	if len(memexs) == 0 {
		return p, nil
	}
	selection.Conditions = memexs
	selection.SetChildren(p)
	return selection, nil
}

// buildProjectionFieldNameFromDeferredCausets builds the field name, causet name and database name when field memex is a column reference.
func (b *CausetBuilder) buildProjectionFieldNameFromDeferredCausets(origField *ast.SelectField, colNameField *ast.DeferredCausetNameExpr, name *types.FieldName) (colName, origDefCausName, tblName, origTblName, dbName perceptron.CIStr) {
...
// buildProjectionFieldNameFromExpressions builds the field name when field memex is a normal memex.
func (b *CausetBuilder) buildProjectionFieldNameFromExpressions(ctx context.Context, field *ast.SelectField) (perceptron.CIStr, error) {
...
func (b *CausetBuilder) buildProjectionField(ctx context.Context, p LogicalCauset, field *ast.SelectField, expr memex.Expression) (*memex.DeferredCauset, *types.FieldName, error) {
	...
	// Correlated column won't affect the final output names. So we can put it in any of the three logic causet.
	// Don't put it into the first causet just for simplifying the codes.
...
func (b *CausetBuilder) buildProjection(ctx context.Context, p LogicalCauset, fields []*ast.SelectField, mapper map[*ast.AggregateFuncExpr]int, windowMapper map[*ast.WindowFuncExpr]int, considerWindow bool, expandGenerateDeferredCauset bool) (LogicalCauset, int, error) {
	b.optFlag |= flagEliminateProjection
	b.curClause = fieldList
	proj := LogicalProjection{Exprs: make([]memex.Expression, 0, len(fields))}.Init(b.ctx, b.getSelectOffset())
	schemaReplicant := memex.NewSchema(make([]*memex.DeferredCauset, 0, len(fields))...)
	oldLen := 0
...
func (b *CausetBuilder) buildDistinct(child LogicalCauset, length int) (*LogicalAggregation, error) {
	b.optFlag = b.optFlag | flagBuildKeyInfo
	b.optFlag = b.optFlag | flagPushDownAgg
	plan4Agg := LogicalAggregation{
		AggFuncs:     make([]*aggregation.AggFuncDesc, 0, child.Schema().Len()),
		GroupByItems: memex.DeferredCauset2Exprs(child.Schema().Clone().DeferredCausets[:length]),
	...
	for _, col := range child.Schema().DeferredCausets {
		aggDesc, err := aggregation.NewAggFuncDesc(b.ctx, ast.AggFuncFirstRow, []memex.Expression{col}, false)
		if err != nil {
			return nil, err
		}
...
func unionJoinFieldType(a, b *types.FieldType) *types.FieldType {
	...
	if resultTp.Tp == allegrosql.TypeNewDecimal {
		...
		// Non-decimal results will be unsigned when the first ALLEGROALLEGROSQL memex result in the union is unsigned.
	...
	memex.SetBinFlagOrBinStr(b, resultTp)
	return resultTp
}

func (b *CausetBuilder) buildProjection4Union(ctx context.Context, u *LogicalUnionAll) {
	unionDefCauss := make([]*memex.DeferredCauset, 0, u.children[0].Schema().Len())
	...
	for i, col := range u.children[0].Schema().DeferredCausets {
		...
		unionDefCauss = append(unionDefCauss, &memex.DeferredCauset{
			RetType:  resultTp,
			UniqueID: b.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
		})
	}
	u.schemaReplicant = memex.NewSchema(unionDefCauss...)
	u.names = names
	...
	for childID, child := range u.children {
		exprs := make([]memex.Expression, len(child.Schema().DeferredCausets))
		for i, srcDefCaus := range child.Schema().DeferredCausets {
			dstType := unionDefCauss[i].RetType
			srcType := srcDefCaus.RetType
			if !srcType.Equal(dstType) {
				exprs[i] = memex.BuildCastFunction4Union(b.ctx, srcDefCaus, dstType)
			} else {
				exprs[i] = srcDefCaus
			}
		}
...
func (b *CausetBuilder) buildSetOpr(ctx context.Context, setOpr *ast.SetOprStmt) (LogicalCauset, error) {
	// Because INTERSECT has higher precedence than UNION and EXCEPT. We build it first.
	selectCausets := make([]LogicalCauset, 0, len(setOpr.SelectList.Selects))
	...
	for i := 0; i < len(selects); i++ {
		...
		selectCauset, afterSetOpr, err := b.buildIntersect(ctx, intersects)
		if err != nil {
			return nil, err
		}
		selectCausets = append(selectCausets, selectCauset)
		afterSetOprs = append(afterSetOprs, afterSetOpr)
	}
	setOprCauset, err := b.buildExcept(ctx, selectCausets, afterSetOprs)
	if err != nil {
		return nil, err
	}

	oldLen := setOprCauset.Schema().Len()
	...
	if setOpr.OrderBy != nil {
		setOprCauset, err = b.buildSort(ctx, setOprCauset, setOpr.OrderBy.Items, nil, nil)
		if err != nil {
			return nil, err
		}
	}
	...
	if setOpr.Limit != nil {
		setOprCauset, err = b.buildLimit(setOprCauset, setOpr.Limit)
		if err != nil {
			return nil, err
		}
	}
	...
	// If there are extra memexs generated from `ORDER BY` clause, generate a `Projection` to remove them.
	if oldLen != setOprCauset.Schema().Len() {
		proj := LogicalProjection{Exprs: memex.DeferredCauset2Exprs(setOprCauset.Schema().DeferredCausets[:oldLen])}.Init(b.ctx, b.getSelectOffset())
		proj.SetChildren(setOprCauset)
		schemaReplicant := memex.NewSchema(setOprCauset.Schema().Clone().DeferredCausets[:oldLen]...)
		for _, col := range schemaReplicant.DeferredCausets {
			col.UniqueID = b.ctx.GetStochastikVars().AllocCausetDeferredCausetID()
		}
		proj.names = setOprCauset.OutputNames()[:oldLen]
		proj.SetSchema(schemaReplicant)
		return proj, nil
	}
	return setOprCauset, nil
}

func (b *CausetBuilder) buildSemiJoinForSetOperator(
	leftOriginCauset LogicalCauset,
	rightCauset LogicalCauset,
	joinType JoinType) (leftCauset LogicalCauset, err error) {
	leftCauset, err = b.buildDistinct(leftOriginCauset, leftOriginCauset.Schema().Len())
	if err != nil {
		return nil, err
	}
	joinCauset := LogicalJoin{JoinType: joinType}.Init(b.ctx, b.getSelectOffset())
	joinCauset.SetChildren(leftCauset, rightCauset)
	joinCauset.SetSchema(leftCauset.Schema())
	joinCauset.names = make([]*types.FieldName, leftCauset.Schema().Len())
	copy(joinCauset.names, leftCauset.OutputNames())
	for j := 0; j < len(rightCauset.Schema().DeferredCausets); j++ {
		leftDefCaus, rightDefCaus := leftCauset.Schema().DeferredCausets[j], rightCauset.Schema().DeferredCausets[j]
		eqCond, err := memex.NewFunction(b.ctx, ast.NullEQ, types.NewFieldType(allegrosql.TypeTiny), leftDefCaus, rightDefCaus)
		if err != nil {
			return nil, err
		}
		if leftDefCaus.RetType.Tp != rightDefCaus.RetType.Tp {
			joinCauset.OtherConditions = append(joinCauset.OtherConditions, eqCond)
		} else {
			joinCauset.EqualConditions = append(joinCauset.EqualConditions, eqCond.(*memex.ScalarFunction))
		}
	}
	return joinCauset, nil
}
...
// higher precedence.
func (b *CausetBuilder) buildIntersect(ctx context.Context, selects []*ast.SelectStmt) (LogicalCauset, *ast.SetOprType, error) {
	leftCauset, err := b.buildSelect(ctx, selects[0])
	if err != nil {
		return nil, nil, err
	}
	if len(selects) == 1 {
		return leftCauset, selects[0].AfterSetOperator, nil
	}

	columnNums := leftCauset.Schema().Len()
	for i := 1; i < len(selects); i++ {
		rightCauset, err := b.buildSelect(ctx, selects[i])
		if err != nil {
			return nil, nil, err
		}
		if rightCauset.Schema().Len() != columnNums {
			return nil, nil, ErrWrongNumberOfDeferredCausetsInSelect.GenWithStackByArgs()
		}
		leftCauset, err = b.buildSemiJoinForSetOperator(leftCauset, rightCauset, SemiJoin)
		if err != nil {
			return nil, nil, err
		}
	}
...
func (b *CausetBuilder) buildExcept(ctx context.Context, selects []LogicalCauset, afterSetOpts []*ast.SetOprType) (LogicalCauset, error) {
	unionCausets := []LogicalCauset{selects[0]}
	tmpAfterSetOpts := []*ast.SetOprType{nil}
	columnNums := selects[0].Schema().Len()
	for i := 1; i < len(selects); i++ {
		rightCauset := selects[i]
		if rightCauset.Schema().Len() != columnNums {
			return nil, ErrWrongNumberOfDeferredCausetsInSelect.GenWithStackByArgs()
		}
		if *afterSetOpts[i] == ast.Except {
			leftCauset, err := b.buildUnion(ctx, unionCausets, tmpAfterSetOpts)
			if err != nil {
				return nil, err
			}
			leftCauset, err = b.buildSemiJoinForSetOperator(leftCauset, rightCauset, AntiSemiJoin)
			if err != nil {
				return nil, err
			}
			unionCausets = []LogicalCauset{leftCauset}
			tmpAfterSetOpts = []*ast.SetOprType{nil}
		} else {
			unionCausets = append(unionCausets, rightCauset)
			tmpAfterSetOpts = append(tmpAfterSetOpts, afterSetOpts[i])
		}
	}
	return b.buildUnion(ctx, unionCausets, tmpAfterSetOpts)
}

func (b *CausetBuilder) buildUnion(ctx context.Context, selects []LogicalCauset, afterSetOpts []*ast.SetOprType) (LogicalCauset, error) {
	if len(selects) == 1 {
		return selects[0], nil
	}
	distinctSelectCausets, allSelectCausets, err := b.divideUnionSelectCausets(ctx, selects, afterSetOpts)
	if err != nil {
		return nil, err
	}

	unionDistinctCauset := b.buildUnionAll(ctx, distinctSelectCausets)
	if unionDistinctCauset != nil {
		unionDistinctCauset, err = b.buildDistinct(unionDistinctCauset, unionDistinctCauset.Schema().Len())
		if err != nil {
			return nil, err
		}
		if len(allSelectCausets) > 0 {
			// Can't change the memexs order in order to get the correct column info.
			allSelectCausets = append([]LogicalCauset{unionDistinctCauset}, allSelectCausets...)
		}
	}

	unionAllCauset := b.buildUnionAll(ctx, allSelectCausets)
	unionCauset := unionDistinctCauset
	if unionAllCauset != nil {
		unionCauset = unionAllCauset
	}

	return unionCauset, nil
}

// divideUnionSelectCausets resolves union's select stmts to logical plans.
...
func (b *CausetBuilder) divideUnionSelectCausets(ctx context.Context, selects []LogicalCauset, setOprTypes []*ast.SetOprType) (distinctSelects []LogicalCauset, allSelects []LogicalCauset, err error) {
	firstUnionAllIdx := 0
	columnNums := selects[0].Schema().Len()
...
func (b *CausetBuilder) buildUnionAll(ctx context.Context, subCauset []LogicalCauset) LogicalCauset {
	if len(subCauset) == 0 {
		return nil
	}
	u := LogicalUnionAll{}.Init(b.ctx, b.getSelectOffset())
	u.children = subCauset
...
func (t *itemTransformer) Enter(inNode ast.Node) (ast.Node, bool) {
	switch n := inNode.(type) {
	case *driver.ParamMarkerExpr:
		newNode := memex.ConstructPositionExpr(n)
		return newNode, true
	}
	return inNode, false
}
...
func (b *CausetBuilder) buildSort(ctx context.Context, p LogicalCauset, byItems []*ast.ByItem, aggMapper map[*ast.AggregateFuncExpr]int, windowMapper map[*ast.WindowFuncExpr]int) (*LogicalSort, error) {
...
// For ordinary memex, node should be uint64 constant value.
// For prepared memex, node is string. We should convert it to uint64.
func getUintFromNode(ctx stochastikctx.Context, n ast.Node) (uVal uint64, isNull bool, isExpectedType bool) {
	...
	case *driver.ParamMarkerExpr:
		...
		param, err := memex.ParamMarkerExpression(ctx, v)
		if err != nil {
			return 0, false, false
		}
		str, isNull, err := memex.GetStringFromConstant(ctx, param)
		if err != nil {
			return 0, false, false
		}
		if isNull {
...
func (b *CausetBuilder) buildLimit(src LogicalCauset, limit *ast.Limit) (LogicalCauset, error) {
...
func matchField(f *ast.SelectField, col *ast.DeferredCausetNameExpr, ignoreAsName bool) bool {
	// if col specify a causet name, resolve from causet source directly.
	if col.Name.Block.L == "" {
		if f.AsName.L == "" || ignoreAsName {
			...
			// a memex without as name can't be matched.
			return false
		}
...
func (a *havingWindowAndOrderbyExprResolver) resolveFromCauset(v *ast.DeferredCausetNameExpr, p LogicalCauset) (int, error) {
	idx, err := memex.FindFieldName(p.OutputNames(), v.Name)
	if err != nil {
		return -1, err
	}
	if idx < 0 {
...
func (a *havingWindowAndOrderbyExprResolver) Leave(n ast.Node) (node ast.Node, ok bool) {
	...
	case *ast.DeferredCausetNameExpr:
		...
		if resolveFieldsFirst {
			...
			if index == -1 {
				if a.curClause == orderByClause {
					index, a.err = a.resolveFromCauset(v, a.p)
			...
			var err error
			index, err = a.resolveFromCauset(v, a.p)
			_ = err
		...
		if index == -1 {
			...
			for _, names := range a.outerNames {
				idx, err1 := memex.FindFieldName(names, v.Name)
				if err1 != nil {
					a.err = err1
					return node, false
				}
...
// When we rewrite the order by / having memex, we will find column in map at first.
func (b *CausetBuilder) resolveHavingAndOrderBy(sel *ast.SelectStmt, p LogicalCauset) (
...
func (b *CausetBuilder) extractAggFuncs(fields []*ast.SelectField) ([]*ast.AggregateFuncExpr, map[*ast.AggregateFuncExpr]int) {
...
func (b *CausetBuilder) resolveWindowFunction(sel *ast.SelectStmt, p LogicalCauset) (
...
func (g *gbyResolver) Leave(inNode ast.Node) (ast.Node, bool) {
	...
	switch v := inNode.(type) {
	case *ast.DeferredCausetNameExpr:
		idx, err := memex.FindFieldName(g.names, v.Name)
		if idx < 0 || !g.inExpr {
			var index int
	...
	case *ast.PositionExpr:
		pos, isNull, err := memex.PosFromPositionExpr(g.ctx, v)
		if err != nil {
		...
		if pos < 1 || pos > len(g.fields) {
			g.err = errors.Errorf("Unknown column '%d' in 'group memex'", pos)
			return inNode, false
		}
...
func buildFuncDependDefCaus(p LogicalCauset, cond ast.ExprNode) (*types.FieldName, *types.FieldName) {
	...
	lIdx, err := memex.FindFieldName(p.OutputNames(), lDefCausExpr.Name)
	if err != nil {
		return nil, nil
	}
	rIdx, err := memex.FindFieldName(p.OutputNames(), rDefCausExpr.Name)
	if err != nil {
		return nil, nil
	}
...
func buildWhereFuncDepend(p LogicalCauset, where ast.ExprNode) map[*types.FieldName]*types.FieldName {
...
func buildJoinFuncDepend(p LogicalCauset, from ast.ResultSetNode) map[*types.FieldName]*types.FieldName {
	switch x := from.(type) {
	case *ast.Join:
		if x.On == nil {
			return nil
...
func checkDefCausFuncDepend(
	...
	for _, index := range tblInfo.Indices {
		...
		for _, indexDefCaus := range index.DeferredCausets {
			...
			iIdx, err := memex.FindFieldName(p.OutputNames(), cn)
			if err != nil || iIdx < 0 {
				funcDepend = false
				break
			}
	...
	for _, colInfo := range tblInfo.DeferredCausets {
		...
		pIdx, err := memex.FindFieldName(p.OutputNames(), pkName)
		if err != nil {
...
func (b *CausetBuilder) checkOnlyFullGroupBy(p LogicalCauset, sel *ast.SelectStmt) (err error) {
	if sel.GroupBy != nil {
...
func (b *CausetBuilder) checkOnlyFullGroupByWithGroupClause(p LogicalCauset, sel *ast.SelectStmt) error {
	...
	for _, byItem := range sel.GroupBy.Items {
		...
		if colExpr, ok := expr.(*ast.DeferredCausetNameExpr); ok {
			idx, err := memex.FindFieldName(p.OutputNames(), colExpr.Name)
			if err != nil || idx < 0 {
				continue
			}
...
func (b *CausetBuilder) checkOnlyFullGroupByWithOutGroupClause(p LogicalCauset, fields []*ast.SelectField) error {
...
func (c *colNameResolver) Leave(inNode ast.Node) (ast.Node, bool) {
	switch v := inNode.(type) {
	case *ast.DeferredCausetNameExpr:
		idx, err := memex.FindFieldName(c.p.OutputNames(), v.Name)
		if err == nil && idx >= 0 {
...
func allDefCausFromExprNode(p LogicalCauset, n ast.Node, names map[*types.FieldName]struct{}) {
...
func (b *CausetBuilder) resolveGbyExprs(ctx context.Context, p LogicalCauset, gby *ast.GroupByClause, fields []*ast.SelectField) (LogicalCauset, []memex.Expression, error) {
	b.curClause = groupByClause
	exprs := make([]memex.Expression, 0, len(gby.Items))
...
func (b *CausetBuilder) unfoldWildStar(p LogicalCauset, selectFields []*ast.SelectField) (resultList []*ast.SelectField, err error) {
...
func (b *CausetBuilder) pushHintWithoutBlockWarning(hint *ast.BlockOptimizerHint) {
	var sb strings.Builder
	ctx := format.NewRestoreCtx(0, &sb)
	if err := hint.Restore(ctx); err != nil {
		return
	}
	errMsg := fmt.Sprintf("Hint %s is inapplicable. Please specify the causet names in the arguments.", sb.String())
	b.ctx.GetStochastikVars().StmtCtx.AppendWarning(ErrInternal.GenWithStack(errMsg))
}

func (b *CausetBuilder) pushBlockHints(hints []*ast.BlockOptimizerHint, nodeType utilhint.NodeType, currentLevel int) {
	...
	for _, hint := range hints {
		// Set warning for the hint that requires the causet name.
...
func (b *CausetBuilder) popBlockHints() {
...
func (b *CausetBuilder) appendUnmatchedIndexHintWarning(indexHints []indexHintInfo, usedForIndexMerge bool) {
	for _, hint := range indexHints {
		if !hint.matched {
			...
			errMsg := fmt.Sprintf("%s(%s) is inapplicable, check whether the causet(%s.%s) exists",
				hintTypeString,
				hint.indexString(),
				hint.dbName,
				hint.tblName,
...
func (b *CausetBuilder) appendUnmatchedJoinHintWarning(joinType string, joinTypeAlias string, hintBlocks []hintBlockInfo) {
	...
	errMsg := fmt.Sprintf("There are no matching causet names for (%s) in optimizer hint %s%s. Maybe you can use the causet alias name",
		strings.Join(unMatchedBlocks, ", "), restore2JoinHint(joinType, hintBlocks), joinTypeAlias)
	b.ctx.GetStochastikVars().StmtCtx.AppendWarning(ErrInternal.GenWithStack(errMsg))
}

func (b *CausetBuilder) appendUnmatchedStorageHintWarning(tiflashBlocks, einsteindbBlocks []hintBlockInfo) {
	...
	errMsg := fmt.Sprintf("There are no matching causet names for (%s) in optimizer hint %s. Maybe you can use the causet alias name",
		strings.Join(append(unMatchedTiFlashBlocks, unMatchedEinsteinDBBlocks...), ", "),
		restore2StorageHint(tiflashBlocks, einsteindbBlocks))
	b.ctx.GetStochastikVars().StmtCtx.AppendWarning(ErrInternal.GenWithStack(errMsg))
}

// BlockHints returns the *blockHintInfo of CausetBuilder.
func (b *CausetBuilder) BlockHints() *blockHintInfo {
...
func (b *CausetBuilder) buildSelect(ctx context.Context, sel *ast.SelectStmt) (p LogicalCauset, err error) {
	b.pushSelectOffset(sel.QueryBlockOffset)
	b.pushBlockHints(sel.BlockHints, utilhint.TypeSelect, sel.QueryBlockOffset)
	defer func() {
		b.popSelectOffset()
		// causet hints are only visible in the current SELECT memex.
		b.popBlockHints()
	}()
	enableNoopFuncs := b.ctx.GetStochastikVars().EnableNoopFuncs
	if sel.SelectStmtOpts != nil {
		if sel.SelectStmtOpts.CalcFoundRows && !enableNoopFuncs {
			err = memex.ErrFunctionsNoopImpl.GenWithStackByArgs("ALLEGROSQL_CALC_FOUND_ROWS")
			return nil, err
		}
	...
	if sel.LockInfo != nil && sel.LockInfo.LockType != ast.SelectLockNone {
		if sel.LockInfo.LockType == ast.SelectLockInShareMode && !enableNoopFuncs {
			err = memex.ErrFunctionsNoopImpl.GenWithStackByArgs("LOCK IN SHARE MODE")
			return nil, err
		}
	...
	if oldLen != p.Schema().Len() {
		proj := LogicalProjection{Exprs: memex.DeferredCauset2Exprs(p.Schema().DeferredCausets[:oldLen])}.Init(b.ctx, b.getSelectOffset())
		proj.SetChildren(p)
		schemaReplicant := memex.NewSchema(p.Schema().Clone().DeferredCausets[:oldLen]...)
		for _, col := range schemaReplicant.DeferredCausets {
			col.UniqueID = b.ctx.GetStochastikVars().AllocCausetDeferredCausetID()
		}
...
func (b *CausetBuilder) buildBlockDual() *LogicalBlockDual {
	b.handleHelper.pushMap(nil)
	return LogicalBlockDual{RowCount: 1}.Init(b.ctx, b.getSelectOffset())
}

func (ds *DataSource) newExtraHandleSchemaDefCaus() *memex.DeferredCauset {
	return &memex.DeferredCauset{
		RetType:  types.NewFieldType(allegrosql.TypeLonglong),
		UniqueID: ds.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
		ID:       perceptron.ExtraHandleID,
		OrigName: fmt.Sprintf("%v.%v.%v", ds.DBName, ds.blockInfo.Name, perceptron.ExtraHandleName),
	}
}

// getStatsBlock gets statistics information for a causet specified by "blockID".
// A pseudo statistics causet is returned in any of the following scenario:
// 1. milevadb-server started and statistics handle has not been initialized.
// 2. causet event count from statistics is zero.
...
func (b *CausetBuilder) buildDataSource(ctx context.Context, tn *ast.BlockName, asName *perceptron.CIStr) (LogicalCauset, error) {
	dbName := tn.Schema
	...
	var columns []*causet.DeferredCauset
	if b.inUFIDelateStmt {
		// create causet t(a int, b int).
		// Imagine that, There are 2 MilevaDB instances in the cluster, name A, B. We add a column `c` to causet t in the MilevaDB cluster.
		...
		// If we use tbl.DefCauss() here, the uFIDelate memex, will ignore the col `c`, and the data `3` will lost.
		columns = tbl.WriblockDefCauss()
	} else if b.inDeleteStmt {
		// All hidden columns are needed because we need to delete the memex index that consists of hidden columns.
		columns = tbl.FullHiddenDefCaussAndVisibleDefCauss()
	} else {
		columns = tbl.DefCauss()
	}
	var statisticBlock *statistics.Block
	if _, ok := tbl.(causet.PartitionedBlock); !ok {
	...
	ds := DataSource{
		...
		TblDefCauss:             make([]*memex.DeferredCauset, 0, len(columns)),
	...
	for i, col := range columns {
		...
		newDefCaus := &memex.DeferredCauset{
			UniqueID: stochastikVars.AllocCausetDeferredCausetID(),
			ID:       col.ID,
	...
	if blockInfo.IsCommonHandle {
		ds.commonHandleDefCauss, ds.commonHandleLens = memex.IndexInfo2DefCauss(ds.DeferredCausets, ds.schemaReplicant.DeferredCausets, blocks.FindPrimaryIndex(blockInfo))
	}
...
func (b *CausetBuilder) timeRangeForSummaryBlock() QueryTimeRange {
...
func (b *CausetBuilder) buildMemBlock(_ context.Context, dbName perceptron.CIStr, blockInfo *perceptron.BlockInfo) (LogicalCauset, error) {
	// We can use the `blockInfo.DeferredCausets` directly because the memory causet has
	// a sblock schemaReplicant and there is no online DBS on the memory causet.
	schemaReplicant := memex.NewSchema(make([]*memex.DeferredCauset, 0, len(blockInfo.DeferredCausets))...)
	...
	for _, col := range blockInfo.DeferredCausets {
		...
		// NOTE: Rewrite the memex if memory causet supports generated columns in the future
		newDefCaus := &memex.DeferredCauset{
			UniqueID: b.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
			ID:       col.ID,
			RetType:  &col.FieldType,
	...
	// NOTE: Add a `LogicalUnionScan` if we support uFIDelate memory causet in the future
	p := LogicalMemBlock{
		DBName:    dbName,
		BlockInfo: blockInfo,
	}.Init(b.ctx, b.getSelectOffset())
...
// BuildDataSourceFromView is used to build LogicalCauset from view
func (b *CausetBuilder) BuildDataSourceFromView(ctx context.Context, dbName perceptron.CIStr, blockInfo *perceptron.BlockInfo) (LogicalCauset, error) {
	...
	selectLogicalCauset, err := b.Build(ctx, selectNode)
	if err != nil {
	...
	if blockInfo.View.Security == perceptron.SecurityDefiner {
		if pm := privilege.GetPrivilegeManager(b.ctx); pm != nil {
			for _, v := range b.visitInfo {
				if !pm.RequestVerificationWithUser(v.EDB, v.causet, v.column, v.privilege, blockInfo.View.Definer) {
	...
	if len(blockInfo.DeferredCausets) != selectLogicalCauset.Schema().Len() {
		return nil, ErrViewInvalid.GenWithStackByArgs(dbName.O, blockInfo.Name.O)
	}

	return b.buildProjUponView(ctx, dbName, blockInfo, selectLogicalCauset)
}

func (b *CausetBuilder) buildProjUponView(ctx context.Context, dbName perceptron.CIStr, blockInfo *perceptron.BlockInfo, selectLogicalCauset Causet) (LogicalCauset, error) {
	columnInfo := blockInfo.DefCauss()
	defcaus := selectLogicalCauset.Schema().Clone().DeferredCausets
	outputNamesOfUnderlyingSelect := selectLogicalCauset.OutputNames().Shallow()
	...
	if blockInfo.View.DefCauss != nil {
		...
		for _, info := range columnInfo {
			idx := memex.FindFieldNameIdxByDefCausName(selectLogicalCauset.OutputNames(), info.Name.L)
			if idx == -1 {
				return nil, ErrViewInvalid.GenWithStackByArgs(dbName.O, blockInfo.Name.O)
			}
			defcaus = append(defcaus, selectLogicalCauset.Schema().DeferredCausets[idx])
			outputNamesOfUnderlyingSelect = append(outputNamesOfUnderlyingSelect, selectLogicalCauset.OutputNames()[idx])
		}
	}

	projSchema := memex.NewSchema(make([]*memex.DeferredCauset, 0, len(blockInfo.DeferredCausets))...)
	projExprs := make([]memex.Expression, 0, len(blockInfo.DeferredCausets))
	...
	for i, name := range outputNamesOfUnderlyingSelect {
		...
		projNames = append(projNames, &types.FieldName{
			// TblName is the of view instead of the name of the underlying causet.
			TblName:     blockInfo.Name,
		...
		projSchema.Append(&memex.DeferredCauset{
	...
	projUponView.SetChildren(selectLogicalCauset.(LogicalCauset))
	projUponView.SetSchema(projSchema)
	return projUponView, nil
}

// buildApplyWithJoinType builds apply plan with outerCauset and innerCauset, which apply join with particular join type for
// every event from outerCauset and the whole innerCauset.
func (b *CausetBuilder) buildApplyWithJoinType(outerCauset, innerCauset LogicalCauset, tp JoinType) LogicalCauset {
	...
	ap.SetChildren(outerCauset, innerCauset)
	ap.names = make([]*types.FieldName, outerCauset.Schema().Len()+innerCauset.Schema().Len())
	copy(ap.names, outerCauset.OutputNames())
	ap.SetSchema(memex.MergeSchema(outerCauset.Schema(), innerCauset.Schema()))
	// Note that, tp can only be LeftOuterJoin or InnerJoin, so we don't consider other outer joins.
	if tp == LeftOuterJoin {
		b.optFlag = b.optFlag | flagEliminateOuterJoin
		resetNotNullFlag(ap.schemaReplicant, outerCauset.Schema().Len(), ap.schemaReplicant.Len())
	}
	for i := outerCauset.Schema().Len(); i < ap.Schema().Len(); i++ {
		ap.names[i] = types.EmptyName
	}
	return ap
}

// buildSemiApply builds apply plan with outerCauset and innerCauset, which apply semi-join for every event from outerCauset and the whole innerCauset.
func (b *CausetBuilder) buildSemiApply(outerCauset, innerCauset LogicalCauset, condition []memex.Expression, asScalar, not bool) (LogicalCauset, error) {
	b.optFlag = b.optFlag | flagPredicatePushDown | flagBuildKeyInfo | flagDecorrelate

	join, err := b.buildSemiJoin(outerCauset, innerCauset, condition, asScalar, not)
	if err != nil {
		return nil, err
	}
...
func (b *CausetBuilder) buildMaxOneRow(p LogicalCauset) LogicalCauset {
	maxOneRow := LogicalMaxOneRow{}.Init(b.ctx, b.getSelectOffset())
	maxOneRow.SetChildren(p)
	return maxOneRow
}

func (b *CausetBuilder) buildSemiJoin(outerCauset, innerCauset LogicalCauset, onCondition []memex.Expression, asScalar bool, not bool) (*LogicalJoin, error) {
	joinCauset := LogicalJoin{}.Init(b.ctx, b.getSelectOffset())
	for i, expr := range onCondition {
		onCondition[i] = expr.Decorrelate(outerCauset.Schema())
	}
	joinCauset.SetChildren(outerCauset, innerCauset)
	joinCauset.AttachOnConds(onCondition)
	joinCauset.names = make([]*types.FieldName, outerCauset.Schema().Len(), outerCauset.Schema().Len()+innerCauset.Schema().Len()+1)
	copy(joinCauset.names, outerCauset.OutputNames())
	if asScalar {
		newSchema := outerCauset.Schema().Clone()
		newSchema.Append(&memex.DeferredCauset{
			RetType:  types.NewFieldType(allegrosql.TypeTiny),
			UniqueID: b.ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
		})
		joinCauset.names = append(joinCauset.names, types.EmptyName)
		joinCauset.SetSchema(newSchema)
		if not {
			joinCauset.JoinType = AntiLeftOuterSemiJoin
		} else {
			joinCauset.JoinType = LeftOuterSemiJoin
		}
	} else {
		joinCauset.SetSchema(outerCauset.Schema().Clone())
		if not {
			joinCauset.JoinType = AntiSemiJoin
		} else {
			joinCauset.JoinType = SemiJoin
		}
	}
	// Apply forces to choose hash join currently, so don't worry the hints will take effect if the semi join is in one apply.
	if b.BlockHints() != nil {
		outerAlias := extractBlockAlias(outerCauset, joinCauset.blockOffset)
		innerAlias := extractBlockAlias(innerCauset, joinCauset.blockOffset)
		if b.BlockHints().ifPreferMergeJoin(outerAlias, innerAlias) {
			joinCauset.preferJoinType |= preferMergeJoin
		}
		if b.BlockHints().ifPreferHashJoin(outerAlias, innerAlias) {
			joinCauset.preferJoinType |= preferHashJoin
		}
		if b.BlockHints().ifPreferINLJ(innerAlias) {
			joinCauset.preferJoinType = preferRightAsINLJInner
		}
		if b.BlockHints().ifPreferINLHJ(innerAlias) {
			joinCauset.preferJoinType = preferRightAsINLHJInner
		}
		if b.BlockHints().ifPreferINLMJ(innerAlias) {
			joinCauset.preferJoinType = preferRightAsINLMJInner
		}
		// If there're multiple join hints, they're conflict.
		if bits.OnesCount(joinCauset.preferJoinType) > 1 {
...
type TblDefCausPosInfo struct {
	...
	IsCommonHandle bool // TODO: fix redesign uFIDelate join causet and remove me!
}
...
func (c TblDefCausPosInfoSlice) FindHandle(colOrdinal int) (int, bool) {
	...
	if c[rangeBehindOrdinal-1].IsCommonHandle {
		// TODO: fix redesign uFIDelate join causet to fix me.
		return 0, false
	}
...
func (b *CausetBuilder) buildUFIDelate(ctx context.Context, uFIDelate *ast.UFIDelateStmt) (Causet, error) {
	b.pushSelectOffset(0)
	b.pushBlockHints(uFIDelate.BlockHints, utilhint.TypeUFIDelate, 0)
	defer func() {
		b.popSelectOffset()
		// causet hints are only visible in the current UFIDelATE memex.
		b.popBlockHints()
	}()

	// uFIDelate subquery causet should be forbidden
	...
	if b.ctx.GetStochastikVars().TxnCtx.IsPessimistic {
		if uFIDelate.BlockRefs.BlockRefs.Right == nil {
			...
			// We only need do this optimization for single causet uFIDelate which is the most common case.
			// When BlockRefs.Right is nil, it is single causet uFIDelate.
	...
	proj := LogicalProjection{Exprs: memex.DeferredCauset2Exprs(p.Schema().DeferredCausets[:oldSchemaLen])}.Init(b.ctx, b.getSelectOffset())
	proj.SetSchema(memex.NewSchema(make([]*memex.DeferredCauset, oldSchemaLen)...))
	proj.names = make(types.NameSlice, len(p.OutputNames()))
	copy(proj.names, p.OutputNames())
	copy(proj.schemaReplicant.DeferredCausets, p.Schema().DeferredCausets[:oldSchemaLen])
	proj.SetChildren(p)
	p = proj
	...
	uFIDelt.SelectCauset, _, err = DoOptimize(ctx, b.ctx, b.optFlag&^flagEliminateProjection, p)
	if err != nil {
		return nil, err
	}
	...
	tblID2Handle, err := resolveIndicesForTblID2Handle(b.handleHelper.tailMap(), uFIDelt.SelectCauset.Schema())
	if err != nil {
		return nil, err
	}
	tblID2block := make(map[int64]causet.Block, len(tblID2Handle))
...
func GetUFIDelateDeferredCausets(ctx stochastikctx.Context, orderedList []*memex.Assignment, schemaLen int) ([]bool, error) {
	...
	for _, v := range orderedList {
		if !ctx.GetStochastikVars().AllowWriteRowID && v.DefCaus.ID == perceptron.ExtraHandleID {
			return nil, errors.Errorf("insert, uFIDelate and replace memexs for _milevadb_rowid are not supported.")
		}
		idx := v.DefCaus.Index
		assignFlag[idx] = true
	}
...
func checkUFIDelateList(ctx stochastikctx.Context, tblID2block map[int64]causet.Block, uFIDelt *UFIDelate) error {
	assignFlags, err := GetUFIDelateDeferredCausets(ctx, uFIDelt.OrderedList, uFIDelt.SelectCauset.Schema().Len())
	if err != nil {
		return err
	}
...
func (b *CausetBuilder) buildUFIDelateLists(
	ctx context.Context,
	blockList []*ast.BlockName,
	list []*ast.Assignment,
	p LogicalCauset,
) (newList []*memex.Assignment,
	po LogicalCauset,
	...
	for _, assign := range list {
		idx, err := memex.FindFieldName(p.OutputNames(), assign.DeferredCauset)
		if err != nil {
	...
	for _, tn := range blockList {
		...
		for i, colInfo := range blockInfo.DeferredCausets {
			...
			// see https://dev.allegrosql.com/doc/refman/8.0/en/create-causet-generated-columns.html
			if ok && !isDefault {
	...
	newList = make([]*memex.Assignment, 0, p.Schema().Len())
	...
	for i, assign := range allAssignments {
		...
		col := p.Schema().DeferredCausets[idx]
		name := p.OutputNames()[idx]
		var newExpr memex.Expression
		var np LogicalCauset
		if i < len(list) {
			// If assign `DEFAULT` to column, fill the `defaultExpr.Name` before rewrite memex
		...
		if _, isConst := newExpr.(*memex.Constant); !isConst {
			allAssignmentsAreConstant = false
		}
		p = np
		newList = append(newList, &memex.Assignment{DefCaus: col, DefCausName: name.DefCausName, Expr: newExpr})
		dbName := name.DBName.L
		// To solve issue#10028, we need to get database name by the causet alias name.
...
func (b *CausetBuilder) buildDelete(ctx context.Context, delete *ast.DeleteStmt) (Causet, error) {
	b.pushSelectOffset(0)
	b.pushBlockHints(delete.BlockHints, utilhint.TypeDelete, 0)
	defer func() {
		b.popSelectOffset()
		// causet hints are only visible in the current DELETE memex.
		b.popBlockHints()
	}()
	...
	del.SelectCauset, _, err = DoOptimize(ctx, b.ctx, b.optFlag, p)
	if err != nil {
		return nil, err
	}

	tblID2Handle, err := resolveIndicesForTblID2Handle(handleDefCaussMap, del.SelectCauset.Schema())
	if err != nil {
		return nil, err
	}
	...
	if del.IsMultiBlock {
		// tblID2BlockName is the causet map value is an array which contains causet aliases.
		// Block ID may not be unique for deleting multiple blocks, for memexs like
		// `delete from t as t1, t as t2`, the same causet has two alias, we have to identify a causet
...
func resolveIndicesForTblID2Handle(tblID2Handle map[int64][]HandleDefCauss, schemaReplicant *memex.Schema) (map[int64][]HandleDefCauss, error) {
...
// matchingDeletingBlock checks whether this column is from the causet which is in the deleting list.
...
// buildProjectionForWindow builds the projection for memexs in the window specification that is not an column,
// so after the projection, window functions only needs to deal with columns.
func (b *CausetBuilder) buildProjectionForWindow(ctx context.Context, p LogicalCauset, spec *ast.WindowSpec, args []ast.ExprNode, aggMap map[*ast.AggregateFuncExpr]int) (LogicalCauset, []property.Item, []property.Item, []memex.Expression, error) {
	b.optFlag |= flagEliminateProjection
	...
	proj := LogicalProjection{Exprs: make([]memex.Expression, 0, projLen)}.Init(b.ctx, b.getSelectOffset())
	proj.SetSchema(memex.NewSchema(make([]*memex.DeferredCauset, 0, projLen)...))
	...
	newArgList := make([]memex.Expression, 0, len(args))
	for _, arg := range args {
		...
		p = np
		switch newArg.(type) {
		case *memex.DeferredCauset, *memex.Constant:
			newArgList = append(newArgList, newArg)
			continue
		}
...
func (b *CausetBuilder) buildArgs4WindowFunc(ctx context.Context, p LogicalCauset, args []ast.ExprNode, aggMap map[*ast.AggregateFuncExpr]int) ([]memex.Expression, error) {
	b.optFlag |= flagEliminateProjection
...
func (b *CausetBuilder) buildByItemsForWindow(
	ctx context.Context,
	p LogicalCauset,
...
) (LogicalCauset, []property.Item, error) {
	transformer := &itemTransformer{}
	for _, item := range items {
		...
		if col, ok := it.(*memex.DeferredCauset); ok {
...
func (b *CausetBuilder) buildWindowFunctionFrameBound(ctx context.Context, spec *ast.WindowSpec, orderByItems []property.Item, boundClause *ast.FrameBound) (*FrameBound, error) {
	...
	bound.CalcFuncs = make([]memex.Expression, len(orderByItems))
	bound.CmpFuncs = make([]memex.CompareFunc, len(orderByItems))
	if bound.Type == ast.CurrentRow {
		for i, item := range orderByItems {
			col := item.DefCaus
			bound.CalcFuncs[i] = col
			bound.CmpFuncs[i] = memex.GetCmpFunction(b.ctx, col, col)
		}
		return bound, nil
	}

	col := orderByItems[0].DefCaus
	// TODO: We also need to raise error for non-deterministic memexs, like rand().
	...
	expr := memex.Constant{Value: val, RetType: boundClause.Expr.GetType()}
	...
	if boundClause.Unit != ast.TimeUnitInvalid {
		...
		bound.CalcFuncs[0], err = memex.NewFunctionBase(b.ctx, funcName, col.RetType, col, &expr, &unit)
		if err != nil {
			return nil, err
		}
		bound.CmpFuncs[0] = memex.GetCmpFunction(b.ctx, orderByItems[0].DefCaus, bound.CalcFuncs[0])
		return bound, nil
	}
	// When the order is asc:
	//   `+` for following, and `-` for the preceding
	...
	bound.CalcFuncs[0], err = memex.NewFunctionBase(b.ctx, funcName, col.RetType, col, &expr)
	if err != nil {
		return nil, err
	}
	bound.CmpFuncs[0] = memex.GetCmpFunction(b.ctx, orderByItems[0].DefCaus, bound.CalcFuncs[0])
...
// paramMarkerInPrepareChecker checks whether the given ast tree has paramMarker and is in prepare memex.
...
func (pc *paramMarkerInPrepareChecker) Enter(in ast.Node) (out ast.Node, skipChildren bool) {
	switch v := in.(type) {
	case *driver.ParamMarkerExpr:
		pc.inPrepareStmt = !v.InInterDircute
		return v, true
	}
	return in, false
}
...
func (b *CausetBuilder) buildWindowFunctionFrame(ctx context.Context, spec *ast.WindowSpec, orderByItems []property.Item) (*WindowFrame, error) {
...
func (b *CausetBuilder) checkWindowFuncArgs(ctx context.Context, p LogicalCauset, windowFuncExprs []*ast.WindowFuncExpr, windowAggMap map[*ast.AggregateFuncExpr]int) error {
...
func (b *CausetBuilder) buildWindowFunctions(ctx context.Context, p LogicalCauset, groupedFuncs map[*ast.WindowSpec][]*ast.WindowFuncExpr, aggMap map[*ast.AggregateFuncExpr]int) (LogicalCauset, map[*ast.WindowFuncExpr]int, error) {
...
func (b *CausetBuilder) checkOriginWindowSpecs(funcs []*ast.WindowFuncExpr, orderByItems []property.Item) error {
	for _, f := range funcs {
		if f.IgnoreNull {
...
func (b *CausetBuilder) checkOriginWindowFrameBound(bound *ast.FrameBound, spec *ast.WindowSpec, orderByItems []property.Item) error {
...
func (b *CausetBuilder) handleDefaultFrame(spec *ast.WindowSpec, windowFuncName string) (*ast.WindowSpec, bool) {
...
func (b *CausetBuilder) groupWindowFuncs(windowFuncs []*ast.WindowFuncExpr) (map[*ast.WindowSpec][]*ast.WindowFuncExpr, error) {
...
// Privilege check should use OrigName, while memex may use AsName.
