...
func (p *basePhysicalCauset) StatsCount() float64 {
	return p.stats.RowCount
}

// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalBlockDual) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalMemBlock) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalShow) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
func getFakeStats(schemaReplicant *memex.Schema) *property.StatsInfo {
	profile := &property.StatsInfo{
		RowCount:    1,
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalShowDBSJobs) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
	// A fake count, just to avoid panic now.
...
func RecursiveDeriveStats4Test(p LogicalCauset) (*property.StatsInfo, error) {
	return p.recursiveDeriveStats(nil)
}

// GetStats4Test is a exporter just for test.
func GetStats4Test(p LogicalCauset) *property.StatsInfo {
	return p.statsInfo()
}

func (p *baseLogicalCauset) recursiveDeriveStats(colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	childStats := make([]*property.StatsInfo, len(p.children))
	childSchema := make([]*memex.Schema, len(p.children))
...
// ExtractDefCausGroups implements LogicalCauset ExtractDefCausGroups interface.
func (p *baseLogicalCauset) ExtractDefCausGroups(_ [][]*memex.DeferredCauset) [][]*memex.DeferredCauset {
	return nil
}

// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *baseLogicalCauset) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if len(childStats) == 1 {
		p.stats = childStats[0]
		return p.stats, nil
	}
	if len(childStats) > 1 {
		err := ErrInternal.GenWithStack("LogicalCausets with more than one child should implement their own DeriveStats().")
		return nil, err
	}
	if p.stats != nil {
		return p.stats, nil
...
func (ds *DataSource) getGroupNDVs(colGroups [][]*memex.DeferredCauset) []property.GroupNDV {
	if colGroups == nil {
		return nil
	}
	...
	for idxID, idx := range tbl.Indices {
		...
		copy(idxDefCauss, tbl.Idx2DeferredCausetIDs[idxID])
...
func (ds *DataSource) initStats(colGroups [][]*memex.DeferredCauset) {
	...
	if ds.statisticBlock == nil {
		ds.statisticBlock = getStatsBlock(ds.ctx, ds.blockInfo, ds.causet.Meta().ID)
	}
...
func (ds *DataSource) deriveStatsByFilter(conds memex.CNFExprs, filledPaths []*soliton.AccessPath) *property.StatsInfo {
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (ds *DataSource) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	...
	for i, expr := range ds.pushedDownConds {
		ds.pushedDownConds[i] = memex.PushDownNot(ds.ctx, expr)
	}
...
// DeriveStats implements LogicalCauset DeriveStats interface.
func (ts *LogicalBlockScan) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (_ *property.StatsInfo, err error) {
	ts.Source.initStats(nil)
	// PushDownNot here can convert query 'not (a != 1)' to 'a = 1'.
	for i, expr := range ts.AccessConds {
		// TODO The memexs may be shared by BlockScan and several IndexScans, there would be redundant
		// `PushDownNot` function call in multiple `DeriveStats` then.
		ts.AccessConds[i] = memex.PushDownNot(ts.ctx, expr)
	}
...
// DeriveStats implements LogicalCauset DeriveStats interface.
func (is *LogicalIndexScan) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	is.Source.initStats(nil)
	for i, expr := range is.AccessConds {
		is.AccessConds[i] = memex.PushDownNot(is.ctx, expr)
	}
	is.stats = is.Source.deriveStatsByFilter(is.AccessConds, nil)
	if len(is.AccessConds) == 0 {
		is.Ranges = ranger.FullRange()
	}
	is.IdxDefCauss, is.IdxDefCausLens = memex.IndexInfo2PrefixDefCauss(is.DeferredCausets, selfSchema.DeferredCausets, is.Index)
	is.FullIdxDefCauss, is.FullIdxDefCausLens = memex.IndexInfo2DefCauss(is.DeferredCausets, selfSchema.DeferredCausets, is.Index)
...
func (ds *DataSource) generateIndexMergeOrPaths() {
	...
	for i, cond := range ds.pushedDownConds {
		...
		for _, item := range dnfItems {
			cnfItems := memex.SplitCNFItems(item)
...
func (ds *DataSource) accessPathsForConds(conditions []memex.Expression, usedIndexCount int) []*soliton.AccessPath {
	...
	for i := 0; i < usedIndexCount; i++ {
		...
		if ds.possibleAccessPaths[i].IsBlockPath() {
			...
			if len(path.BlockFilters) > 0 || len(path.AccessConds) == 0 {
				...
				// For example, a allegrosql `select * from t where a > 1 or (b < 2 and c > 3)` and causet `t` with indexes
				// on a and b separately. we can generate a `IndexMergePath` with causet filter `a > 1 or (b < 2 and c > 3)`.
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalSelection) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalUnionAll) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalLimit) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (lt *LogicalTopN) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if lt.stats != nil {
		return lt.stats, nil
	}
...
func getCardinality(defcaus []*memex.DeferredCauset, schemaReplicant *memex.Schema, profile *property.StatsInfo) float64 {
	cardinality := 1.0
...
func (p *LogicalProjection) getGroupNDVs(colGroups [][]*memex.DeferredCauset, childProfile *property.StatsInfo, selfSchema *memex.Schema) []property.GroupNDV {
	if len(colGroups) == 0 || len(childProfile.GroupNDVs) == 0 {
		return nil
	}
	exprDefCaus2ProjDefCaus := make(map[int64]int64)
	for i, expr := range p.Exprs {
		exprDefCaus, ok := expr.(*memex.DeferredCauset)
		if !ok {
			continue
		}
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalProjection) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	...
	for i, expr := range p.Exprs {
		defcaus := memex.ExtractDeferredCausets(expr)
...
// ExtractDefCausGroups implements LogicalCauset ExtractDefCausGroups interface.
func (p *LogicalProjection) ExtractDefCausGroups(colGroups [][]*memex.DeferredCauset) [][]*memex.DeferredCauset {
	if len(colGroups) == 0 {
		return nil
	}
	...
	extracted := make([][]*memex.DeferredCauset, 0, len(extDefCausGroups))
	for _, defcaus := range extDefCausGroups {
		exprs := make([]*memex.DeferredCauset, len(defcaus))
		allDefCauss := true
		for i, offset := range defcaus {
			col, ok := p.Exprs[offset].(*memex.DeferredCauset)
		...
		if allDefCauss {
			extracted = append(extracted, memex.SortDeferredCausets(exprs))
		}
	}
	return extracted
}

func (la *LogicalAggregation) getGroupNDVs(colGroups [][]*memex.DeferredCauset, childProfile *property.StatsInfo, selfSchema *memex.Schema, gbyDefCauss []*memex.DeferredCauset) []property.GroupNDV {
	if len(colGroups) == 0 || len(childProfile.GroupNDVs) == 0 {
		return nil
	}
	...
	// except for using the independent assumption, unless we can use stats of memex index.
	gbyDefCauss = memex.SortDeferredCausets(gbyDefCauss)
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (la *LogicalAggregation) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	childProfile := childStats[0]
	gbyDefCauss := make([]*memex.DeferredCauset, 0, len(la.GroupByItems))
	for _, gbyExpr := range la.GroupByItems {
		defcaus := memex.ExtractDeferredCausets(gbyExpr)
...
// ExtractDefCausGroups implements LogicalCauset ExtractDefCausGroups interface.
func (la *LogicalAggregation) ExtractDefCausGroups(_ [][]*memex.DeferredCauset) [][]*memex.DeferredCauset {
	...
	if len(gbyDefCauss) > 0 {
		return [][]*memex.DeferredCauset{memex.SortDeferredCausets(gbyDefCauss)}
	}
	return nil
}

func (p *LogicalJoin) getGroupNDVs(colGroups [][]*memex.DeferredCauset, childStats []*property.StatsInfo) []property.GroupNDV {
	outerIdx := int(-1)
...
func (p *LogicalJoin) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		// Reload GroupNDVs since colGroups may have changed.
...
// ExtractDefCausGroups implements LogicalCauset ExtractDefCausGroups interface.
func (p *LogicalJoin) ExtractDefCausGroups(colGroups [][]*memex.DeferredCauset) [][]*memex.DeferredCauset {
	leftJoinKeys, rightJoinKeys, _, _ := p.GetJoinKeys()
	extracted := make([][]*memex.DeferredCauset, 0, 2+len(colGroups))
	if len(leftJoinKeys) > 1 && (p.JoinType == InnerJoin || p.JoinType == LeftOuterJoin || p.JoinType == RightOuterJoin) {
		extracted = append(extracted, memex.SortDeferredCausets(leftJoinKeys), memex.SortDeferredCausets(rightJoinKeys))
	}
	var outerSchema *memex.Schema
	if p.JoinType == LeftOuterJoin || p.JoinType == LeftOuterSemiJoin || p.JoinType == AntiLeftOuterSemiJoin {
...
func (la *LogicalApply) getGroupNDVs(colGroups [][]*memex.DeferredCauset, childStats []*property.StatsInfo) []property.GroupNDV {
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (la *LogicalApply) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if la.stats != nil {
		// Reload GroupNDVs since colGroups may have changed.
...
// ExtractDefCausGroups implements LogicalCauset ExtractDefCausGroups interface.
func (la *LogicalApply) ExtractDefCausGroups(colGroups [][]*memex.DeferredCauset) [][]*memex.DeferredCauset {
	var outerSchema *memex.Schema
	...
	extracted := make([][]*memex.DeferredCauset, len(offsets))
...
func getSingletonStats(schemaReplicant *memex.Schema) *property.StatsInfo {
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalMaxOneRow) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, _ [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		return p.stats, nil
	}
...
func (p *LogicalWindow) getGroupNDVs(colGroups [][]*memex.DeferredCauset, childStats []*property.StatsInfo) []property.GroupNDV {
	if len(colGroups) > 0 {
		return childStats[0].GroupNDVs
	}
	return nil
...
// DeriveStats implement LogicalCauset DeriveStats interface.
func (p *LogicalWindow) DeriveStats(childStats []*property.StatsInfo, selfSchema *memex.Schema, childSchema []*memex.Schema, colGroups [][]*memex.DeferredCauset) (*property.StatsInfo, error) {
	if p.stats != nil {
		// Reload GroupNDVs since colGroups may have changed.
		p.stats.GroupNDVs = p.getGroupNDVs(colGroups, childStats)
		return p.stats, nil
...
// ExtractDefCausGroups implements LogicalCauset ExtractDefCausGroups interface.
func (p *LogicalWindow) ExtractDefCausGroups(colGroups [][]*memex.DeferredCauset) [][]*memex.DeferredCauset {
	if len(colGroups) == 0 {
		return nil
	}
