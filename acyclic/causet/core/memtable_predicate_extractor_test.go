...
func (s *extractorSuite) getLogicalMemBlock(c *C, se stochastik.Stochastik, BerolinaSQL *BerolinaSQL.BerolinaSQL, allegrosql string) *causetcore.LogicalMemBlock {
	stmt, err := BerolinaSQL.ParseOneStmt(allegrosql, "", "")
	c.Assert(err, IsNil)

	ctx := context.Background()
	builder := causetcore.NewCausetBuilder(se, s.dom.SchemaReplicant(), &hint.BlockHintProcessor{})
	plan, err := builder.Build(ctx, stmt)
	c.Assert(err, IsNil)

	logicalCauset, err := causetcore.LogicalOptimize(ctx, builder.GetOptFlag(), plan.(causetcore.LogicalCauset))
	c.Assert(err, IsNil)
	...
	logicalMemBlock := leafCauset.(*causetcore.LogicalMemBlock)
...
func (s *extractorSuite) TestClusterConfigBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetcore.ClusterBlockExtractor)
...
func (s *extractorSuite) TestClusterLogBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetcore.ClusterLogBlockExtractor)
		c.Assert(clusterConfigExtractor.NodeTypes, DeepEquals, ca.nodeTypes, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		c.Assert(clusterConfigExtractor.Instances, DeepEquals, ca.instances, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		c.Assert(clusterConfigExtractor.SkipRequest, DeepEquals, ca.skipRequest, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		if ca.startTime > 0 {
...
func (s *extractorSuite) TestMetricBlockExtractor(c *C) {
	...
	parseTime := func(c *C, s string) time.Time {
		t, err := time.ParseInLocation(causetcore.MetricBlockTimeFormat, s, time.Local)
		c.Assert(err, IsNil)
		return t
	}
	...
	for _, ca := range cases {
		logicalMemBlock := s.getLogicalMemBlock(c, se, BerolinaSQL, ca.allegrosql)
		c.Assert(logicalMemBlock.Extractor, NotNil)
		metricBlockExtractor := logicalMemBlock.Extractor.(*causetcore.MetricBlockExtractor)
...
func (s *extractorSuite) TestMetricsSummaryBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		extractor := logicalMemBlock.Extractor.(*causetcore.MetricSummaryBlockExtractor)
...
func (s *extractorSuite) TestInspectionResultBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetcore.InspectionResultBlockExtractor)
		if len(ca.rules) > 0 {
...
func (s *extractorSuite) TestInspectionSummaryBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetcore.InspectionSummaryBlockExtractor)
		if len(ca.rules) > 0 {
			c.Assert(clusterConfigExtractor.Memrules, DeepEquals, ca.rules, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		}
		if len(ca.names) > 0 {
...
func (s *extractorSuite) TestInspectionMemruleBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetcore.InspectionMemruleBlockExtractor)
		if len(ca.tps) > 0 {
