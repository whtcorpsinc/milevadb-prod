...
func (a *aggregationEliminateChecker) tryToEliminateAggregation(agg *LogicalAggregation) *LogicalProjection {
	...
	schemaByGroupby := memex.NewSchema(agg.groupByDefCauss...)
...
func ConvertAggToProj(agg *LogicalAggregation, schemaReplicant *memex.Schema) (bool, *LogicalProjection) {
	proj := LogicalProjection{
		Exprs: make([]memex.Expression, 0, len(agg.AggFuncs)),
...
// rewriteExpr will rewrite the aggregate function to memex doesn't contain aggregate function.
func rewriteExpr(ctx stochastikctx.Context, aggFunc *aggregation.AggFuncDesc) (bool, memex.Expression) {
	switch aggFunc.Name {
...
func rewriteCount(ctx stochastikctx.Context, exprs []memex.Expression, targetTp *types.FieldType) memex.Expression {
	...
	isNullExprs := make([]memex.Expression, 0, len(exprs))
	for _, expr := range exprs {
		if allegrosql.HasNotNullFlag(expr.GetType().Flag) {
			isNullExprs = append(isNullExprs, memex.NewZero())
		} else {
			isNullExpr := memex.NewFunctionInternal(ctx, ast.IsNull, types.NewFieldType(allegrosql.TypeTiny), expr)
			isNullExprs = append(isNullExprs, isNullExpr)
		}
	}

	innerExpr := memex.ComposeDNFCondition(ctx, isNullExprs...)
	newExpr := memex.NewFunctionInternal(ctx, ast.If, targetTp, innerExpr, memex.NewZero(), memex.NewOne())
	return newExpr
}

func rewriteBitFunc(ctx stochastikctx.Context, funcType string, arg memex.Expression, targetTp *types.FieldType) memex.Expression {
	// For not integer type. We need to cast(cast(arg as signed) as unsigned) to make the bit function work.
	innerCast := memex.WrapWithCastAsInt(ctx, arg)
	outerCast := wrapCastFunction(ctx, innerCast, targetTp)
	var finalExpr memex.Expression
	if funcType != ast.AggFuncBitAnd {
		finalExpr = memex.NewFunctionInternal(ctx, ast.Ifnull, targetTp, outerCast, memex.NewZero())
	} else {
		finalExpr = memex.NewFunctionInternal(ctx, ast.Ifnull, outerCast.GetType(), outerCast, &memex.Constant{Value: types.NewUintCauset(math.MaxUint64), RetType: targetTp})
	}
	return finalExpr
}

// wrapCastFunction will wrap a cast if the targetTp is not equal to the arg's.
func wrapCastFunction(ctx stochastikctx.Context, arg memex.Expression, targetTp *types.FieldType) memex.Expression {
	if arg.GetType() == targetTp {
		return arg
	}
	return memex.BuildCastFunction(ctx, arg, targetTp)
}

func (a *aggregationEliminator) optimize(ctx context.Context, p LogicalCauset) (LogicalCauset, error) {
