...
func (s *testCausetSuite) TestPosetDagCausetBuilderBasePhysicalCauset(c *C) {
	...
	for i, tt := range input {
		...
		s.testData.OnRecord(func() {
			output[i].ALLEGROALLEGROSQL = tt
			output[i].Best = core.ToString(p)
			output[i].Hints = hint.RestoreOptimizerHints(core.GenHintsFromPhysicalCauset(p))
		})
		c.Assert(core.ToString(p), Equals, output[i].Best, Commentf("for %s", tt))
		c.Assert(hint.RestoreOptimizerHints(core.GenHintsFromPhysicalCauset(p)), Equals, output[i].Hints, Commentf("for %s", tt))
	}
}
...
func (s *testCausetSuite) TestJoinHints(c *C) {
	...
	for i, test := range input {
		...
		c.Assert(hint.RestoreOptimizerHints(core.GenHintsFromPhysicalCauset(p)), Equals, output[i].Hints, comment)
	}
}
...
func (s *testCausetSuite) TestQueryBlockHint(c *C) {
	...
	for i, tt := range input {
		...
		s.testData.OnRecord(func() {
			output[i].ALLEGROALLEGROSQL = tt
			output[i].Causet = core.ToString(p)
			output[i].Hints = hint.RestoreOptimizerHints(core.GenHintsFromPhysicalCauset(p))
		})
		c.Assert(core.ToString(p), Equals, output[i].Causet, comment)
		c.Assert(hint.RestoreOptimizerHints(core.GenHintsFromPhysicalCauset(p)), Equals, output[i].Hints, comment)
	}
}
...
func testPosetDagCausetBuilderSplitAvg(c *C, root core.PhysicalCauset) {
