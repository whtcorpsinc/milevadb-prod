...
type HandleDefCauss interface {
	...
	ResolveIndices(schemaReplicant *memex.Schema) (HandleDefCauss, error)
	// IsInt returns if the HandleDefCauss is a single tnt column.
	IsInt() bool
	// String implements the fmt.Stringer interface.
	String() string
	// GetDefCaus gets the column by idx.
	GetDefCaus(idx int) *memex.DeferredCauset
...
func (cb *CommonHandleDefCauss) ResolveIndices(schemaReplicant *memex.Schema) (HandleDefCauss, error) {
	ncb := &CommonHandleDefCauss{
		tblInfo: cb.tblInfo,
		idxInfo: cb.idxInfo,
		sc:      cb.sc,
		columns: make([]*memex.DeferredCauset, len(cb.columns)),
	}
	for i, col := range cb.columns {
		...
		ncb.columns[i] = newDefCaus.(*memex.DeferredCauset)
	}
	return ncb, nil
}
...
func (cb *CommonHandleDefCauss) GetDefCaus(idx int) *memex.DeferredCauset {
...
func NewCommonHandleDefCauss(sc *stmtctx.StatementContext, tblInfo *perceptron.BlockInfo, idxInfo *perceptron.IndexInfo,
	blockDeferredCausets []*memex.DeferredCauset) *CommonHandleDefCauss {
	defcaus := &CommonHandleDefCauss{
		tblInfo: tblInfo,
		idxInfo: idxInfo,
		sc:      sc,
		columns: make([]*memex.DeferredCauset, len(idxInfo.DeferredCausets)),
	}
...
// ResolveIndices implements the ekv.HandleDefCauss interface.
func (ib *IntHandleDefCauss) ResolveIndices(schemaReplicant *memex.Schema) (HandleDefCauss, error) {
	newDefCaus, err := ib.col.ResolveIndices(schemaReplicant)
	if err != nil {
		return nil, err
	}
	return &IntHandleDefCauss{col: newDefCaus.(*memex.DeferredCauset)}, nil
}
...
// GetDefCaus implements the ekv.HandleDefCauss interface.
func (ib *IntHandleDefCauss) GetDefCaus(idx int) *memex.DeferredCauset {
	if idx != 0 {
		return nil
	}
	return ib.col
...
func NewIntHandleDefCauss(col *memex.DeferredCauset) HandleDefCauss {
