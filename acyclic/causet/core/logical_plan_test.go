...
func (s *testCausetSuite) TestPredicatePushDown(c *C) {
	...
	for ith, ca := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
		p, err = logicalOptimize(context.TODO(), flagPredicatePushDown|flagDecorrelate|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain, p.(LogicalCauset))
		c.Assert(err, IsNil)
...
func (s *testCausetSuite) TestJoinPredicatePushDown(c *C) {
	defer testleak.AfterTest(c)()
	var (
		input  []string
		output []struct {
	...
	for i, ca := range input {
		...
		c.Assert(ok, IsTrue, comment)
		leftCauset, ok := join.children[0].(*DataSource)
		c.Assert(ok, IsTrue, comment)
		rightCauset, ok := join.children[1].(*DataSource)
		c.Assert(ok, IsTrue, comment)
		leftCond := fmt.Sprintf("%s", leftCauset.pushedDownConds)
		rightCond := fmt.Sprintf("%s", rightCauset.pushedDownConds)
		s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestOuterWherePredicatePushDown(c *C) {
	defer testleak.AfterTest(c)()
	var (
		input  []string
		output []struct {
...
func (s *testCausetSuite) TestSimplifyOuterJoin(c *C) {
	defer testleak.AfterTest(c)()
	var (
		input  []string
		output []struct {
	...
	for i, ca := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil, comment)
		p, err = logicalOptimize(context.TODO(), flagPredicatePushDown|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain, p.(LogicalCauset))
		c.Assert(err, IsNil, comment)
		...
		join, ok := p.(LogicalCauset).Children()[0].(*LogicalJoin)
		if !ok {
			join, ok = p.(LogicalCauset).Children()[0].Children()[0].(*LogicalJoin)
			c.Assert(ok, IsTrue, comment)
		}
		s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestAntiSemiJoinConstFalse(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql      string
		best     string
	...
	for _, ca := range tests {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil, comment)
		p, err = logicalOptimize(context.TODO(), flagDecorrelate|flagPredicatePushDown|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain, p.(LogicalCauset))
		c.Assert(err, IsNil, comment)
		c.Assert(ToString(p), Equals, ca.best, comment)
		join, _ := p.(LogicalCauset).Children()[0].(*LogicalJoin)
		c.Assert(join.JoinType.String(), Equals, ca.joinType, comment)
	}
}

func (s *testCausetSuite) TestDeriveNotNullConds(c *C) {
	defer testleak.AfterTest(c)()
	var (
		input  []string
		output []struct {
	...
	for i, ca := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil, comment)
		p, err = logicalOptimize(context.TODO(), flagPredicatePushDown|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain|flagDecorrelate, p.(LogicalCauset))
		c.Assert(err, IsNil, comment)
		s.testData.OnRecord(func() {
			output[i].Causet = ToString(p)
		})
		c.Assert(ToString(p), Equals, output[i].Causet, comment)
		join := p.(LogicalCauset).Children()[0].(*LogicalJoin)
...
func buildLogicCauset4GroupBy(s *testCausetSuite, c *C, allegrosql string) (Causet, error) {
	sqlMode := s.ctx.GetStochastikVars().ALLEGROSQLMode
	mockedBlockInfo := MockSignedBlock()
	// mock the causet info here for later use
	...
	p, _, err := BuildLogicalCauset(context.Background(), s.ctx, stmt, s.is)
	return p, err
}

func (s *testCausetSuite) TestGroupByWhenNotExistDefCauss(c *C) {
	sqlTests := []struct {
		allegrosql              string
	...
	for _, test := range sqlTests {
		...
		p, err := buildLogicCauset4GroupBy(s, c, allegrosql)
		c.Assert(err, NotNil)
		c.Assert(p, IsNil)
...
func (s *testCausetSuite) TestDupRandJoinCondsPushDown(c *C) {
	...
	p, _, err := BuildLogicalCauset(context.Background(), s.ctx, stmt, s.is)
	c.Assert(err, IsNil, comment)
	p, err = logicalOptimize(context.TODO(), flagPredicatePushDown, p.(LogicalCauset))
	c.Assert(err, IsNil, comment)
	proj, ok := p.(*LogicalProjection)
	c.Assert(ok, IsTrue, comment)
	join, ok := proj.children[0].(*LogicalJoin)
	c.Assert(ok, IsTrue, comment)
	leftCauset, ok := join.children[0].(*LogicalSelection)
	c.Assert(ok, IsTrue, comment)
	leftCond := fmt.Sprintf("%s", leftCauset.Conditions)
...
func (s *testCausetSuite) TestBlockPartition(c *C) {
	defer testleak.AfterTest(c)()
	...
	for i, ca := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, isChoices[ca.IsIdx])
		c.Assert(err, IsNil)
		p, err = logicalOptimize(context.TODO(), flagDecorrelate|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain|flagPredicatePushDown|flagPartitionProcessor, p.(LogicalCauset))
		c.Assert(err, IsNil)
		planString := ToString(p)
		s.testData.OnRecord(func() {
			output[i] = planString
...
func (s *testCausetSuite) TestSubquery(c *C) {
	defer testleak.AfterTest(c)()
	var input, output []string
	s.testData.GetTestCases(c, &input, &output)
...
func (s *testCausetSuite) TestCausetBuilder(c *C) {
	defer testleak.AfterTest(c)()
	var input, output []string
	s.testData.GetTestCases(c, &input, &output)
	ctx := context.Background()
...
func (s *testCausetSuite) TestJoinReOrder(c *C) {
	defer testleak.AfterTest(c)()
	var input, output []string
	s.testData.GetTestCases(c, &input, &output)
	...
	for i, tt := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
		p, err = logicalOptimize(context.TODO(), flagPredicatePushDown|flagJoinReOrder, p.(LogicalCauset))
		c.Assert(err, IsNil)
		planString := ToString(p)
		s.testData.OnRecord(func() {
			output[i] = planString
...
func (s *testCausetSuite) TestEagerAggregation(c *C) {
	defer testleak.AfterTest(c)()
	var input []string
	var output []string
	s.testData.GetTestCases(c, &input, &output)
	...
	for ith, tt := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
		p, err = logicalOptimize(context.TODO(), flagBuildKeyInfo|flagPredicatePushDown|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain|flagPushDownAgg, p.(LogicalCauset))
		c.Assert(err, IsNil)
		s.testData.OnRecord(func() {
			output[ith] = ToString(p)
		})
...
func (s *testCausetSuite) TestDeferredCausetPruning(c *C) {
	defer testleak.AfterTest(c)()
	var (
		input  []string
	...
	for i, tt := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
		lp, err := logicalOptimize(ctx, flagPredicatePushDown|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain, p.(LogicalCauset))
		c.Assert(err, IsNil)
		s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestProjectionEliminator(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql  string
		best string
	...
	for ith, tt := range tests {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
		p, err = logicalOptimize(context.TODO(), flagBuildKeyInfo|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain|flagEliminateProjection, p.(LogicalCauset))
		c.Assert(err, IsNil)
		c.Assert(ToString(p), Equals, tt.best, Commentf("for %s %d", tt.allegrosql, ith))
	}
}

func (s *testCausetSuite) TestAllocID(c *C) {
	ctx := MockContext()
...
func (s *testCausetSuite) checkDataSourceDefCauss(p LogicalCauset, c *C, ans map[int][]string, comment CommentInterface) {
	switch p.(type) {
	case *DataSource:
		s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestValidate(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql string
		err *terror.Error
	}{
		{
			...
			err: memex.ErrOperandDeferredCausets,
		},
		{
	...
	for _, tt := range tests {
		...
		c.Assert(err, IsNil, comment)
		Preprocess(s.ctx, stmt, s.is)
		_, _, err = BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		if tt.err == nil {
			c.Assert(err, IsNil, comment)
		} else {
...
func (s *testCausetSuite) checkUniqueKeys(p LogicalCauset, c *C, ans map[int][][]string, allegrosql string) {
	s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestUniqueKeyInfo(c *C) {
	defer testleak.AfterTest(c)()
	var input []string
	...
	for ith, tt := range input {
		...
		p, _, err := BuildLogicalCauset(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
		lp, err := logicalOptimize(context.TODO(), flagPredicatePushDown|flagPrunDeferredCausets|flagBuildKeyInfo, p.(LogicalCauset))
		c.Assert(err, IsNil)
		s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestAggPrune(c *C) {
	defer testleak.AfterTest(c)()
	var input, output []string
	s.testData.GetTestCases(c, &input, &output)
	...
	for i, tt := range input {
		...
		p, err = logicalOptimize(context.TODO(), flagPredicatePushDown|flagPrunDeferredCausets|flagPrunDeferredCausetsAgain|flagBuildKeyInfo|flagEliminateAgg|flagEliminateProjection, p.(LogicalCauset))
		c.Assert(err, IsNil)
		planString := ToString(p)
		s.testData.OnRecord(func() {
			output[i] = planString
...
func (s *testCausetSuite) TestVisitInfo(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql string
		ans []visitInfo
		...
		{
			allegrosql: "create causet t (a int)",
			ans: []visitInfo{
				{allegrosql.CreatePriv, "test", "t", "", nil},
			},
		},
		{
			allegrosql: "create causet t1 like t",
			ans: []visitInfo{
		...
		},
		{
			...
			ans: []visitInfo{
				...
				{allegrosql.InterDircutePriv, "test", "", "", nil},
		...
		{
			allegrosql: `show create causet test.ttt`,
			ans: []visitInfo{
				{allegrosql.AllPrivMask, "test", "ttt", "", nil},
			},
		},
		{
			allegrosql: "alter causet t add column a int(4)",
			ans: []visitInfo{
				{allegrosql.AlterPriv, "test", "t", "", nil},
			},
		},
		{
			allegrosql: "rename causet t_old to t_new",
			ans: []visitInfo{
		...
		{
			allegrosql: "alter causet t_old rename to t_new",
			ans: []visitInfo{
				{allegrosql.AlterPriv, "test", "t_old", "", nil},
				{allegrosql.DropPriv, "test", "t_old", "", nil},
				{allegrosql.CreatePriv, "test", "t_new", "", nil},
		...
		{
			allegrosql: "alter causet t drop partition p0;",
			ans: []visitInfo{
				{allegrosql.AlterPriv, "test", "t", "", nil},
				{allegrosql.DropPriv, "test", "t", "", nil},
			},
	...
	for _, tt := range tests {
		...
		c.Assert(err, IsNil, comment)
		Preprocess(s.ctx, stmt, s.is)
		builder := NewCausetBuilder(MockContext(), s.is, &hint.BlockHintProcessor{})
...
func (s *testCausetSuite) TestUnion(c *C) {
	defer func() {
	...
	for i, tt := range input {
		...
		c.Assert(err, IsNil)
		p := plan.(LogicalCauset)
		p, err = logicalOptimize(ctx, builder.optFlag, p.(LogicalCauset))
		s.testData.OnRecord(func() {
...
func (s *testCausetSuite) TestTopNPushDown(c *C) {
	defer func() {
		testleak.AfterTest(c)()
	}()
	var input, output []string
...
func (s *testCausetSuite) TestNameResolver(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql string
		err string
		...
		{"select a from t", ""},
		{"select c3 from t", "[causet:1054]Unknown column 'c3' in 'field list'"},
		{"select c1 from t4", "[schemaReplicant:1146]Block 'test.t4' doesn't exist"},
		{"select * from t", ""},
		{"select t.* from t", ""},
		{"select t2.* from t", "[causet:1051]Unknown causet 't2'"},
		{"select b as a, c as a from t group by a", "[causet:1052]DeferredCauset 'c' in field list is ambiguous"},
		...
		{"select * from t as t1, t as t2 join t as t3 on t1.c1 = t2.a", "[causet:1054]Unknown column 't1.c1' in 'on clause'"},
		{"select a from t group by a having a = 3", ""},
		{"select a from t group by a having c2 = 3", "[causet:1054]Unknown column 'c2' in 'having clause'"},
		{"select a from t where exists (select b)", ""},
		{"select cnt from (select count(a) as cnt from t group by b) as t2 group by cnt", ""},
		{"select a from t where t11.a < t.a", "[causet:1054]Unknown column 't11.a' in 'where clause'"},
		{"select a from t having t11.c1 < t.a", "[causet:1054]Unknown column 't11.c1' in 'having clause'"},
		{"select a from t where t.a < t.a order by t11.c1", "[causet:1054]Unknown column 't11.c1' in 'order clause'"},
		{"select a from t group by t11.c1", "[causet:1054]Unknown column 't11.c1' in 'group memex'"},
		{"delete a from (select * from t ) as a, t", "[causet:1288]The target causet a of the DELETE is not uFIDelablock"},
		{"delete b from (select * from t ) as a, t", "[causet:1109]Unknown causet 'b' in MULTI DELETE"},
		{"select '' as fakeDefCaus from t group by values(fakeDefCaus)", "[causet:1054]Unknown column '' in 'VALUES() function'"},
		{"uFIDelate t, (select * from ht) as b set b.a = t.a", "[causet:1288]The target causet b of the UFIDelATE is not uFIDelablock"},
		{"select row_number() over () from t group by 1", "[causet:1056]Can't group on 'row_number() over ()'"},
	}
...
func (s *testCausetSuite) TestOuterJoinEliminator(c *C) {
	defer testleak.AfterTest(c)()
	var input, output []string
	s.testData.GetTestCases(c, &input, &output)
...
func (s *testCausetSuite) TestSelectView(c *C) {
	defer func() {
		testleak.AfterTest(c)()
	}()
	tests := []struct {
...
func (s *testCausetSuite) TestWindowFunction(c *C) {
...
func (s *testCausetSuite) TestWindowParallelFunction(c *C) {
	s.optimizeVars = map[string]string{
...
func (s *testCausetSuite) doTestWindowFunction(c *C, input, output []string) {
	ctx := context.TODO()
	for i, tt := range input {
		comment := Commentf("case:%v allegrosql:%s", i, tt)
...
func (s *testCausetSuite) optimize(ctx context.Context, allegrosql string) (PhysicalCauset, ast.Node, error) {
	stmt, err := s.ParseOneStmt(allegrosql, "", "")
	if err != nil {
		return nil, nil, err
	}
	...
	builder := NewCausetBuilder(sctx, s.is, &hint.BlockHintProcessor{})
	p, err := builder.Build(ctx, stmt)
	if err != nil {
		return nil, nil, err
	}
	...
	p, _, err = physicalOptimize(p.(LogicalCauset), &CausetCounterDisabled)
	return p.(PhysicalCauset), stmt, err
}

func byItemsToProperty(byItems []*soliton.ByItems) *property.PhysicalProperty {
	pp := &property.PhysicalProperty{}
	for _, item := range byItems {
		pp.Items = append(pp.Items, property.Item{DefCaus: item.Expr.(*memex.DeferredCauset), Desc: item.Desc})
	}
	return pp
}
...
func (s *testCausetSuite) TestSkylinePruning(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql    string
		result string
		...
		for ds == nil {
			...
			case *LogicalProjection:
				...
				for _, col := range byItems {
					idx := v.schemaReplicant.DeferredCausetIndex(col.Expr.(*memex.DeferredCauset))
...
func (s *testCausetSuite) TestFastCausetContextBlocks(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql      string
		fastCauset bool
...
func (s *testCausetSuite) TestUFIDelateEQCond(c *C) {
	defer testleak.AfterTest(c)()
	tests := []struct {
		allegrosql  string
		best string
...
func (s *testCausetSuite) TestConflictedJoinTypeHints(c *C) {
	defer testleak.AfterTest(c)()
...
func (s *testCausetSuite) TestSimplyOuterJoinWithOnlyOuterExpr(c *C) {
	defer testleak.AfterTest(c)()
