...
func (s *testMemoSuite) SetUpSuite(c *C) {
	testleak.BeforeTest()
	s.is = schemareplicant.MockSchemaReplicant([]*perceptron.BlockInfo{causetcore.MockSignedBlock()})
	s.sctx = causetcore.MockContext()
...
func (s *testMemoSuite) TestGrouFIDeleleteAll(c *C) {
	expr := NewGroupExpr(causetcore.LogicalSelection{}.Init(s.sctx, 0))
	g := NewGroupWithSchema(expr, s.schemaReplicant)
	c.Assert(g.Insert(NewGroupExpr(causetcore.LogicalLimit{}.Init(s.sctx, 0))), IsTrue)
	c.Assert(g.Insert(NewGroupExpr(causetcore.LogicalProjection{}.Init(s.sctx, 0))), IsTrue)
...
func (s *testMemoSuite) TestGroupFingerPrint(c *C) {
	...
	c.Assert(err, IsNil)
	p1, _, err := causetcore.BuildLogicalCauset(context.Background(), s.sctx, stmt1, s.is)
	c.Assert(err, IsNil)
	logic1, ok := p1.(causetcore.LogicalCauset)
	c.Assert(ok, IsTrue)
	// Causet tree should be: DataSource -> Selection -> Projection
	proj, ok := logic1.(*causetcore.LogicalProjection)
	c.Assert(ok, IsTrue)
	sel, ok := logic1.Children()[0].(*causetcore.LogicalSelection)
	c.Assert(ok, IsTrue)
	...
	limit := causetcore.LogicalLimit{}.Init(proj.SCtx(), 0)
	...
	newSelection := causetcore.LogicalSelection{
...
func (s *testMemoSuite) TestGroupGetFirstElem(c *C) {
	expr0 := NewGroupExpr(causetcore.LogicalProjection{}.Init(s.sctx, 0))
	expr1 := NewGroupExpr(causetcore.LogicalLimit{}.Init(s.sctx, 0))
	expr2 := NewGroupExpr(causetcore.LogicalProjection{}.Init(s.sctx, 0))
	expr3 := NewGroupExpr(causetcore.LogicalLimit{}.Init(s.sctx, 0))
	expr4 := NewGroupExpr(causetcore.LogicalProjection{}.Init(s.sctx, 0))
...
func (impl *fakeImpl) GetCauset() causetcore.PhysicalCauset               { return impl.plan }
...
func (s *testMemoSuite) TestGetInsertGroupImpl(c *C) {
	g := NewGroupWithSchema(NewGroupExpr(causetcore.LogicalLimit{}.Init(s.sctx, 0)), s.schemaReplicant)
	...
	impl = &fakeImpl{plan: &causetcore.PhysicalLimit{}}
...
func (s *testMemoSuite) TestFirstElemAfterDelete(c *C) {
	oldExpr := NewGroupExpr(causetcore.LogicalLimit{Count: 10}.Init(s.sctx, 0))
	g := NewGroupWithSchema(oldExpr, s.schemaReplicant)
	newExpr := NewGroupExpr(causetcore.LogicalLimit{Count: 20}.Init(s.sctx, 0))
	g.Insert(newExpr)
...
func (s *testMemoSuite) TestBuildKeyInfo(c *C) {
	...
	c.Assert(err, IsNil)
	p2, _, err := causetcore.BuildLogicalCauset(context.Background(), s.sctx, stmt2, s.is)
	c.Assert(err, IsNil)
	logic2, ok := p2.(causetcore.LogicalCauset)
	c.Assert(ok, IsTrue)
	...
	newSel := causetcore.LogicalSelection{}.Init(s.sctx, 0)
	...
	newLimit := causetcore.LogicalLimit{Count: 1}.Init(s.sctx, 0)
