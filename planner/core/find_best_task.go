...
func (ds *DataSource) convertToIndexScan(prop *property.PhysicalProperty, candidate *candidatePath) (task task, err error) {
	...
	cop.cst = cost
	task = cop
	if cop.blockPlan != nil && ds.blockInfo.IsCommonHandle {
		cop.commonHandleDefCauss = ds.commonHandleDefCauss
	}
	if candidate.isMatchProp {
		if cop.blockPlan != nil && !ds.blockInfo.IsCommonHandle {
			col, isNew := cop.blockPlan.(*PhysicalBlockScan).appendExtraHandleDefCaus(ds)
...
func (is *PhysicalIndexScan) addPushedDownSelection(copTask *copTask, p *DataSource, path *soliton.AccessPath, finalStats *property.StatsInfo) {
	...
	if len(blockConds) > 0 {
		...
		blockSel.SetChildren(copTask.blockPlan)
		copTask.blockPlan = blockSel
	}
}
