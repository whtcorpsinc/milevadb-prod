...
func (p *PhysicalTopN) canPushDown(cop *copTask) bool {
	...
	return expression.CanExprsPushDown(p.ctx.GetStochastikVars().StmtCtx, exprs, p.ctx.GetClient(), cop.getStoreType())
}
...
func (p *PhysicalTopN) attach2Task(tasks ...task) task {
	...
	if copTask, ok := t.(*copTask); ok && p.canPushDown(copTask) && len(copTask.rootTaskConds) == 0 {
		...
		if !copTask.indexPlanFinished && p.allDefCaussFromSchema(copTask.indexPlan.Schema()) {
			pushedDownTopN = p.getPushedDownTopN(copTask.indexPlan)
			copTask.indexPlan = pushedDownTopN
		} else {
			copTask.finishIndexPlan()
			pushedDownTopN = p.getPushedDownTopN(copTask.blockPlan)
			copTask.blockPlan = pushedDownTopN
		}
...
func CheckAggCanPushCop(sctx stochastikctx.Context, aggFuncs []*aggregation.AggFuncDesc, groupByItems []expression.Expression, storeType ekv.StoreType) bool {
...
func (p *basePhysicalAgg) newPartialAggregate(copTaskType ekv.StoreType) (partial, final PhysicalPlan) {
	...
	if copTaskType == ekv.MilevaDB {
		// For partial agg of MilevaDB cop task, since MilevaDB interlock reuse the MilevaDB executor,
...
func (p *PhysicalStreamAgg) attach2Task(tasks ...task) task {
	...
	if cop, ok := t.(*copTask); ok {
		...
		if cop.extraHandleDefCaus != nil || len(cop.rootTaskConds) > 0 {
			...
			if partialAgg != nil {
				if cop.blockPlan != nil {
					cop.finishIndexPlan()
					partialAgg.SetChildren(cop.blockPlan)
					cop.blockPlan = partialAgg
				} else {
