...
func (e *Explain) explainPlanInRowFormat(p Plan, taskType, driverSide, indent string, isLastChild bool) (err error) {
	...
	case *PhysicalBlockReader:
		...
		err = e.explainPlanInRowFormat(x.blockPlan, "cop["+storeType+"]", "", childIndent, true)
	case *PhysicalIndexReader:
		err = e.explainPlanInRowFormat(x.indexPlan, "cop[einsteindb]", "", childIndent, true)
	case *PhysicalIndexLookUpReader:
		err = e.explainPlanInRowFormat(x.indexPlan, "cop[einsteindb]", "(Build)", childIndent, false)
		err = e.explainPlanInRowFormat(x.blockPlan, "cop[einsteindb]", "(Probe)", childIndent, true)
	case *PhysicalIndexMergeReader:
		for _, pchild := range x.partialPlans {
			err = e.explainPlanInRowFormat(pchild, "cop[einsteindb]", "(Build)", childIndent, false)
		}
		err = e.explainPlanInRowFormat(x.blockPlan, "cop[einsteindb]", "(Probe)", childIndent, true)
	case *Insert:
		if x.SelectPlan != nil {
...
func (e *Explain) prepareTaskDot(p PhysicalPlan, taskTp string, buffer *bytes.Buffer) {
	...
	for planQueue := []PhysicalPlan{p}; len(planQueue) > 0; planQueue = planQueue[1:] {
		...
		case *PhysicalBlockReader:
			pipelines = append(pipelines, fmt.Sprintf("\"%s\" -> \"%s\"\n", copPlan.ExplainID(), copPlan.blockPlan.ExplainID()))
			CausetTasks = append(CausetTasks, copPlan.blockPlan)
		case *PhysicalIndexReader:
			pipelines = append(pipelines, fmt.Sprintf("\"%s\" -> \"%s\"\n", copPlan.ExplainID(), copPlan.indexPlan.ExplainID()))
			CausetTasks = append(CausetTasks, copPlan.indexPlan)
		case *PhysicalIndexLookUpReader:
			pipelines = append(pipelines, fmt.Sprintf("\"%s\" -> \"%s\"\n", copPlan.ExplainID(), copPlan.blockPlan.ExplainID()))
			pipelines = append(pipelines, fmt.Sprintf("\"%s\" -> \"%s\"\n", copPlan.ExplainID(), copPlan.indexPlan.ExplainID()))
			CausetTasks = append(CausetTasks, copPlan.blockPlan)
		...
		case *PhysicalIndexMergeReader:
			for i := 0; i < len(copPlan.partialPlans); i++ {
				...
				CausetTasks = append(CausetTasks, copPlan.partialPlans[i])
			}
			if copPlan.blockPlan != nil {
				pipelines = append(pipelines, fmt.Sprintf("\"%s\" -> \"%s\"\n", copPlan.ExplainID(), copPlan.blockPlan.ExplainID()))
				CausetTasks = append(CausetTasks, copPlan.blockPlan)
			}
		}
