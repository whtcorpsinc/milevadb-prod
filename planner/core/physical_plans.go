...
// PhysicalLock is the physical operator of lock, which is used for `select ... for uFIDelate` clause.
...
func DefCauslectPlanStatsVersion(plan PhysicalPlan, statsInfos map[string]uint64) map[string]uint64 {
	...
	case *PhysicalBlockReader:
		statsInfos = DefCauslectPlanStatsVersion(copPlan.blockPlan, statsInfos)
	case *PhysicalIndexReader:
		statsInfos = DefCauslectPlanStatsVersion(copPlan.indexPlan, statsInfos)
	...
	case *PhysicalIndexScan:
		statsInfos[copPlan.Block.Name.O] = copPlan.stats.StatsVersion
