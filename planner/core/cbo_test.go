...
func (s *testAnalyzeSuite) TestCBOWithoutAnalyze(c *C) {
	...
	testKit.MustQuery("explain select * from t1, t2 where t1.a = t2.a").Check(testkit.Rows(
		...
		"│   └─BlockFullScan_13 6.00 cop[einsteindb] block:t2 keep order:false, stats:pseudo",
		"└─BlockReader_12(Probe) 5.99 root  data:Selection_11",
		"  └─Selection_11 5.99 cop[einsteindb]  not(isnull(test.t1.a))",
		"    └─BlockFullScan_10 6.00 cop[einsteindb] block:t1 keep order:false, stats:pseudo",
	))
...
func (s *testAnalyzeSuite) TestEstimation(c *C) {
	...
	testKit.MustQuery("explain select count(*) from t group by a").Check(testkit.Rows(
		...
		"    └─BlockFullScan_8 8.00 cop[einsteindb] block:t keep order:false",
	))
}
...
func (s *testAnalyzeSuite) TestOutdatedAnalyze(c *C) {
	...
	testKit.MustQuery("explain select * from t where a <= 5 and b <= 5").Check(testkit.Rows(
		...
		"  └─BlockFullScan_5 80.00 cop[einsteindb] block:t keep order:false",
	))
	statistics.RatioOfPseudoEstimate.CausetStore(0.7)
	testKit.MustQuery("explain select * from t where a <= 5 and b <= 5").Check(testkit.Rows(
		"BlockReader_7 8.84 root  data:Selection_6",
		"└─Selection_6 8.84 cop[einsteindb]  le(test.t.a, 5), le(test.t.b, 5)",
		"  └─BlockFullScan_5 80.00 cop[einsteindb] block:t keep order:false, stats:pseudo",
	))
}
...
func (s *testAnalyzeSuite) TestInconsistentEstimation(c *C) {
	...
	tk.MustQuery("explain select * from t use index(ab) where a = 5 and c = 5").
		Check(testkit.Rows(
			...
			"├─IndexRangeScan_5(Build) 12.50 cop[einsteindb] block:t, index:ab(a, b) range:[5,5], keep order:false",
			"└─Selection_7(Probe) 10.00 cop[einsteindb]  eq(test.t.c, 5)",
			"  └─BlockRowIDScan_6 12.50 cop[einsteindb] block:t keep order:false",
		))
}
