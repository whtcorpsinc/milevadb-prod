...
var MetricBlockMap = map[string]MetricBlockDef{
	...
	"milevadb_kv_region_error_total_count": {
	...
	"milevadb_kv_write_num": {
		PromQL:   "histogram_quantile($QUANTILE, sum(rate(milevadb_einsteindbclient_txn_write_kv_num_bucket{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (le, instance))",
		Labels:   []string{"instance"},
		Quantile: 1,
	...
	"einsteindb_scheduler_keys_read": {
		PromQL:   `histogram_quantile($QUANTILE, sum(rate(einsteindb_scheduler_kv_command_key_read_bucket{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (le,instance,type))`,
		Labels:   []string{"instance", "type"},
		Quantile: 0.99,
		Comment:  "The quantile count of keys read by command",
	},
	"einsteindb_scheduler_keys_read_avg": {
		PromQL:  `sum(rate(einsteindb_scheduler_kv_command_key_read_sum{$LABEL_CONDITIONS}[$RANGE_DURATION])) / sum(rate(einsteindb_scheduler_kv_command_key_read_count{$LABEL_CONDITIONS}[$RANGE_DURATION])) `,
		Labels:  []string{"instance", "type"},
		Comment: "The average count of keys read by command",
	},
	"einsteindb_scheduler_keys_written": {
		PromQL:   `histogram_quantile($QUANTILE, sum(rate(einsteindb_scheduler_kv_command_key_write_bucket{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (le,instance,type))`,
		Labels:   []string{"instance", "type"},
		Quantile: 0.99,
		Comment:  "The quantile of count of keys written by a command",
	},
	"einsteindb_scheduler_keys_written_avg": {
		PromQL:  `sum(rate(einsteindb_scheduler_kv_command_key_write_sum{$LABEL_CONDITIONS}[$RANGE_DURATION])) / sum(rate(einsteindb_scheduler_kv_command_key_write_count{$LABEL_CONDITIONS}[$RANGE_DURATION])) `,
		Labels:  []string{"instance", "type"},
		Comment: "The average count of keys written by a command",
	},
	"einsteindb_scheduler_scan_details": {
		PromQL:  `sum(rate(einsteindb_scheduler_kv_scan_details{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (tag,instance,req,cf)`,
	...
	"einsteindb_scheduler_scan_details_total_num": {
		PromQL: `sum(increase(einsteindb_scheduler_kv_scan_details{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (tag,instance,req,cf)`,
		Labels: []string{"instance", "tag", "req", "cf"},
	},
	...
	"einsteindb_snapshot_kv_count": {
		PromQL:   `histogram_quantile($QUANTILE, sum(rate(einsteindb_snapshot_kv_count_bucket{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (le,instance))`,
		Labels:   []string{"instance"},
		Quantile: 0.9999,
	...
	"milevadb_kv_backoff_total_count": {
	...
	"milevadb_kv_backoff_total_time": {
	...
	"milevadb_kv_request_total_count": {
	...
	"milevadb_kv_request_total_time": {
	...
	"milevadb_kv_write_size_total_count": {
	...
	"milevadb_kv_write_total_num": {
		PromQL:  "sum(increase(milevadb_einsteindbclient_txn_write_kv_num_sum{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance)",
		Labels:  []string{"instance"},
		Comment: "The total num of ekv write in transaction execution",
	},
	"milevadb_kv_write_total_size": {
	...
	"einsteindb_cop_kv_cursor_operations_total_count": {
	...
	"einsteindb_scheduler_keys_read_total_count": {
		PromQL:  "sum(increase(einsteindb_scheduler_kv_command_key_read_count{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance,type)",
		Labels:  []string{"instance", "type"},
		Comment: "The total count of keys read by a command",
	},
	"einsteindb_scheduler_keys_total_read": {
		PromQL:  "sum(increase(einsteindb_scheduler_kv_command_key_read_sum{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance,type)",
		Labels:  []string{"instance", "type"},
		Comment: "The total count of keys read by command",
	},
	"einsteindb_scheduler_keys_total_written": {
		PromQL:  "sum(increase(einsteindb_scheduler_kv_command_key_write_sum{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance,type)",
		Labels:  []string{"instance", "type"},
		Comment: "The total count of keys written by a command",
	},
	"einsteindb_scheduler_keys_written_total_count": {
		PromQL:  "sum(increase(einsteindb_scheduler_kv_command_key_write_count{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance,type)",
		Labels:  []string{"instance", "type"},
		Comment: "The total count of keys written by a command",
	},
	"einsteindb_snapshot_kv_count_total_count": {
		PromQL:  "sum(increase(einsteindb_snapshot_kv_count_count{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance)",
		Labels:  []string{"instance"},
		Comment: "The total count of number of KV within a snapshot",
	},
	"einsteindb_snapshot_kv_total_count": {
		PromQL:  "sum(increase(einsteindb_snapshot_kv_count_sum{$LABEL_CONDITIONS}[$RANGE_DURATION])) by (instance)",
		Labels:  []string{"instance"},
