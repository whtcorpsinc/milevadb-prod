...
func (s *testVarsutilSuite) TestNewStochastikVars(c *C) {
	...
	c.Assert(vars.FoundInPlanCache, Equals, DefMilevaDBFoundInPlanCache)
...
func (s *testVarsutilSuite) TestVarsutil(c *C) {
	...
	err = SetStochastikSystemVar(v, MilevaDBFoundInPlanCache, types.NewStringCauset("1"))
	c.Assert(err, IsNil)
	val, err = GetStochastikSystemVar(v, MilevaDBFoundInPlanCache)
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "0")
	c.Assert(v.systems[MilevaDBFoundInPlanCache], Equals, "1")
...
func (s *testVarsutilSuite) TestConcurrencyVariables(c *C) {
	...
	err = SetStochastikSystemVar(vars, MilevaDBExecutorConcurrency, types.NewIntCauset(int64(exeConcurrency)))
	c.Assert(err, IsNil)
	c.Assert(vars.indexLookupConcurrency, Equals, ConcurrencyUnset)
