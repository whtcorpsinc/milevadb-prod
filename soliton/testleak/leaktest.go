...
func interestingGoroutines() (gs []string) {
	...
	ignoreList := []string{
		"created by github.com/whtcorpsinc/milevadb.init",
		"testing.RunTests",
		"check.(*resultTracker).start",
		"check.(*suiteRunner).runFunc",
		"check.(*suiteRunner).parallelRun",
		"localstore.(*dbStore).scheduler",
		"einsteindb.(*noGCHandler).Start",
		"dbs.(*dbs).start",
		"dbs.(*delRange).startEmulator",
		"petri.NewPetri",
		"testing.(*T).Run",
		"petri.(*Petri).LoadPrivilegeLoop",
		"petri.(*Petri).UFIDelateBlockStatsLoop",
		"testing.Main(",
		"runtime.goexit",
		...
		"github.com/whtcorpsinc/goleveldb/leveldb/soliton.(*BufferPool).drain",
		"github.com/whtcorpsinc/goleveldb/leveldb.(*EDB).compactionError",
		"github.com/whtcorpsinc/goleveldb/leveldb.(*EDB).mpoolDrain",
		"go.etcd.io/etcd/pkg/logutil.(*MergeLogger).outputLoop",
		"go.etcd.io/etcd/v3/pkg/logutil.(*MergeLogger).outputLoop",
		"oracles.(*FIDelOracle).uFIDelateTS",
		"einsteindb.(*einsteindbStore).runSafePointChecker",
		"einsteindb.(*RegionCache).asyncCheckAndResolveLoop",
		"github.com/whtcorpsinc/badger",
		"github.com/ngaut/entangledstore/einsteindb.(*MVCCStore).runUFIDelateSafePointLoop",
	}
...
// Now it's only used in the milevadb_test.go.
