...
const (
	...
	TreeBody = '│'
	// TreeMidbseNode indicates this operator is not the last child of the
...
func PrettyIdentifier(id, indent string, isLastChild bool) string {
	...
	for i := len(indentBytes) - 1; i >= 0; i-- {
		...
		// 2. treeMidbseNode, if this operator is not the last child..
		if isLastChild {
			indentBytes[i] = TreeLastNode
		} else {
			indentBytes[i] = TreeMidbseNode
		}
		break
	}
