...
func Copy(src string) string {
	return string(replog.Slice(src))
}
...
// (`"`) depending on whether allegrosql.ModeANSIQuotes is enabled.
func Escape(str string, sqlMode allegrosql.ALLEGROSQLMode) string {
	var quote string
	if sqlMode&allegrosql.ModeANSIQuotes != 0 {
		quote = `"`
	} else {
		quote = "`"
	}
