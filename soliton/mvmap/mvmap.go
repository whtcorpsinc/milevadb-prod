...
type MVMap struct {
	...
	dataStore  dataStore
	hashBlock  map[uint64]entryAddr
	length     int
}
...
func NewMVMap() *MVMap {
	m := new(MVMap)
	m.hashBlock = make(map[uint64]entryAddr)
...
func (m *MVMap) Put(key, value []byte) {
	...
	oldEntryAddr := m.hashBlock[hashKey]
	...
	m.hashBlock[hashKey] = newEntryAddr
	m.length++
}
