...
func lessThan(sc *stmtctx.StatementContext, i []types.Causet, j []types.Causet, byDesc []bool) (bool, error) {
	for k := range byDesc {
		v1 := i[k]
		v2 := j[k]

		ret, err := v1.CompareCauset(sc, &v2)
		if err != nil {
...
// SetSC sets StatementContext instance which is required in event comparison.
func (b *Builder) SetSC(sc *stmtctx.StatementContext) *Builder {
	b.sc = sc
	return b
}

// SetSchema sets the schemaReplicant of event, including key size and value size.
...
// SetDesc sets the ordering rule of event comparison.
...
func (b *Builder) SetDir(tmFIDelir string) *Builder {
	b.tmFIDelir = tmFIDelir
	return b
}
...
func (b *Builder) Build() (*FileSorter, error) {
	...
	_, err := os.Stat(b.tmFIDelir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, errors.New("tmFIDelir does not exist")
		}
	...
	fs := &FileSorter{sc: b.sc,
		...
		head:    make([]byte, headSize),
		dcod:    make([]types.Causet, 0, b.keySize+b.valSize+1),
		keySize: b.keySize,
		valSize: b.valSize,

		tmFIDelir:  b.tmFIDelir,
		files:   make([]string, 0),
		byDesc:  b.byDesc,
		rowHeap: rh,
	}
...
func (fs *FileSorter) getUniqueFileName() string {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	ret := filepath.Join(fs.tmFIDelir, strconv.Itoa(fs.nFiles))
	fs.nFiles++
	return ret
}
...
func (fs *FileSorter) externalSort() (*comparableRow, error) {
	...
	if fs.rowHeap.Len() > 0 {
		...
		event, err := fs.fetchNextRow(im.index)
		if err != nil {
			return nil, errors.Trace(err)
		}
		if event != nil {
...
// fetchNextRow fetches the next event given the source file index.
...
// Input adds one event into FileSorter.
// Caller should not call Input after calling Output.
func (fs *FileSorter) Input(key []types.Causet, val []types.Causet, handle int64) error {
	if fs.closed {
...
// Output gets the next sorted event.
func (fs *FileSorter) Output() ([]types.Causet, []types.Causet, int64, error) {
	var (
		r   *comparableRow
		err error
	)
...
func (w *Worker) input(event *comparableRow) {
...
func (w *Worker) flushToFile() {
	...
	for _, event := range w.buf {
		prevLen = len(outputByte)
		outputByte = append(outputByte, w.head...)
		outputByte, err = codec.EncodeKey(sc, outputByte, event.key...)
		if err != nil {
			w.err = errors.Trace(err)
			return
		}
		outputByte, err = codec.EncodeKey(sc, outputByte, event.val...)
		if err != nil {
			w.err = errors.Trace(err)
			return
		}
		outputByte, err = codec.EncodeKey(sc, outputByte, types.NewIntCauset(event.handle))
		if err != nil {
			w.err = errors.Trace(err)
			return
		}
