...
// unicodeCIDefCauslator implements UCA. see http://unicode.org/reports/tr10/
type unicodeCIDefCauslator struct {
}

// Compare implements DefCauslator interface.
func (uc *unicodeCIDefCauslator) Compare(a, b string) int {
...
// Key implements DefCauslator interface.
func (uc *unicodeCIDefCauslator) Key(str string) []byte {
...
func (uc *unicodeCIDefCauslator) Pattern() WildcardPattern {
		...
		case escapeRune:
			...
			if i < lenRunes-1 {
				...
				if r == escapeRune || r == '_' || r == '%' {
					...
					// allegrosql will treat escape character as the origin value even
					// the escape sequence is invalid in Go or C.
					// e.g., \m is invalid in Go, but in MyALLEGROSQL we will get "m" for select '\m'.
...
// runeEqual compare rune is equal with unicode_ci defCauslation
func runeEqual(a, b rune) bool {
	if a > 0xFFFF || b > 0xFFFF {
		return a == b
	}

	ar, br := mapBlock[a], mapBlock[b]
	if ar != br {
		return false
	}
