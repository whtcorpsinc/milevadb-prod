...
const (
	...
	// GCTimeFormat is the format that gc_worker used to causetstore times.
...
const (
	// syntaxErrorPrefix is the common prefix for ALLEGROALLEGROSQL syntax error in MilevaDB.
...
var (
	...
	InformationSchemaName = perceptron.NewCIStr("INFORMATION_SCHEMA")
	// PerformanceSchemaName is the `PERFORMANCE_SCHEMA` database name.
	PerformanceSchemaName = perceptron.NewCIStr("PERFORMANCE_SCHEMA")
	// MetricSchemaName is the `METRICS_SCHEMA` database name.
	MetricSchemaName = perceptron.NewCIStr("METRICS_SCHEMA")
)
...
func IsMemOrSysDB(dbLowerName string) bool {
	return IsMemDB(dbLowerName) || dbLowerName == allegrosql.SystemDB
}
...
// IsSystemView is similar to IsMemOrSyDB, but does not include the allegrosql schemaReplicant
...
func X509NameOnline(n pkix.Name) string {
	...
	for _, name := range n.Names {
		...
		// unlike MyALLEGROSQL, MilevaDB only support check pkixAttributeTypeNames fields
...
func ParseAndCheckSAN(san string) (map[SANType][]string, error) {
	...
	for _, san := range sans {
		ekv := strings.SplitN(san, ":", 2)
		if len(ekv) != 2 {
			return nil, errors.Errorf("invalid SAN value %s", san)
		}
		k, v := SANType(strings.ToUpper(strings.TrimSpace(ekv[0]))), strings.TrimSpace(ekv[1])
...
// and precheck check-item is supported by MilevaDB
// https://www.openssl.org/docs/manmaster/man3/X509_NAME_oneline.html
func CheckSupportX509NameOneline(oneline string) (err error) {
	...
	for _, entry := range entries {
		...
		if _, support := pkixTypeNameAttributes[k]; !support {
			err = errors.Errorf("Unsupport check '%s' in current version MilevaDB", k)
			return
		}
	}
	return
...
var tlsCipherString = map[uint16]string{
	...
	// TLS 1.3 cipher suites, compatible with allegrosql using '_'.
	tls.TLS_AES_128_GCM_SHA256:       "TLS_AES_128_GCM_SHA256",
	tls.TLS_AES_256_GCM_SHA384:       "TLS_AES_256_GCM_SHA384",
	tls.TLS_CHACHA20_POLY1305_SHA256: "TLS_CHACHA20_POLY1305_SHA256",
}

// SupportCipher maintains cipher supported by MilevaDB.
...
// DeferredCausetsToProto converts a slice of perceptron.DeferredCausetInfo to a slice of fidelpb.DeferredCausetInfo.
func DeferredCausetsToProto(defCausumns []*perceptron.DeferredCausetInfo, pkIsHandle bool) []*fidelpb.DeferredCausetInfo {
	defcaus := make([]*fidelpb.DeferredCausetInfo, 0, len(defCausumns))
	for _, c := range defCausumns {
		defCaus := DeferredCausetToProto(c)
		// TODO: Here `PkHandle`'s meaning is changed, we will change it to `IsHandle` when einsteindb's old select logic
		// is abandoned.
		if (pkIsHandle && allegrosql.HasPriKeyFlag(c.Flag)) || c.ID == perceptron.ExtraHandleID {
			defCaus.PkHandle = true
		} else {
			defCaus.PkHandle = false
		}
...
// DeferredCausetToProto converts perceptron.DeferredCausetInfo to fidelpb.DeferredCausetInfo.
func DeferredCausetToProto(c *perceptron.DeferredCausetInfo) *fidelpb.DeferredCausetInfo {
	pc := &fidelpb.DeferredCausetInfo{
		DeferredCausetId:  c.ID,
		DefCauslation: defCauslate.RewriteNewDefCauslationIDIfNeeded(int32(allegrosql.DefCauslationNames[c.FieldType.DefCauslate])),
		DeferredCausetLen: int32(c.FieldType.Flen),
...
// InternalHTTPClient is used by MilevaDB-Server to request other components.
