...
type RowContainer struct {
	...
	actionSpill *SpillDiskCausetAction
}
...
// GetRow returns the event the ptr pointed to.
...
// CausetActionSpill returns a SpillDiskCausetAction for spilling over to disk.
func (c *RowContainer) CausetActionSpill() *SpillDiskCausetAction {
	if c.actionSpill == nil {
		c.actionSpill = &SpillDiskCausetAction{
			c:    c,
...
// CausetActionSpillForTest returns a SpillDiskCausetAction for spilling over to disk for test.
func (c *RowContainer) CausetActionSpillForTest() *SpillDiskCausetAction {
	c.actionSpill = &SpillDiskCausetAction{
		c: c,
...
// SpillDiskCausetAction implements memory.SuperCowOrNoCausetOnExceed for chunk.List. If
// the memory quota of a query is exceeded, SpillDiskCausetAction.CausetAction is
// triggered.
type SpillDiskCausetAction struct {
	c              *RowContainer
	fallbackCausetAction memory.SuperCowOrNoCausetOnExceed
	m              sync.Mutex
	once           sync.Once
...
type spillStatusCond struct {
	*sync.Cond
	// status indicates different stages for the CausetAction
...
func (a *SpillDiskCausetAction) setStatus(status spillStatus) {
	a.cond.L.Lock()
	defer a.cond.L.Unlock()
	a.cond.status = status
}

func (a *SpillDiskCausetAction) getStatus() spillStatus {
	a.cond.L.Lock()
	defer a.cond.L.Unlock()
	return a.cond.status
}

// CausetAction sends a signal to trigger spillToDisk method of RowContainer
// and if it is already triggered before, call its fallbackCausetAction.
func (a *SpillDiskCausetAction) CausetAction(t *memory.Tracker) {
	a.m.Lock()
	defer a.m.Unlock()
	...
	if a.fallbackCausetAction != nil {
		a.fallbackCausetAction.CausetAction(t)
	}
}

// Reset resets the status for SpillDiskCausetAction.
func (a *SpillDiskCausetAction) Reset() {
	a.m.Lock()
	defer a.m.Unlock()
...
func (a *SpillDiskCausetAction) SetFallback(fallback memory.SuperCowOrNoCausetOnExceed) {
	a.fallbackCausetAction = fallback
}

// SetLogHook sets the hook, it does nothing just to form the memory.SuperCowOrNoCausetOnExceed interface.
func (a *SpillDiskCausetAction) SetLogHook(hook func(uint64)) {}

// WaitForTest waits all goroutine have gone.
func (a *SpillDiskCausetAction) WaitForTest() {
	a.testWg.Wait()
}
...
type SortedRowContainer struct {
	*RowContainer
	ptrM struct {
		sync.RWMutex
		// rowPtrs causetstore the chunk index and event index for each event.
	...
	actionSpill *SortAndSpillDiskCausetAction
}
...
func NewSortedRowContainer(fieldType []*types.FieldType, chunkSize int, ByItemsDesc []bool,
	keyDeferredCausets []int, keyCmpFuncs []CompareFunc) *SortedRowContainer {
	return &SortedRowContainer{RowContainer: NewRowContainer(fieldType, chunkSize),
		ByItemsDesc: ByItemsDesc, keyDeferredCausets: keyDeferredCausets, keyCmpFuncs: keyCmpFuncs}
}
...
func (c *SortedRowContainer) lessRow(rowI, rowJ Row) bool {
	for i, defCausIdx := range c.keyDeferredCausets {
...
func (c *SortedRowContainer) keyDeferredCausetsLess(i, j int) bool {
...
func (c *SortedRowContainer) Sort() {
	...
	sort.Slice(c.ptrM.rowPtrs, c.keyDeferredCausetsLess)
...
// GetSortedRow returns the event the idx pointed to.
...
// CausetActionSpill returns a SortAndSpillDiskCausetAction for sorting and spilling over to disk.
func (c *SortedRowContainer) CausetActionSpill() *SortAndSpillDiskCausetAction {
	if c.actionSpill == nil {
		c.actionSpill = &SortAndSpillDiskCausetAction{
			c:               c,
			SpillDiskCausetAction: c.RowContainer.CausetActionSpill(),
		}
	}
	return c.actionSpill
}

// CausetActionSpillForTest returns a SortAndSpillDiskCausetAction for sorting and spilling over to disk for test.
func (c *SortedRowContainer) CausetActionSpillForTest() *SortAndSpillDiskCausetAction {
	c.actionSpill = &SortAndSpillDiskCausetAction{
		c:               c,
		SpillDiskCausetAction: c.RowContainer.CausetActionSpillForTest(),
	}
	return c.actionSpill
}

// SortAndSpillDiskCausetAction implements memory.SuperCowOrNoCausetOnExceed for chunk.List. If
// the memory quota of a query is exceeded, SortAndSpillDiskCausetAction.CausetAction is
// triggered.
type SortAndSpillDiskCausetAction struct {
	c *SortedRowContainer
	*SpillDiskCausetAction
}

// CausetAction sends a signal to trigger sortAndSpillToDisk method of RowContainer
// and if it is already triggered before, call its fallbackCausetAction.
func (a *SortAndSpillDiskCausetAction) CausetAction(t *memory.Tracker) {
	a.m.Lock()
	defer a.m.Unlock()
...
// SetFallback sets the fallback action.
func (a *SortAndSpillDiskCausetAction) SetFallback(fallback memory.SuperCowOrNoCausetOnExceed) {
	a.fallbackCausetAction = fallback
}

// SetLogHook sets the hook, it does nothing just to form the memory.SuperCowOrNoCausetOnExceed interface.
func (a *SortAndSpillDiskCausetAction) SetLogHook(hook func(uint64)) {}

// WaitForTest waits all goroutine have gone.
func (a *SortAndSpillDiskCausetAction) WaitForTest() {
	a.testWg.Wait()
}
