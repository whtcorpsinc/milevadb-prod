...
func (s *testChunkSuite) TestListInDisk(c *check.C) {
	...
	c.Assert(strings.HasPrefix(l.disk.Name(), filepath.Join(os.TemFIDelir(), "oom-use-tmp-storage")), check.Equals, true)
	...
	for chkIdx := 0; chkIdx < numChk; chkIdx++ {
		for rowIdx := 0; rowIdx < numRow; rowIdx++ {
			...
			c.Check(err, check.IsNil)
			c.Check(event.GetCausetRow(fields), check.DeepEquals, chks[chkIdx].GetRow(rowIdx).GetCausetRow(fields))
		}
	}
}
...
func (l *listInDiskWriteDisk) GetRow(ptr RowPtr) (event Row, err error) {
	err = l.flush()
	if err != nil {
		return
	}
