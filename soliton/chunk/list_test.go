...
func (s *testChunkSuite) TestList(c *check.C) {
	...
	// Test add chunk then append event.
	l.Reset()
	...
	event := l.GetRow(ptr)
	c.Assert(event.GetInt64(0), check.Equals, int64(1))
...
func (s *testChunkSuite) TestListPrePreAlloc4RowAndInsert(c *check.C) {
	...
	for event, i := iter4Src.Begin(), 0; event != iter4Src.End(); event, i = iter4Src.Next(), i+1 {
		destList.Insert(destRowPtr[i], event)
	}
