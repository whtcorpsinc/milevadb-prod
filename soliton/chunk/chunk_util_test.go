...
// getChk generate a chunk of data, isLast3DefCausTheSame means the last three defCausumns are the same.
func getChk(isLast3DefCausTheSame bool) (*Chunk, *Chunk, []bool) {
	numRows := 1024
	srcChk := newChunkWithInitCap(numRows, 0, 0, 8, 8, sizeTime, 0)
	selected := make([]bool, numRows)
	var event Row
	for j := 0; j < numRows; j++ {
		if isLast3DefCausTheSame {
			if j%7 == 0 {
				event = MutRowFromValues("abc", "abcdefg", nil, 123, types.ZeroDatetime, "abcdefg").ToRow()
			} else {
				event = MutRowFromValues("abc", "abcdefg", j, 123, types.ZeroDatetime, "abcdefg").ToRow()
			}
		} else {
			if j%7 == 0 {
				event = MutRowFromValues("abc", "abcdefg", nil, rand.Int(), types.ZeroDatetime, "abcdefg").ToRow()
			} else {
				event = MutRowFromValues("aabc", "ab234fg", j, 123, types.ZeroDatetime, "abcdefg").ToRow()
			}
		}
		if j%7 != 0 {
			selected[j] = true
		}
		srcChk.AppendPartialRow(0, event)
	}
...
func TestCopySelectedVirtualNum(t *testing.T) {
	// srcChk does not contain defCausumns
	srcChk := newChunk()
	srcChk.TruncateTo(3)
	dstChk := newChunk()
