...
func (it *Iterator4Chunk) Next() Row {
	...
	event := it.chk.GetRow(int(it.cursor))
	it.cursor++
	return event
}
...
func (it *iterator4RowPtr) Next() Row {
	...
	event := it.li.GetRow(it.ptrs[it.cursor])
	it.cursor++
	return event
}
...
func (it *iterator4RowContainer) Current() Row {
	...
	event, err := it.c.GetRow(RowPtr{uint32(it.chkIdx), uint32(it.rowIdx)})
	if err != nil {
		it.err = err
		it.ReachEnd()
		return it.End()
...
func (it *multiIterator) Current() Row {
	...
	event := it.curIter.Current()
	if event == it.curIter.End() {
		it.err = it.curIter.Error()
	}
	return event
}
