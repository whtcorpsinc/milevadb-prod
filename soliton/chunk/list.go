...
// RowPtr is used to get a event from a list.
...
// AppendRow appends a event to the List, the event is copied to the List.
func (l *List) AppendRow(event Row) RowPtr {
...
func (l *List) Add(chk *Chunk) {
	...
	if chk.NumRows() == 0 {
		...
		panic("chunk appended to List should have at least 1 event")
	}
...
func (l *List) preAlloc4Row(event Row) (ptr RowPtr) {
	...
	chk := l.chunks[chkIdx]
	rowIdx := chk.preAlloc(event)
	l.length++
	return RowPtr{ChkIdx: uint32(chkIdx), RowIdx: rowIdx}
}

// Insert inserts `event` on the position specified by `ptr`.
// Note: Insert will cover the origin data, it should be called after
// PreAlloc.
func (l *List) Insert(ptr RowPtr, event Row) {
	l.chunks[ptr.ChkIdx].insert(int(ptr.RowIdx), event)
}

// ListWalkFunc is used to walk the list.
// If error is returned, it will stop walking.
type ListWalkFunc = func(event Row) error

// Walk iterate the list and call walkFunc for each event.
