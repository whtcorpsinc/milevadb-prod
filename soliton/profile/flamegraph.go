...
// add the value from a sample into the flamegraph PosetDag.
...
func (n *flamegraphNode) add(sample *profile.Sample) {
	...
	for {
		...
		// The previous implementation in MilevaDB identify nodes using location ID,
...
// defCauslectFuncUsage defCauslect the value by given function name
func (n *flamegraphNode) defCauslectFuncUsage(name string) int64 {
	if n.name == name {
		return n.cumValue
	}
	if len(n.children) == 0 {
	...
	for _, child := range n.children {
		usage = child.defCauslectFuncUsage(name) + usage
	}
	return usage
}
...
type flamegraphDefCauslector struct {
	rows      [][]types.Causet
...
func newFlamegraphDefCauslector(p *profile.Profile) *flamegraphDefCauslector {
	locations := make(map[uint64]*profile.Location, len(p.Location))
	for _, loc := range p.Location {
		locations[loc.ID] = loc
	}
	return &flamegraphDefCauslector{locations: locations}
}

func (c *flamegraphDefCauslector) locationName(locID uint64) (funcName, fileLine string) {
...
func (c *flamegraphDefCauslector) defCauslectChild(
	...
	c.rows = append(c.rows, types.MakeCausets(
	...
	for i, child := range children {
		c.defCauslectChild(child, depth+1, indent4Child, node.cumValue, i == len(children)-1)
	}
}

func (c *flamegraphDefCauslector) defCauslect(root *flamegraphNode) {
	c.rows = append(c.rows, types.MakeCausets("root", "100%", "100%", 0, 0, "root"))
	...
	for i, child := range children {
		c.rootChild = i + 1
		c.defCauslectChild(child, 1, indent4Child, root.cumValue, i == len(children)-1)
	}
}
