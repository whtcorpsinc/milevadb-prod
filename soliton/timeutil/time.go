...
func inferTZNameFromFileName(path string) (string, error) {
	...
	// "/var/EDB/timezone/tz/2018e.1.0/zoneinfo/Asia/Shanghai"
...
// SystemLocation returns time.SystemLocation's IANA timezone location. It is MilevaDB's global timezone location.
...
// SetSystemTZ sets systemTZ by the value loaded from allegrosql.milevadb.
func SetSystemTZ(name string) {
	setSysTZOnce.Do(func() {
		systemTZ.CausetStore(name)
	})
}
