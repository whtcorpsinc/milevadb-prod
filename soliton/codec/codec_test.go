...
func (s *testCodecSuite) TestCodecKey(c *C) {
	...
	block := []struct {
		...
		{
			types.MakeCausets(int64(1)),
			types.MakeCausets(int64(1)),
		},

		{
			types.MakeCausets(float32(1), float64(3.15), []byte("123"), "123"),
			types.MakeCausets(float64(1), float64(3.15), []byte("123"), []byte("123")),
		},
		{
			types.MakeCausets(uint64(1), float64(3.15), []byte("123"), int64(-1)),
			types.MakeCausets(uint64(1), float64(3.15), []byte("123"), int64(-1)),
		},

		{
			types.MakeCausets(true, false),
			types.MakeCausets(int64(1), int64(0)),
		},

		{
			types.MakeCausets(nil),
			types.MakeCausets(nil),
		},

		{
			types.MakeCausets(types.NewBinaryLiteralFromUint(100, -1), types.NewBinaryLiteralFromUint(100, 4)),
			types.MakeCausets(uint64(100), uint64(100)),
		},

		{
			types.MakeCausets(types.Enum{Name: "a", Value: 1}, types.Set{Name: "a", Value: 1}),
			types.MakeCausets(uint64(1), uint64(1)),
		},
	}
	...
	var raw types.Causet
	raw.SetRaw([]byte("raw"))
	_, err := EncodeKey(sc, nil, raw)
	c.Assert(err, NotNil)
}

func estimateValuesSize(sc *stmtctx.StatementContext, vals []types.Causet) (int, error) {
	size := 0
...
func (s *testCodecSuite) TestCodecKeyCompare(c *C) {
	...
	block := []struct {
		...
		{
			types.MakeCausets(3.15),
			types.MakeCausets(3.12),
			1,
		},
		{
			types.MakeCausets("abc"),
			types.MakeCausets("abcd"),
			-1,
		},
		{
			types.MakeCausets("abcdefgh"),
			types.MakeCausets("abcdefghi"),
			-1,
		},
		{
			types.MakeCausets(1, "abc"),
			types.MakeCausets(1, "abcd"),
			-1,
		},
		{
			types.MakeCausets(1, "abc", "def"),
			types.MakeCausets(1, "abcd", "af"),
			-1,
		},
		{
			types.MakeCausets(3.12, "ebc", "def"),
			types.MakeCausets(2.12, "abcd", "af"),
			1,
		},
		{
			types.MakeCausets([]byte{0x01, 0x00}, []byte{0xFF}),
			types.MakeCausets([]byte{0x01, 0x00, 0xFF}),
			-1,
		},
		{
			types.MakeCausets([]byte{0x01}, uint64(0xFFFFFFFFFFFFFFF)),
			types.MakeCausets([]byte{0x01, 0x10}, 0),
			-1,
		},
		{
			types.MakeCausets(0),
		...
		{
			types.MakeCausets([]byte{0x00}),
			types.MakeCausets(nil),
			1,
		},
		{
			types.MakeCausets(math.SmallestNonzeroFloat64),
			types.MakeCausets(nil),
			1,
		},
		{
			types.MakeCausets(int64(math.MinInt64)),
			types.MakeCausets(nil),
			1,
		},
		{
			types.MakeCausets(1, int64(math.MinInt64), nil),
			types.MakeCausets(1, nil, uint64(math.MaxUint64)),
			1,
		},
		{
			types.MakeCausets(1, []byte{}, nil),
			types.MakeCausets(1, nil, 123),
			1,
		},
		{
			types.MakeCausets(parseTime(c, "2011-11-11 00:00:00"), 1),
			types.MakeCausets(parseTime(c, "2011-11-11 00:00:00"), 0),
			1,
		},
		{
			types.MakeCausets(parseDuration(c, "00:00:00"), 1),
			types.MakeCausets(parseDuration(c, "00:00:01"), 0),
			-1,
		},
		{
			[]types.Causet{types.MinNotNullCauset()},
			[]types.Causet{types.MaxValueCauset()},
			-1,
		},
	}
	sc := &stmtctx.StatementContext{TimeZone: time.Local}
...
func parseTime(c *C, s string) types.Time {
	...
	m, err := types.ParseTime(sc, s, allegrosql.TypeDatetime, types.DefaultFsp)
	c.Assert(err, IsNil)
	return m
}
...
func (s *testCodecSuite) TestTime(c *C) {
	...
	sc := &stmtctx.StatementContext{TimeZone: time.Local}
	for _, t := range tbl {
		m := types.NewCauset(parseTime(c, t))
		...
		c.Assert(types.NewCauset(t), DeepEquals, m)
	}
	...
	for _, t := range tblCmp {
		m1 := types.NewCauset(parseTime(c, t.Arg1))
		m2 := types.NewCauset(parseTime(c, t.Arg2))
...
func (s *testCodecSuite) TestDuration(c *C) {
	...
	for _, t := range tbl {
		...
		b, err := EncodeKey(sc, nil, types.NewCauset(m))
		c.Assert(err, IsNil)
		v, err := Decode(b, 1)
		c.Assert(err, IsNil)
		m.Fsp = types.MaxFsp
		c.Assert(v, DeepEquals, types.MakeCausets(m))
	}
	...
	for _, t := range tblCmp {
		...
		b1, err := EncodeKey(sc, nil, types.NewCauset(m1))
		c.Assert(err, IsNil)
		b2, err := EncodeKey(sc, nil, types.NewCauset(m2))
		c.Assert(err, IsNil)
...
func (s *testCodecSuite) TestDecimal(c *C) {
	...
	for _, t := range tbl {
		...
		c.Assert(err, IsNil)
		b, err := EncodeKey(sc, nil, types.NewCauset(dec))
		c.Assert(err, IsNil)
		v, err := Decode(b, 1)
		c.Assert(err, IsNil)
		c.Assert(v, HasLen, 1)
	...
	for _, t := range tblCmp {
		d1 := types.NewCauset(t.Arg1)
		dec1, err := d1.ToDecimal(sc)
		c.Assert(err, IsNil)
		d1.SetMysqlDecimal(dec1)
		d2 := types.NewCauset(t.Arg2)
	...
	decimalCauset := types.NewCauset(d)
	decimalCauset.SetLength(20)
	decimalCauset.SetFrac(5)
	_, err = EncodeValue(sc, nil, decimalCauset)
	c.Assert(err, IsNil)

	sc.OverflowAsWarning = true
	decimalCauset.SetLength(12)
	decimalCauset.SetFrac(10)
	_, err = EncodeValue(sc, nil, decimalCauset)
	c.Assert(err, IsNil)
}
...
func (s *testCodecSuite) TestJSON(c *C) {
	...
	datums := make([]types.Causet, 0, len(tbl))
	for _, t := range tbl {
		var d types.Causet
...
func (s *testCodecSuite) TestCut(c *C) {
	...
	block := []struct {
		...
		{
			types.MakeCausets(float32(1), float64(3.15), []byte("123456789012345")),
			types.MakeCausets(float64(1), float64(3.15), []byte("123456789012345")),
		},
		{
			types.MakeCausets(types.NewDecFromInt(0), types.NewDecFromFloatForTest(-1.3)),
			types.MakeCausets(types.NewDecFromInt(0), types.NewDecFromFloatForTest(-1.3)),
		},
		{
			types.MakeCausets(json.CreateBinary("abc")),
			types.MakeCausets(json.CreateBinary("abc")),
		},
	}
	sc := &stmtctx.StatementContext{TimeZone: time.Local}
	...
	b, err := EncodeValue(sc, nil, types.NewCauset(42))
	c.Assert(err, IsNil)
	rem, n, err := CutDeferredCausetID(b)
	c.Assert(err, IsNil)
...
func (s *testCodecSuite) TestSetRawValues(c *C) {
	sc := &stmtctx.StatementContext{TimeZone: time.Local}
	datums := types.MakeCausets(1, "abc", 1.1, []byte("def"))
	rowData, err := EncodeValue(sc, nil, datums...)
	c.Assert(err, IsNil)
	values := make([]types.Causet, 4)
	err = SetRawValues(rowData, values)
	c.Assert(err, IsNil)
	for i, rawVal := range values {
		c.Assert(rawVal.HoTT(), Equals, types.HoTTRaw)
...
func (s *testCodecSuite) TestDecodeOneToChunk(c *C) {
	...
	for defCausIdx, tp := range tps {
		for rowIdx := 0; rowIdx < rowCount; rowIdx++ {
			got := chk.GetRow(rowIdx).GetCauset(defCausIdx, tp)
			expect := datums[defCausIdx]
			if got.IsNull() {
				c.Assert(expect.IsNull(), IsTrue)
			} else {
				if got.HoTT() != types.HoTTMysqlDecimal {
					cmp, err := got.CompareCauset(sc, &expect)
					c.Assert(err, IsNil)
...
func (s *testCodecSuite) TestHashGroup(c *C) {
	...
	chk1.Reset()
	chk1.DeferredCauset(0).AppendMyDecimal(types.NewDecFromStringForTest("-123.123456789"))
	chk1.DeferredCauset(0).AppendMyDecimal(types.NewDecFromStringForTest("-123.123456789"))
	chk1.DeferredCauset(0).AppendMyDecimal(types.NewDecFromStringForTest("-123.123456789"))
	...
	tp1.Flen = 20
	tp1.Decimal = 5
	_, err := HashGroupKey(sc, 3, chk1.DeferredCauset(0), buf1, tp1)
	c.Assert(err, NotNil)
	tp2 := tp
	tp2.Flen = 12
	tp2.Decimal = 10
	_, err = HashGroupKey(sc, 3, chk1.DeferredCauset(0), buf1, tp2)
	c.Assert(err, NotNil)
}
...
func datumsForTest(sc *stmtctx.StatementContext) ([]types.Causet, []*types.FieldType) {
	...
	block := []struct {
		...
		{nil, types.NewFieldType(allegrosql.TypeNull)},
		{nil, types.NewFieldType(allegrosql.TypeLonglong)},
		{nil, types.NewFieldType(allegrosql.TypeFloat)},
		{nil, types.NewFieldType(allegrosql.TypeDate)},
		{nil, types.NewFieldType(allegrosql.TypeDuration)},
		{nil, types.NewFieldType(allegrosql.TypeNewDecimal)},
		{nil, types.NewFieldType(allegrosql.TypeEnum)},
		{nil, types.NewFieldType(allegrosql.TypeSet)},
		{nil, types.NewFieldType(allegrosql.TypeBit)},
		{nil, types.NewFieldType(allegrosql.TypeJSON)},
		{nil, types.NewFieldType(allegrosql.TypeVarchar)},
		{nil, types.NewFieldType(allegrosql.TypeDouble)},
		{int64(1), types.NewFieldType(allegrosql.TypeTiny)},
		{int64(1), types.NewFieldType(allegrosql.TypeShort)},
		{int64(1), types.NewFieldType(allegrosql.TypeInt24)},
		{int64(1), types.NewFieldType(allegrosql.TypeLong)},
		{int64(-1), types.NewFieldType(allegrosql.TypeLong)},
		{int64(1), types.NewFieldType(allegrosql.TypeLonglong)},
		{uint64(1), types.NewFieldType(allegrosql.TypeLonglong)},
		{float32(1), types.NewFieldType(allegrosql.TypeFloat)},
		{float64(1), types.NewFieldType(allegrosql.TypeDouble)},
		{types.NewDecFromInt(1), types.NewFieldType(allegrosql.TypeNewDecimal)},
		{types.NewDecFromStringForTest("1.123"), decType},
		{"abc", types.NewFieldType(allegrosql.TypeString)},
		{"def", types.NewFieldType(allegrosql.TypeVarchar)},
		{"ghi", types.NewFieldType(allegrosql.TypeVarString)},
		{[]byte("abc"), types.NewFieldType(allegrosql.TypeBlob)},
		{[]byte("abc"), types.NewFieldType(allegrosql.TypeTinyBlob)},
		{[]byte("abc"), types.NewFieldType(allegrosql.TypeMediumBlob)},
		{[]byte("abc"), types.NewFieldType(allegrosql.TypeLongBlob)},
		{types.CurrentTime(allegrosql.TypeDatetime), types.NewFieldType(allegrosql.TypeDatetime)},
		{types.CurrentTime(allegrosql.TypeDate), types.NewFieldType(allegrosql.TypeDate)},
		{types.NewTime(types.FromGoTime(time.Now()), allegrosql.TypeTimestamp, types.DefaultFsp), types.NewFieldType(allegrosql.TypeTimestamp)},
		{types.Duration{Duration: time.Second, Fsp: 1}, types.NewFieldType(allegrosql.TypeDuration)},
		{types.Enum{Name: "a", Value: 1}, &types.FieldType{Tp: allegrosql.TypeEnum, Elems: []string{"a"}}},
		{types.Set{Name: "a", Value: 1}, &types.FieldType{Tp: allegrosql.TypeSet, Elems: []string{"a"}}},
		{types.BinaryLiteral{100}, &types.FieldType{Tp: allegrosql.TypeBit, Flen: 8}},
		{json.CreateBinary("abc"), types.NewFieldType(allegrosql.TypeJSON)},
...
func (s *testCodecSuite) TestDecodeRange(c *C) {
	...
	c.Assert(err, IsNil)
	for i, causet := range datums1 {
		cmp, err := causet.CompareCauset(nil, &datums[i])
		c.Assert(err, IsNil)
...
func testHashChunkRowEqual(c *C, a, b interface{}, equal bool) {
	...
	types.DefaultTypeForValue(a, tp1, allegrosql.DefaultCharset, allegrosql.DefaultDefCauslationName)
	chk1 := chunk.New([]*types.FieldType{tp1}, 1, 1)
	d := types.Causet{}
	d.SetValue(a, tp1)
	chk1.AppendCauset(0, &d)

	tp2 := new(types.FieldType)
	types.DefaultTypeForValue(b, tp2, allegrosql.DefaultCharset, allegrosql.DefaultDefCauslationName)
	chk2 := chunk.New([]*types.FieldType{tp2}, 1, 1)
	d = types.Causet{}
	d.SetValue(b, tp2)
	chk2.AppendCauset(0, &d)
...
func (s *testCodecSuite) TestHashChunkRow(c *C) {
	...
	defCausIdx := make([]int, len(tps))
	for i := 0; i < len(tps); i++ {
		defCausIdx[i] = i
	}
	h := crc32.NewIEEE()
	err1 := HashChunkRow(sc, h, chk.GetRow(0), tps, defCausIdx, buf)
	sum1 := h.Sum32()
	h.Reset()
	err2 := HashChunkRow(sc, h, chk.GetRow(0), tps, defCausIdx, buf)
	sum2 := h.Sum32()
...
func (s *testCodecSuite) TestHashChunkDeferredCausets(c *C) {
	sc := &stmtctx.StatementContext{TimeZone: time.Local}
	buf := make([]byte, 1)
	datums, tps := datumsForTest(sc)
	...
	// Test hash value of the first 12 `Null` defCausumns
	for i := 0; i < 12; i++ {
		c.Assert(chk.GetRow(0).IsNull(i), Equals, true)
		err1 := HashChunkSelected(sc, vecHash, chk, tps[i], i, buf, hasNull, sel, false)
		err2 := HashChunkRow(sc, rowHash[0], chk.GetRow(0), tps, defCausIdx[i:i+1], buf)
		err3 := HashChunkRow(sc, rowHash[1], chk.GetRow(1), tps, defCausIdx[i:i+1], buf)
		err4 := HashChunkRow(sc, rowHash[2], chk.GetRow(2), tps, defCausIdx[i:i+1], buf)
		c.Assert(err1, IsNil)
		c.Assert(err2, IsNil)
		c.Assert(err3, IsNil)
		c.Assert(err4, IsNil)
	...
	// Test hash value of every single defCausumn that is not `Null`
