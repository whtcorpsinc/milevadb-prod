...
func (s *testDecimalSuite) TestDecimalCodec(c *C) {
	...
	for _, input := range inputs {
		...
		causet := types.NewCauset(v)

		b, err := EncodeDecimal([]byte{}, causet.GetMysqlDecimal(), causet.Length(), causet.Frac())
		c.Assert(err, IsNil)
		_, d, prec, frac, err := DecodeDecimal(b)
		if causet.Length() != 0 {
			c.Assert(prec, Equals, causet.Length())
			c.Assert(frac, Equals, causet.Frac())
		} else {
			prec1, frac1 := causet.GetMysqlDecimal().PrecisionAndFrac()
