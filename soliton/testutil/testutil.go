...
// datumEqualsChecker is a checker for CausetEquals.
type datumEqualsChecker struct {
	*check.CheckerInfo
}

// CausetEquals checker verifies that the obtained value is equal to
// the expected value.
// For example:
//     c.Assert(value, CausetEquals, NewCauset(42))
var CausetEquals check.Checker = &datumEqualsChecker{
	&check.CheckerInfo{Name: "CausetEquals", Params: []string{"obtained", "expected"}},
}
...
func (checker *datumEqualsChecker) Check(params []interface{}, names []string) (result bool, error string) {
	...
	paramFirst, ok := params[0].(types.Causet)
	if !ok {
		panic("the first param should be causet")
	}
	paramSecond, ok := params[1].(types.Causet)
	if !ok {
		panic("the second param should be causet")
	}
	sc := new(stmtctx.StatementContext)
	res, err := paramFirst.CompareCauset(sc, &paramSecond)
	if err != nil {
		panic(err)
	}
	return res == 0, ""
...
func MustNewCommonHandle(c *check.C, values ...interface{}) ekv.Handle {
...
// CommonHandleSuite is used to adapt ekv.CommonHandle to existing ekv.IntHandle tests.
//  Usage:
...
func (c *commonHandleSuiteNewHandleBuilder) Common(vs ...interface{}) ekv.Handle {
	c.commonVals = vs
	return c.Build()
}

func (c *commonHandleSuiteNewHandleBuilder) Build() ekv.Handle {
	if c.isCommon {
		encoded, err := codec.EncodeKey(new(stmtctx.StatementContext), nil, types.MakeCausets(c.commonVals...)...)
		if err != nil {
			panic(err)
		}
		ch, err := ekv.NewCommonHandle(encoded)
...
// the expected handle.
// For example:
//     c.Assert(value, HandleEquals, ekv.IntHandle(42))
...
func (checker *handleEqualsChecker) Check(params []interface{}, names []string) (result bool, error string) {
	...
	param1, ok1 := params[0].(ekv.Handle)
	param2, ok2 := params[1].(ekv.Handle)
	if !ok1 || !ok2 {
		return false, "Argument to " + checker.Name + " must be ekv.Handle"
	}
...
// OnRecord execute the function to uFIDelate result.
func (t *TestData) OnRecord(uFIDelateFunc func()) {
	if record {
		uFIDelateFunc()
	}
}
...
// ConvertALLEGROSQLWarnToStrings converts []ALLEGROSQLWarn to []string.
func (t *TestData) ConvertALLEGROSQLWarnToStrings(warns []stmtctx.ALLEGROSQLWarn) (rs []string) {
...
func (a *autoRandom) MaskSortHandles(handles []int64, shardBitsCount int, fieldType byte) []int64 {
	typeBitsLength := allegrosql.DefaultLengthOfMysqlTypes[fieldType] * 8
