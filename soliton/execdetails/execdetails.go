...
type StmtInterDircDetails struct {
	...
	WaitFIDelResFIDeluration   int64
	WriteALLEGROSQLResFIDeluration time.Duration
}
...
const (
	// CopTimeStr represents the sum of cop-task time spend in MilevaDB distALLEGROSQL.
	CopTimeStr = "Cop_time"
	// ProcessTimeStr represents the sum of process time of all the interlock tasks.
	ProcessTimeStr = "Process_time"
	// WaitTimeStr means the time of all interlock wait.
...
// CopRuntimeStats defCauslects cop tasks' execution info.
type CopRuntimeStats struct {
	sync.Mutex

	// stats stores the runtime statistics of interlock tasks.
	// The key of the map is the einsteindb-server address. Because a einsteindb-server can
	// have many region leaders, several interlock tasks can be sent to the
	// same einsteindb-server instance. We have to use a list to maintain all tasks
...
func (e *RuntimeStatsDefCausl) GetCopStats(planID int) *CopRuntimeStats {
	e.mu.Lock()
	defer e.mu.Unlock()
...
// ExistsRootStats checks if the planID exists in the rootStats defCauslection.
func (e *RuntimeStatsDefCausl) ExistsRootStats(planID int) bool {
	e.mu.Lock()
	defer e.mu.Unlock()
...
// ExistsCopStats checks if the planID exists in the copStats defCauslection.
func (e *RuntimeStatsDefCausl) ExistsCopStats(planID int) bool {
	e.mu.Lock()
	defer e.mu.Unlock()
