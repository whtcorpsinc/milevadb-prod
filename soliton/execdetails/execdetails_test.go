...
func TestCopRuntimeStats(t *testing.T) {
	...
	cop := stats.GetCopStats(blockScanID)
	...
	rootStats := stats.GetRootStats(blockReaderID)
	if rootStats == nil {
		t.Fatal("block_reader")
	}
	if stats.ExistsRootStats(blockReaderID) == false {
		t.Fatal("block_reader not exists")
	}
}
...
func TestRootRuntimeStats(t *testing.T) {
	...
	pid := 1
	stmtStats := NewRuntimeStatsDefCausl()
