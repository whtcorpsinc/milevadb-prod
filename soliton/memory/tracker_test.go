...
func (s *testSuite) TestOOMCausetAction(c *C) {
	tracker := NewTracker(1, 100)
	// make sure no panic here.
	tracker.Consume(10000)

	tracker = NewTracker(1, 100)
	action := &mockCausetAction{}
	tracker.SetSuperCowOrNoCausetOnExceed(action)
	...
	// test fallback
	action1 := &mockCausetAction{}
	action2 := &mockCausetAction{}
	tracker.SetSuperCowOrNoCausetOnExceed(action1)
	tracker.FallbackOldAndSetNewCausetAction(action2)
...
type mockCausetAction struct {
	called   bool
	fallback SuperCowOrNoCausetOnExceed
}

func (a *mockCausetAction) SetLogHook(hook func(uint64)) {
}

func (a *mockCausetAction) CausetAction(t *Tracker) {
	if a.called && a.fallback != nil {
		a.fallback.CausetAction(t)
		return
	}
	a.called = true
}

func (a *mockCausetAction) SetFallback(fallback SuperCowOrNoCausetOnExceed) {
	a.fallback = fallback
}
...
func (s *testSuite) TestErrorCode(c *C) {
	c.Assert(int(terror.ToALLEGROSQLError(errMemExceedThreshold).Code), Equals, errno.ErrMemExceedThreshold)
}
