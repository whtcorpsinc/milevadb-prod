...
import (
	...
	"github.com/whtcorpsinc/milevadb/soliton/tracing"
)
...
func (s *testTraceSuite) TestSpanFromContext(c *C) {
	...
	// test milevadb tracing
	defCauslectedSpan := make([]basictracer.RawSpan, 1)
	sp := tracing.NewRecordedTrace("test", func(sp basictracer.RawSpan) {
		defCauslectedSpan = append(defCauslectedSpan, sp)
	})
	sp.Finish()
	...
	c.Assert(defCauslectedSpan[0].Operation, NotNil)
}
...
func (s *testTraceSuite) TestChildSpanFromContext(c *C) {
	...
	// verify second span's operation is not nil, this way we can ensure
	// callback logic works.
	c.Assert(defCauslectedSpan[1].Operation, NotNil)

}

func (s *testTraceSuite) TestFollowFrom(c *C) {
	var defCauslectedSpans []basictracer.RawSpan
	// first start a root span
	sp1 := tracing.NewRecordedTrace("test", func(sp basictracer.RawSpan) {
		defCauslectedSpans = append(defCauslectedSpans, sp)
	})
	sp2 := sp1.Tracer().StartSpan("follow_from", opentracing.FollowsFrom(sp1.Context()))
	sp1.Finish()
	sp2.Finish()
	c.Assert(defCauslectedSpans[1].Operation, Equals, "follow_from")
	c.Assert(defCauslectedSpans[1].ParentSpanID, Not(Equals), uint64(0))
	// only root span has 0 parent id
	c.Assert(defCauslectedSpans[0].ParentSpanID, Equals, uint64(0))
}
...
func (s *testTraceSuite) TestCreateSapnBeforeSetupGlobalTracer(c *C) {
	...
	// droped.
	c.Assert(len(defCauslectedSpans), Equals, 1)
}
...
func (s *testTraceSuite) TestTreeRelationship(c *C) {
	...
	// then causetstore such root span into context
	...
	// ensure length of defCauslectedSpans is greather than 0
	c.Assert(len(defCauslectedSpans), Greater, 0)
	if len(defCauslectedSpans) > 0 {
		c.Assert(defCauslectedSpans[0].Operation, Equals, "test")
		c.Assert(defCauslectedSpans[1].Operation, Equals, "parent")
		c.Assert(defCauslectedSpans[2].Operation, Equals, "child")
		// check tree relationship
		// sp1 is parent of sp2. And sp2 is parent of sp3.
		c.Assert(defCauslectedSpans[1].ParentSpanID, Equals, defCauslectedSpans[0].Context.SpanID)
		c.Assert(defCauslectedSpans[2].ParentSpanID, Equals, defCauslectedSpans[1].Context.SpanID)
	}
}
