...
func (s *testSuite5) TestSetVar(c *C) {
	...
	tk.MustExec("SET SESSION milevadb_skip_isolation_level_check = 0")
	...
	tk.MustExec("SET SESSION milevadb_skip_isolation_level_check = 1")
...
func (s *testSuite5) TestEnableNoopFunctionsVar(c *C) {
	...
	tk.MustQuery(`select @@global.milevadb_enable_noop_functions;`).Check(testkit.Events("0"))
	tk.MustQuery(`select get_lock("lock", 10)`).Check(testkit.Events("1"))
	tk.MustQuery(`select release_lock("lock")`).Check(testkit.Events("1"))
