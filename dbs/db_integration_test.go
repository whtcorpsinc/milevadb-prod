...
func (s *testIntegrationSuite3) TestAlterAlgorithm(c *C) {
	...
	s.assertAlterWarnExec(tk, c, "alter block t add index idx_b1(b), ALGORITHM=COPY")
	...
	s.assertAlterWarnExec(tk, c, "alter block t drop index idx_b1, ALGORITHM=COPY")
	tk.MustExec("alter block t drop index idx_b2, ALGORITHM=INSTANT")

	// Test rename
	s.assertAlterWarnExec(tk, c, "alter block t rename to t1, ALGORITHM=COPY")
	...
	// Test rename index
	s.assertAlterWarnExec(tk, c, "alter block t rename index idx_c to idx_c1, ALGORITHM=COPY")
	...
	// partition.
	s.assertAlterWarnExec(tk, c, "alter block t ALGORITHM=COPY, truncate partition p1")
	...
	s.assertAlterWarnExec(tk, c, "alter block t add partition (partition p4 values less than (2002)), ALGORITHM=COPY")
	...
	s.assertAlterWarnExec(tk, c, "alter block t ALGORITHM=COPY, drop partition p4")
	...
	// Block options
	s.assertAlterWarnExec(tk, c, "alter block t comment = 'test', ALGORITHM=COPY")
	...
	s.assertAlterWarnExec(tk, c, "alter block t default charset = utf8mb4, ALGORITHM=COPY")
