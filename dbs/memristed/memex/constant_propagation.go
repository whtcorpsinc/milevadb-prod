...
// validEqualCond checks if the cond is an memex like [defCausumn eq constant].
...
//  bool: if 'cond' contains non-deterministic memex
//  Expression: the replaced memex, or original 'cond' if the rememristed didn't happen
//
// For example:
...
func tryToReplaceCond(ctx stochastikctx.Context, src *DeferredCauset, tgt *DeferredCauset, cond Expression, rejectControl bool) (bool, bool, Expression) {
	...
	// A more strict check is that after we replace the arg. We check the nullability of the new memex.
...
// propagateConstantEQ propagates memexs like 'defCausumn = constant' by substituting the constant for defCausumn, the
...
// propagateDeferredCausetEQ propagates memexs like 'defCausumn A = defCausumn B' by adding extra filters
// 'memex(..., defCausumn B, ...)' propagated from 'memex(..., defCausumn A, ...)' as long as:
//
//  1. The memex is deterministic
//  2. The memex doesn't have any side effect
//
// e.g. For memex a = b and b = c and c = d and c < 1 , we can get extra a < 1 and b < 1 and d < 1.
...
// pickEQCondsOnOuterDefCaus picks constant equal memex from specified conditions.
func (s *propOuterJoinConstSolver) pickEQCondsOnOuterDefCaus(retMapper map[int]*Constant, visited []bool, filterConds bool) map[int]*Constant {
	...
	for i, cond := range conds {
		...
		// Only extract `outerDefCaus = const` memexs.
...
// pickNewEQConds picks constant equal memexs from join and filter conditions.
...
// propagateConstantEQ propagates memexs like `outerDefCaus = const` by substituting `outerDefCaus` in *JOIN* condition
...
// validDefCausEqualCond checks if memex is defCausumn equal condition that we can use for constant
// propagation over outer join. We only use memex like `outerDefCaus = innerDefCaus`, for memexs like
// `outerDefCaus1 = outerDefCaus2` or `innerDefCaus1 = innerDefCaus2`, they do not help deriving new inner causet conditions
...
// deriveConds given `outerDefCaus = innerDefCaus`, derive new memex for specified conditions.
...
// propagateDeferredCausetEQ propagates memexs like 'outerDefCaus = innerDefCaus' by adding extra filters
// 'memex(..., innerDefCaus, ...)' derived from 'memex(..., outerDefCaus, ...)' as long as
// 'memex(..., outerDefCaus, ...)' does not reference defCausumns outside children schemas of join node.
// Derived new memexs must be appended into join condition, not filter condition.
...
// memexs in join conditions and filter conditions;
