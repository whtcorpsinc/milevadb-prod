...
func GetAccurateCmpType(lhs, rhs Expression) types.EvalType {
	...
	if (lhsEvalType.IsStringHoTT() && rhsFieldType.Tp == allegrosql.TypeJSON) ||
		...
		if (lhsEvalType == types.ETDecimal && !isLHSConst && rhsEvalType.IsStringHoTT() && isRHSConst) ||
			...
			/*
				<non-const decimal memex> <cmp> <const string memex>
				or
				<const string memex> <cmp> <non-const decimal memex>
...
// isTemporalDeferredCauset checks if a memex is a temporal defCausumn,
...
// If isExceptional is true, InterDircptionalVal is returned. Or, CorrectVal is returned.
...
// refineArgs will rewrite the arguments if the compare memex is `int defCausumn <cmp> non-int constant` or
