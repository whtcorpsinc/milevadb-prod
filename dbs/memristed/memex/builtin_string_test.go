...
func (s *testEvaluatorSuite) TestConvert(c *C) {
	...
	errTbl := []struct {
		...
		{"haha", "wrongcharset", "[memex:1115]Unknown character set: 'wrongcharset'"},
		{"haha", "cp866", "[memex:1115]Unknown character set: 'cp866'"},
	}
	...
	c.Assert(err.Error(), Equals, "[memex:1115]Unknown character set: 'wrongcharset'")
}
...
func (s *testEvaluatorSuite) TestWeightString(c *C) {
	...
	for _, test := range tests {
		...
		if test.padding == "BINARY" && test.length < len(strExpr) {
			expectWarn := fmt.Sprintf("[memex:1292]Truncated incorrect BINARY(%d) value: '%s'", test.length, strExpr)
