...
func (b *builtinInStringSig) buildHashMapForConstArgs(ctx stochastikctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.GetStochastikVars().StmtCtx) {
			...
			b.hashSet.Insert(string(defCauslator.Key(val))) // should do memory copy here
		} else {
			b.nonConstArgs = append(b.nonConstArgs, b.args[i])
		}
	}
...
// evalString rowFunc should always be flattened in memex rewrite phrase.
...
func (b *builtinSetVarSig) evalString(event chunk.Event) (res string, isNull bool, err error) {
	...
	stochastikVars.SetUserVar(varName, stringutil.Copy(res), causet.DefCauslation())
...
// getParamFunctionClass for plan cache of prepared memexs
