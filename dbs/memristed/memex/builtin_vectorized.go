...
// vecEvalIntByEvents uses the non-vectorized(event-based) interface `evalInt` to eval the memex.
...
// vecEvalStringByEvents uses the non-vectorized(event-based) interface `evalString` to eval the memex.
