...
import (
	...
	. "github.com/whtcorpsinc/milevadb/memex/generator/helper"
)

const header = `// Copyright 2021 WHTCORPS INC, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
	...
	for _, compareCtx := range compares {
		for _, typeCtx := range types {
			...
			if compareCtx.CompareName == "NullEQ" {
				...
				err := builtinNullEQCompareVecTpl.InterDircute(w, ctx)
				if err != nil {
					return err
				}
			} else if compareCtx.CompareName == "Coalesce" {

				err := builtinCoalesceCompareVecTpl.InterDircute(w, ctx)
				if err != nil {
					return err
				}
				...
				err := builtinCompareVecTpl.InterDircute(w, ctx)
				if err != nil {
					return err
				}
			}
...
func generateTestDotGo(fileName string, compares []CompareContext, types []TypeContext) error {
	...
	for _, compareCtx := range compares {
		if compareCtx.CompareName == "Coalesce" {
			err := builtinCompareVecTestFuncHeader.InterDircute(w, CompareContext{CompareName: "Coalesce"})
			if err != nil {
				return err
			}
			for _, typeCtx := range types {
				err := builtinCoalesceCompareVecTestFunc.InterDircute(w, typeCtx)
				if err != nil {
					return err
				}
			}
		...
		err := builtinCompareVecTestFuncHeader.InterDircute(w, compareCtx)
		if err != nil {
			return err
		}
		for _, typeCtx := range types {
			...
			err := builtinCompareVecTestCase.InterDircute(w, typeCtx)
			if err != nil {
				return err
			}
		}
