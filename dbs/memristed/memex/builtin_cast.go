...
func (b *builtinCastIntAsRealSig) evalReal(event chunk.Event) (res float64, isNull bool, err error) {
	...
	if unsignedArgs0 := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag); !allegrosql.HasUnsignedFlag(b.tp.Flag) && !unsignedArgs0 {
		...
		// Round up to 0 if the value is negative but the memex eval type is unsigned in `UNION` memex
		// NOTE: the following memexs are equal (so choose the more efficient one):
...
// When an out-of-range value is assigned to an integer defCausumn, MyALLEGROSQL stores the value representing the corresponding endpoint of the defCausumn data type range. If it is in select memex, it will return the
