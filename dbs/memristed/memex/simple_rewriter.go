...
// ParseSimpleExprWithBlockInfo parses simple memex string to Expression.
// The memex string must only reference the defCausumn in causet Info.
...
// ParseSimpleExprCastWithBlockInfo parses simple memex string to Expression.
...
// RewriteSimpleExprWithBlockInfo rewrites simple ast.ExprNode to memex.Expression.
...
// ParseSimpleExprsWithSchema parses simple memex string to Expression.
// The memex string must only reference the defCausumn in the given schemaReplicant.
...
// ParseSimpleExprsWithNames parses simple memex string to Expression.
...
// RewriteSimpleExprWithNames rewrites simple ast.ExprNode to memex.Expression.
...
// RewriteSimpleExprWithSchema rewrites simple ast.ExprNode to memex.Expression.
...
func (sr *simpleRewriter) rewriteDeferredCauset(nodeDefCausName *ast.DeferredCausetNameExpr) (*DeferredCauset, error) {
	...
	return nil, errBadField.GenWithStackByArgs(nodeDefCausName.Name.Name.O, "memex")
}
...
func (sr *simpleRewriter) Leave(originInNode ast.Node) (retNode ast.Node, ok bool) {
	...
	case *ast.SetDefCauslationExpr:
		...
		// SetDefCauslationExpr sets the defCauslation explicitly, even when the evaluation type of the memex is non-string.
		if _, ok := arg.(*DeferredCauset); ok {
			// Wrap a cast here to avoid changing the original FieldType of the defCausumn memex.
