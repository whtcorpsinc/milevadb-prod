...
// Expression represents all scalar memex in ALLEGROALLEGROSQL.
type Expression interface {
	...
	// HashCode creates the hashcode for memex which can be used to identify itself from other memex.
...
// CNFExprs stands for a CNF memex.
...
// IsEQCondFromIn checks if an memex is equal condition converted from `[not] in (subq)`.
...
// EvalBool evaluates memex list to a boolean value. The first returned value
// indicates bool result of the memex list, the second returned value indicates
// whether the result of the memex list is null, it can only be true when the
	...
	for _, expr := range exprList {
		...
		for i := range sel {
			if isZero[i] == -1 {
				...
				// The null flag may be set to false later by other memexs in some cases.
				nulls[sel[i]] = true
				sel[j] = sel[i]
				j++
				continue
...
// And it selects the method for evaluating memex based on
// the environment variables and whether the memex can be vectorized.
...
// composeConditionWithBinaryOp composes condition with binary operator into a balance deep tree, which benefits a lot for pb causetDecoder/causetCausetEncoder.
...
// ComposeCNFCondition composes CNF items into a balance deep CNF tree, which benefits a lot for pb causetDecoder/causetCausetEncoder.
...
// FlattenDNFConditions extracts DNF memex's leaf item.
...
// FlattenCNFConditions extracts CNF memex's leaf item.
...
type Assignment struct {
	...
	// DefCausName indicates its original defCausumn name in causet schemaReplicant. It's used for outputting helping message when executing meets some errors.
...
func DeferredCausetInfos2DeferredCausetsAndNames(ctx stochastikctx.Context, dbName, tblName perceptron.CIStr, defCausInfos []*perceptron.DeferredCausetInfo, tblInfo *perceptron.BlockInfo) ([]*DeferredCauset, types.NameSlice, error) {
	...
	for i, defCaus := range defCausInfos {
		...
		newDefCaus := &DeferredCauset{
			...
			ID:       defCaus.ID,
			UniqueID: ctx.GetStochastikVars().AllocCausetDeferredCausetID(),
...
// IsBinaryLiteral checks whether an memex is a binary literal
...
func canFuncBePushed(sf *ScalarFunction, storeType ekv.StoreType) bool {
	// Use the failpoint to control whether to push down an memex in the integration test.
	// Push down all memex if the `failpoint memex` is `all`, otherwise, check
	// whether scalar function's name is contained in the enabled memex list (e.g.`ne,eq,lt`).
...
// DefaultExprPushDownBlacklist indicates the memexs which can not be pushed down to EinsteinDB.
...
func canScalarFuncPushDown(scalarFunc *ScalarFunction, pc PbConverter, storeType ekv.StoreType) bool {
	...
	if spacetimedata := scalarFunc.Function.spacetimedata(); spacetimedata != nil {
		var err error
		_, err = proto.Marshal(spacetimedata)
		if err != nil {
			logutil.BgLogger().Error("encode spacetimedata", zap.Any("spacetimedata", spacetimedata), zap.Error(err))
			return false
		}
	}
	return true
