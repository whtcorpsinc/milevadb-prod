...
type Constant struct {
	...
	// DeferredExpr holds deferred function in CausetCache cached plan.
	// it's only used to represent non-deterministic functions(see memex.DeferredFunctions)
	// in CausetCache cached plan, so let them can be evaluated until cached item be used.
	DeferredExpr Expression
	// ParamMarker holds param index inside stochastikVars.PreparedParams.
	// It's only used to reference a user variable provided in the `EXECUTE` memex or `COM_EXECUTE` binary protodefCaus.
...
// GetUserVar returns the corresponding user variable presented in the `EXECUTE` memex or `COM_EXECUTE` command.
