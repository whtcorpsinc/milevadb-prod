...
func NewDistAggFunc(expr *fidelpb.Expr, fieldTps []*types.FieldType, sc *stmtctx.StatementContext) (Aggregation, error) {
	args := make([]memex.Expression, 0, len(expr.Children))
	for _, child := range expr.Children {
		arg, err := memex.PBToExpr(child, fieldTps, sc)
		if err != nil {
			return nil, err
		}
...
func newAggFunc(funcName string, args []memex.Expression, hasDistinct bool) aggFunction {
...
func CheckAggPushDown(aggFunc *AggFuncDesc, storeType ekv.StoreType) bool {
	...
	if ret {
		ret = memex.IsPushDownEnabled(strings.ToLower(aggFunc.Name), storeType)
	}
	return ret
}
