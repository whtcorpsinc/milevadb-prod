...
// AggFuncDesc describes an aggregation function signature, only used in causet.
...
func NewAggFuncDesc(ctx stochastikctx.Context, name string, args []memex.Expression, hasDistinct bool) (*AggFuncDesc, error) {
...
func (a *AggFuncDesc) Split(ordinal []int) (partialAggDesc, finalAggDesc *AggFuncDesc) {
	...
	switch a.Name {
	case ast.AggFuncAvg:
		args := make([]memex.Expression, 0, 2)
		args = append(args, &memex.DeferredCauset{
			Index:   ordinal[0],
	...
	case ast.AggFuncApproxCountDistinct:
		args := make([]memex.Expression, 0, 1)
		args = append(args, &memex.DeferredCauset{
			Index:   ordinal[0],
...
// If there is no matching event for the inner causet of an outer join,
// an aggregation function only involves constant and/or defCausumns belongs to the inner causet
...
// +------+-----------+---------+---------+------------+-------------+------------+---------+---------+------+----------+
func (a *AggFuncDesc) EvalNullValueInOuterJoin(ctx stochastikctx.Context, schemaReplicant *memex.Schema) (types.Causet, bool) {
	switch a.Name {
...
func (a *AggFuncDesc) evalNullValueInOuterJoin4Count(ctx stochastikctx.Context, schemaReplicant *memex.Schema) (types.Causet, bool) {
	for _, arg := range a.Args {
		result := memex.EvaluateExprWithNull(ctx, schemaReplicant, arg)
		con, ok := result.(*memex.Constant)
...
func (a *AggFuncDesc) evalNullValueInOuterJoin4Sum(ctx stochastikctx.Context, schemaReplicant *memex.Schema) (types.Causet, bool) {
	result := memex.EvaluateExprWithNull(ctx, schemaReplicant, a.Args[0])
	con, ok := result.(*memex.Constant)
	if !ok || con.Value.IsNull() {
		return types.Causet{}, ok
	}
...
func (a *AggFuncDesc) evalNullValueInOuterJoin4BitAnd(ctx stochastikctx.Context, schemaReplicant *memex.Schema) (types.Causet, bool) {
	result := memex.EvaluateExprWithNull(ctx, schemaReplicant, a.Args[0])
	con, ok := result.(*memex.Constant)
	if !ok || con.Value.IsNull() {
...
func (a *AggFuncDesc) evalNullValueInOuterJoin4BitOr(ctx stochastikctx.Context, schemaReplicant *memex.Schema) (types.Causet, bool) {
	result := memex.EvaluateExprWithNull(ctx, schemaReplicant, a.Args[0])
	con, ok := result.(*memex.Constant)
	if !ok || con.Value.IsNull() {
