...
// baseFuncDesc describes an function signature, only used in causet.
...
func newBaseFuncDesc(ctx stochastikctx.Context, name string, args []memex.Expression) (baseFuncDesc, error) {
...
func (a *baseFuncDesc) clone() *baseFuncDesc {
	...
	newTp := *a.RetTp
	clone.RetTp = &newTp
	clone.Args = make([]memex.Expression, len(a.Args))
	for i := range a.Args {
...
func (a *baseFuncDesc) typeInfer4GroupConcat(ctx stochastikctx.Context) {
	...
	// TODO: a.Args[i] = memex.WrapWithCastAsString(ctx, a.Args[i])
}

func (a *baseFuncDesc) typeInfer4MaxMin(ctx stochastikctx.Context) {
	_, argIsScalaFunc := a.Args[0].(*memex.ScalarFunction)
	if argIsScalaFunc && a.Args[0].GetType().Tp == allegrosql.TypeFloat {
		...
		// error would happen when extracting the evaluation of a.Args[0] to a ProjectionInterDirc.
		tp := types.NewFieldType(allegrosql.TypeDouble)
		tp.Flen, tp.Decimal = allegrosql.MaxRealWidth, types.UnspecifiedLength
		types.SetBinChsClnFlag(tp)
		a.Args[0] = memex.BuildCastFunction(ctx, a.Args[0], tp)
	}
...
func (a *baseFuncDesc) typeInfer4BitFuncs(ctx stochastikctx.Context) {
	...
	// TODO: a.Args[0] = memex.WrapWithCastAsInt(ctx, a.Args[0])
}
...
func (a *baseFuncDesc) typeInfer4LeadLag(ctx stochastikctx.Context) {
	if len(a.Args) <= 2 {
		...
		a.RetTp = memex.InferType4ControlFuncs(a.Args[0], a.Args[2])
	}
}
...
func (a *baseFuncDesc) WrapCastForAggArgs(ctx stochastikctx.Context) {
	...
	var castFunc func(ctx stochastikctx.Context, expr memex.Expression) memex.Expression
	switch retTp := a.RetTp; retTp.EvalType() {
	case types.ETInt:
		castFunc = memex.WrapWithCastAsInt
	case types.ETReal:
		castFunc = memex.WrapWithCastAsReal
	case types.ETString:
		castFunc = memex.WrapWithCastAsString
	case types.ETDecimal:
		castFunc = memex.WrapWithCastAsDecimal
	case types.ETDatetime, types.ETTimestamp:
		castFunc = func(ctx stochastikctx.Context, expr memex.Expression) memex.Expression {
			return memex.WrapWithCastAsTime(ctx, expr, retTp)
		}
	case types.ETDuration:
		castFunc = memex.WrapWithCastAsDuration
	case types.ETJson:
		castFunc = memex.WrapWithCastAsJSON
	default:
	...
	for i := range a.Args {
		...
		if defCaus, ok := a.Args[i].(*memex.DeferredCauset); ok {
