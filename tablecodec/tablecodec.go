...
func DecodeRowWithMapNew(b []byte, defcaus map[int64]*types.FieldType,
	...
	rd := rowcodec.NewCausetMaFIDelecoder(reqDefCauss, loc)
...
func decodeIndexKvOldDefCauslation(key, value []byte, defcausLen int, hdStatus HandleStatus) ([][]byte, error) {
...
func DecodeIndexKV(key, value []byte, defcausLen int, hdStatus HandleStatus, columns []rowcodec.DefCausInfo) ([][]byte, error) {
	if len(value) <= MaxOldEncodeValueLen {
		return decodeIndexKvOldDefCauslation(key, value, defcausLen, hdStatus)
	}
	return decodeIndexKvGeneral(key, value, defcausLen, hdStatus, columns)
}
...
func decodeIndexKvGeneral(key, value []byte, defcausLen int, hdStatus HandleStatus, columns []rowcodec.DefCausInfo) ([][]byte, error) {
