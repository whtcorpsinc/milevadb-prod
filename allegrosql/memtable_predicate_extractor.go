...
// and push the predicates down to the data retrieving on reading memory causet stage.
//
// e.g:
...
// 3. Passing the extractor to the `ClusterReaderInterDircInterDirc` interlock
// 4. InterlockingDirectorate sends requests to the target components instead of all of the components
type MemBlockPredicateExtractor interface {
	...
	Extract(stochastikctx.Context, *memex.Schema, []*types.FieldName, []memex.Expression) (remained []memex.Expression)
...
func (helper extractHelper) extractDefCausInConsExpr(extractDefCauss map[int64]*types.FieldName, expr *memex.ScalarFunction) (string, []types.Causet) {
	args := expr.GetArgs()
	col, isDefCaus := args[0].(*memex.DeferredCauset)
	if !isDefCaus {
		return "", nil
	}
	...
	// All memexs in IN must be a constant
	// SELECT * FROM t1 WHERE c IN ('1', '2')
	var results []types.Causet
	for _, arg := range args[1:] {
		constant, ok := arg.(*memex.Constant)
...
func (helper extractHelper) extractDefCausBinaryOpConsExpr(extractDefCauss map[int64]*types.FieldName, expr *memex.ScalarFunction) (string, []types.Causet) {
	args := expr.GetArgs()
	...
	for i := 0; i < 2; i++ {
		var isDefCaus bool
		col, isDefCaus = args[i].(*memex.DeferredCauset)
		if isDefCaus {
			colIdx = i
			break
		}
	...
	// The `lhs/rhs` of EQ memex must be a constant
	// SELECT * FROM t1 WHERE c='rhs'
	// SELECT * FROM t1 WHERE 'lhs'=c
	constant, ok := args[1-colIdx].(*memex.Constant)
	if !ok || constant.DeferredExpr != nil || constant.ParamMarker != nil {
		return "", nil
	}
...
// extract the OR memex, e.g:
// SELECT * FROM t1 WHERE c1='a' OR c1='b' OR c1='c'
func (helper extractHelper) extractDefCausOrExpr(extractDefCauss map[int64]*types.FieldName, expr *memex.ScalarFunction) (string, []types.Causet) {
	args := expr.GetArgs()
	lhs, ok := args[0].(*memex.ScalarFunction)
	if !ok {
		return "", nil
	}
	rhs, ok := args[1].(*memex.ScalarFunction)
	if !ok {
		return "", nil
	}
	// Define an inner function to avoid populate the outer scope
	var extract = func(extractDefCauss map[int64]*types.FieldName, fn *memex.ScalarFunction) (string, []types.Causet) {
...
func (helper extractHelper) extractDefCaus(
	...
	predicates []memex.Expression,
	extractDefCausName string,
	valueToLower bool,
) (
	remained []memex.Expression,
	skipRequest bool,
	result set.StringSet,
) {
	remained = make([]memex.Expression, 0, len(predicates))
	...
	for _, expr := range predicates {
		fn, ok := expr.(*memex.ScalarFunction)
		if !ok {
			continue
		}
		var colName string
		...
		// There are no data if the low-level interlock skip request, so the filter can be droped
		if skipRequest {
			remained = remained[:0]
			break
		}
...
func (helper extractHelper) extractLikePatternDefCaus(
	...
	// support perl-like CNF regular memex: (?=expr1)(?=expr2).
	// e.g:
	...
	for _, expr := range predicates {
		...
		var pattern string
		// We use '|' to combine DNF regular memex: .*a.*|.*b.*
		// e.g:
...
func (helper extractHelper) extractOrLikePattern(
	...
	predicates := memex.SplitDNFItems(orFunc)
	if len(predicates) == 0 {
		return false, ""
	}
	...
	for _, predicate := range predicates {
		fn, ok := predicate.(*memex.ScalarFunction)
		if !ok {
			return false, ""
		}
...
func (helper extractHelper) findDeferredCauset(schemaReplicant *memex.Schema, names []*types.FieldName, colName string) map[int64]*types.FieldName {
...
// For the memex that push down to the interlock, the function name is different with normal compare function,
...
func (helper extractHelper) getTimeFunctionName(fn *memex.ScalarFunction) string {
...
func (helper extractHelper) extractDefCauss(
	...
	valueToLower bool) ([]memex.Expression, bool, map[string]set.StringSet) {
...
// ClusterBlockExtractor is used to extract some predicates of cluster causet.
...
// GetMetricBlockPromQL uses to get the promQL of metric causet.
...
type SlowQueryExtractor struct {
	...
	StartTime   time.Time
	EndTime     time.Time
	// Enable is true means the interlock should use the time range to locate the slow-log file that need to be parsed.
	// Enable is false, means the interlock should keep the behavior compatible with before, which is only parse the
...
type BlockStorageStatsExtractor struct {
	...
	// SkipRequest means the where clause always false, we don't need to request any component.
	SkipRequest bool
	// BlockSchema represents blockSchema applied to, and we should apply all causet disk usage if there is no schemaReplicant specified.
	// e.g: SELECT * FROM information_schema.disk_usage WHERE block_schema in ('test', 'information_schema').
	BlockSchema set.StringSet
	// BlockName represents blockName applied to, and we should apply all causet disk usage if there is no causet specified.
	// e.g: SELECT * FROM information_schema.disk_usage WHERE causet in ('schemata', 'blocks').
...
func (e *BlockStorageStatsExtractor) explainInfo(p *PhysicalMemBlock) string {
	...
	if len(e.BlockName) > 0 {
		r.WriteString(fmt.Sprintf("causet:[%s]", extractStringFromStringSet(e.BlockName)))
	}
	return r.String()
}
...
// TiFlashSystemBlockExtractor is used to extract some predicates of tiflash system causet.
type TiFlashSystemBlockExtractor struct {
	...
	// MilevaDBBlocks represents milevadbBlocks applied to, and we should apply all milevadb causet if there is no causet specified.
