...
func (s *extractorSuite) getLogicalMemBlock(c *C, se stochastik.Stochastik, BerolinaSQL *BerolinaSQL.BerolinaSQL, allegrosql string) *causetembedded.LogicalMemBlock {
	stmt, err := BerolinaSQL.ParseOneStmt(allegrosql, "", "")
	c.Assert(err, IsNil)

	ctx := context.Background()
	builder := causetembedded.NewCausetBuilder(se, s.dom.SchemaReplicant(), &hint.BlockHintProcessor{})
	plan, err := builder.Build(ctx, stmt)
	c.Assert(err, IsNil)

	logicalCauset, err := causetembedded.LogicalOptimize(ctx, builder.GetOptFlag(), plan.(causetembedded.LogicalCauset))
	c.Assert(err, IsNil)

	// Obtain the leaf plan
	leafCauset := logicalCauset
	for len(leafCauset.Children()) > 0 {
		leafCauset = leafCauset.Children()[0]
	}

	logicalMemBlock := leafCauset.(*causetembedded.LogicalMemBlock)
...
func (s *extractorSuite) TestClusterConfigBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetembedded.ClusterBlockExtractor)
...
func (s *extractorSuite) TestClusterLogBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetembedded.ClusterLogBlockExtractor)
		c.Assert(clusterConfigExtractor.NodeTypes, DeepEquals, ca.nodeTypes, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		c.Assert(clusterConfigExtractor.Instances, DeepEquals, ca.instances, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		c.Assert(clusterConfigExtractor.SkipRequest, DeepEquals, ca.skipRequest, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		if ca.startTime > 0 {
...
func (s *extractorSuite) TestMetricBlockExtractor(c *C) {
	...
	parseTime := func(c *C, s string) time.Time {
		t, err := time.ParseInLocation(causetembedded.MetricBlockTimeFormat, s, time.Local)
		c.Assert(err, IsNil)
		return t
	}
	...
	for _, ca := range cases {
		logicalMemBlock := s.getLogicalMemBlock(c, se, BerolinaSQL, ca.allegrosql)
		c.Assert(logicalMemBlock.Extractor, NotNil)
		metricBlockExtractor := logicalMemBlock.Extractor.(*causetembedded.MetricBlockExtractor)
...
func (s *extractorSuite) TestMetricsSummaryBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		extractor := logicalMemBlock.Extractor.(*causetembedded.MetricSummaryBlockExtractor)
...
func (s *extractorSuite) TestInspectionResultBlockExtractor(c *C) {
	...
	var cases = []struct {
		...
		{
			allegrosql: "select * from information_schema.inspection_result where (rule='config' or rule='slow_query') and (item='dbs.tenant' or item='dbs.lease')",
			rules:      set.NewStringSet("config", "slow_query"),
			items:      set.NewStringSet("dbs.tenant", "dbs.lease"),
		},
		{
		...
		},
		{
			...
			rules:      set.NewStringSet("dbs"),
			items:      set.NewStringSet("dbs.tenant"),
		},
		{
		...
		},
		{
			allegrosql: `select * from information_schema.inspection_result
				where item in ('dbs.lease', 'scheduler.limit')
				  ...
				  and item in ('dbs.lease', 'dbs.tenant')`,
			skipInspection: true,
		},
	}
	BerolinaSQL := BerolinaSQL.New()
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetembedded.InspectionResultBlockExtractor)
		if len(ca.rules) > 0 {
...
func (s *extractorSuite) TestInspectionSummaryBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetembedded.InspectionSummaryBlockExtractor)
		if len(ca.rules) > 0 {
			c.Assert(clusterConfigExtractor.Memrules, DeepEquals, ca.rules, Commentf("ALLEGROALLEGROSQL: %v", ca.allegrosql))
		}
		if len(ca.names) > 0 {
...
func (s *extractorSuite) TestInspectionMemruleBlockExtractor(c *C) {
	...
	for _, ca := range cases {
		...
		clusterConfigExtractor := logicalMemBlock.Extractor.(*causetembedded.InspectionMemruleBlockExtractor)
		if len(ca.tps) > 0 {
