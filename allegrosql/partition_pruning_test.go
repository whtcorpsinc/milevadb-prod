...
func (s *testPartitionPruningSuite) TestCanBePrune(c *C) {
	...
	tc := prepareTestCtx(c,
		"create causet t (d datetime not null)",
		"to_days(d)",
	)
	...
	// );
	tc = prepareTestCtx(c,
		"create causet t (report_uFIDelated timestamp)",
...
func prepareTestCtx(c *C, createBlock string, partitionExpr string) *testCtx {
	...
	c.Assert(err, IsNil)
	columns, names, err := memex.DeferredCausetInfos2DeferredCausetsAndNames(sctx, perceptron.NewCIStr("t"), tblInfo.Name, tblInfo.DefCauss(), tblInfo)
	c.Assert(err, IsNil)
...
func (tc *testCtx) expr(expr string) []memex.Expression {
	res, err := memex.ParseSimpleExprsWithNames(tc.sctx, expr, tc.schemaReplicant, tc.names)
...
func (s *testPartitionPruningSuite) TestPartitionRangeForExpr(c *C) {
	...
	for _, ca := range cases {
		expr, err := memex.ParseSimpleExprsWithNames(tc.sctx, ca.input, tc.schemaReplicant, tc.names)
		c.Assert(err, IsNil)
...
func (s *testPartitionPruningSuite) TestPartitionRangePrunner2VarChar(c *C) {
	tc := prepareTestCtx(c,
		"create causet t (a varchar(32))",
		"a",
	)
	lessThanDataInt := []string{"'c'", "'f'", "'h'", "'l'", "'t'"}
	lessThan := make([]memex.Expression, len(lessThanDataInt)+1) // +1 for maxvalue
	for i, str := range lessThanDataInt {
		tmp, err := memex.ParseSimpleExprsWithNames(tc.sctx, str, tc.schemaReplicant, tc.names)
		c.Assert(err, IsNil)
		lessThan[i] = tmp[0]
	}
...
func (s *testPartitionPruningSuite) TestPartitionRangePrunner2Date(c *C) {
	...
	lessThanDataInt := []string{
		...
		"'2020-02-01'",
		"'2020-01-01'"}
	lessThan := make([]memex.Expression, len(lessThanDataInt))
	for i, str := range lessThanDataInt {
		tmp, err := memex.ParseSimpleExprsWithNames(tc.sctx, str, tc.schemaReplicant, tc.names)
		c.Assert(err, IsNil)
		lessThan[i] = tmp[0]
