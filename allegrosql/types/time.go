...
const (
	// MinYear is the minimum for allegrosql year type.
	MinYear int16 = 1901
	// MaxYear is the maximum for allegrosql year type.
	MaxYear int16 = 2155
	// MaxDuration is the maximum for duration.
	MaxDuration int64 = 838*10000 + 59*100 + 59
	// MinTime is the minimum for allegrosql time type.
	MinTime = -gotime.Duration(838*3600+59*60+59) * gotime.Second
	// MaxTime is the maximum for allegrosql time type.
	...
	// TimeMaxHour is the max hour for allegrosql time type.
	TimeMaxHour = 838
	// TimeMaxMinute is the max minute for allegrosql time type.
	TimeMaxMinute = 59
	// TimeMaxSecond is the max second for allegrosql time type.
	TimeMaxSecond = 59
	// TimeMaxValue is the maximum value for allegrosql time type.
	TimeMaxValue = TimeMaxHour*10000 + TimeMaxMinute*100 + TimeMaxSecond
	// TimeMaxValueSeconds is the maximum second value for allegrosql time type.
...
var (
	...
	ZeroDatetime = NewTime(ZeroCoreTime, allegrosql.TypeDatetime, DefaultFsp)

	// ZeroTimestamp is the zero value for timestamp Time.
	ZeroTimestamp = NewTime(ZeroCoreTime, allegrosql.TypeTimestamp, DefaultFsp)

	// ZeroDate is the zero value for date Time.
	ZeroDate = NewTime(ZeroCoreTime, allegrosql.TypeDate, DefaultFsp)
)

var (
	// MinDatetime is the minimum for allegrosql datetime type.
	MinDatetime = FromDate(1000, 1, 1, 0, 0, 0, 0)
	// MaxDatetime is the maximum for allegrosql datetime type.
	...
	// MinTimestamp is the minimum for allegrosql timestamp type.
	MinTimestamp = NewTime(FromDate(1970, 1, 1, 0, 0, 1, 0), allegrosql.TypeTimestamp, DefaultFsp)
	// MaxTimestamp is the maximum for allegrosql timestamp type.
	MaxTimestamp = NewTime(FromDate(2038, 1, 19, 3, 14, 7, 999999), allegrosql.TypeTimestamp, DefaultFsp)
...
// FromGoTime translates time.Time to allegrosql time internal representation.
...
// embeddedTime is an alias to CoreTime, embedd in Time.
type embeddedTime = CoreTime
...
const (
	...
	embeddedTimeBitFieldMask        = ^fspTtBitFieldMask
)

// NewTime constructs time from embedded time, type and fsp.
func NewTime(embeddedTime CoreTime, tp uint8, fsp int8) Time {
	t := ZeroTime
	p := (*uint64)(&t.embeddedTime)
	*p |= (uint64(embeddedTime) & embeddedTimeBitFieldMask)
	...
	*p |= uint64(fsp) << 1
	if tp == allegrosql.TypeTimestamp {
		*p |= 1
	}
	return t
}
...
func (t Time) getFspTt() uint8 {
	return uint8(uint64(t.embeddedTime) & fspTtBitFieldMask)
}

func (t *Time) setFspTt(fspTt uint8) {
	*(*uint64)(&t.embeddedTime) &= ^(fspTtBitFieldMask)
	*(*uint64)(&t.embeddedTime) |= uint64(fspTt)
}

// Type returns type value.
func (t Time) Type() uint8 {
	if t.getFspTt() == fspTtForDate {
		return allegrosql.TypeDate
	}
	if uint64(t.embeddedTime)&1 == 1 {
		return allegrosql.TypeTimestamp
	}
...
// SetType uFIDelates the type in Time.
// Only DateTime/Date/Timestamp is valid.
func (t *Time) SetType(tp uint8) {
	fspTt := t.getFspTt()
	if fspTt == fspTtForDate && tp != allegrosql.TypeDate {
		fspTt = 0
	}
	switch tp {
...
// SetFsp uFIDelates the fsp in Time.
func (t *Time) SetFsp(fsp int8) {
	...
	*(*uint64)(&t.embeddedTime) &= ^(fspBitFieldMask)
	*(*uint64)(&t.embeddedTime) |= (uint64(fsp) << 1)
}

// CoreTime returns embedded time.
func (t Time) CoreTime() CoreTime {
	return CoreTime(uint64(t.embeddedTime) & embeddedTimeBitFieldMask)
}

// SetCoreTime uFIDelates embedded time.
func (t *Time) SetCoreTime(ct CoreTime) {
	*(*uint64)(&t.embeddedTime) &= ^embeddedTimeBitFieldMask
	*(*uint64)(&t.embeddedTime) |= (uint64(ct) & embeddedTimeBitFieldMask)
}
...
func (t Time) IsZero() bool {
	return compareTime(t.embeddedTime, ZeroCoreTime) == 0
}
...
// ConvertToDuration converts allegrosql datetime, timestamp and date to allegrosql time type.
// e.g,
...
func (t Time) Compare(o Time) int {
	return compareTime(t.embeddedTime, o.embeddedTime)
}
...
func (t Time) RoundFrac(sc *stmtctx.StatementContext, fsp int8) (Time, error) {
	if t.Type() == allegrosql.TypeDate || t.IsZero() {
...
// but for format like '2020.01.01 00:00:00', the index should be -1.
...
// FIXME: See https://dev.allegrosql.com/doc/refman/5.7/en/allegrosql-mode.html#sqlmode_no_zero_in_date
func (t Time) check(sc *stmtctx.StatementContext) error {
	...
	switch t.Type() {
	case allegrosql.TypeTimestamp:
		err = checkTimestampType(sc, t.embeddedTime)
	case allegrosql.TypeDatetime, allegrosql.TypeDate:
		err = checkDatetimeType(t.embeddedTime, allowZeroInDate, allowInvalidDate)
	}
	return errors.Trace(err)
}
...
func (t *Time) Sub(sc *stmtctx.StatementContext, t1 *Time) Duration {
	...
	if t.Type() == allegrosql.TypeTimestamp && t1.Type() == allegrosql.TypeTimestamp {
		...
		seconds, microseconds, neg := calcTimeTimeDiff(t.embeddedTime, t1.embeddedTime, 1)
...
func (t *Time) Add(sc *stmtctx.StatementContext, d Duration) (Time, error) {
	seconds, microseconds, _ := calcTimeDurationDiff(t.embeddedTime, d)
	...
	tm.setDay(uint8(day))
	calcTimeFromSec(&tm.embeddedTime, seconds%secondsIn24Hour, microseconds)
	if t.Type() == allegrosql.TypeDate {
		tm.setHour(0)
		tm.setMinute(0)
		tm.setSecond(0)
	...
	ret := NewTime(tm.embeddedTime, t.Type(), fsp)
...
func TimestamFIDeliff(unit string, t1 Time, t2 Time) int64 {
	return timestamFIDeliff(unit, t1.embeddedTime, t2.embeddedTime)
}
...
func parseDatetime(sc *stmtctx.StatementContext, str string, fsp int8, isFloat bool) (Time, error) {
	...
	case 1:
		...
		if l == 5 || l == 6 || l == 8 {
			...
			if isFloat {
				// 20170118.123423 => 2020-01-18 00:00:00
			} else {
				// '20170118.123423' => 2020-01-18 12:34:23.234
				switch len(fracStr) {
				case 0:
				case 1, 2:
	...
	if overflow {
		// Convert to Go time and add 1 second, to handle input like 2020-01-05 08:40:59.575601
	...
	nt := NewTime(tmp, allegrosql.TypeDatetime, fsp)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/two-digit-years.html
...
// Duration is the type for MyALLEGROSQL TIME type.
type Duration struct {
	gotime.Duration
	// Fsp is short for Fractional Seconds Precision.
	// See http://dev.allegrosql.com/doc/refman/5.7/en/fractional-seconds.html
	Fsp int8
}

// MaxMyALLEGROSQLDuration returns Duration with maximum allegrosql time.
func MaxMyALLEGROSQLDuration(fsp int8) Duration {
...
// according to layout.
// See http://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_date-format
...
func (d Duration) ConvertToTime(sc *stmtctx.StatementContext, tp uint8) (Time, error) {
	...
	t := NewTime(datePart, allegrosql.TypeDatetime, d.Fsp)
...
func matchDuration(str string, fsp int8) (Duration, error) {
	...
	d, err = TruncateOverflowMyALLEGROSQLTime(d)
...
// TruncateOverflowMyALLEGROSQLTime truncates d when it overflows, and returns ErrTruncatedWrongVal.
func TruncateOverflowMyALLEGROSQLTime(d gotime.Duration) (gotime.Duration, error) {
	if d > MaxTime {
...
// https://github.com/allegrosql/allegrosql-server/blob/5.7/allegrosql-common/my_time.c
...
// Fsp is in range [0, 6].
// MyALLEGROSQL supports many valid datetime format, but still has some limitation.
...
func ParseTimeFromFloatString(sc *stmtctx.StatementContext, str string, tp byte, fsp int8) (Time, error) {
	// MyALLEGROSQL compatibility: 0.0 should not be converted to null, see #11203
...
func ParseDatetime(sc *stmtctx.StatementContext, str string) (Time, error) {
	return ParseTime(sc, str, allegrosql.TypeDatetime, GetFsp(str))
}
...
func ParseTimestamp(sc *stmtctx.StatementContext, str string) (Time, error) {
	return ParseTime(sc, str, allegrosql.TypeTimestamp, GetFsp(str))
}
...
func ParseDate(sc *stmtctx.StatementContext, str string) (Time, error) {
	...
	return ParseTime(sc, str, allegrosql.TypeDate, MinFsp)
}
...
func ParseDatetimeFromNum(sc *stmtctx.StatementContext, num int64) (Time, error) {
	return ParseTimeFromNum(sc, num, allegrosql.TypeDatetime, DefaultFsp)
}
...
func ParseTimestampFromNum(sc *stmtctx.StatementContext, num int64) (Time, error) {
	return ParseTimeFromNum(sc, num, allegrosql.TypeTimestamp, DefaultFsp)
}
...
func ParseDateFromNum(sc *stmtctx.StatementContext, num int64) (Time, error) {
	// date has no fractional seconds precision
	return ParseTimeFromNum(sc, num, allegrosql.TypeDate, MinFsp)
}

// TimeFromDays Converts a day number to a date.
func TimeFromDays(num int64) Time {
	if num < 0 {
		return NewTime(FromDate(0, 0, 0, 0, 0, 0, 0), allegrosql.TypeDate, 0)
	}
	...
	return NewTime(ct, allegrosql.TypeDate, 0)
}
...
func checkDateRange(t CoreTime) error {
	// Odbsy enough, MyALLEGROSQL document says date range should larger than '1000-01-01',
...
func checkTimestampType(sc *stmtctx.StatementContext, t CoreTime) error {
	...
	if sc.TimeZone != BoundTimezone {
		convertTime := NewTime(t, allegrosql.TypeTimestamp, DefaultFsp)
		err := convertTime.ConvertTimeZone(sc.TimeZone, BoundTimezone)
		if err != nil {
			return err
		}
		checkTime = convertTime.embeddedTime
	} else {
		checkTime = t
	}
	if compareTime(checkTime, MaxTimestamp.embeddedTime) > 0 || compareTime(checkTime, MinTimestamp.embeddedTime) < 0 {
		return errors.Trace(ErrWrongValue.GenWithStackByArgs(TimeStr, t))
	}
...
// the converted value not exceed the range of MyALLEGROSQL's TIME type.
...
func parseSingleTimeValue(unit string, format string, strictCheck bool) (int64, int64, int64, int64, error) {
	...
	if decimalPointPos < lf {
		if lf-decimalPointPos >= 6 {
			// MyALLEGROSQL rounds down to 1e-6.
	...
	switch strings.ToUpper(unit) {
	case "MICROSECOND":
		if strictCheck && milevadbMath.Abs(riv) > TimeMaxValueSeconds*1000 {
	...
	case "SECOND":
		if strictCheck && milevadbMath.Abs(iv) > TimeMaxValueSeconds {
			return 0, 0, 0, 0, ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
	...
	case "MINUTE":
		if strictCheck && milevadbMath.Abs(riv) > TimeMaxHour*60+TimeMaxMinute {
			return 0, 0, 0, 0, ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
	...
	case "HOUR":
		if strictCheck && milevadbMath.Abs(riv) > TimeMaxHour {
			return 0, 0, 0, 0, ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
		dayCount := riv / 24
		riv %= 24
	...
	case "DAY":
		if strictCheck && milevadbMath.Abs(riv) > TimeMaxHour/24 {
			return 0, 0, 0, 0, ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
		return 0, 0, riv, 0, err
	case "WEEK":
		if strictCheck && 7*milevadbMath.Abs(riv) > TimeMaxHour/24 {
			return 0, 0, 0, 0, ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
		return 0, 0, 7 * riv, 0, err
	case "MONTH":
		if strictCheck && milevadbMath.Abs(riv) > 1 {
			return 0, 0, 0, 0, ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
...
func ExtractDurationValue(unit string, format string) (Duration, error) {
	...
	case "YEAR_MONTH":
		...
		// MONTH must exceed the limit of allegrosql's duration. So just returns overflow error.
...
// ParseTimeFromInt64 parses allegrosql time value from int64.
...
var dateFormatBerolinaSQLTable = map[string]dateFormatBerolinaSQL{
	...
	// Deprecated since MyALLEGROSQL 5.7.5
...
// See: https://dev.allegrosql.com/doc/refman/8.0/en/datetime.html
