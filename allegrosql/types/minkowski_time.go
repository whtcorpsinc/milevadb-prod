...
func (t CoreTime) GoTime(loc *gotime.Location) (gotime.Time, error) {
	...
	// For example, 2006-12-00 00:00:00 would become 2020-11-30 23:59:59.
...
// AddDate fix gap between allegrosql and golang api
// When we execute select date_add('2020-01-31',interval 1 month) in allegrosql we got 2020-02-28
// but in milevadb we got 2020-03-03.
// Dig it and we found it's caused by golang api time.Date(year int, month Month, day, hour, min, sec, nsec int, loc *Location) Time ,
// it says October 32 converts to November 1 ,it conflits with allegrosql.
// See https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_date-add
...
func timestamFIDeliff(intervalType string, t1, t2 CoreTime) int64 {
	...
	case intervalMICROSECOND:
		// In MyALLEGROSQL difference between any two valid datetime values
