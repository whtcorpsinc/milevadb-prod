...
func (s *testTimeSuite) TestTimeEncoding(c *C) {
	tests := []struct {
		...
		{2020, 9, 16, 0, 0, 0, 0, allegrosql.TypeDatetime, 0, 0b1111110001110011000000000000000000000000000000000000000000000},
		{2020, 12, 31, 23, 59, 59, 999999, allegrosql.TypeTimestamp, 3, 0b1111110001111001111110111111011111011111101000010001111110111},
		{2020, 1, 5, 0, 0, 0, 0, allegrosql.TypeDate, 0, 0b1111110010000010010100000000000000000000000000000000000001110},
	}
	...
	for ith, tt := range tests {
		...
		c.Check(t.CoreTime(), Equals, ct, Commentf("%d embedded time failed.", ith))
...
func (s *testTimeSuite) TestDateTime(c *C) {
	...
	causet := []struct {
		...
		{"1701020301.", "2020-01-02 03:01:00"},
		{"1701020304.1", "2020-01-02 03:04:01.0"},
		{"1701020302.11", "2020-01-02 03:02:11.00"},
		{"170102036", "2020-01-02 03:06:00"},
		{"170102039.", "2020-01-02 03:09:00"},
		{"170102037.11", "2020-01-02 03:07:11.00"},
		{"2020-01-01 18", "2020-01-01 18:00:00"},
		{"18-01-01 18", "2020-01-01 18:00:00"},
		{"2020.01.01", "2020-01-01 00:00:00.00"},
	...
	fspTbl := []struct {
		...
		{"20170118.123", 6, "2020-01-18 12:03:00.000000"},
		...
		{"170105084059.575601", 0, "2020-01-05 08:41:00"},
		{"2020-01-05 23:59:59.575601", 0, "2020-01-06 00:00:00"},
		{"2020-01-31 23:59:59.575601", 0, "2020-02-01 00:00:00"},
		{"2020-00-05 23:59:58.575601", 3, "2020-00-05 23:59:58.576"},
		{"2020.00.05 23:59:58.575601", 3, "2020-00-05 23:59:58.576"},
		{"2020/00/05 23:59:58.575601", 3, "2020-00-05 23:59:58.576"},
		{"2020/00/05-23:59:58.575601", 3, "2020-00-05 23:59:58.576"},
	}
	...
	for _, test := range fspTbl {
		t, err := types.ParseTime(sc, test.Input, allegrosql.TypeDatetime, test.Fsp)
		c.Assert(err, IsNil)
		c.Assert(t.String(), Equals, test.Expect)
	}

	t, _ := types.ParseTime(sc, "121231113045.9999999", allegrosql.TypeDatetime, 6)
...
func (s *testTimeSuite) TestDate(c *C) {
	...
	causet := []struct {
		...
		{"2020-06-01 12:12:12", "2020-06-01"},
...
func (s *testTimeSuite) TestCodec(c *C) {
	...
	// MyALLEGROSQL timestamp value doesn't allow month=0 or day=0.
	t, err := types.ParseTimestamp(sc, "2020-12-00 00:00:00")
	c.Assert(err, NotNil)
	...
	t1 := types.NewTime(types.FromGoTime(time.Now()), allegrosql.TypeTimestamp, 0)
	packed, err := t1.ToPackedUint()
	c.Assert(err, IsNil)

	t2 := types.NewTime(types.ZeroCoreTime, allegrosql.TypeTimestamp, 0)
	...
	t3 := types.NewTime(types.ZeroCoreTime, allegrosql.TypeDatetime, 0)
	...
	t4 := types.NewTime(types.ZeroCoreTime, allegrosql.TypeDatetime, 0)
	...
	for _, test := range tbl {
		t, err := types.ParseTime(sc, test, allegrosql.TypeDatetime, types.MaxFsp)
		c.Assert(err, IsNil)

		packed, _ = t.ToPackedUint()

		dest := types.NewTime(types.ZeroCoreTime, allegrosql.TypeDatetime, types.MaxFsp)
...
func (s *testTimeSuite) TestParseTimeFromNum(c *C) {
	...
	for ith, test := range causet {
		...
		if test.ExpectDateTimeError {
			...
			c.Assert(err, IsNil)
			c.Assert(t.Type(), Equals, allegrosql.TypeDatetime)
		}
		...
		if test.ExpectTimeStampError {
			...
			c.Assert(t.Type(), Equals, allegrosql.TypeTimestamp)
		}
		...
		if test.ExpectDateTimeError {
			...
			c.Assert(err, IsNil)
			c.Assert(t.Type(), Equals, allegrosql.TypeDate)
		}
...
func (s *testTimeSuite) TestToNumber(c *C) {
	...
	for _, test := range tblDate {
		t, err := types.ParseTime(sc, test.Input, allegrosql.TypeDate, 0)
		c.Assert(err, IsNil)
		c.Assert(t.ToNumber().String(), Equals, test.Expect)
	}
...
func (s *testTimeSuite) TestRoundFrac(c *C) {
	...
	tbl := []struct {
		...
		// TODO: MyALLEGROSQL can handle this case, but we can't.
		// {"2012-01-00 23:59:59.999999", 3, "2012-01-01 00:00:00.000"},
	}

	for _, t := range tbl {
		v, err := types.ParseTime(sc, t.Input, allegrosql.TypeDatetime, types.MaxFsp)
		c.Assert(err, IsNil)
	...
	tbl = []struct {
		...
		{"2020-11-25 07:25:45.123456", 4, "2020-11-25 07:25:45.1235"},
		{"2020-11-25 07:25:45.123456", 5, "2020-11-25 07:25:45.12346"},
		{"2020-11-25 07:25:45.123456", 0, "2020-11-25 07:25:45"},
		{"2020-11-25 07:25:45.123456", 2, "2020-11-25 07:25:45.12"},
		{"2020-11-26 11:30:45.999999", 4, "2020-11-26 11:30:46.0000"},
		{"2020-11-26 11:30:45.999999", 0, "2020-11-26 11:30:46"},
		{"2020-11-26 11:30:45.999999", 3, "2020-11-26 11:30:46.000"},
	}
...
func (s *testTimeSuite) TestConvert(c *C) {
	...
	tbl := []struct {
		...
		{"2020-01-05 08:40:59.575601", 0, "08:41:00"},
		{"2020-01-05 23:59:59.575601", 0, "00:00:00"},
		{"0000-00-00 00:00:00", 6, "00:00:00"},
	}

	for _, t := range tbl {
		v, err := types.ParseTime(sc, t.Input, allegrosql.TypeDatetime, t.Fsp)
		c.Assert(err, IsNil)
	...
	for _, t := range tblDuration {
		...
		t, err := v.ConvertToTime(sc, allegrosql.TypeDatetime)
		c.Assert(err, IsNil)
...
func (s *testTimeSuite) TestCompare(c *C) {
	...
	for _, t := range tbl {
		v1, err := types.ParseTime(sc, t.Arg1, allegrosql.TypeDatetime, types.MaxFsp)
		c.Assert(err, IsNil)
	...
	v1, err := types.ParseTime(sc, "2011-10-10 11:11:11", allegrosql.TypeDatetime, types.MaxFsp)
	c.Assert(err, IsNil)
...
func (s *testTimeSuite) TestTamestamFIDeliff(c *C) {
	tests := []struct {
		unit   string
		t1     types.CoreTime
		t2     types.CoreTime
		...
		{"QUARTER", types.FromDate(2000, 1, 12, 0, 0, 0, 0), types.FromDate(2020, 1, 1, 0, 0, 0, 0), 63},
		{"QUARTER", types.FromDate(2020, 1, 1, 0, 0, 0, 0), types.FromDate(2000, 1, 12, 0, 0, 0, 0), -63},
	}

	for _, test := range tests {
		t1 := types.NewTime(test.t1, allegrosql.TypeDatetime, 6)
		t2 := types.NewTime(test.t2, allegrosql.TypeDatetime, 6)
		c.Assert(types.TimestamFIDeliff(test.unit, t1, t2), Equals, test.expect)
	}
}
...
func (s *testTimeSuite) TestConvertTimeZone(c *C) {
	...
	tests := []struct {
		...
		{types.FromDate(2020, 1, 1, 0, 0, 0, 0), time.UTC, loc, types.FromDate(2020, 1, 1, 8, 0, 0, 0)},
		{types.FromDate(2020, 1, 1, 8, 0, 0, 0), loc, time.UTC, types.FromDate(2020, 1, 1, 0, 0, 0, 0)},
...
func (s *testTimeSuite) TestTimeAdd(c *C) {
	tbl := []struct {
		...
		{"2020-01-18", "12:30:59", "2020-01-18 12:30:59"},
		{"2020-01-18 01:01:01", "12:30:59", "2020-01-18 13:32:00"},
		{"2020-01-18 01:01:01.123457", "12:30:59", "2020-01-18 13:32:0.123457"},
		{"2020-01-18 01:01:01", "838:59:59", "2020-02-22 00:01:00"},
		{"2020-08-21 15:34:42", "-838:59:59", "2020-07-17 16:34:43"},
		{"2020-08-21", "01:01:01.001", "2020-08-21 01:01:01.001"},
	}
	...
	for _, t := range tbl {
		...
		c.Assert(err, IsNil)
		result, err := types.ParseTime(sc, t.Ret, allegrosql.TypeDatetime, types.MaxFsp)
		c.Assert(err, IsNil)
...
func (s *testTimeSuite) TestTruncateOverflowMyALLEGROSQLTime(c *C) {
	t := types.MaxTime + 1
	res, err := types.TruncateOverflowMyALLEGROSQLTime(t)
...
func (s *testTimeSuite) TestCheckTimestamp(c *C) {
	...
	tests = []struct {
		...
		tz:             losAngelesTz,
		input:          types.FromDate(2020, 3, 11, 1, 0, 50, 0),
		expectRetError: false,
	}, {
		tz:             losAngelesTz,
		input:          types.FromDate(2020, 3, 11, 2, 0, 16, 0),
		expectRetError: true,
	}, {
		tz:             losAngelesTz,
		input:          types.FromDate(2020, 3, 11, 3, 0, 20, 0),
		expectRetError: false,
	}, {
		tz:             shanghaiTz,
		input:          types.FromDate(2020, 3, 11, 1, 0, 50, 0),
		...
		tz:             londonTz,
		input:          types.FromDate(2020, 3, 31, 0, 0, 20, 0),
		expectRetError: false,
	}, {
		tz:             londonTz,
		input:          types.FromDate(2020, 3, 31, 1, 0, 20, 0),
		expectRetError: true,
	}, {
		tz:             londonTz,
		input:          types.FromDate(2020, 3, 31, 2, 0, 20, 0),
		expectRetError: false,
	},
	}
...
func (s *testTimeSuite) TestCurrentTime(c *C) {
	res := types.CurrentTime(allegrosql.TypeTimestamp)
	c.Assert(res.Type(), Equals, allegrosql.TypeTimestamp)
	c.Assert(res.Fsp(), Equals, int8(0))
}

func (s *testTimeSuite) TestInvalidZero(c *C) {
	in := types.NewTime(types.ZeroCoreTime, allegrosql.TypeTimestamp, types.DefaultFsp)
	c.Assert(in.InvalidZero(), Equals, true)
	in.SetCoreTime(types.FromDate(2020, 00, 00, 00, 00, 00, 00))
	c.Assert(in.InvalidZero(), Equals, true)
	in.SetCoreTime(types.FromDate(2020, 04, 12, 12, 00, 00, 00))
	c.Assert(in.InvalidZero(), Equals, false)
}

func (s *testTimeSuite) TestGetFsp(c *C) {
	res := types.GetFsp("2020:04:12 14:00:00.123456")
	c.Assert(res, Equals, int8(6))

	res = types.GetFsp("2020:04:12 14:00:00.1234567890")
	c.Assert(res, Equals, int8(6))

	res = types.GetFsp("2020:04:12 14:00:00.1")
	c.Assert(res, Equals, int8(1))

	res = types.GetFsp("2020:04:12 14:00:00")
	c.Assert(res, Equals, int8(0))
}

func (s *testTimeSuite) TestExtractDatetimeNum(c *C) {
	in := types.NewTime(types.FromDate(2020, 04, 12, 14, 00, 00, 0000), allegrosql.TypeTimestamp, types.DefaultFsp)
	...
	c.Assert(err, IsNil)
	c.Assert(res, Equals, int64(2020))
	...
	in = types.NewTime(types.FromDate(0000, 00, 00, 00, 00, 00, 0000), allegrosql.TypeTimestamp, types.DefaultFsp)
...
func (s *testTimeSuite) TestParseDurationValue(c *C) {
	tbl := []struct {
		...
		{"-1:1", "DAY_HOUR", 0, 0, -1, -3600000000000, nil},
		{"-aa1bb1", "DAY_HOUR", 0, 0, -1, -3600000000000, nil},
		{"2020-12", "YEAR_MONTH", 2020, 12, 0, 0, nil},
...
func (s *testTimeSuite) TestParseTimeFromInt64(c *C) {
	...
	c.Assert(output.Type(), Equals, allegrosql.TypeDatetime)
	c.Assert(output.Year(), Equals, 2020)
...
func (s *testTimeSuite) TestTimeSub(c *C) {
	tbl := []struct {
		...
		{"2020-01-18 01:01:01", "2020-01-18 00:00:01", "01:01:00"},
		{"2020-01-18 01:01:01", "2020-01-18 01:01:01", "00:00:00"},
		{"2020-04-12 18:20:00", "2020-04-12 14:00:00", "04:20:00"},
	}
	...
	for _, t := range tbl {
		v1, err := types.ParseTime(sc, t.Arg1, allegrosql.TypeDatetime, types.MaxFsp)
		c.Assert(err, IsNil)
		v2, err := types.ParseTime(sc, t.Arg2, allegrosql.TypeDatetime, types.MaxFsp)
		c.Assert(err, IsNil)
...
func (s *testTimeSuite) TestCheckMonthDay(c *C) {
	...
	for _, t := range dates {
		tt := types.NewTime(t.date, allegrosql.TypeDate, types.DefaultFsp)
		err := tt.Check(sc)
		if t.isValidDate {
			c.Check(err, IsNil)
		} else {
...
func BenchmarkTimeAdd(b *testing.B) {
	...
	arg1, _ := types.ParseTime(sc, "2020-01-18", allegrosql.TypeDatetime, types.MaxFsp)
