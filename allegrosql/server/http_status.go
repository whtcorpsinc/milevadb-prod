...
import (
	...
	"github.com/tiancaiamao/apFIDelash/traceapp"
	...
	static "sourcegraph.com/sourcegraph/apFIDelash-data"
)
...
func (s *Server) startHTTPServer() {
	...
	if host, port, err := net.SplitHostPort(s.statusAddr); err == nil {
		...
		baseURL := &url.URL{
			Scheme: soliton.InternalHTTPSchema(),
			Host:   fmt.Sprintf("%s:%s", host, port),
		}
		router.HandleFunc("/web/trace", traceapp.HandleMilevaDB).Name("Trace Viewer")
	...
	serverMux.HandleFunc("/debug/zip", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="milevadb_debug"`+time.Now().Format("20060102150405")+".zip"))
		...
		_, err = fw.Write([]byte(printer.GetMilevaDBInfo()))
		terror.Log(err)

		err = zw.Close()
		terror.Log(err)
	})
	fetcher := sqlInfoFetcher{causetstore: einsteindbHandlerTool.CausetStore}
	serverMux.HandleFunc("/debug/sub-optimal-plan", fetcher.zipInfoForALLEGROSQL)
	...
	failpoint.Inject("enableTestAPI", func() {
		...
		router.Handle("/test/{mod}/{op}", &testHandler{einsteindbHandlerTool, 0})
	})
...
func (s *Server) handleStatus(w http.ResponseWriter, req *http.Request) {
	...
	st := status{
		...
		Version:     allegrosql.ServerVersion,
		GitHash:     versioninfo.MilevaDBGitHash,
	}
